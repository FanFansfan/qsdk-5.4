...
void pbcHyfi20GetPipeMsgCB(s32 fd, void *eloop_ctx, void *sock_ctx) {
    ...
    } else if (strncmp(buf, CONFIG_CHANNEL, strlen(CONFIG_CHANNEL)) == 0) {
        apac_mib_channel_mode_update_cb(pData, buf);
    } else if (strncmp(buf, CONFIG_MODE, strlen(CONFIG_MODE)) == 0) {
        apac_mib_channel_mode_update_cb(pData, buf);
    }
#endif
    else {
...
int pbcGetBssid(char *ifName, u8 *bssid) {
    ...
    if (getBSSID_cfg80211(wlanIfWd->ctx, ifName, (struct ether_addr *)bssid) < 0 ) {
...
int pbcGetName(char *ifName, size_t size, char *name) {
    ...
    if (getName_cfg80211(wlanIfWd->ctx, ifName, name) < 0) {
        dprintf(MSG_ERROR, "%s: ioctl() failed, ifName: %s.\n", __func__, ifName);
        goto err;
    }
