		...
		(struct new_fdb_table *new_table, int number)
{
	...
	struct new_fdb_table *new_entry = new_table + i;
        if((new_entry->ageing_timer_value > 0) && (new_entry->is_common == 0)
...
static void wrap_create_vap(struct new_fdb_table *entry)
{
    ...
    new_entry->is_common = 0;
    new_entry->success_flag = OPERATION_IN_PROGRESS;
    data->table_no++;
...
static void wrap_delete_vap(struct fdb_table *entry)
{
    ...
    entry->success_flag = OPERATION_IN_PROGRESS;
...
static void wrap_check_success_flag()
{
	...
	if(entry->success_flag == TO_BE_DELETED)
	{
	    /*
	    ...
	    */
            ...
            entry->is_common = last_entry->is_common;
...
/* main function which takes care of creating and deleting vaps based on
 * entries in bridge table.
 */
void *wrap_main_function()
{
	...
	/* If old table is empty and new table has entries, then create vap
	 * for all the entries in new table.
	 */
	...
	/* If old table is not empty and new table is empty, then
	 * delete all the vaps corresponding to the entries in old table.
	 */
					...
					&& data->delete_enable == 1)
	{
	    ...
	    {
		struct fdb_table *entry=table+i;
		if(entry->success_flag == OPERATION_SUCCESS)
		{
		    wrap_delete_vap(entry);
		}
	    }
	}
	/* If both new and old table are having some entries, then compare the
	 * entries in the old and new table and flag common entries in both
	 * new and old table.
	 *
	 * Delete all vaps from old table if flag is set to 0.
	 * Create vaps for all entries in new table if flag is 0.
	 */
	else if(data->table_no != 0 && data->new_table_no != 0)
	{
		...
		{
		    struct fdb_table *entry = table+i;
		    if((entry->is_common == 0) && (entry->success_flag == OPERATION_SUCCESS))
		    {
			wrap_delete_vap(entry);
		    }
		}
...
/* Add a static entry  which will not be deleted even if the bridge entry gets
 * deleted. This needs to be deleted manually with command.
 */
static void wrap_add_static_entry(unsigned char *mac_addr)
{
    ...
    entry->success_flag = STATIC_ENTRY;
    data->table_no++;

}

/* Set success flag based on whether creation or deletion of vap happened
 * successfully
 */
static void wrap_set_success_flag(unsigned char *mac_addr, int flag)
{
    int i;
    for(i=0; i<data->table_no; i++)
    {
...
void *wrap_check_socket()
{
	...
	if(message == INTERNAL_MESSAGE)
	{
	    ...
	    if(type == WIRED_PSTA_ADD)
	    {
		if(value == FLAG_SUCCESS)
		{
		    wrap_set_success_flag(mac_addr,0);
		}
		else if(value == FLAG_FAIL)
		{
		    global_vap_limit_flag = 0;
		    wrap_set_success_flag(mac_addr,2);
		}
	    }
	    else if(type == WIRED_PSTA_REMOVE)
	    {
		if(value == FLAG_SUCCESS)
		{
		    global_vap_limit_flag = 0;
		...
		wrapd_printf("ERROR: Message format wrong\n");
	}
    }
out:
    wrapd_printf("ERROR: Exiting thread %s\n", __func__);
