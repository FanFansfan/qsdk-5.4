...
#define HTCAP_SIZE 150
#define VHTCAP_SIZE 300
#define HTCAP_FILE_SIZE 100
#define VHTCAP_FILE_SIZE 250
...
typedef struct {
    ...
    uint8_t tpscale; /* Transmit power scale factor */
    ...
    uint8_t rpt_max_phy; /*Enable Repeater Max Phy feature, 0 or 1; 0 if unspecified*/
    uint8_t scan_over_cac; /*Enable scan over DFS wait time, 0 or 1; 0 if unspecified*/
    uint8_t acs_pcaconly; /* Enable ACS preCAC only flag, 0 or 1; 0 if disabled */
    uint8_t dcs_enable;/*Enable DCS flag,0 or 1 or 2 or 3; 0 if disabled */
...
typedef struct {
    ...
    uint8_t  greenap_ps_enable;
    uint32_t greenap_ps_trans_time;
    uint32_t sec_comp_6g;
    uint32_t keymgmt_mask_6g;
