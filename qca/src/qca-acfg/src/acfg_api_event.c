...
parse_wsupp_event(char *event_buf, acfg_os_event_t *event)
{
    ...
    if (ACFG_N_STR_MATCH(event_buf, WSUPP_AP_STA_CONNECTED))
    {
        ...
        acfg_os_strcpy(ap_sta_conn->raw_message, event_buf,sizeof(ap_sta_conn->raw_message));
        ...
        acfg_os_strcpy(wpa_conn->raw_message, event_buf,sizeof(wpa_conn->raw_message));
        ...
        acfg_os_strcpy(eap_status->raw_message, event_buf,sizeof(eap_status->raw_message));
                ...
                WSUPP_CTRL_EVENT_WPS_NEW_AP_SETTINGS))
    {
        ...
        acfg_os_strcpy(wps_new_ap_setting->raw_message, event_buf,sizeof(wps_new_ap_setting->raw_message));
        ...
        acfg_os_strcpy(wps_success->raw_message, event_buf,sizeof(wps_success->raw_message));
    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_WPS_ENROLLEE_SEEN))
    {
        ...
        acfg_os_strcpy(wps_enr->raw_message, event_buf,sizeof(wps_enr->raw_message));
    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONF_RECEIVED))
    {
        acfg_wsupp_dpp_conf_received_t *dpp_conf_recevied;
        event->id = ACFG_EV_DPP_EVENT_CONF_RECEIVED;
        dpp_conf_recevied = (void*) &event->data;
        acfg_os_strcpy(dpp_conf_recevied->raw_message, event_buf,sizeof(dpp_conf_recevied->raw_message));

    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONFOBJ_AKM))
    {
        acfg_wsupp_dpp_confobj_akm_t *dpp_confobj_akm;
        event->id = ACFG_EV_DPP_EVENT_CONFOBJ_AKM;
        dpp_confobj_akm = (void*) &event->data;
        acfg_os_strcpy(dpp_confobj_akm->raw_message, event_buf,sizeof(dpp_confobj_akm->raw_message));

    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONFOBJ_SSID))
    {
        acfg_wsupp_dpp_confobj_ssid_t *dpp_confobj_ssid;
        event->id = ACFG_EV_DPP_EVENT_CONFOBJ_SSID;
        dpp_confobj_ssid = (void*) &event->data;
        acfg_os_strcpy(dpp_confobj_ssid->raw_message, event_buf,sizeof(dpp_confobj_ssid->raw_message));

    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONFOBJ_PASS))
    {
        acfg_wsupp_dpp_confobj_pass_t *dpp_confobj_pass;
        event->id = ACFG_EV_DPP_EVENT_CONFOBJ_PASS;
        dpp_confobj_pass = (void*) &event->data;
        acfg_os_strcpy(dpp_confobj_pass->raw_message, event_buf,sizeof(dpp_confobj_pass->raw_message));

    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONFOBJ_CONNECTOR))
    {
        acfg_wsupp_dpp_confobj_connector_t *dpp_confobj_connector;
        event->id = ACFG_EV_DPP_EVENT_CONFOBJ_CONNECTOR;
        dpp_confobj_connector = (void*) &event->data;
        acfg_os_strcpy(dpp_confobj_connector->raw_message, event_buf,sizeof(dpp_confobj_connector->raw_message));

    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONFOBJ_CSIGN))
    {
        acfg_wsupp_dpp_confobj_csign_t *dpp_confobj_csign;
        event->id = ACFG_EV_DPP_EVENT_CONFOBJ_CSIGN;
        dpp_confobj_csign = (void*) &event->data;
        acfg_os_strcpy(dpp_confobj_csign->raw_message, event_buf,sizeof(dpp_confobj_csign->raw_message));

    } else if (ACFG_N_STR_MATCH(event_buf,
                WSUPP_CTRL_EVENT_DPP_CONFOBJ_NETACCESSKEY))
    {
        acfg_wsupp_dpp_confobj_netaccesskey_t *dpp_confobj_netaccesskey;
        event->id = ACFG_EV_DPP_EVENT_CONFOBJ_NETACCESSKEY;
        dpp_confobj_netaccesskey = (void*) &event->data;
        acfg_os_strcpy(dpp_confobj_netaccesskey->raw_message, event_buf,sizeof(dpp_confobj_netaccesskey->raw_message));
        ...
        raw = (void *)&event->data;
        acfg_os_strcpy(raw->raw_message, event_buf,sizeof(raw->raw_message));
    }
...

{
    ...
    switch(msg->id)
    {
        ...
        case ACFG_EV_HW_MODE_CHANGE_STATUS:
            ...
            break;
        case ACFG_EV_DPP_EVENT_CONF_RECEIVED:
            {
                if (event->dpp_conf_received) {
                    event->dpp_conf_received(ifname,
                            &pdata->dpp_conf_received);
                }
            }
            break;
        case ACFG_EV_DPP_EVENT_CONFOBJ_AKM:
            {
                if (event->dpp_confobj_akm) {
                    event->dpp_confobj_akm(ifname,
                            &pdata->dpp_confobj_akm);
                }
            }
            break;
        case ACFG_EV_DPP_EVENT_CONFOBJ_SSID:
            {
                if (event->dpp_confobj_ssid) {
                    event->dpp_confobj_ssid(ifname,
                            &pdata->dpp_confobj_ssid);
                }
            }
            break;
        case ACFG_EV_DPP_EVENT_CONFOBJ_PASS:
            {
                if (event->dpp_confobj_pass) {
                    event->dpp_confobj_pass(ifname,
                            &pdata->dpp_confobj_pass);
                }
            }
            break;
        case ACFG_EV_DPP_EVENT_CONFOBJ_CONNECTOR:
            {
                if (event->dpp_confobj_connector) {
                    event->dpp_confobj_connector(ifname,
                            &pdata->dpp_confobj_connector);
                }
            }
            break;
        case ACFG_EV_DPP_EVENT_CONFOBJ_CSIGN:
            {
                if (event->dpp_confobj_csign) {
                    event->dpp_confobj_csign(ifname,
                            &pdata->dpp_confobj_csign);
                }
            }
            break;
        case ACFG_EV_DPP_EVENT_CONFOBJ_NETACCESSKEY:
            {
                if (event->dpp_confobj_netaccesskey) {
                    event->dpp_confobj_netaccesskey(ifname,
                            &pdata->dpp_confobj_netaccesskey);
                }
            }
            break;
    }
...
acfg_ctrl_iface_detach (int32_t sock)
{
    ...
    acfg_os_snprintf(cmd, sizeof(cmd), "%s", WPA_CTRL_IFACE_DETACH_CMD);
...
acfg_ctrl_iface_attach (int32_t sock)
{
    ...
    acfg_os_snprintf(cmd, sizeof(cmd), "%s", WPA_CTRL_IFACE_ATTACH_CMD);
    len = sizeof (replybuf);
    if (acfg_ctrl_iface_send(sock, cmd, strlen(cmd), replybuf, &len) < 0) {
        ...
        acfg_event_mode_t  mode)
{
    ...
    while (1) {
        ...
        for (i = 0; i < list.num_iface; i++) {
            acfg_os_snprintf((char *)wsupp_h[i].ifname, sizeof(wsupp_h[i].ifname),
                     "%s", list.iface[i]);
