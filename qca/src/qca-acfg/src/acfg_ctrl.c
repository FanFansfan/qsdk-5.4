        ...
        struct wsupp_ev_handle *wsupp_h)
{
    ...
    acfg_os_snprintf(wsupp_h->local.sun_path, sizeof (wsupp_h->local.sun_path),
    ...
    acfg_os_snprintf(wsupp_h->dest.sun_path, sizeof (wsupp_h->dest.sun_path),
...
acfg_ctrl_iface_present (uint8_t *ifname, acfg_opmode_t opmode)
{
    ...
    if (opmode == ACFG_OPMODE_STA) {
        acfg_os_snprintf(wsupp_h.dest.sun_path,
...
{   
    ...
    acfg_os_snprintf(app_sock_h->local.sun_path, 
...
acfg_open_app_sock(struct sockaddr_un *local)
{
    ...
    acfg_os_snprintf(local->sun_path, sizeof (local->sun_path),
    ...
    acfg_os_snprintf(dest.sun_path, sizeof (dest.sun_path),
