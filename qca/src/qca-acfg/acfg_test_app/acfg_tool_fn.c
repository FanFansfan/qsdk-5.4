...
int wrap_set_profile(char *params[])
{
    ...
    for (i = 0; i < new_profile->num_vaps; i++) {
        acfg_os_strcpy((char *)new_profile->vap_params[i].radio_name,
                (char *)new_profile->radio_params.radio_name, sizeof(new_profile->vap_params[i].radio_name));
    }
    acfg_os_strcpy(ctrl_hapd, new_profile->ctrl_hapd, sizeof(ctrl_hapd));
    acfg_os_strcpy(ctrl_wpasupp, new_profile->ctrl_wpasupp, sizeof(ctrl_wpasupp));
...
param_info_t wrap_set_acs_pcaconly_params[] = {
        {"wifi", PARAM_STRING, "radio name"},
        {"acs_pcaconly", PARAM_UINT32, "acs pre-cac only"}
};

int wrap_set_acs_pcaconly(char *params[])
{
    uint32_t status = QDF_STATUS_E_FAILURE;
    uint32_t acs_pcaconly;

    get_uint32(params[1], (uint32_t *)&acs_pcaconly);

    status = acfg_set_acs_pcaconly((uint8_t *)params[0], acs_pcaconly);

    return acfg_to_os_status(status);
}

param_info_t wrap_get_acs_pcaconly_params[] = {
        {"wifi", PARAM_STRING, "radio name"},
};

int wrap_get_acs_pcaconly(char *params[])
{
    uint32_t status = QDF_STATUS_E_FAILURE;
    uint32_t acs_pcaconly;

    status = acfg_get_acs_pcaconly((uint8_t *)params[0], &acs_pcaconly);

    msg("ACS pre-cac-only: %u", acs_pcaconly);
...
int wrap_set_ssid(char *params[])
{
    ...
    if (acfg_os_strcpy((char *)ssid.ssid, params[1],
...
int wrap_set_vap_vendor_param(char *params[])
{
    ...
    if(strcmp(params[3], "str") == 0)
    {
        ...
        if (acfg_os_strcpy((char *)&val, params[2], sizeof(val)) >= sizeof(val)) {
...
addr_ntoa(const uint8_t mac[ACFG_MACADDR_LEN])
{
    ...
    i = acfg_os_snprintf(a, sizeof(a), "%02x:%02x:%02x:%02x:%02x:%02x",
...
int wrap_wps_pin(char *params[])
{
    ...
    if (strncmp(params[1], "set", 3) == 0) {
        ...
        if (acfg_os_strcpy(pin, params[2], sizeof(pin)) >= sizeof(pin)) {
...
param_info_t wrap_gpio_config_params[] = {
    ...
    {"mux_config_val", PARAM_STRING , "Mux config value"},
    {"drive", PARAM_STRING , "drive"},
    {"init_enable", PARAM_STRING , "init_enable"},
};

int wrap_gpio_config(char *params[])
{
    uint32_t status = QDF_STATUS_E_FAILURE ;
    uint32_t gpio_num, input, pull_type, intr_mode, mux_config_val, drive, init_enable;
    printf("%s[%d] params: %s %s %s %s %s %s %s\n", __func__,__LINE__, params[1],params[2],params[3],params[4], params[5], params[6], params[7]);

    get_uint32(params[1], &gpio_num);
    get_uint32(params[2], &input);
    get_uint32(params[3], &pull_type);
    get_uint32(params[4], &intr_mode);
    get_uint32(params[5], &mux_config_val);
    get_uint32(params[6], &drive);
    get_uint32(params[7], &init_enable);

    status = acfg_gpio_config((uint8_t *)params[0], gpio_num, input, pull_type, intr_mode, mux_config_val, drive, init_enable);
    return acfg_to_os_status(status) ;
}
    ...
    for(i=0; i<rs.rs_nrates; i++){
        ...
        if(rate_code&IEEE80211_RATE_BASIC){
            if((rate_code&ACFG_RATE_VAL)==11){
                acfg_os_snprintf(print_str,sizeof(print_str),"%s%s","5.5","(B)");
            }else{
                acfg_os_snprintf(print_str,sizeof(print_str),"%d%s",((rate_code&ACFG_RATE_VAL)/2),"(B)");
            }
        }else{
            acfg_os_snprintf(print_str,sizeof(print_str),"%d",((rate_code&ACFG_RATE_VAL)/2));
        }
        printf("%s ",print_str);
    }
    printf("\n");
...
param_info_t wrap_add_rnr_entry_params[] = {
    {"radio", PARAM_STRING, "radio name"},
    {"param", PARAM_STRING, "parameter value, uid"},
    {"param", PARAM_STRING, "parameter value, num_ap"},
};

static void acfg_user_rnr_init(acfg_user_rnr_data_t *data)
{
    acfg_rnr_nbr_ap_info_t *ap_info = NULL;
    acfg_rnr_tbtt_info_set_t *tbtt_info = NULL;
    uint8_t cnt = 0;
    uint8_t bssid[6] = {0x0,0x3,0x7f,0x1,0x2,0x1};
    uint8_t num_ap = 0;

    num_ap = data->num_ap;
    ap_info = (acfg_rnr_nbr_ap_info_t *)data->user_buf;
    ap_info->op_class = 1;
    ap_info->channel = 36;
    ap_info->hdr_info_len = 13;  //contains all optional fields
    ap_info->hdr_info_cnt = num_ap-1;

    dbg_print_params("%s:op_class=%d, channel=%d, hdr_len=%d,hdr_cnt=%d \n",
            __FUNCTION__, ap_info->op_class, ap_info->channel,
            ap_info->hdr_info_len, ap_info->hdr_info_cnt);

    while (cnt < num_ap) {
        tbtt_info = &(ap_info->tbtt_info[cnt]);
        tbtt_info->tbtt_offset = 100;
        memcpy(tbtt_info->bssid, bssid, 6);
        tbtt_info->bssid[5] += cnt;
        tbtt_info->short_ssid = 0;
        tbtt_info->bss_params.oct_recommended = 0;
        tbtt_info->bss_params.same_ssid = 0;
        tbtt_info->bss_params.mbssid_set = 1;
        tbtt_info->bss_params.tx_bssid = 0;
        ...
        tbtt_info->psd_20mhz = 0;
        dbg_print_params("====tbtt info %d====\n",cnt+1);
        dbg_print_params("tbtt_offset=%d, bssid=0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n",
                tbtt_info->tbtt_offset, tbtt_info->bssid[0], tbtt_info->bssid[1],
                tbtt_info->bssid[2],tbtt_info->bssid[3],
                tbtt_info->bssid[4],tbtt_info->bssid[5]);
        cnt++;
    }
    data->user_buf_len = 4*sizeof(uint8_t) + num_ap*sizeof(acfg_rnr_tbtt_info_set_t);
}

int wrap_add_rnr_entry(char *params[])
{
    uint32_t uid = 0, len, num_ap = 0;
    uint32_t status = QDF_STATUS_E_FAILURE ;
    acfg_user_rnr_data_t *data = NULL;

    len = sizeof(acfg_user_rnr_data_t);
    data = malloc(len);
    if (data == NULL)
        return QDF_STATUS_E_FAILURE;
    memset(data, 0, len);

    get_uint32(params[1], (uint32_t *)&uid);
    get_uint32(params[2], (uint32_t *)&num_ap);
    data->uid = uid;
    data->num_ap = num_ap;

    if (num_ap == 0 || num_ap > 15) {
        printf("%s: num ap cant be 0 or more than 15\n", __func__);
        free(data);
        return QDF_STATUS_E_FAILURE;
    }
    acfg_user_rnr_init(data);

    dbg_print_params("uid=%d, user_buf_len=%d\n",data->uid, data->user_buf_len);
    status = acfg_add_rnr_entry((uint8_t *)params[0], (const uint8_t *)data, len);
    if (status == 0) {
        printf("%s: entry with uid%d added\n",__func__,uid);
    }
...
param_info_t wrap_del_rnr_entry_params[] = {
    {"radio", PARAM_STRING, "radio name"},
    {"param", PARAM_STRING, "parameter value, uid"},
};

int wrap_del_rnr_entry(char *params[])
{
    uint32_t uid, len;
    uint32_t status = QDF_STATUS_E_FAILURE ;
    acfg_user_rnr_data_t data;
    ...
    status = acfg_del_rnr_entry((uint8_t *)params[0], (const uint8_t *)&data, len);
    if (status == 0) {
        printf("%s: entry with uid%d deleted\n",__func__,uid);
    }

    return acfg_to_os_status(status) ;
}

param_info_t wrap_dump_rnr_entries_params[] = {
    {"radio", PARAM_STRING, "radio name"},
};

int wrap_dump_rnr_entries(char *params[])
{
    uint32_t status = QDF_STATUS_E_FAILURE ;

    status = acfg_dump_rnr_entries((uint8_t *)params[0]);

    return acfg_to_os_status(status) ;
}


int wrap_dpp_bootstrap_gen(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_bootstrap_gen(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_bootstrap_remove(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_bootstrap_remove(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_bootstrap_info(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_bootstrap_info(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_configurator_add(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_configurator_add(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_configurator_remove(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_configurator_remove(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_configurator_sign(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_configurator_sign(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_configurator_get_key(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_configurator_get_key(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_bootstrap_get_uri(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_bootstrap_get_uri(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_qr_code(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_qr_code(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_bootstrap_set(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_bootstrap_set(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_configurator_params(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_configurator_params(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_auth_init(char *params[])

{
    uint32_t status = 0;

    status = acfg_dpp_auth_init(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_listen(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_listen(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_stop_listen(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_stop_listen(params[0]);
    return acfg_to_os_status(status);
}

int wrap_dpp_chirp(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_chirp(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_stop_chirp(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_stop_chirp(params[0]);
    return acfg_to_os_status(status);
}

int wrap_dpp_controller_start(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_controller_start(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_controller_stop(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_controller_stop(params[0]);
    return acfg_to_os_status(status);
}

int wrap_dpp_pkex_add(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_pkex_add(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_pkex_remove(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_pkex_remove(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_mud_url_set(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_mud_url_set(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_pfs_set(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_pfs_set(params[0], params[1]);
    return acfg_to_os_status(status);
}

int wrap_dpp_controller_set(char *params[])
{
    uint32_t status = 0;

    status = acfg_dpp_controller_set(params[0], params[1]);
    return acfg_to_os_status(status);
}
...
fntbl_t fntbl[] = {
    ...
    {"acfg_gpio_config", wrap_gpio_config, 8, wrap_gpio_config_params},
    ...
    {"acfg_set_acs_pcaconly", wrap_set_acs_pcaconly, 2, wrap_set_acs_pcaconly_params},
    {"acfg_get_acs_pcaconly", wrap_get_acs_pcaconly, 1, wrap_get_acs_pcaconly_params},
    {"acfg_add_rnr_entry", wrap_add_rnr_entry, 3, wrap_add_rnr_entry_params},
    {"acfg_del_rnr_entry", wrap_del_rnr_entry, 2, wrap_del_rnr_entry_params},
    {"acfg_dump_rnr_entries", wrap_dump_rnr_entries, 1, wrap_dump_rnr_entries_params},
    {"acfg_dpp_bootstrap_gen", wrap_dpp_bootstrap_gen, 2, NULL},
    {"acfg_dpp_bootstrap_remove", wrap_dpp_bootstrap_remove, 2, NULL},
    {"acfg_dpp_bootstrap_info", wrap_dpp_bootstrap_info, 2, NULL},
    {"acfg_dpp_configurator_add", wrap_dpp_configurator_add, 2, NULL},
    {"acfg_dpp_configurator_remove", wrap_dpp_configurator_remove, 2, NULL},
    {"acfg_dpp_configurator_sign", wrap_dpp_configurator_sign, 2, NULL},
    {"acfg_dpp_configurator_get_key", wrap_dpp_configurator_get_key, 2, NULL},
    {"acfg_dpp_bootstrap_get_uri", wrap_dpp_bootstrap_get_uri, 2, NULL},
    {"acfg_dpp_qr_code", wrap_dpp_qr_code, 2, NULL},
    {"acfg_dpp_bootstrap_set", wrap_dpp_bootstrap_set, 2, NULL},
    {"acfg_dpp_configurator_params", wrap_dpp_configurator_params, 2, NULL},
    {"acfg_dpp_auth_init", wrap_dpp_auth_init, 2, NULL},
    {"acfg_dpp_listen", wrap_dpp_listen, 2, NULL},
    {"acfg_dpp_stop_listen", wrap_dpp_stop_listen, 1, NULL},
    {"acfg_dpp_chirp", wrap_dpp_chirp, 2, NULL},
    {"acfg_dpp_stop_chirp", wrap_dpp_stop_chirp, 1, NULL},
    {"acfg_dpp_controller_start", wrap_dpp_controller_start, 2, NULL},
    {"acfg_dpp_controller_stop", wrap_dpp_controller_stop, 1, NULL},
    {"acfg_dpp_pkex_add", wrap_dpp_pkex_add, 2, NULL},
    {"acfg_dpp_pkex_remove", wrap_dpp_pkex_remove, 2, NULL},
    {"acfg_dpp_mud_url_set", wrap_dpp_mud_url_set, 2, NULL},
    {"acfg_dpp_pfs_set", wrap_dpp_pfs_set, 2, NULL},
    {"acfg_dpp_controller_set", wrap_dpp_controller_set, 2, NULL},
    {NULL,NULL},
};
