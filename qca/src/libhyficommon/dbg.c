...
/*
 * dbgModuleFindModuleName -- find module node matching module name
 * Returns NULL if module not found.
*/
static struct dbgModule *dbgModuleFindModuleName(
        const char *Name,
        ...
        char *B)        /* we can scribble over this input */
{
	...
	*End = 0;
    ...
    if (!strcmp(ModuleName, "all")) {
        ...
        MP = dbgModuleFindModuleName(ModuleName, Hash);
        if (MP) {   /* known Module */
            if (GotLevel) {
                MP->Level = Level;
            }
        } else {   /* unknown Module */
             dbgDebug(DBGERR,"%s: Unknown Module:%s \n", __func__, ModuleName);
             return ++NErrors;
        }
    }
    return NErrors;
