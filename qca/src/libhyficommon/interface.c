...
/*private*/ int interfaceVlanConfig(
        struct vlan_config_t * if_request)
{
...
int interfaceAddVlan(const char* Interface, const int VlanID)
{
    ...
    struct vlan_config_t if_request;
    memset(&if_request, 0, sizeof(struct vlan_config_t));

    if_request.cmdType = ADD_VLAN_CMD;

    if (strlcpy(if_request.devName, Interface,
        sizeof(if_request.devName)) >= sizeof(if_request.devName)) {
        return -1;
    }

    if_request.vlanInfo.vlanID = VlanID;
...
int interfaceDelVlan(const char* Interface)
{
    ...
    if_request.cmdType = DEL_VLAN_CMD;
