...
class TestParser(unittest.TestCase):
    ...
    def test_unpack_msg(self):
        ...
        # Test 7: A Multi-AP message
        mac = '\x24\x26\x98\x76\xd4\x4b'
        msg = '\x20\x12\x06\x42\x78\x57\x51\x73\x00\x00\x0C' + \
              '\x00' + mac + '\x05\x01\x00\x01'
        header, payload = parser.unpack_msg(msg)
        self.assertEquals(common.Header._make((common.Version.VERSION2,
                                               False, 18, 0x57784206, 0x7351,
                                               common.ModuleID.MULTI_AP)),
                          header)
        self.assertEquals(multi_ap.APCapability._make(
            (common.ether_ntoa(mac), 5, True, False, True)), payload)

        # Test 8: An unhandled module
