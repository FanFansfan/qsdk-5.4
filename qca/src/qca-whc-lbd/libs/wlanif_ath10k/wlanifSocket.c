...
#undef QCA_MOD_INPUT
#define QCA_MOD_INPUT QCA_MOD_LBD_WLANIF_ATH10K
...
int wlanif_netlink(const char *iface)
{

    struct nl_sock *sock;
    struct nl_msg *nlmsg;
    int ifaceindex;
    int drv_identifier, ret_status, tx_power;

    sock = nl_socket_alloc();
    genl_connect(sock);

    drv_identifier = genl_ctrl_resolve(sock, "nl80211");
//  dbgf(NULL, DBGINFO, "%s:Interface name : %s \n", __func__, iface);
    printf("%s Interface Name %s \n", __func__, iface);

    nl_socket_modify_cb(sock, NL_CB_VALID, NL_CB_CUSTOM, callback,&tx_power)

    nlmsg = nlmsg_alloc();
    ifaceindex = if_nametoindex(iface);
    genlmsg_put(nlmsg, 0, 0, drv_identifier,0, 0, NL80211_CMD_GET_INTERFACE, 0);
    NLA_PUT_U32(nlmsg, NL80211_ATTR_IFINDEX, ifaceindex);
    ret = nl_send_auto_complete(sock, nlmsg);
    printf("%s return status:%d \n", __func__, ret_status);
    nl_recvmsgs_default(sock);
    nl_wait_for_ack(sock);
    dbgf(NULL, DBGINFO, "%s:Tx Power value: %d \n", __func__, tx_power);
    nl_close(sock);
    nl_socket_free(sock);
