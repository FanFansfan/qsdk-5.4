    ...
    if (steeralgCmnIsActiveUpgradeDowngrade(servingBSS->trigger)) {
        if ( stadbEntry_isBandSupported(entry, wlanif_band_6g) ) {
            if ( servingBSS->band == wlanif_band_24g ||
                ...
                preferred11kReqBand = wlanif_band_6g;
            } else  {
                preferred11kReqBand =
                  stadbEntry_isBandSupported(entry, wlanif_band_5g) ? wlanif_band_5g : wlanif_band_24g;
            }
        }
        else {
            preferred11kReqBand =
              servingBSS->band == wlanif_band_24g ? wlanif_band_5g : wlanif_band_24g;
        }

        // For upgrade/downgrade, we ignore any BSSes that are not on the
        // preferred band as there is no sense in measuring them.
        if (wlanif_resolveBandFromFreq(bssInfo->freq) !=
                preferred11kReqBand) {
            return 0;
        }
    } else {
        preferred11kReqBand =
          stadbEntry_isBandSupported(entry, wlanif_band_6g) ? wlanif_band_6g : wlanif_band_5g;
    }
