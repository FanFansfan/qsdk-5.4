...
typedef struct steeralgPriv_t {
    ...
    struct {
        ...
        /// Check if multi 11k request is enabled
        /// or not
        u_int8_t multi11krequest;

        /// Number of seconds allowed for a backhaul capacity measurement to
        /// be considered as recent
        u_int8_t backhaulFreshnessLimit;

        // Max time allowed for a measurement to be considered recent
...
LBD_STATUS steeralgSelect11kMultiChannel(stadbEntry_handle_t entry,
                                            lbd_channelId_t *dot11kChannel, u_int8_t *validchannellist);
/**
