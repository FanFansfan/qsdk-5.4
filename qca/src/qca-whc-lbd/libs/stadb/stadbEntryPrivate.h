...
typedef struct stadbEntryPriv_inNetworkInfo_t {
    ...
    struct {
        ...
        /// The freq on which the device is currently associated (if any).
        u_int16_t freq;
    } assoc;
...
#define STADB_NUM_BCNRPT_MAX 24

typedef struct stadbBeaconReport_t {
    ...
    struct {
        ...
        int8_t rcpi;
    } reportedBcnrptInfo[STADB_NUM_BCNRPT_MAX];
...
typedef struct stadbEntryPriv_t {
    ...
    /// Whether the STA is a backhaul interface on a AP class device.
    LBD_BOOL isBackhaulSTA : 1;

  /// whether Client is sending RCPI in Beacon report as per 11k spec
    u_int8_t rcpiType;
  ...
  /// An array which holds the timestamp of each legacy upgrade steering
    ...
    /// Number of pending beacon report.
    u_int8_t pendingBcnRpt;

    /// Number of pending beacon request to be sent.
    u_int8_t pendingBcnRptReq;

    /// Maximum number of channels selected for 11k request.
    u_int8_t maxNumSelectedChannels;

    /// Channels Requested for a client to do 11k reuest.
    lbd_channelId_t dot11kChannelsReq[STADB_NUM_BCNRPT_MAX];

    /// Frequency requested for a client to do 11k request
    u_int16_t dot11kFreqReq[STADB_NUM_BCNRPT_MAX];

    /// Valid Beacon report for a sta entry.
    stadbBeaconReport_t bcnRpt;
