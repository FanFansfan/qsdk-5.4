...
static void stadbEntryDiagLogRSSI(const stadbEntry_handle_t entry,
                                  const lbd_bssInfo_t *bss, lbd_rssi_t rssi);
...
static void stadbEntryDiagLogBandPHYCapInfo(stadbEntry_handle_t entry,
                                            wlanif_band_e band);
...
LBD_STATUS stadbEntry_markBackhaulSTA(stadbEntry_handle_t entry) {
    if (!entry) {
        return LBD_NOK;
    }

    entry->isBackhaulSTA = LBD_TRUE;
    return LBD_OK;
}

LBD_BOOL stadbEntry_isBackhaulSTA(const stadbEntry_handle_t entry) {
    if (!entry) {
        return LBD_FALSE;
    }

    return entry->isBackhaulSTA;
}
...
#define mask_24g_5g     (( 1 << wlanif_band_24g ) | ( 1 << wlanif_band_5g ))
#define mask_24g_6g     (( 1 << wlanif_band_24g ) | ( 1 << wlanif_band_6g ))
#define mask_5g_6g      (( 1 << wlanif_band_5g ) | ( 1 << wlanif_band_6g ))
#define mask_24g_5g_6g  (( 1 << wlanif_band_24g ) |( 1 << wlanif_band_5g ) | ( 1 << wlanif_band_6g ))
    if (!entry) {
        return LBD_FALSE;
    }
    return ( !(entry->operatingBands ^ mask_24g_5g) |
             !(entry->operatingBands ^ mask_24g_6g) |
             !(entry->operatingBands ^ mask_5g_6g) |
             !(entry->operatingBands ^ mask_24g_5g_6g) );
}
...
LBD_STATUS stadbEntry_setActStatus(const stadbEntry_handle_t entry, LBD_BOOL active,
    ...
    if (actChanged) {
        *actChanged = entry->isAct != active ? LBD_TRUE : LBD_FALSE;
    }

    // Notify observers only when the activity status is
    // different from the existing one
    if (entry->isAct != active) {
        return stadbNotifyActChange(entry, bss, active);
    } else {
...
stadbEntry_handle_t stadbEntryCreate(const struct ether_addr *addr, LBD_BOOL inNetwork,
    ...
    if (entry) {
        ...
        entry->rcpiType = LBD_RCPI_TYPE_UNKNOWN;
        if (entryType != stadbEntryType_outOfNetwork) {
            ...
            entry->inNetworkInfo->assoc.freq = LBD_FREQ_INVALID;
...
stadbEntry_handle_t stadbEntryChangeEntryType(
    ...
    if (newEntry) {
        if (newEntry->entryType == stadbEntryType_outOfNetwork) {
            ...
            newEntry->inNetworkInfo->assoc.freq = LBD_FREQ_INVALID;
...
LBD_STATUS stadbEntryRecordRSSI(stadbEntry_handle_t entry,
    ...
    stadbEntryDiagLogRSSI(entry, bss, rssi);

    return LBD_OK;
}

/**
 * @brief Generate the diagnostic log for an RSSI measurement.
 *
 * This should only be used for actual measurements and not in cases where
 * the RSSI is estimated from other data.
 *
 * @param [in] entry  the STA for which the RSSI is being logged
 * @param [in] bss  the BSS on which the RSSI was measured
 * @param [in] rssi  the actual measurement
 */
static void stadbEntryDiagLogRSSI(const stadbEntry_handle_t entry,
                                  const lbd_bssInfo_t *bss, lbd_rssi_t rssi) {
...
LBD_STATUS stadbEntryMarkAssociated(stadbEntry_handle_t entry,
    ...
    if (isAssociated) {
        ...
        entry->inNetworkInfo->assoc.freq = bss->freq;
...
LBD_STATUS stadbEntry_setClientRcpiType(stadbEntry_handle_t entry, u_int8_t rcpiType) {
    ...
    // update sta entry with client Rcpi Type
    entry->rcpiType = rcpiType;

    return LBD_OK;
}

LBD_STATUS stadbEntry_setPendingBeaconReport(const stadbEntry_handle_t entry, u_int8_t pendingBcnRpt) {
    if (!entry) {
        return LBD_NOK;
    }

    entry->pendingBcnRpt = pendingBcnRpt;

    return LBD_OK;
}

LBD_STATUS stadbEntry_setMaxNumSelectedChannels(const stadbEntry_handle_t entry, u_int8_t maxNumSelectedChannels) {
    if (!entry) {
        return LBD_NOK;
    }

    entry->maxNumSelectedChannels = maxNumSelectedChannels;

    return LBD_OK;
}

LBD_STATUS stadbEntry_setPendingBeaconReportRequest(const stadbEntry_handle_t entry, u_int8_t pendingBcnRptReq) {

    if (!entry) {
        return LBD_NOK;
    }

    entry->pendingBcnRptReq = pendingBcnRptReq;
...
LBD_STATUS stadbEntry_setBeaconReportChannelsRequested(const stadbEntry_handle_t entry,
    ...
    size_t i = 0;
    for (i = 0; i < entry->maxNumSelectedChannels; i++) {
        entry->dot11kChannelsReq[i] = dot11kChannel[i];
        entry->dot11kFreqReq[i] = dot11kFreq[i];
    }
...
void stadbEntry_resetBeaconReport(stadbEntry_handle_t entry) {
    ...
    size_t i = 0;
    for (i = 0; i < 8; i++) { //entry->maxNumSelectedChannels; i++) {
        entry->dot11kChannelsReq[i] = 0;
        entry->dot11kFreqReq[i] = 0;
    }

    entry->pendingBcnRpt = 0;
    entry->bcnRpt.numBcnrpt = 0;
    entry->pendingBcnRptReq = 0;
    entry->maxNumSelectedChannels = 0;
}

u_int8_t stadbEntryUpdateBeaconReport(stadbEntry_handle_t entry, const wlanif_beaconReportEvent_t *bcnrptEvent) {
    size_t i = 0;
    u_int8_t pendingBcnRpt = 0;

    if (!entry) {
        return pendingBcnRpt;
    }

    stadbEntry_getPendingBeaconReport(entry, &pendingBcnRpt);
    if (pendingBcnRpt) {
        stadbEntry_setPendingBeaconReport(entry, --pendingBcnRpt);
    }

    if (bcnrptEvent->valid) {
        entry->bcnRpt.valid = bcnrptEvent->valid;
        lbCopyMACAddr(bcnrptEvent->sta_addr.ether_addr_octet, entry->bcnRpt.sta_addr.ether_addr_octet);
        for (i = 0; i < bcnrptEvent->numBcnrpt; i++) {
            lbCopyBSSInfo(&bcnrptEvent->reportedBcnrptInfo[i].reportedBss,
                          &entry->bcnRpt.reportedBcnrptInfo[entry->bcnRpt.numBcnrpt+i].reportedBss);
            entry->bcnRpt.reportedBcnrptInfo[entry->bcnRpt.numBcnrpt+i].rcpi = bcnrptEvent->reportedBcnrptInfo[i].rcpi;
        }
        entry->bcnRpt.numBcnrpt += bcnrptEvent->numBcnrpt;
    }

    return pendingBcnRpt;
}

LBD_STATUS stadbEntryRetrieveBeaconReport(stadbEntry_handle_t entry, wlanif_beaconReport_t *bcnrpt) {
    size_t i = 0;
    bcnrpt->valid = LBD_FALSE;

    if (!entry) {
        return LBD_NOK;
    }

    lbCopyMACAddr(entry->addr.ether_addr_octet, bcnrpt->sta_addr.ether_addr_octet);
    if (entry->bcnRpt.numBcnrpt) {
        for (i = 0; i < entry->bcnRpt.numBcnrpt; i++) {
            lbCopyBSSInfo(&entry->bcnRpt.reportedBcnrptInfo[i].reportedBss,
                          &bcnrpt->reportedBcnrptInfo[i].reportedBss);
            bcnrpt->reportedBcnrptInfo[i].rcpi = entry->bcnRpt.reportedBcnrptInfo[i].rcpi;
            bcnrpt->valid = LBD_TRUE;
        }
        bcnrpt->numBcnrpt = entry->bcnRpt.numBcnrpt;
        entry->bcnRpt.numBcnrpt = 0;
    }
...
LBD_STATUS stadbEntry_getNumBcnRptReceived(stadbEntry_handle_t entry, size_t *numBcnrpt) {
    if (!entry) {
        return LBD_NOK;
    }
    *numBcnrpt = entry->bcnRpt.numBcnrpt;
    return LBD_OK;
}
...
static void stadbEntrySetSupportedBand(stadbEntry_handle_t entry,
    ...
    entry->operatingBands |= 1 << band;
    if (stadbEntry_isInNetwork(entry) && !wasDualBand && stadbEntry_isDualBand(entry)) {
        if (diaglog_startEntry(mdModuleID_StaDB, stadb_msgId_dualBandUpdate, diaglog_level_demo)) {
            diaglog_writeMAC(&entry->addr);
        ...
        stadbEntry_bssStatsHandle_t servingBSS = stadbEntry_getServingBSS(entry, NULL);
        const lbd_bssInfo_t *bssInfo = stadbEntry_resolveBSSInfo(servingBSS);
        stadbEntryPopulateBSSesFromSameESS(entry, bssInfo, wlanif_band_invalid);
    }
}
...
LBD_BOOL stadbEntry_getClientRcpiType(const stadbEntry_handle_t entry, u_int8_t *clientRcpiType) {
    if (!entry) {
        return LBD_NOK;
    }

    *clientRcpiType = entry->rcpiType;

    if (*clientRcpiType > LBD_RCPI_TYPE_11KCOMPLIANT) {
        dbgf(NULL, DBGERR,
             "SONUNEXPECTED: %s: Client RcpiType:%d",__func__, *clientRcpiType);
        *clientRcpiType = LBD_RCPI_TYPE_UNKNOWN;
        return LBD_NOK;
    }
...
LBD_STATUS stadbEntry_getPendingBeaconReport(const stadbEntry_handle_t entry, u_int8_t *pendingBcnRpt) {
    if (!entry) {
        return LBD_NOK;
    }

    *pendingBcnRpt = entry->pendingBcnRpt;

    return LBD_OK;
}

LBD_STATUS stadbEntry_getMaxNumSelectedChannels(const stadbEntry_handle_t entry, u_int8_t *maxNumSelectedChannels) {
    if (!entry) {
        return LBD_NOK;
    }

    *maxNumSelectedChannels = entry->maxNumSelectedChannels;

    return LBD_OK;
}

LBD_STATUS stadbEntry_getPendingBeaconReportRequest(const stadbEntry_handle_t entry, u_int8_t *pendingBcnRptReq) {

    if (!entry) {
        return LBD_NOK;
    }

    *pendingBcnRptReq = entry->pendingBcnRptReq;
...
LBD_STATUS stadbEntry_getBeaconReportChannelsRequested(const stadbEntry_handle_t entry,
    ...
    size_t i = 0;
    for (i = 0; i < entry->maxNumSelectedChannels; i++) {
        dot11kChannel[i] = entry->dot11kChannelsReq[i];
        dot11kFreq[i] = entry->dot11kFreqReq[i];
    }
...
#define BSS_INFO_STR_LEN 40
#define ASSOC_STR_LEN (BSS_INFO_STR_LEN + 10) // BSS (age)
#define ACTIVITY_STR_LEN 25
...
void stadbEntryPrintSummary(const stadbEntry_handle_t entry,
    ...
    time_t curTime = stadbEntryGetTimestamp();
    cmdf(context, lbMACAddFmt(":") "        %-10u%c%c%c        ",
         lbMACAddData(entry->addr.ether_addr_octet),
         ...
         stadbEntry_isBandSupported(entry, wlanif_band_6g) ? '6' : ' ');

    if (!inNetwork) {
        cmdf(context, " %s  \n", entry->isSteeringDisallowed ? "Steer Disallowed" : "");
        return;
    }
    ...
    cmdf(context, "%s%s%s%s%s%s%s", entry->isBTMSupported ? "BTM " : "",
         entry->isRRMSupported ? "RRM " : "", entry->hasReservedAirtime ? "RA " : "",
         entry->isMUMIMOSupported ? "MU " : "", entry->isStaticSMPS ? "PS " : "",
         entry->isSteeringDisallowed ? "Steer Disallowed " : "",
         entry->isBackhaulSTA ? "bSTA " : "");

    u_int8_t clientClassGroup = 0;
    stadbEntry_getClientClassGroup(entry, &clientClassGroup);
    cmdf(context, "Class:%-3d", clientClassGroup);
    cmdf(context, "RcpiType:%-3d", entry->rcpiType);
    cmdf(context, "\n");
}
...
LBD_STATUS stadbEntry_setFullCapacities(stadbEntry_handle_t entry,
                                        ...
                                        lbd_linkCapacity_t ulCapacity,
                                        lbd_linkCapacity_t dlCapacity) {
    if (!entry || !stadbEntry_isInNetwork(entry) || !bssHandle) {
        return LBD_NOK;
    }
    lbDbgAssertExit(NULL, bssHandle->valid);

    time_t curTime = stadbEntryGetTimestamp();
    if (dlCapacity != LBD_UNKNOWN_LINK_CAP) {
        bssHandle->downlinkInfo.fullCapacity = dlCapacity;
        bssHandle->downlinkInfo.lastUpdateSecs = curTime;
    }
    if (ulCapacity != LBD_UNKNOWN_LINK_CAP) {
        bssHandle->uplinkInfo.fullCapacity = ulCapacity;
        bssHandle->uplinkInfo.lastUpdateSecs = curTime;
    }

    if (diaglog_startEntry(mdModuleID_StaDB,
                           stadb_msgId_fullCapacitiesUpdate,
                           diaglog_level_info)) {
        const lbd_bssInfo_t *bss = stadbEntry_resolveBSSInfo(bssHandle);
        lbDbgAssertExit(NULL, bss);

        diaglog_writeMAC(&entry->addr);
        diaglog_writeBSSInfo(bss);
        diaglog_write16(bssHandle->uplinkInfo.fullCapacity);
        diaglog_write16(bssHandle->downlinkInfo.fullCapacity);
        diaglog_finishEntry();
    }
...
LBD_STATUS stadbEntry_setUplinkRSSI(stadbEntry_handle_t entry,
    ...
    if (!estimated) {
        ...
        // Only generate the log for an actual measurement and not for an
        // estimate
        stadbEntryDiagLogRSSI(entry, bss, rssi);
    }
    return LBD_OK;
...
LBD_STATUS stadbEntry_setUplinkRSSIWithAge(stadbEntry_handle_t entry,
    ...
    if (LBD_OK != stadbEntry_setUplinkRSSI(entry, bssHandle, rssi, LBD_FALSE)) {
        return LBD_NOK;
    }
    time_t curTime = stadbEntryGetTimestamp();
    if (rssiAge > curTime) {
        bssHandle->uplinkInfo.lastUpdateSecs = 0;
    } else {
        bssHandle->uplinkInfo.lastUpdateSecs = curTime - rssiAge;
    }
    return LBD_OK;
}
...
static LBD_BOOL stadbEntryUpdateBetterPHYCapInfo(wlanif_phyCapInfo_t *oldPHYCap,
    ...
    if (!oldPHYCap->valid) {
        *oldPHYCap = *newPHYCap;
        ...
        if (newPHYCap->mode) {
            oldPHYCap->mode = newPHYCap->mode;

            if (newPHYCap->mode | wlanif_phymode_ht) {
                oldPHYCap->htCapInfo = newPHYCap->htCapInfo;
                changed = LBD_TRUE;
            }

            if (newPHYCap->mode | wlanif_phymode_vht) {
                oldPHYCap->vhtCapInfo = newPHYCap->vhtCapInfo;
                changed = LBD_TRUE;
            }

            if (newPHYCap->mode | wlanif_phymode_he) {
                oldPHYCap->heCapInfo = newPHYCap->heCapInfo;
                changed = LBD_TRUE;
            }
        }
    }
...
static LBD_BOOL stadbEntryUpdateBandPHYCapInfo(stadbEntry_handle_t entry,
    ...
    if (stadbEntryUpdateBetterPHYCapInfo(oldPHYCap, newPHYCap)) {
        ...
        // Generate a diagnostic log of the capabilities now.
        stadbEntryDiagLogBandPHYCapInfo(entry, band);
    }
    ...
    for (i = 0; i < entry->inNetworkInfo->numBSSStats; ++i) {
        ...
        if (bssStats->valid && bssStats->bss.vap == vap) {
            ...
            if (bssStats == entry->inNetworkInfo->assoc.bssHandle) {
                ...
                entry->inNetworkInfo->assoc.freq = freq;
            }
            break;
        }
    }
...
static void stadbEntryDiagLogBandPHYCapInfo(stadbEntry_handle_t entry,
                                            wlanif_band_e band) {
    if (diaglog_startEntry(mdModuleID_StaDB, stadb_msgId_bandPHYCapUpdate,
                           diaglog_level_info)) {
        const wlanif_phyCapInfo_t *phyCap = &entry->inNetworkInfo->phyCapInfo[band];
        diaglog_writeMAC(&entry->addr);
        diaglog_write8(band);
        diaglog_write8(phyCap->maxChWidth);
        diaglog_write8(phyCap->numStreams);
        diaglog_write8(phyCap->phyMode);
        diaglog_write8(phyCap->maxMCS);
        diaglog_write8(phyCap->maxTxPower);
        diaglog_finishEntry();
    }
}
...
void stadbEntryPopulateBSSesFromSameESS(stadbEntry_handle_t entry,
    ...
    supported_band = stadbEntry_isBandSupported(entry, wlanif_band_6g) << wlanif_band_6g |
                     stadbEntry_isBandSupported(entry, wlanif_band_5g) << wlanif_band_5g;

    if (LBD_NOK == wlanif_getBSSesSameESS(servingBSS, lastServingESS,
                                          band, &maxNumBSSes, bss, supported_band) ||
        !maxNumBSSes) {
...
wlanif_band_e stadbEntry_resolveBandFromBSSStats(
    ...
    return wlanif_resolveBandFromFreq(bssHandle->bss.freq);
}
...
static stadbEntry_bssStatsHandle_t stadbEntryFindSlotForBSSStats(
    ...
    for (i = 0; i < entry->inNetworkInfo->numBSSStats; ++i) {
        ...
        if (stadbEntryIsSameServingAP(entry, &bssEntry->bss)) {
            numServing++;
            if (wlanif_resolveBandFromFreq(
                    bssEntry->bss.freq) == band &&
...
LBD_STATUS stadbEntryUpdateOpMode(stadbEntry_handle_t entry,
    ...
    wlanif_band_e band = wlanif_resolveBandFromFreq(servingBSSInfo->freq);
    lbDbgAssertExit(NULL, band < wlanif_band_invalid);
...
LBD_STATUS stadbEntryMarkGivenBandSupported(stadbEntry_handle_t entry, u_int8_t bandCap) {
    ...
    if (bandCap & (1 << wlanif_band_6g)) {
        stadbEntrySetSupportedBand(entry, wlanif_band_6g);
    }
    stadbEntryUpdateTimestamp(entry);
    return LBD_OK;
}
...
LBD_STATUS stadbEntryPopulateNonServingPHYInfo(stadbEntry_handle_t entry,
    ...
    wlanif_band_e servingBand, nonServingBand, supported_band;
    ...
    servingBand = wlanif_resolveBandFromFreq(servingBSS->freq);
    if (servingBand == wlanif_band_invalid) {
        return LBD_NOK;
    }

    if (stadbEntry_isBandSupported(entry, wlanif_band_6g)) {
        if ( servingBand == wlanif_band_24g || servingBand == wlanif_band_5g ) {
            nonServingBand = wlanif_band_6g;
        } else{
            nonServingBand =
              stadbEntry_isBandSupported(entry, wlanif_band_5g) ? wlanif_band_5g :
                                                                  wlanif_band_24g;
    ...
    if (nonServingBand == supported_band ) {
