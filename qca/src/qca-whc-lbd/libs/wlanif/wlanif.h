...
#define WLANIF_6G_START_FREQ 5950
#define WLANIF_6G_END_FREQ 7125

#define WLANIF_5G_START_FREQ 5180
#define WLANIF_5G_END_FREQ 5945

#define WLANIF_2G_START_FREQ 2412
#define WLANIF_2G_END_FREQ 2462
...
#define WLANIF_MAX_OPERATING_CLASSES IEEE1905_MAX_OPERATING_CLASSES

#define WLANIF_MAX_CHANNELS_PER_OP_CLASS IEEE1905_MAX_CHANNELS_PER_OP_CLASS

#define WLANIF_MAX_SSID_LEN IEEE1905_MAX_SSID_LEN

#define WLANIF_MAX_RADIOS 4  // looking forward to tri-radio platforms

#define wlanif_heCapInfo_t ieee1905APHeCapabilities_t
...
typedef enum wlanif_event_e {
    ...
    wlanif_event_radar_detected,     /// radar detected

    wlanif_event_retry_hmwds_programming, /// retry hmwds programming
...
typedef struct wlanif_phyCapInfo_t {
    ...
    /// Flag to indicate modes supported by this STA for DE
    u_int8_t mode;

    /// HT Capability Info of the STA for DE
    u_int16_t htCapInfo;

    /// VHT Capability Info of the STA for DE
    u_int32_t vhtCapInfo;

    /// HE Capability Info of the STA for DE
    wlanif_heCapInfo_t heCapInfo;

    /// Capture station assoc time
    u_int32_t assoc_time;
} wlanif_phyCapInfo_t;

// Min number of spatial streams supported
#define WLANIF_MIN_NSS_SUPPORT 1
...
typedef struct wlanif_radarEvent_t {
    /// frequency of radar detection
    u_int16_t freq;
    /// index of interface
    int sysIndex;
} wlanif_radarEvent_t;
...
/// Maximum number of beacon reports for a sta
#define WLANIF_MAX_NUM_BCNRPT_PER_STA 24

/**
 * @brief The format of the wlanif_event_beacon_report event.
 */
typedef struct wlanif_beaconReport_t {
    /// Flag indicating if the beacon report is valid or not
    LBD_BOOL valid;
    ...
    struct {
        ...
        /// The RCPI measurement reported
        int8_t rcpi;
    } reportedBcnrptInfo[WLANIF_MAX_NUM_BCNRPT_PER_STA];

    /// Number of beacon report info contained in this event
    size_t numBcnrpt;
} wlanif_beaconReport_t;
 ...
 */
typedef ieee1905BcnMetricsQuery_t wlanif_bcnMetricsQuery_t;
...
/**
 * @brief Resolve band from frequency number
 *
 * Only consider 2.4 GHz, 5 GHz, 6GHz band for now.
 *
 * @param [in] channum  the frequency number
 *
 ...
 * @param [in] query  structure to create IE for beacon request
 * @param [in] bss  the BSS that is serving the STA
 * @param [in] clientClassGroup client classification group the client belongs to
 * @param [in] useBeaconTable  whether measurements should be done using
 *                             beacon table mode or not
