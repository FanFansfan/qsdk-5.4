...
void wlanifLinkEventsCmnProcessChannelChange(
        ...
        wlanif_band_e band, int sysIndex, u_int16_t freq) {
    ...
    if (wlanifBSteerControlUpdateChannel(state->bsteerControlHandle,
                                         band,
                                         sysIndex,
                                         ((const struct iw_event *)event)->u.freq.m,
                                         freq) != LBD_OK) {
         dbgf(state->dbgModule, DBGERR,
 ...
 *                       generated
 */
static void wlanifLinkEventsCmnHandleIWEvent(wlanifLinkEventsHandle_t state, const u_int8_t *data,
                                             u_int32_t dataLen, wlanif_band_e band, int sysIndex) {
        ...
        switch (event.cmd) {
            ...
            case IWEVEXPIRED:
            {
                if (event.len == IW_EV_LCP_LEN + sizeof(event.u)) {
                    ...
                    wlanifLinkEventsCmnGenerateDisassocEvent(state, &event, sysIndex);
                } else {
                    dbgf(state->dbgModule, DBGERR, "%s: Invalid event length %u (expected %tu)",
            ...
            case IWEVCUSTOM:
            {
                ...
                if (event.len >= IW_EV_POINT_LEN) {
                    memcpy(&event.u.data.length, data + IW_EV_LCP_LEN,
                           ...
                           (((char *) &event.u.data.length) - ((char *) &event)));

                    switch (event.u.data.flags) {
                        case IEEE80211_EV_CHAN_CHANGE:
                            if (event.u.data.length == sizeof(struct chan_change_evt_t) &&
                                data + IW_EV_POINT_LEN + event.u.data.length <= dataEnd) {
                                struct chan_change_evt_t chan_det = {0};

                                memcpy(&chan_det, data + IW_EV_POINT_LEN, event.u.data.length);
                                event.u.freq.m = chan_det.channel;
                                wlanifLinkEventsCmnProcessChannelChange(state, &event, band, 
                                                                        sysIndex, chan_det.freq);
                                wlanifLinkEventsProcessChannelChange(sysIndex,
                                                                     (u_int8_t)chan_det.channel,
                                                                     chan_det.freq);
                            }
                            break;
                        case IEEE80211_EV_RADAR_DETECTED:
                            if (event.u.data.length == sizeof(u_int16_t) &&
                                data + IW_EV_POINT_LEN + event.u.data.length <= dataEnd) {
                                wlanif_radarEvent_t radar;
                                u_int16_t freq;

                                memcpy(&freq, data + IW_EV_POINT_LEN, event.u.data.length);
                                radar.freq = freq;
                                radar.sysIndex = sysIndex;
                                mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_Low,
                                              wlanif_event_radar_detected, &radar,
                                              sizeof(wlanif_radarEvent_t));
                            }
                            break;
                        ...
                        default:
                            dbgf(state->dbgModule, DBGERR,
                                 ...
                                 "remaining=%tu)",
                                 __func__, event.u.data.length, dataEnd - (data + IW_EV_POINT_LEN));
                    } // flags 
                } // valid length
            } // custom
            break;
            default:
            {
                ...
                dbgf(state->dbgModule, DBGERR, "%s: Unhandled event %u len %u", __func__, event.cmd,
                     event.len);
            }
                break;
        }
...
static void wlanifLinkEventsCmnHandleNewLink(wlanifLinkEventsHandle_t state,
    ...
    u_int8_t ifname[IFNAMSIZ + 1];
    LBD_BOOL isRadio = LBD_FALSE;
    if (wlanif_band_invalid == band) {
        if((if_indextoname (ifaceInfo->ifi_index, (char*)ifname) != NULL) &&
           (wlanifBSteerControlIsRadioResolved(state->bsteerControlHandle, ifname) == LBD_OK)) {
                isRadio = LBD_TRUE;
        }
        else {
    ...
    if (wlanifBSteerControlIsIncludedIface(state->bsteerControlHandle,
                                           ifaceInfo->ifi_index) == LBD_NOK) {
        if(isRadio == LBD_TRUE) {
            // Do nothing, valid event.
        }
        else {
            dbgf(state->dbgModule, DBGDUMP,
