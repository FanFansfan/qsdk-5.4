...
wlanif_band_e wlanif_resolveBandFromFreq(u_int16_t freq) {

    if ( freq >= WLANIF_6G_START_FREQ && freq <= WLANIF_6G_END_FREQ )
        return wlanif_band_6g;
    else if ( freq >= WLANIF_5G_START_FREQ && freq <= WLANIF_5G_END_FREQ )
        return wlanif_band_5g;
    else if ( freq >= WLANIF_2G_START_FREQ && freq <= WLANIF_2G_END_FREQ )
