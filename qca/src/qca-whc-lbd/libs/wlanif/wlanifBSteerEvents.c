...
LBD_STATUS wlanifBSteerEventsHandleRemoteAssocUpdate(
    ...
    dbgf(state->dbgModule, DBGINFO,
         "%s: Node " lbMACAddFmt(":") " %s on remote " lbBSSInfoAddFmt(), __func__,
         lbMACAddData(staAddr->ether_addr_octet),
         isAssoc ? "associated" : "disassociated", lbBSSInfoAddData(&assocBSS));
...
static void wlanifBSteerEventsHandleNodeAssociatedInd(
    ...
    if( event->data.bs_node_associated.datarate_info.num_streams < WLANIF_MIN_NSS_SUPPORT ) {
        dbgf(state->dbgModule, DBGINFO,
             "%s: Mark the client as 11G because receving invalid Num of spatial streams from client",
             __func__);
        assocEvent.phyCapInfo.numStreams =  WLANIF_MIN_NSS_SUPPORT;
    }
...
static void wlanifBSteerEventsHandleRSSIMeasurementInd(
    ...
    if(event->data.bs_rssi_measurement.rssi <= 0)
        dbgf(state->dbgModule, DBGERR,
             "SONUNEXPECTED: %s: " lbMACAddFmt(":") " RSSI measurement %u on " lbBSSInfoAddFmt(),
             __func__, lbMACAddData(event->data.bs_rssi_measurement.client_addr),
             event->data.bs_rssi_measurement.rssi, lbBSSInfoAddData(bss));
 ...
 */
static void wlanifVendorCFGEventsHandleBeaconReport(
        wlanifBSteerControlHandle_t state,
        const lbd_bssInfo_t *receivingBss,
        const struct ev_rrm_report_data *report) {
    ...
    if (report->mode != IEEE80211_RRM_MEASRPT_MODE_SUCCESS) {
        bcnrptEvent.valid = LBD_FALSE;
        generateEvent = LBD_TRUE;
    } else {
        int i = 0;
        bcnrptEvent.valid = LBD_TRUE;
        ieee80211_bcnrpt_t *bcnrpt = (ieee80211_bcnrpt_t *)report->bcnrpt;
        while (i < IEEE80211_RRM_NUM_BCNRPT_MAX) {
            ...
            if (wlanifBSteerControlGetBSSInfoFromBSSID(
                ...
                dbgf(state->dbgModule, DBGINFO, "%s: Beacon report from " lbMACAddFmt(":")
                     ": " lbBSSInfoAddFmt() " rcpi: (0x%x) %d numBcnrpt %zu", __func__,
                     lbMACAddData(bcnrptEvent.sta_addr.ether_addr_octet),
                     lbBSSInfoAddData(&reportedBss), bcnrpt->rcpi, (int8_t)bcnrpt->rcpi, bcnrptEvent.numBcnrpt);
            } else {
 ...
 */
 void wlanifVendorCFGEventsHandleRRMReportInd(
        ...
        lbd_bssInfo_t *bss) {
    const struct ev_rrm_report_data *report = &event->data.rrm_report;
    switch (report->rrm_type) {
        case BSTEERING_RRM_TYPE_BCNRPT:
            wlanifVendorCFGEventsHandleBeaconReport(state, bss, report);
            break;
        default:
...
void wlanifVendorCFGEventsHandleWNMEvent(
    ...
    type = (u_int8_t *)event;
// Do below operation to know event is of type BTM Response/Query
    type += 2*sizeof(int);
    ...
    switch (*type) {
        case BSTM_RESP_DATA:
            dbgf(state->dbgModule, DBGINFO,
                 ...
                 __func__, lbMACAddData(event->data.wnm_response.peer_mac),
                 event->data.wnm_response.token,
                 event->data.wnm_response.status,
                 event->data.wnm_response.reject_code,
                 event->data.wnm_response.termination_delay,
                 lbMACAddData(event->data.wnm_response.target_mac));

            wlanif_btmResponseEvent_t resp;
            resp.bss = *bss;
            resp.dialog_token = event->data.wnm_response.token;
            lbCopyMACAddr(event->data.wnm_response.peer_mac,
                          resp.sta_addr.ether_addr_octet);
            resp.status = event->data.wnm_response.status;
            resp.reject_code = event->data.wnm_response.reject_code;
            resp.termination_delay = event->data.wnm_response.termination_delay;
            lbCopyMACAddr(event->data.wnm_response.target_mac,
        ...
        case BSTM_QUERY_DATA:
            dbgf(state->dbgModule, DBGINFO,
                 "%s: Received BTM Query from " lbMACAddFmt(":")
                 ": Dialog Token: %u, Reason: %u ",
                 __func__, lbMACAddData(event->data.wnm_query.peer_mac),
                 event->data.wnm_query.token,
                 event->data.wnm_query.reason);

            wlanif_btmQueryEvent_t query;
            query.dialog_token = event->data.wnm_query.token;
            lbCopyMACAddr(event->data.wnm_query.peer_mac,
                          query.sta_addr.ether_addr_octet);
            query.reason = event->data.wnm_query.reason;
...
#endif
        default:
            dbgf(state->dbgModule, DBGINFO, "%s: Unhandled WNM msg \n",
                 __func__);
             break;
    }
}
...
void wlanifVendorCFGEventsHandleSMPSUpdateInd(
    ...
    ath_vendorcfg_event_t *event,
    lbd_bssInfo_t *bss) {
    struct ieee80211_smps_update_data *update =
        (struct ieee80211_smps_update_data *)&event->data.smps_update;

    dbgf(state->dbgModule, DBGINFO,
         "%s: SM Power Save mode received from " lbMACAddFmt(":") " on "
         lbBSSInfoAddFmt() ": %sSTATIC", __func__,
         lbMACAddData(update->macaddr), lbBSSInfoAddData(bss),
         update->is_static ? "" : "NOT ");

    wlanif_smpsUpdateEvent_t smpsEvent;
    lbCopyMACAddr(update->macaddr, smpsEvent.sta_addr.ether_addr_octet);
 ...
 */
 void wlanifVendorCFGEventsHandleOpModeUpdateInd(
   ...
   struct ieee80211_opmode_update_data *update =
        (struct ieee80211_opmode_update_data *)&event->data.opmode_update;

    dbgf(state->dbgModule, DBGINFO,
         "%s: Operating Mode Notification received from " lbMACAddFmt(":")
         " on " lbBSSInfoAddFmt() ", Max bandwidth: %u, Num of spatial streams: %u",
         __func__, lbMACAddData(update->macaddr), lbBSSInfoAddData(bss),
         update->max_chwidth, update->num_streams);

    wlanif_opmodeUpdateEvent_t opModeEvent;
    lbCopyMACAddr(update->macaddr, opModeEvent.sta_addr.ether_addr_octet);
    lbCopyBSSInfo(bss, &opModeEvent.bss);
    opModeEvent.maxChWidth = wlanifMapToBandwidth(state->dbgModule,
                                                  update->max_chwidth);
    opModeEvent.numStreams = update->num_streams;


   mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_High,
                  wlanif_event_opmode_update, &opModeEvent,
                           sizeof(wlanif_opmodeUpdateEvent_t));
    return;
}
