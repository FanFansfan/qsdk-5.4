...
static void stamonCmnHandleStaTputEstComplete(struct mdEventNode *event);
...
static struct profileElement stamonCmnElementDefaultTable[] = {
    ...
    { STAMON_RSSI_MEASUREMENT_NUM_SAMPLES_W6_KEY, "5" },
    { STAMON_AGE_LIMIT_KEY,                       "5" },
    { STAMON_LEGACY_CLIENT_AGE_LIMIT_KEY,         "20"},
    ...
    { STAMON_LOW_AP_STEER_RSSI_XING_THRESHOLD_W6_KEY,    "0"},
    { STAMON_DISABLE_STEERING_INACTIVE_LEGACY_CLIENTS,   "1"},
    { STAMON_DISABLE_STEERING_ACTIVE_LEGACY_CLIENTS,     "1"},
    { STAMON_DISABLE_STEERING_11K_UNFRIENDLY_CLIENTS,    "1"},
    { NULL, NULL }
};
...
LBD_STATUS stamon_init(void) {
    ...
    stamonState.config.instRSSINumSamples[wlanif_band_5g] =
        profileGetOptsInt(mdModuleID_StaMon,
                          ...
                          stamonCmnElementDefaultTable);
    stamonState.config.instRSSINumSamples[wlanif_band_6g] =
        profileGetOptsInt(mdModuleID_StaMon,
                          STAMON_RSSI_MEASUREMENT_NUM_SAMPLES_W6_KEY,
                          stamonCmnElementDefaultTable);
    stamonState.config.freshnessLimit =
        profileGetOptsInt(mdModuleID_StaMon,
                          STAMON_AGE_LIMIT_KEY,
                          stamonCmnElementDefaultTable);
    stamonState.config.legacyClientFreshnessLimit =
        profileGetOptsInt(mdModuleID_StaMon, STAMON_LEGACY_CLIENT_AGE_LIMIT_KEY,
                          stamonCmnElementDefaultTable);
    ...
    if (profileGetOptsIntArray(mdModuleID_StaMon,
        STAMON_LOW_AP_STEER_RSSI_XING_THRESHOLD_W6_KEY,
        stamonCmnElementDefaultTable, value) == LBD_NOK) {
        dbgf(stamonState.dbgModule, DBGERR,
             "[Band %u] Unable to parse %s",
             wlanif_band_6g, STAMON_LOW_AP_STEER_RSSI_XING_THRESHOLD_W6_KEY);
        return LBD_NOK;
    }

    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        stamonState.config.apSteerLowRSSIXingThresholds[wlanif_band_6g][index] = value[index];
    }

    stamonState.config.disableSteeringInactiveLegacyClients = profileGetOptsInt(
        mdModuleID_StaMon, STAMON_DISABLE_STEERING_INACTIVE_LEGACY_CLIENTS,
        stamonCmnElementDefaultTable);

    stamonState.config.disableSteeringActiveLegacyClients = profileGetOptsInt(
        mdModuleID_StaMon, STAMON_DISABLE_STEERING_ACTIVE_LEGACY_CLIENTS,
        stamonCmnElementDefaultTable);

    stamonState.config.disableSteering11kUnfriendlyClients = profileGetOptsInt(
        mdModuleID_StaMon, STAMON_DISABLE_STEERING_11K_UNFRIENDLY_CLIENTS,
        stamonCmnElementDefaultTable);
    ...
    mdListenTableRegister(mdModuleID_Estimator, estimator_event_staTputEstComplete,
                          stamonCmnHandleStaTputEstComplete);
...
static void stamonCmnTriggerActivityOrSteering(stadbEntry_handle_t entry,
    ...
    stamonMakeLegacyAPSteerDecision(entry);
    ...
    if (isActive) {
        ...
        if (activityUpdate) {
            // Triggered by STA becoming idle (transition from active to idle)
            estimator_abortLegacySteerMetrics(entry);
        }
...
static void stamonCmnHandleTxRateXing(struct mdEventNode *event) {
    ...
    stadbEntry_handle_t entry = stadb_find(&xing->sta_addr);
    ...
    wlanif_band_e band = wlanif_resolveBandFromFreq(xing->bss.freq);
    ...
    if (  stadbEntry_isBandSupported(entry, wlanif_band_6g) ) {
        if (((band == wlanif_band_6g) && (xing->xing != wlanif_xing_down)) ||
            ((band == wlanif_band_5g) && (xing->xing != wlanif_xing_up)) ||
...
static void stamonCmnMakeSteerDecisionActive(stadbEntry_handle_t entry) {
    ...
    do {
        ...
        if (LBD_OK == stadbEntry_getPolluted(entry, servingBSS, &polluted, NULL) &&
            ...
            stamonCmnTriggerActiveSteering(entry, staAddr,
                                           steerexec_reason_interferenceAvoidance);
            break;
        }
        ...
        stamonMakeSteerDecisionActive(entry, staAddr, band, &staStats, rateEligibility);
    } while (0);
}

/**
 * @brief React to STA stats completion event
 */
static void stamonCmnHandleStaTputEstComplete(struct mdEventNode *event) {
    const estimator_staTputEstCompleteEvent_t *info =
        (const estimator_staTputEstCompleteEvent_t *)event->Data;
    stadbEntry_handle_t entry = stadb_find(&info->staAddr);
    if (!entry) {
        dbgf(stamonState.dbgModule, DBGERR,
             "%s: Received STA throughput estimate complete event from unknown MAC "
             "address: " lbMACAddFmt(":"),
             __func__, lbMACAddData(info->staAddr.ether_addr_octet));
        return;
    }

    LBD_BOOL isActive;
    if (stadbEntry_getActStatus(entry, &isActive, NULL) == LBD_OK && isActive) {
        stamonCmnMakeSteerDecisionActive(entry);
    }
}
...
LBD_STATUS stamonCmnGetUplinkRSSI(stadbEntry_handle_t entry,
    ...
    time_t ageLimit = 0xFFFFFFFF;
    u_int8_t probeCount = 0;
    if (!stadbEntry_isRRMSupported(entry)) {
        // Legacy client
        ageLimit = stamonState.config.legacyClientFreshnessLimit;
    } else {
        ageLimit = stamonState.config.freshnessLimit;
    }
    lbd_rssi_t rssi = stadbEntry_getUplinkRSSI(entry, servingBSS, &ageSecs, &probeCount);
    if (rssi == LBD_INVALID_RSSI || ageSecs > ageLimit || probeCount) {
        ...
        // Request RSSI only if the serving AP is self otherwise return LBD_NOK
        if (bss->apId == LBD_APID_SELF) {
            wlanif_band_e associatedBand =
                wlanif_resolveBandFromFreq(bss->freq);
            if (LBD_NOK == wlanif_requestStaRSSI(
                               bss, staAddr,
...
void stamonCmnTriggerActiveSteering(stadbEntry_handle_t entry,
    ...
    if (LBD_NOK == estimator_estimateSTADataMetrics(entry, trigger)) {
        dbgf(stamonState.dbgModule, DBGINFO,
             "%s: Failed to collect metrics for " lbMACAddFmt(":"), __func__,
