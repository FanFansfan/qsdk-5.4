 ...
 * to steerexec_steer
 */
#define STEEREXEC_MAX_CANDIDATES (WLANIF_MAX_RADIOS - 1)

/**
 * Maximum number of channels that can be provided in a call
 * to steerexec_allowAssoc
 */
#define STEEREXEC_MAX_ALLOW_ASSOC (WLANIF_MAX_RADIOS)
...
typedef enum steerexec_reason_e {
    ...
    // Legacy AP Steering while active
    steerexec_reason_activeLegacyAPSteering,

    // Legacy AP Steering while idle
    steerexec_reason_idleLegacyAPSteering,
...
#define STEEREXEC_TARGET_LOW_RSSI_THRESHOLD_W6_KEY "TargetLowRSSIThreshold_W6"
