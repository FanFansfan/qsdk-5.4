...
#undef QCA_MOD_INPUT
#define QCA_MOD_INPUT QCA_MOD_LBD_STEEREXEC
...
typedef struct steerexecImplCmnSteeringStateLegacy_t {
    ...
    lbd_channelId_t disabledChannelList[WLANIF_MAX_RADIOS];

    /// Set of disabled frequency (used for pre-assoc steering)
    u_int16_t disabledFreqList[WLANIF_MAX_RADIOS];
...
static const char *steerexec_SteeringReasonString[] = {
    ...
    "ActiveLegacyAPSteering",
    "IdleLegacyAPSteering",
    "Invalid"
};
...
static LBD_STATUS steerexecImplCmnChannelDelta(
    ...
    u_int16_t *enabledFreqList,
    u_int8_t *disabledChannelCount,
    lbd_channelId_t *disabledChannelList,
    u_int16_t *disabledFreqList );
...
static void steerexecImplCmnUpdateChannelSet(
    ...
    const u_int16_t *disabledFreqList);
...
LBD_STATUS steerexecImplCmnAllowAssoc(
    ...
    if (channelCount > STEEREXEC_MAX_ALLOW_ASSOC || !channelCount || !channelList || !freqList) {
        return LBD_NOK;
    }
    ...
    if (steerexecImplCmnReconcileSteerChannel(
        ...
        channelCount, channelList, freqList, &willSteer) != LBD_OK) {
        return LBD_NOK;
    }
...
LBD_STATUS steerexecImplCmnSteer(steerexecImplCmnHandle_t exec,
    ...
    if ( stadbEntry_isBandSupported(entry, wlanif_band_6g) ) {
        blacklistAutoClear =
          wlanif_resolveBandFromFreq(candidateList[0].freq) == wlanif_band_6g ? LBD_TRUE :
                                                                                LBD_FALSE;
    } else {
        blacklistAutoClear =
         wlanif_resolveBandFromFreq(candidateList[0].freq) == wlanif_band_5g ? LBD_TRUE :
                                                                               LBD_FALSE;
    }
...
steerexec_steerEligibility_e steerexecImplCmnDetermineSteeringEligibility(
    ...
    if (stadbEntry_isSteeringDisallowed(entry) ||
        LBD_INVALID_AIRTIME != stadbEntry_getReservedAirtime(entry, stats)) {
        return steerexec_steerEligibility_none;
    }
...
static LBD_STATUS steerexecImplCmnTimeoutBlacklistChannel(
    ...
    for (i = channelsToChange - 1; i >= 0 ; i--) {
        ...
        if (bandmon_isChannelOverloaded(state->legacy.disabledChannelList[i],
                                        state->legacy.disabledFreqList[i],
                                        &isOverloaded) != LBD_OK) {
            dbgf(exec->dbgModule, DBGERR,
        ...
        if (i != state->legacy.disabledChannelCount) {
            ...
            memmove(&state->legacy.disabledFreqList[i],
                    &state->legacy.disabledFreqList[i+1],
                    (state->legacy.disabledChannelCount - i) *
                    sizeof(state->legacy.disabledFreqList[0]));
        }
    }
...
static LBD_STATUS steerexecImplCmnEnableAllDisabledChannels(
    ...
    if (wlanif_setChannelStateForSTA(state->legacy.disabledChannelCount,
                                     ...
                                     &state->legacy.disabledFreqList[0],
                                     staAddr,
                                     lastServingESS,
                                     LBD_TRUE /* enable */) != LBD_OK) {
        dbgf(state->context->dbgModule, DBGERR,
        ...
        u_int16_t newfreqList[WLANIF_MAX_RADIOS];
...
static LBD_STATUS steerexecImplCmnRemoveAllBlacklists(
    ...
    LBD_BOOL wasBlacklisted = LBD_FALSE;
    ...
    if (state->candidateCount ||
        ...
        if (state->candidateCount >
            (sizeof(state->candidateList) / sizeof(state->candidateList[0]))) {
            // This case should generally not happen and could possibly even
            // be an assert rather than a conditional check.
            status = LBD_NOK;
        } else {
    ...
    // Only if all possible removals were successful should we mark the STA as
    // no longer blacklisted.
    if (status == LBD_OK && wasBlacklisted) {
        steerexecImplCmnMarkAsNotBlacklisted(state);
    }
...
static void steerexecImplCmnLowRSSIObserver(stadbEntry_handle_t entry, void *cookie) {
    ...
    if(state->state == steerexecImplCmn_state_waiting_response) {
        // BTM is in progress dont clear blacklist
        return;
    }
...
static LBD_BOOL steerexecImplCmnIsTargetChannelRSSIOK(
    ...
    wlanif_band_e targetBand =
        wlanif_resolveBandFromFreq(freq);
...
static LBD_BOOL steerexecImplCmnIsTargetRSSIOKChannel(
    ...
    u_int16_t wlanifFreqList[WLANIF_MAX_RADIOS];

    // Get the set of channels from the radio
    if (steerexecImplCmnGetAndValidateRadioChannelList(state,
                                                       &wlanifChannelCount,
                                                       &wlanifChannelList[0],
                                                       &wlanifFreqList[0]) != LBD_OK) {
        return LBD_FALSE;
    }
    ...
    params.enabledChannelCount = steerexecImplCmnCopyAllNotOnList(
        ...
        state->legacy.disabledChannelCount,
        &state->legacy.disabledChannelList[0],
        &params.enabledChannelList[0], NULL, NULL );
...
static void steerexecImplCmnHandleAssocPreAssoc(
    ...
    wlanif_band_e assocBand = wlanif_resolveBandFromFreq(assocBSS->freq),
                  supported_band;
...
static void steerexecImplCmnUpdateBlacklistOnSteerCompletion(
    ...
    for (i = 0; i < blacklistCount; i++) {
        if (blacklist[i].channelId == channelToEnable ||
            wlanif_resolveBandFromFreq(blacklist[i].freq) == wlanif_band_24g) {
...
static LBD_STATUS steerexecImplCmnGetAndValidateRadioChannelList(
    ...
    *channelCount = wlanif_getChannelList(channelList, NULL /* chwidthList */,freqList,
                                          WLANIF_MAX_RADIOS);
...
static u_int8_t steerexecImplCmnCopyAllNotOnList(
    ...
    for (i = 0; i < count1; i++) {
        if (!steerexecImplCmnIsOnChannelList(count2, list2, list1[i])) {
            ...
            if ( FreqList && outFreqList ){
                outFreqList[outCount] = FreqList[i];
            }
            outCount++;
        }
    }
...
static void steerexecImplCmnResetChannelList(
    ...
    const u_int16_t *updatedFreqList,
    u_int8_t *enabledChannelCount,
    lbd_channelId_t *enabledChannelList,
    u_int16_t *enabledFreqList,
    u_int8_t *disabledChannelCount,
    lbd_channelId_t *disabledChannelList,
    const u_int16_t *radioFreqList,
    u_int16_t *disabledFreqList ) {
    ...
    *enabledChannelCount = updatedChannelCount;
    memcpy(enabledChannelList, &updatedChannelList[0],
           updatedChannelCount * sizeof(lbd_channelId_t));
    memcpy(enabledFreqList, &updatedFreqList[0], updatedChannelCount * sizeof(u_int16_t));
    ...
    *disabledChannelCount =
        steerexecImplCmnCopyAllNotOnList(radioChannelCount, radioChannelList,
                                      ...
                                      disabledChannelList, radioFreqList, disabledFreqList );
}
...
static LBD_STATUS steerexecImplCmnReconcileSteerChannel(
    ...
    *willSteer = LBD_FALSE;
    ...
    u_int16_t freqlistEnable[WLANIF_MAX_RADIOS], freqlistDisable[WLANIF_MAX_RADIOS];

    if (steerexecImplCmnChannelDelta(state, channelCount, channelList, freqList,
                                  &countEnable, &listEnable[0],&freqlistEnable[0],
                                  &countDisable, &listDisable[0], &freqlistDisable[0]) == LBD_NOK) {
        // Error occurred.
        return LBD_NOK;
...
static LBD_STATUS steerexecImplCmnChannelDelta(
    ...
    *disabledChannelCount = 0;
    ...
    if (steerexecImplCmnGetAndValidateRadioChannelList(state, &wlanifChannelCount,
                                                    &wlanifChannelList[0], &wlanifFreqList[0]) != LBD_OK) {
        return LBD_NOK;
    }
    ...
    lbd_channelId_t updatedChannelList[WLANIF_MAX_RADIOS];
    u_int16_t updatedFreqList[WLANIF_MAX_RADIOS];

    memcpy(&updatedChannelList[0], channelList,
           channelCount * sizeof(lbd_channelId_t));
    memcpy(&updatedFreqList[0], freqList, channelCount * sizeof(u_int16_t));
    ...
    for (i = 0; i < wlanifChannelCount; i++) {
        ...
        if (!match) {
            ...
            if (!match) {
                ...
                disabledFreqList[*disabledChannelCount] = wlanifFreqList[i];
                (*disabledChannelCount)++;
            }
        }
    }
...
static void steerexecImplCmnUpdateChannelSet(
    ...
    lbd_channelId_t temp[WLANIF_MAX_RADIOS];
    u_int16_t tempFreq[WLANIF_MAX_RADIOS];
    ...
    memcpy(&state->legacy.disabledFreqList, &tempFreq,
           countDisabled * sizeof(u_int16_t));
    ...
    memcpy(&state->legacy.disabledFreqList[countDisabled], disabledFreqList,
           disabledChannelCount * sizeof(u_int16_t));
