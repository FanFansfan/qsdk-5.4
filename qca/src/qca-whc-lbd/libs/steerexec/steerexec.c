...
static struct profileElement steerexecElementDefaultTable[] = {
    ...
    { STEEREXEC_TARGET_LOW_RSSI_THRESHOLD_W6_KEY,    "15" },
    // 15 minutes
...
LBD_STATUS steerexec_allowAssoc(stadbEntry_handle_t entry,
    ...
    return steerexecImplCmnAllowAssoc(steerexecState.steerexecImplCmn,
                                   entry, channelCount, channelList, freqList, ignored);
}
...
static LBD_STATUS steerexecReadConfig(steerexecImplCmnConfig_t *config,
    ...
    if (profileGetOptsIntArray(mdModuleID_SteerExec,
        STEEREXEC_TARGET_LOW_RSSI_THRESHOLD_W6_KEY,
        steerexecElementDefaultTable, value) == LBD_NOK) {
        dbgf(dbgModule, DBGERR,
             "Unable to parse %s",
             STEEREXEC_TARGET_LOW_RSSI_THRESHOLD_W6_KEY);
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->targetLowRSSIThreshold[wlanif_band_6g][index] = value[index];
    }
...
static const char *steerexecMenuSteerHelp[] = {
    ...
    "Usage:",
    "\tsteer <mac> <ap> <channel> <frequency> <ess> [<ap> <channel> <frequency> <ess>]: steer the MAC address to the given candidate(s)."
 ...
 *         otherwise
 */
static LBD_STATUS steerexecMenuAssignUint16(struct cmdContext *context,
                                           const char *arg,
                                           u_int16_t *val) {
    if (!cmdWordDigits(arg)) {
        // Don't print an error here, could just be out of arguments
    ...
    int tempInt = atoi(arg);
    if (wlanif_resolveBandFromFreq(tempInt) == wlanif_band_invalid) {
        cmdf(context, "steerexec command frequency argument out of range, should be a valid frequency value : '%d'\n", tempInt);
        return LBD_NOK;
    }
    *val = tempInt;
...
static LBD_STATUS steerexecMenuGetBSSInfo(struct cmdContext *context,
    ...
    *arg = cmdWordNext(*arg);
    if (steerexecMenuAssignUint16(context, *arg, &outBSS->freq) != LBD_OK) {
        return LBD_NOK;
    }
...
void steerexecMenuSteerHandler(struct cmdContext *context,
    ...
    if (!i) {
        cmdf(context, "steerexec 'steer' command needs at least 1 candidate. "
             "Candidate should be specified as <ap> <channel> <frequency> <ess>\n",
             arg);
        return;
    }
...
static const char *steerexecMenuAllowAssocHelp[] = {
    ...
    "Usage:",
    "\tallow_assoc <mac> <channel> [<channel>] <freq> [<frequency>]: allow the MAC address to associate "
...
void steerexecMenuAllowAssocHandler(struct cmdContext *context,
    ...
    u_int16_t freq[STEEREXEC_MAX_ALLOW_ASSOC], tfreq;
    size_t channelCount;
    ...
    for (channelCount = 0; channelCount < STEEREXEC_MAX_ALLOW_ASSOC; channelCount++) {
        ...
        tfreq = atoi(arg);
        if ( !( (tfreq >= WLANIF_6G_START_FREQ && tfreq <= WLANIF_6G_END_FREQ) ||
                (tfreq >= WLANIF_5G_START_FREQ && tfreq <= WLANIF_5G_END_FREQ) ||
                (tfreq >= WLANIF_2G_START_FREQ && tfreq <= WLANIF_2G_END_FREQ) ) ) {
            cmdf(context, "estimator 'airtime' command invalid freq '%s'\n",
                 arg);
            return;
        }
        freq[channelCount]=tfreq;
    }
    ...
    LBD_BOOL ignored;
    if (steerexec_allowAssoc(entry, channelCount, &channels[0], &freq[0], &ignored) != LBD_OK) {
