...
struct bandmonPriv_t {
    ...
    /// The number of previous active channels.
    size_t prevNumActiveChannels;
...
/**
 * @brief Find the matching entry for the channel utilization by the frequency
 *        number.
 *
 * @param [in] freq  the freq number
 *
 * @return the handle to the entry, or NULL if no match was found
 */
struct bandmonChannelUtilizationInfo_t *
 ...
 * @param [in] frequency on which the utilization was measured
 * @param [in] utilization   the utilization value (0-100)
 */
void bandmonCmnHandleChanUtil(lbd_channelId_t channelId,
                              u_int16_t freq,
