...
bool LOWIClient::setLogLevel (eLogLevel log_level)
{
  ...
  if (0 != log_set_local_level_for_tag (LOWIUtilsExtn::TAG, level))
  {
    retVal = false;
  }
  return retVal;
...
bool LOWIClient::init ()
{
  ...
  do
  {
    ...
    log_verbose (TAG, "Client thread id = %llx", threadid);

    char clientname [CLIENT_NAME_LEN] = {0};
    snprintf(clientname, sizeof(clientname), "%llx", threadid);
    ...
    mLOWIClientReceiver = new (std::nothrow) LOWIClientReceiver
        (MQ_SERVER_SOCKET_NAME, mLocalMsgQueue, mConn, this, mClientName);
...
LOWIResponse* LOWIClient::processRequest(LOWIRequest* const request)
{
  ...
  // pick the timeout according to the request
  if (LOWIUtils::isBackgroundScan(request) ||
      (LOWIRequest::LOWI_RTT_RM_CHANNEL_REQUEST == reqType) ||
      (LOWIRequest::LOWI_ENABLE_RESPONDER_REQUEST == reqType))
  {
    // The time out is given by the definition STATUS_BLOCKING_TIMEOUT_SEC which is
    // currently set to 5 sec.
    timeout.tv_sec = now + STATUS_BLOCKING_TIMEOUT_SEC;
  }
  else if (LOWIRequest::CAPABILITY_SUBSCRIPTION == reqType)
  {
    // The time out is given by the definition CAPS_SUBCRIPTION_BLOCKING_TIMEOUT_SEC
    // which is currently set to 60 sec.
    timeout.tv_sec = now + CAPS_SUBCRIPTION_BLOCKING_TIMEOUT_SEC;
  }
  pTimeout = &timeout;
...
int LOWIClient::newMsg(InMemoryStream * new_buffer)
{
  ...
  do
  {
    ...
    if (NULL == resp)
    {
      log_debug (TAG, "Unable to parse the InPostcard");
      break;
    }
    ...
    log_verbose (TAG, "%s: resp to blocking caller... rspType(%s) mBlock(%d) mBlockBg(%d)",
                 __FUNCTION__, LOWIUtils::to_string(resp->getResponseType()), mBlock, mBlockBg);
    ...
    if (mBlock || mBlockBg)
    {
      switch(resp->getResponseType())
      {
        ...
        case LOWIResponse::BGSCAN_CAPABILITIES:
        case LOWIResponse::BGSCAN_CHANNELS_SUPPORTED:
        case LOWIResponse::BGSCAN_CACHED_RESULTS:
        case LOWIResponse::BGSCAN_CACHED_RESULTS_VECTOR:
        case LOWIResponse::CAPABILITY_SUBSCRIPTION:
        case LOWIResponse::LOWI_RTT_RM_CHANNEL_RESPONSE:
          {
            log_debug (TAG, "%s response received", LOWIUtils::to_string(resp->getResponseType()));
  ...
  if(0 != result)
  {
    log_debug(TAG, "newMsg failed %d", result);
  }
