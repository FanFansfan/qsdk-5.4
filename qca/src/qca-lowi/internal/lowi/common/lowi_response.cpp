...
const char* const LOWICapabilities::TAG = "LOWICapabilities";
...
LOWICapabilities::LOWICapabilities ()
{
  ...
  bgscanSupported               = false;
  bwSupport                     = 0;
  preambleSupport               = 0;
  mcVersion                     = 0;
  supportedCapsWigig            = LOWI_NO_SCAN_SUPPORTED;
  supportedCapablities          = LOWI_NO_SCAN_SUPPORTED;
}

void LOWICapabilities::PrintCapabilities ()
{
  log_verbose(TAG, "%s: Supported - discoveryScan(%u) rangingScan(%u) activeScan(%u)"
              "dualSidedRanging11mc(%u) bgscan(%u) bwSupport(0x%x) mcVersion(%u)"
              "Capablities(0x%x) CapsWigig(0x%x) ", __FUNCTION__,
              discoveryScanSupported, rangingScanSupported, activeScanSupported,
              dualSidedRangingSupported11mc, bgscanSupported, bwSupport, mcVersion,
              supportedCapablities, supportedCapsWigig);
}
