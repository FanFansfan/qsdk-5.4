...
{
  setRequestOriginator(rhs.mRequestOriginator);
...
{
  if(NULL == request_originator)
  {
    mRequestOriginator = NULL;
    log_error (TAG, "Invalid request Originator : NULL");
    return;
  }
  size_t len = strlen(request_originator) + 1;
  mRequestOriginator = new (std::nothrow) char [len];
  if (NULL == mRequestOriginator)
  {
    ...
    bool fullBeaconScanResponse)
{
  ...
  do
  {
    ...
    req->timeoutTimestamp = timeoutTimestamp;
    req->bufferCacheRequest = bufferCacheRequest;
    req->fullBeaconScanResponse = fullBeaconScanResponse;
  } while (0);
...
(uint32 requestId,
    ...
    eRequestMode mode, bool fullBeaconScanResponse)
{
  LOWIDiscoveryScanRequest* req = NULL;
  do
  {
...
LOWINodeInfo::LOWINodeInfo ()
{
  ...
  num_pkts_per_meas    = 5;
  num_retries_per_meas = 0;
  ftmRangingParameters = 0;
  ...
  FTM_SET_PTSF_TIMER_NO_PREF(ftmRangingParameters); // indicates no preferred time for
                                                    // start of first burst
  // Default Param Control
  paramControl = LOWI_USE_PARAMS_FROM_CACHE;

  // Default phymode. When phymode is not specified by the client
  // LOWI will provide from cache if available.
...
:LOWINodeInfo()
{
  ...
  /* Default to period of 0 */
  meas_period = 0;

  /* Default to 0 measurements */
  num_measurements = 0;
}
...
std::string LOWIRequest::get_interface() const
{
  return mInterface;
}

void LOWIRequest::set_interface(std::string iface)
{
  mInterface = iface;
}
...
{
  mBssid = bssid;
  log_debug(TAG, "LOWISendLCIRequest: macAddr(" LOWI_MACADDR_FMT ")",
            LOWI_MACADDR(mBssid));
}
...
LOWIFTMRangingRequest::LOWIFTMRangingRequest(uint32 requestId, LOWIMacAddress bssid,
  ...
  : LOWIRequest(requestId),
    ...
    mNodes(nodes)
{
  log_debug(TAG, "LOWIFTMRRNodeInfo: macAddr(" LOWI_MACADDR_FMT ")",
            LOWI_MACADDR(mDestBssid));
}
/////////////////////////////
// LOWIConfigRequest
/////////////////////////////
LOWIConfigRequest::LOWIConfigRequest (uint32 requestId, eConfigRequestMode reqMode)
: LOWIRequest (requestId)
{
  log_verbose (TAG, "LOWIConfigRequest");
  mLowiVariant = LOWI_AP;
  mLowiGlobalLogLevel = EL_INFO;
  mLowiGlobalLogFlag = false;
  mConfigRequestMode = reqMode;
}
