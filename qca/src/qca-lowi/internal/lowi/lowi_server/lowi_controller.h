 ...
 */
class LOWIController:
public qc_loc_fw::MqClientControllerBase, public LOWIScanResultReceiverListener,
...
protected:
  ...
  /** RangingScanResultReceiver*/
  LOWIScanResultReceiver*        mWigigRangingScanResultReceiver;
  /** BgScanResultReceiver   */
  LOWIScanResultReceiver*        mBgScanResultReceiver;
  /** ScanRequestSender   */
  ...
  /** WiGig Driver Interface */
  LOWIWifiDriverInterface*       mWigigDriver;
  /** Timer Data & callback */
  ...
  LOWIBackgroundScanMgr*         mBackgroundScanMgr;
  int32                          mMaxQueueSize;
  int32                          mFreshScanThreshold;
  bool                           mWifiStateEnabled;
  bool                           mWigigStateEnabled;
  bool                           mWigigDriverRttSupported;
  bool                           mReadDriverCacheDone;
  bool                           mLOWILPPresent;
  LOWIScanResultReceiver*        mLPScanResultReceiver;
  bool                           mUseLowiLp;
  bool                           mUseLowiLpRanging;
  ...
  /** wigig state received from RTM message */
  eWifiIntfState                 mNlWigigStatus;
  int                            mLogLevel;
  int                            mCacheSize;
  LOWICapabilities               mWifiCaps;
  LOWICapabilities               mWigigCaps;
  /** peer types */
  enum ePeerTypes
  {
    WIFI_PEERS,
   ...
   */
  virtual LOWIResponse* generateResponse (LOWIRequest& req,
   ...
   */
  virtual bool sendErrorResponse (LOWIRequest& req,
   ...
   * @param LOWIRequest*
   */
  virtual void processRequest (LOWIRequest* request);

  /**
   * Handles the failure in sending request to the driver
   * @param erequestStatus Request status
   * @param, LOWIRequest* Pointer to the request
   * @return bool true - success, false otherwise
   */
  virtual bool handleRequestFailure (eRequestStatus req_status,
                                     LOWIRequest* request);
   ...
   * @return true if able to generate the response, false otherwise
   */
  virtual bool sendResponse (LOWIRequest& req, LOWIMeasurementResult* meas_result);
private:
  /**
   * Issues a new request from the pending queue if any
   * If there is no pending request, issues a passive listening request
   */
  virtual void issuePendingRequest ();
  /**
   * Issues the request to the wifi driver.
   * @param LOWIRequest* LOWIRequest to be sent
   * @return true if successfully sent, false otherwise
   */
  virtual bool issueRequest (LOWIRequest* req);
...
protected:
   ...
   */
  virtual eRequestStatus requestMeasurements (LOWIRequest* r);
  /**
  ...
  /**
   * Checks is wigig driver is enabled or not
   * @return true if wigig driver is enabled or false
   */
  bool isWigigEnabled();
  ...
  /**
   * Creates the WiGig driver
   * @return Returns true if success, false otherwise
   */
  bool createWigigDriver ();

  /**
   * Creates the Scan Result Receiver threads
   * @return Returns true if success, false otherwise
   */
  virtual bool createScanResultReceiver ();

  /**
   * Creates the WiGig Scan Result Receiver threads
   * @return Returns true if success, false otherwise
   */
  virtual bool createWigigRangingScanResultReceiver()
  {
    return false;
  }
  ...
  /**
   * Copies wifiCapabilities to controller
   */
  void getWifiCapabilities();

  /**
   * Copies wigigCapabilities to controller
   */
  void getWigigCapabilities();

  /**
   * Terminates the Wifi driver
   */
  void terminateWifiDriver ();

  /**
   * Terminates the WiGig driver
   */
  void terminateWigigDriver ();
  ...
  /**
   * Terminates the Wigig Scan Result Receiver thread
   */
  void terminateWigigRangingScanResultReceiver();
  ...
  * @return bool: true if scheduler successfully created, false if not
   ...
   */
  virtual LOWIRequest* getCurrReqPtr();
   ...
   * function checks the location capabilities of the received AP in the
   * lowi-cache. It then forms the command to send to the wpa_supplicant
   * accordingly.
   *
   * @param LOWIRequest* request pointer
   */
  virtual void processLocANQPRequest(LOWIRequest* /*request*/)
  {
  }
  ...
  /**
   * Notify the Disocvery scan receiver that the Wlan interface
   * status has changed. This is done via internal message.
   *
   * @param None
   */
  void notifyWlanInterfaceStatusChange();

  /**
   * Empty the list and delete the objects.
   *
   * @param list - List to be emptied.
   */
  void emptyList(List<LOWIRequest *>& list);

  /**
   * Checks whether the wireless driver (either wifi or wigig) is
   * enabled. If not enabled, it discards the measurements in the
   * LOWILocalMsg msg.
   *
   * @param LOWILocalMsg *: LOWILocalMsg containing the measurements
   * @return bool: false: driver was disabled and measurements were
   *         discarded, true: driver was enabled measurements were not
   *         discarded.
   */
  bool isWirelessIntfEnabled(LOWILocalMsg *msg);

  /**
   * Read and load the lowi.conf file paramters.
   *
   * @param None
   */
  void loadConfigItems();
  /**
   * Process the LOWI Config request.
   * Currently only Log config mode is supported.
   * This will set the passed tags to their corresponding
   * log levels. Also if global log flag is enabled, global
   * log level will be overriden with the passed global log value.
   *
   * @param LOWIConfigRequest *: LOWIConfigRequest containing the config req.
   * @return bool true if success, false otherwise.
   */
  bool ProcessConfigRequest(LOWIConfigRequest* req);
  /**
   * Timercallback class will notify us about the callback
   */
  ...
  /**
   * Background scan manager class uses request and response management
   * functions from the LOWIController class
   */
  friend class LOWIBackgroundScanMgr;
...
public:
  ...
  /**
   * Notification callback that the wifi/wigig driver interface status changes
   * are received from netlinksocketreceiver thread.
   * The notification comes in the NetLinkSocketReceiver thread context.
   * Post the status to the BlockingQueue.
   ** The message will then be read by the LOWIController in its own thread
   * context.
   *
   * @param LOWIDriverInterface &: driver interface information
   */
  void intfStateReceived(LOWIDriverInterface &result);

  /**
   * Notifies the listener about the driver interface state changes
   * @param LOWIDriverInterface: wlan/wigig driver interface state
   */
  virtual void updateIntfState (LOWIDriverInterface &intf);

  /**
   * Provides caller with pointer to LOWI Cache object.
   */
  LOWICacheManager* getCacheManager() const { return mCacheManager; }

protected:
  /** Allows any specific initializations in this function for the
   * sub classes
  ...
  /**
   * Generates LOWIResponse for a background scan request.
   * @param LOWIRequest& Request for which the response is to be genarated
   * @param LOWIMeasurementResult* Measurement Result
   */
  virtual LOWIResponse* generateBgScanResponse (LOWIRequest& /*req*/,
      LOWIMeasurementResult* /*meas_result*/)
  { return NULL; }

  /**
   * Handles a batching subscription request (START / STOP)
   * @param LOWIRequest* Request for batching subscription
   * @return code (0 - Batching subscription request handled)
                   (-1 - Batching subscription request but can not be handled
                   (-2 - Not a Batching subscription request)
   */
  virtual int32 handleBatchingSubscriptionRequest (LOWIRequest* /*req*/)
  { return -2; }

  /**
   * Handles a capability subscription request
   * @param LOWIRequest* Request for capability subscription
   * @return code (0 - Batching subscription request handled)
                   (-1 - Batching subscription request but can not be handled
                   (-2 - Not a Batching subscription request)
   */
  virtual int32 handleCapabilitySubscriptionRequest (LOWIRequest* /*req*/)
  { return -2; }

  /**
   * Sends the response to Capability subscription request.
   * Updates the list with only valid requests and drops the
   * requests silently that have expired.
   * Notifies to the requests in the list only if the results
   * correspond to capabilities response.
   * @param LOWIMeasurementResult* Pointer to results received
   * @return bool Returns true if able to generate any response
   *                      false otherwise.
   */
  virtual bool sendCapabilitiesSubscriptionResponse (
      LOWIMeasurementResult* /*meas_result*/)
  { return true; }

  /**
   * Sends the Batching status response to all the subscribers
   * @param LOWIMeasurementResult* Pointer to results received
   * @return bool Returns true if able to generate any response
   *                      false otherwise.
   */
  virtual bool sendBatchingStatusResponseToAllSubscribers (
      LOWIMeasurementResult* /*meas_result*/)
  { return true; }

  /**
   * Sends the status responses to Batching subscribers.
   * Notifies to the client if the results
   * correspond to batching subscriptions.
   * @param LOWIMeasurementResult* Pointer to results received
   * @return bool Returns true if able to generate any response
   *                      false otherwise.
   */
  virtual bool sendBatchingStatusResponse (LOWIMeasurementResult* /*meas_result*/)
  { return true; }

  /**
   * Creates a background scan manager (BSM) for managing all bgscan and hotlist
   * related requests and responses
   * @return bool: true if BSM successfully created, false if not
   */
  virtual bool createBackgroundScanMgr()
  { return true; }

  /**
   * Handles the LOWI-LP requests
   * @param LOWIRequest* Request for discovery scan.(currently discovery scan only)
   * @return code (0 - discovery scan successfully sent to LOWI-LP)
                  (-1 - even though LOWI-LP is present, it returned error
                        while sending the request)
                  (-2 - Either not a Discovery scan request or LOWI-LP is not
                        present,in this case, handle it in discoveryscanreceiver)
   */
  virtual int32 handleLowiLpRequest (LOWIRequest* /*request*/)
  {
    return -2;
  }

 /**
  * Handles the Ranging request through LOWI-LP
  * @param: LOWIRequest* Pointer to Ranging Request
  * @return int32 0 (handled), -1 (Not supported), -2 (Supported, yet failed to handle)
  */
  virtual int32 handleLowiLpRangingRequest (LOWIRangingScanRequest* /*request*/)
 {
   return -1;
 }

  /**
   * Sends the status responses to discovery scan response
   * received from the LOWI-LP.
   * @param LOWIMeasurementResult* Pointer to results received
   * @return bool Returns true if able to generate any response
   *                      false otherwise.
   */
  virtual bool sendLOWILpDiscoveryScanResponse (LOWIMeasurementResult* /*meas_result*/)
  {
    return false;
  }

  /**
   * Requests the Background scan
   * @param LOWIRequest* Pointer to request
   * @return bool true if SUCCESS, false otherwise
   */
  virtual bool requestBackgroundScan (LOWIRequest* /*req*/)
  {
    return false;
  }

  /**
   * Process the Measurement results from LOWI-LP
   * @param LOWIMeasurementResult* Pointer to measurement results
   */
  virtual void processMeasurementResultsLP (LOWIMeasurementResult* /*meas_result*/)
  {
  }

  /**
   * Check if the build is userdebug build or not
   *
   * @return bool: true if it is userdebug build, false otherwise.
   */
