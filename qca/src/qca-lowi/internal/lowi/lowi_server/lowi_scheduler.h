...
// constants used for encoding latitude and longitude
#define POW_2_25 33554432    // pow(2,25)
#define POW_2_34 17179869184 // pow(2,34)
// constant used for encoding altitude
#define POW_2_8  256         // pow(2,8)
...
private:
  ...
  /** Current Wigig request */
  LOWIRequest *mWigigCurrentRequest;

  /** Queue to keep requests that can't be serviced right away */
  Queue<LOWIRequest *>   mWigigRequestQueue;
...
/**
   ...
   * request. Specifically, it saves the request id used by the scheduler to
   * handle the request, and changes the state of the node WIFI_NODE_MEAS_IN_PROGRESS
   ...
   * @param rttType: rtt type wifi node
  ...
  /**
   * Retrieves Ranging information for each AP to be ranged with
   *
   * @param vector<LOWIPeriodicNodeInfo>&: vector containing the
   *         BSSID and channel information of the APs to range
   *         with
   * @param vector<LOWIPeriodicNodeInfo>&: vector containing all
   *              the necessary ranging information for each AP
   */
  void getRangingInfoFromRequest(vector<LOWIPeriodicNodeInfo> &v,
                                 vector<LOWIPeriodicNodeInfo> &vNodes);

  /**
   * Creates a ranging request that can be sent to the driver. APs to be ranged
   * with came in the internal message. Any additional information needed to
   * construct the ranging request is obtained from the cache.
   *
   * @param iReq: internal message coming from the driver
   *
   * @return bool: true if success, else false
   */
  bool HandleFTMRangeReq(LOWIInternalMessage *iReq);

  /**
  * populates the incoming LCI structure with the LCI of the
  * Sydney Opera house.
  *
  * @param LOWILCIRprtInfo&: LCI structure to populate with LCI of the
  *                 Sydney Opera House.
  *
  * @return None
  */
  void loadSydneyOperaHouseLCI(LOWILCIRprtInfo& lciInfo);

  /**
  * populates the incoming LCI structure with unknown LCI.
  *
  * @param LOWILCIRprtInfo&: LCI structure to populate with unknown LCI
  *
  * @return None
  */
  void loadUnknownLCI(LOWILCIRprtInfo& lciInfo);

  /**
  * Obtains all necessary information to put together an LCI report and sends it
  * to the driver.
  *
  * @param iReq: internal message coming from the driver
  *
  * @return bool: true if success, else false
  */
  bool HandleLCIReq(LOWILCIReqMessage *iReq);

  /**
   * Encode the latitude or longitude per the IEEE std. 2s complement, 9
   * bits before binary point, 25 after, MSB first.
   * @param a: latitude or longitude in degrees
   * @return uint64: encoded value
   */
  uint64 encodeLatLon(double a)
  {
    uint64 val = (a < 0) ? (uint64)(POW_2_34 - round((-1) * a * (double)POW_2_25)):
                           (uint64)(round(a * (double)POW_2_25));
    return val;
  }

  /**
   * Encode the altitude per the IEEE std. 22 bits before binary point, 8 after,
   * MSB first.
   * @param alt: altitude in meters
   * @return uint32: encoded altitude
   */
  uint32 encodeAltitude(double alt)
  {
    return (uint32)(round(alt * (double)POW_2_8));
  }

  /**
   * Takes an incoming LOWIRequest intended for the wigig driver, and attempts to
   * send it to the driver. If the driver is busy servicing a request, the
   * incoming request is placed in a pending queue to be processed later.
   *
   * @param LOWIRequest &: incoming LOWIRequest
   * @return int: 0: success, -1: failure
   */
  int processWigigRangRequest(LOWIRequest &request);

  /**
   * This function takes an incoming LOWIRequest intended for the wigig driver
   * and tries to send it to the wigig ranging scan result receiver after
   * ensuring that the wigig driver interface is up and that there is a valid
   * wigig driver object that will handle the request.
   *
   * @param LOWIRequest *: incoming LOWIRequest
   * @return LOWIResponse::eScanStatus: status of the operation
   */
  LOWIResponse::eScanStatus requestMeasurements(LOWIRequest *request);
  ...
  /**
   * Issues a new request from the pending queue if there are requests in the
   * pending queue and if there is, currently, no request being serviced.
   */
  void issueRequest();
...
public:
  ...
  /**
   * Cancel the current request. This removes the requested peers from the
   * database and sends a notification to the client.
   */
  void cancelCurrentRequest();

  /**
   * This function removes the all wigig peers from the database
   * @param LOWIController::ePeerTypes: indicates which peer type to remove
   */
  void removePeers(LOWIController::ePeerTypes remove);
};
} // namespace qc_loc_fw
