...
private:
   ...
   * The cache is updated in different ways if the measurement
   * is a discovery scan measurement or if the measurement is from
   * LOWI-LP or of it is a ranging measureent.
   *
   * @param List<LOWIScanMeasurement> Measurements to be kept in
   *         cache
   * @param bool Indicates if result is from LOWI-LP
   * @param bool Indicates if Ranging Measurements
  ...
  /**
   * Update Ranging info only from scan measurement
   *
   * @param LOWIScanMeasurement& Measurement to be updated.
   * @param LOWIScanMeasurement& Incoming new measurement
   *                           info.
   * @param bool Indicates if Ranging scan measurements
   *
   */
  void updateScanMeasInfo(LOWIScanMeasurement &scanMeas,
...
public:
  ...
  bool putInCache (vector<LOWIScanMeasurement*> & measurements,
      bool result_from_lowilp, bool isRangingScan = false);
  ...
  /**
   * Check if mac address is in LOWI cache. If it is, retrieve the frequency
   *
   * @param mac: mac address of peer to check
   * @param freq: frequency of the peer in the cache
   *
   * @return bool: true if mac addr is in cache, else false
   */
  bool getFreqFromCache(LOWIMacAddress mac, uint32 &freq);

  /**
   * Updates the Result received from LOWI-LP with the additional data
   * in the cache. This additional data is received by LOWI-AP in passive
   * listening and cached. LOWI-LP does not provide this data so need to
   * update the end result to the clients.
   * @param LOWIMeasurementResult& Referenece to LOWIMeasurementResult
   * @return bool true if success in update, false otherwise
   */
  bool updateResultRecdFrmLP (LOWIMeasurementResult& res);
