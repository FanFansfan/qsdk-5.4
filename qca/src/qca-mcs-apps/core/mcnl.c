	...
	int sync)
{
	...
	/* Do it only once per context, save a system call, since thread pid is
	 * bound by the mc netlink listener, and pid is in the 1-2^24 */
	if (!SelfPid)
		SelfPid = getpid();
	if (sync) {
		myPid = SelfPid + (1 << 24);
