...
void mcManagerMCProcess(struct mcManagerSnooper *snooper, struct __mc_mdb_entry *mdbEntry, int32_t num_entries)
{
	...
	for (i = 0; i < num_entries; i++, mdbEntry++) {
		...
		if (!node) {
			...
			if (ifTemp->table == NULL || ifTemp->temptable == NULL) {
	...
	HMapForEachSafe(node, n, &snooper->ifTableMap) {
		...
		if (ifTemp->sync) {
			...
			if (!modified) {
				modified = memcmp(ifTemp->table->entry, ifTemp->temptable->entry, sizeof(struct __mc_floodtbl_entry) * ifTemp->table->entryCnt);
			}
			if (modified) {
