 ...
 * @Notes:
 *
 * Copyright (c) 2014-2015, 2017, 2019, 2021 Qualcomm Technologies, Inc.
 *
 * All Rights Reserved.
...
static u_int32_t switchFindPortmap(struct ether_addr *mac, u_int32_t vid)
{
	...
	entry.vid = vid;
	switchCommandExcute(SW_API_FDB_FIND, switchWrapperState.deviceId, &entry);
...
{
	return switchCommandExcute(SW_API_IGMP_SG_ENTRY_QUERY, switchWrapperState.deviceId, entry);
}
...
static int switchAddIgmpMldRules()
{
	...
	switchCommandExcuteVar(SW_API_ACL_LIST_CREAT, 3, switchWrapperState.deviceId, 10, 10);
	...
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, switchWrapperState.deviceId, 10, 0, 1, &rule);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 10, 0, 0, 1);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 10, 0, 0, 2);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 10, 0, 0, 3);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 10, 0, 0, 4);

	/*Rules for MLD packets */
	switchCommandExcuteVar(SW_API_ACL_LIST_CREAT, 3, switchWrapperState.deviceId, 20, 20);
	...
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, switchWrapperState.deviceId, 20, 0, 1, &rule);
	rule.icmp_type_val = 131; /*Multicast Listener Report*/
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, switchWrapperState.deviceId, 20, 1, 1, &rule);
	rule.icmp_type_val = 132; /*Multicast Listener Done*/
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, switchWrapperState.deviceId, 20, 2, 1, &rule);
	rule.icmp_type_val = 143; /*Multicast Listener Report, Version 2*/
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, switchWrapperState.deviceId, 20, 3, 1, &rule);

	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 20, 0, 0, 1);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 20, 0, 0, 2);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 20, 0, 0, 3);
	switchCommandExcuteVar(SW_API_ACL_LIST_BIND, 5, switchWrapperState.deviceId, 20, 0, 0, 4);

	/*Ensure ACL engine working*/
	switchCommandExcuteVar(SW_API_ACL_STATUS_SET, 2, switchWrapperState.deviceId, 1);
...
int switchInitSnooping(void)
{
	...
	switchWrapperState.rootPort =
		atoi(profileGetOpts(mdModuleID_Interface, "SwitchCpuPort",
			switchWrapperDefaulttable));
	switchWrapperState.deviceId =
		atoi(profileGetOpts(mdModuleID_Interface, "SwitchDeviceId",
switchWrapperDefaulttable));
	...
	switchCommandExcute(SW_API_IGMP_RP_SET, switchWrapperState.deviceId, 1 << switchWrapperState.rootPort);
...
{
	return switchCommandExcute(SW_API_IGMP_SG_ENTRY_SET, switchWrapperState.deviceId, entry);
}

/*Clear the group Rules*/
int switchClearGroupRule(swIgmpForwardEntry * entry)
{
	return switchCommandExcute(SW_API_IGMP_SG_ENTRY_CLEAR, switchWrapperState.deviceId, entry);
}
...
int switchAddFdb(struct ether_addr *mac, swBitsMap map)
{
	...
	entry.addr = *mac;
	entry.port.map = map;
	return switchCommandExcute(SW_API_FDB_RESV_ADD, switchWrapperState.deviceId, &entry);
}
...
int switchClearFdb(struct ether_addr *mac, swBitsMap map)
{
	...
	entry.addr = *mac;
	entry.port.map = map;
	return switchCommandExcute(SW_API_FDB_RESV_DEL, switchWrapperState.deviceId, &entry);
}
