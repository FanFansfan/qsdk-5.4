...
#ifdef QCA_GCC_7_SUPPORT
    PRIN_LOG("TOA                   :%lx",type1_resp->toa);
    PRIN_LOG("TOD                   :%lx",type1_resp->tod);
    PRIN_LOG("RTT                   :%lx",(((type1_resp->toa - type1_resp->tod))));
#else
...
#ifdef QCA_GCC_7_SUPPORT
    PRIN_LOG("Request timestamp     :%lx",sresp->req_tstamp);
    PRIN_LOG("Response timestamp    :%lx",sresp->resp_tstamp);
#else
...
#ifdef QCA_GCC_7_SUPPORT
    PRIN_LOG("STA last timestamp :%lx",sta_info->tstamp);
#else
...
#ifdef QCA_GCC_7_SUPPORT
    PRIN_LOG("Associated AP's tsf   :%lx\n",tsfresp->assoc_tsf);
    PRIN_LOG("Probing AP's tsf      :%lx\n",tsfresp->prob_tsf);
#else
...
#ifdef QCA_GCC_7_SUPPORT
    PRIN_LOG("Request timestamp     :%lx",mresp->req_tstamp);
    PRIN_LOG("Response timestamp    :%lx",mresp->resp_tstamp);
#else
