...
static int _vendor_ioctl_getparam_stainfo(struct net_device *dev, athcfg_wcmd_data_t *iiData)
{
    ...
    if ((wlan_vdev_is_up(vap->vdev_obj) == QDF_STATUS_SUCCESS) && (ni))
    {
        ...
        if (wlan_getsnr(vap, &snr_info, WLAN_SNR_RX) == -1)
            iiData->station_info.rx_rssi = snr_info.avg_snr;
        else
            iiData->station_info.rx_rssi = ni->ni_snr; /* go back to 802.11 layer's */
        if (wlan_getsnr(vap, &snr_info, WLAN_SNR_BEACON) == -1)
            iiData->station_info.rx_rssi_beacon = snr_info.avg_snr;
        else
            iiData->station_info.rx_rssi_beacon = ni->ni_snr; /* go back to 802.11 layer's */
                                        ...
                                        ieee80211_recv_status *rs)
{
    ...
    {
        osvsp->tm_latestRssiCombined = rs->rs_snr;
        for (i = 0; i < IEEE80211_MAX_ANTENNA; i++)
        {
            osvsp->tm_latestRssiCtl[i] = rs->rs_snrctl[i];
            osvsp->tm_latestRssiExt[i] = rs->rs_snrextn[i];
        }
...
static QDF_STATUS _vendor_ioctl_getparam_scan_resultCb(void *arg, wlan_scan_entry_t se)
{
    ...
    sr->isr_snr = util_scan_entry_snr(se);
...
static int _vendor_ioctl_getparam_rssi(struct net_device *dev, athcfg_wcmd_data_t *iiData)
{
    ...
    wlan_getsnr(vap, (wlan_snr_info *)(&iiData->snr.bc_avg_snr), WLAN_SNR_BEACON);
    wlan_getsnr(vap, (wlan_snr_info *)(&iiData->snr.data_avg_snr), WLAN_SNR_RX_DATA);
