...
struct qca_notifier_block {
    struct notifier_block *notifier;
    atomic_t count;
};

static inline int
qca_register_netdevice_notifier(struct qca_notifier_block *qca_device_notifier)
{
    int ret = 0;

    if (!qdf_atomic_read(&qca_device_notifier->count)) {
        ret = register_netdevice_notifier(qca_device_notifier->notifier);
    }

    qdf_atomic_inc(&qca_device_notifier->count);
    return ret;
}

static inline int
qca_unregister_netdevice_notifier(struct qca_notifier_block *qca_device_notifier)
{
    int ret = 0;

    if (qdf_atomic_dec_and_test(&qca_device_notifier->count)) {
        ret = unregister_netdevice_notifier(qca_device_notifier->notifier);
    }
