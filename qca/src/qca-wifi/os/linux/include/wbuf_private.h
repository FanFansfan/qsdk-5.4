...
#define M_INCORRECT_PMF_KEY   N_INCORRECT_PMF_KEY
...
#define M_SMPSACTM_SET  N_SMPSACTM_SET
#define M_SMPSACTM_IS   N_SMPSACTM_IS
...
#define wbuf_set_smpsactframe(wbuf)         __wbuf_set_smpsactframe(wbuf)
#define wbuf_is_smpsactframe(wbuf)          __wbuf_is_smpsactframe(wbuf)
...
static INLINE void
__wbuf_set_incorrect_pmf_key(struct sk_buff *skb)
{
     M_FLAG_SET(skb, M_INCORRECT_PMF_KEY);
}

static INLINE int
__wbuf_is_incorrect_pmf_key(struct sk_buff *skb)
{
    if (M_FLAG_GET(skb, M_INCORRECT_PMF_KEY))
        return 1;
    else
        return 0;
}
...
static INLINE void
__wbuf_set_smpsactframe(struct sk_buff *skb)
{
    M_SMPSACTM_SET(skb);
}

static INLINE int
__wbuf_is_smpsactframe(struct sk_buff *skb)
{
    if (M_SMPSACTM_IS(skb))
        return 1;
    else
        return 0;
}
