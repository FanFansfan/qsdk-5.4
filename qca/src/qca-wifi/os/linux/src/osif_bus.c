...
bus_alloc_nonconsistent(void *hwdev, size_t size, dma_addr_t *dma_handle)
{
	...
	return dma_alloc_attrs(devhandle->bdev == NULL ? NULL:&(pdev->dev), size, dma_handle, GFP_ATOMIC,
...
bus_free_nonconsistent(void *hwdev, size_t size, void *vaddr, dma_addr_t dma_handle)
{
	...
	pdev = (struct pci_dev *)devhandle->bdev;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 24)
	return dma_free_attrs(devhandle->bdev == NULL ? NULL:&(pdev->dev), size, vaddr, dma_handle,
				DMA_ATTR_NON_CONSISTENT);
#else
