...
enum osif_nss_vdev_cmd {
    ...
    OSIF_NSS_VDEV_ENABLE_IGMP_ME,
    OSIF_NSS_WIFI_VDEV_ENABLE_HLOS_TID_OVERRIDE,
...
int32_t osif_nss_ol_vap_create(struct ieee80211vap *vap, osif_dev *osif, nss_if_num_t nss_ifnum);
...
void osif_nss_vdev_send_cmd(struct nss_ctx_instance *nss_wifiol_ctx, nss_if_num_t if_num, enum nss_wifi_vdev_cmd cmd, uint32_t value);
...
int32_t osif_nss_ext_vdev_hardstart(struct sk_buff *skb, struct net_device *dev);
...
struct nss_vdev_ops {
...
#endif
    void (* ic_osif_nss_vdev_me_dump_deny_list)(osif_dev *osifp);
    void (* ic_osif_nss_vdev_me_add_deny_member)(osif_dev *osifp,
                                uint32_t grp_ipaddr, uint32_t netmask);
    ...
    void (* ic_osif_nss_vdev_me_del_deny_member)(osif_dev *osifp, uint32_t grp_ipaddr, uint32_t netmask);
    ...
    int32_t (* ic_osif_nss_vap_create)(struct ieee80211vap *vap,
                                osif_dev *osif, nss_if_num_t nss_ifnum);
    ...
    int (*ic_osif_nss_vdev_set_peer_next_hop)(osif_dev *osifp, uint8_t *addr, nss_if_num_t if_num);
    ...
    bool (* ic_osif_nss_vdev_prepare_wifi_mac_db)(osif_dev *osif);
    bool (* ic_osif_nss_vdev_wifi_mac_db_pool_entries_send)(osif_dev *osif);
    bool (* ic_osif_nss_vdev_wifi_mac_db_ready)(osif_dev *osif);
    bool (* osif_nss_li_vdev_wifi_mac_db_reset_state)(osif_dev *osif);
    void (* ic_osif_nss_vdev_me_add_hmmc_member)(osif_dev *osifp, uint32_t ipaddr, uint32_t netmask);
    void (* ic_osif_nss_vdev_me_del_hmmc_member)(osif_dev *osifp, uint32_t ipaddr, uint32_t netmask);
};

#ifdef QCA_SUPPORT_WDS_EXTENDED
struct nss_ext_vdev_ops {
	int (*ic_osif_nss_ext_vdev_alloc)(osif_peer_dev *osifp, struct ieee80211com *ic);
	QDF_STATUS (*ic_osif_nss_ext_vdev_register)(osif_peer_dev *osifp, struct ieee80211com *ic);
	QDF_STATUS (*ic_osif_nss_ext_vdev_up)(osif_peer_dev *osifp);
	QDF_STATUS (*ic_osif_nss_ext_vdev_down)(osif_peer_dev *osifp);
	QDF_STATUS  (*ic_osif_nss_ext_vdev_cfg_wds_peer)(osif_peer_dev *osifp, struct ieee80211com *ic);
	void (*ic_osif_nss_ext_vdev_delete)(osif_peer_dev *osifp);
	QDF_STATUS (*ic_osif_nss_ext_vdev_change_mac_addr)(osif_peer_dev *osifp, uint8_t *addr);
	enum osif_nss_error_types (*ic_osif_nss_ext_vdev_xmit)(osif_peer_dev *osifp, struct sk_buff *skb);
};
#endif /* QCA_SUPPORT_WDS_EXTENDED */
...
bool osif_nss_ol_be_check_valid_vdev_for_id(struct cdp_soc_t *soc, uint8_t vdev_id);
uint8_t osif_nss_wifili_get_vdevid_fromvdev(void *vdev, uint8_t *vdev_id);
bool osif_nss_wifili_check_valid_vdev_for_id(struct cdp_soc_t *soc, uint8_t vdev_id);
...
void osif_nss_ol_be_vdev_txinfo_handler(struct ol_ath_softc_net80211 *scn, struct sk_buff *skb,
                             struct nss_wifi_vdev_per_packet_metadata *wifi_metadata, bool is_raw);
uint32_t osif_nss_ol_be_vdev_get_per_pkt_vdev_id_check(struct ol_ath_softc_net80211 *scn);
...
uint8_t osif_nss_ol_be_get_peer_mac_by_id(osif_dev *osifp, uint32_t peer_id, uint8_t *peer_mac);
...
bool osif_nss_ol_be_vdev_prepare_wifi_mac_db(osif_dev *osdev);
bool osif_nss_li_vdev_prepare_wifi_mac_db(osif_dev *osdev);
bool osif_nss_ol_be_vdev_wifi_mac_db_pool_entries_send(osif_dev *osdev);
bool osif_nss_li_vdev_wifi_mac_db_pool_entries_send(osif_dev *osdev);
bool osif_nss_li_vdev_wifi_mac_db_is_ready(osif_dev *osifp);
bool osif_nss_ol_be_vdev_wifi_mac_db_is_ready(osif_dev *osifp);
bool osif_nss_ol_be_vdev_wifi_mac_db_reset_state(osif_dev *osifp);
...
struct nss_wifi_internal_ops {
    ...
    bool (* ic_osif_nss_ol_check_valid_vdev_for_id)(struct cdp_soc_t *cdp_soc, uint8_t vdev_id);
    ...
    void (* ic_osif_nss_ol_vdev_data_receive_meshmode_rxinfo)(struct net_device *dev,
                                struct sk_buff *skb);
    uint8_t (* ic_osif_nss_ol_get_peer_mac_by_id)(osif_dev *osifp, uint32_t peer_id, uint8_t *peer_mac);
    ...
    void (* ic_osif_nss_rsim_rx_decap)(os_if_t osif, qdf_nbuf_t *pdeliver_list_head, qdf_nbuf_t *pdeliver_list_tail, uint8_t *peer_mac);
#ifdef QCA_SUPPORT_WDS_EXTENDED
    bool (* ic_osif_nss_ext_vdev_rx)(osif_dev *osdev, struct sk_buff  *nbuf, struct napi_struct *napi);
#endif /* QCA_SUPPORT_WDS_EXTENDED */
    uint32_t (* ic_osif_nss_ol_li_vdev_get_per_pkt_vdev_id_check)(struct ol_ath_softc_net80211 *scn);
};
