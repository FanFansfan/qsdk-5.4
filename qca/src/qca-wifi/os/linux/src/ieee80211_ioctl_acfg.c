/*
 * Copyright (c) 2014-2015,2017-2021 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
                ...
                uint8_t *buffer, int32_t Length)
{
    ...
    return ieee80211_ucfg_set_phymode(vap, s, len, true);
}
             ...
             acfg_event_type_t type, acfg_event_data_t *acfg_event)
{
	...
	case WL_EVENT_TYPE_DISASSOC_AP:
        ...
        case WL_EVENT_TYPE_WDT_EVENT:
            ...
            memset(wdt->dump_file, '\0', FW_DUMP_FILE_NAME_SIZE);
            strlcpy(wdt->dump_file, acfg_event->dump_file,
                    FW_DUMP_FILE_NAME_SIZE);
    	    break;
#endif
#if UMAC_SUPPORT_ACFG
...
void acfg_process_msg(wbuf_t msg)
{
    ...
    if ((acfg_hdr.msg_type == ACFG_PKT_TYPE_GPR)) {
        ...
        if (ic->ic_gpr_enable && !acfg_gpr_rate_check(vap,acfg_frame)) {
            /*update time period*/
            if (qdf_ktime_to_ms(ms_to_ktime(acfg_frame->period)) !=
                    qdf_ktime_to_ms(ic->ic_gpr_period)) {
                ic->ic_gpr_period = ms_to_ktime(acfg_frame->period);
                ic->ic_period = acfg_frame->period;
...
#if QCA_SUPPORT_GPR && ACFG_NETLINK_TX
    ...
    if (rinfo.rate && rinfo.mcs) {
        ...
        if(sta_info) {
            ...
            if (wlan_node_getsnr(node, &snr_info, WLAN_SNR_RX) == 0) {
                sta_info->diag.rssi = snr_info.avg_snr;
            }
            ...
            /* Get timestamp */
            qdf_time_ktime_get_real_time(&time);
            ...
            /* steps to get mcs, chanwidth, sgi and nss*/
