...
extern int osif_nss_be_vdev_set_peer_nexthop(osif_dev *osif, uint8_t *addr, nss_if_num_t if_num);
...
extern void
ol_ath_mgmt_handler(struct wlan_objmgr_pdev *pdev, struct ol_ath_softc_net80211 *scn, wbuf_t wbuf, struct ieee80211_frame * wh, struct mgmt_rx_event_params rx_event, bool null_data_handler);
...
{
    nss_if_num_t ifnum  = soc->nss_soc.nss_sifnum;
    struct nss_wifi_msg *wifimsg = NULL;
    nss_tx_status_t status;
    void *nss_wifiol_ctx = soc->nss_soc.nss_sctx;
    int radio_id = soc->nss_soc.nss_wifiol_id;

    qdf_info("%s :NSS %pK radio %d if_num %d",__FUNCTION__, nss_wifiol_ctx, radio_id, soc->nss_soc.nss_sifnum);
    ...
    for (i = 0 ; i < msg->nentries ; i++) {
        ...
        if (peer) {
            ...
            if ((vap) && ieee80211_vap_wnm_is_set(vap) && ieee80211_wnm_bss_is_set(vap->wnm)) {
                ni = ieee80211_find_node(ic, peer->mac_addr.raw, WLAN_NSS_IF_ID);
                if (ni) {
        ...
        case NSS_WIFI_STATS_MSG:
            {
                ...
                if (!scn) {
                    qdf_err("scn is NULL, not processing NSS messgage ");
                    break;
                }
                if(soc->soc_attached == 0) {
                    if(pdev_obj != NULL)
        ...
        case NSS_WIFI_RX_MGMT_NULL_TYPE:
            {
                ...
                if (!(htt_pdev->ar_rx_ops->is_fcs_err(rx_mpdu_desc))) {
                    ...
                    ol_ath_mgmt_handler(pdev_obj, scn, nbuf, wh, rx_event, true);
                    discard = 0;
                }
...
static int osif_nss_ol_pktlog_cfg(struct ol_ath_softc_net80211 *scn, int enable)
{
    ...
    qdf_info("pktlog hdrsize %zu msdu_id_offset %d ", scn->pl_dev->pktlog_hdr_size, scn->pl_dev->msdu_id_offset);
#endif
    wifimsg = qdf_mem_malloc(sizeof(struct nss_wifi_msg));
    if (!wifimsg) {
        qdf_print("%s: Failed to allocate memory \n",__func__);
...
static int osif_nss_ol_peer_set_vlan_id(struct ol_ath_softc_net80211 *scn, uint8_t vdev_id,
 ...
 */
static void osif_nss_send_cmd(struct nss_ctx_instance *nss_wifiol_ctx, nss_if_num_t if_num,
                                                       ...
                                                       uint8_t pkt_type, uint8_t cipher_type, uint8_t mic_key[])
{
    ...
    int ifnum = -1;
    struct nss_wifi_msg *wifimsg = NULL;
    struct nss_wifi_peer_security_type_msg *sec_msg = NULL;
    nss_tx_status_t status;
    nss_wifi_msg_callback_t msg_cb;
    struct ol_txrx_pdev_t *pdev;
    struct ol_txrx_peer_t *peer;
    ...
    ifnum = scn->nss_radio.nss_rifnum;
    pdev = osif_nss_ol_get_legacy_pdev_from_scn(scn);
    if (!nss_wifiol_ctx || ifnum == -1 || !pdev) {
        return -1;
    }
    ...
    memset(wifimsg, 0, sizeof(struct nss_wifi_msg));
    sec_msg = &wifimsg->msg.securitymsg;
    sec_msg->peer_id = peer->peer_ids[0];
    ...
    /*
     * Send peer security message
     */
    nss_cmn_msg_init(&wifimsg->cm, ifnum, NSS_WIFI_PEER_SECURITY_CFG,
            sizeof(struct nss_wifi_peer_security_type_msg), msg_cb, NULL);

    status = nss_wifi_tx_msg(nss_wifiol_ctx, wifimsg);
    qdf_mem_free(wifimsg);
    if (status != NSS_TX_SUCCESS) {
        ol_txrx_peer_unref_delete((ol_txrx_peer_handle)peer);
	qdf_print ("Peer security message failed to send to Offload");
        return -1;
    }
...
static QDF_STATUS
osif_nss_ol_set_peer_isolation(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac,
                               uint8_t vdev_id, uint8_t value)
{
    ...
    nss_if_num_t ifnum = -1;
    struct nss_wifi_msg *wifimsg = NULL;
    struct nss_wifi_peer_isolation_msg *iso_msg = NULL;
    nss_tx_status_t status;
    nss_wifi_msg_callback_t msg_cb;
    struct ol_txrx_pdev_t *pdev;
    struct ol_txrx_peer_t *peer;
    ...
    memset(wifimsg, 0, sizeof(struct nss_wifi_msg));
    iso_msg = &wifimsg->msg.isolation_msg;
    iso_msg->peer_id = peer->peer_ids[0];
    iso_msg->isolation = value;

    msg_cb = (nss_wifi_msg_callback_t)osif_nss_ol_event_receive;

    /*
     * Send peer isolation message
     */
    nss_cmn_msg_init(&wifimsg->cm, ifnum, NSS_WIFI_PEER_ISOLATION_MSG,
            sizeof(struct nss_wifi_peer_isolation_msg), msg_cb, NULL);
...
static QDF_STATUS osif_nss_ol_stats_clr(struct ol_ath_softc_net80211 *scn, uint8_t vdev_id)
{
    return QDF_STATUS_SUCCESS;
}

static inline bool osif_nss_ol_ext_vdev_rx(osif_dev *osdev, struct sk_buff *nbuf, struct napi_struct *napi)
{
    return QDF_STATUS_SUCCESS;
}

static inline QDF_STATUS osif_nss_wifiol_peer_set_4addr_cfg(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac,
 ...
 */
static void osif_nss_wifiol_set_dbdc_no_backhaul_radio(struct ieee80211com* ic, bool enable)
{
    return;
}

static inline QDF_STATUS osif_nss_wifiol_peer_update_auth_flag(struct ol_ath_softc_net80211 *scn, uint8_t *peer_mac,
                                                      uint8_t vdev_id, uint32_t auth_flag)
{
    ...
    int ifnum = -1;
    struct nss_wifi_msg *wifimsg = NULL;
    struct nss_wifi_peer_auth_msg *auth_msg = NULL;
    nss_tx_status_t status;
    nss_wifi_msg_callback_t msg_cb;
    struct ol_txrx_pdev_t *pdev;
    struct ol_txrx_peer_t *peer;
    ...
    if (!peer) {
       qdf_print ("%s: Peer is NULL ",__FUNCTION__);
       return QDF_STATUS_E_FAILURE;
    }
    radio_id = pdev->nss_wifiol_id;
    wifimsg = qdf_mem_malloc(sizeof(struct nss_wifi_msg));
    ...
    memset(wifimsg, 0, sizeof(struct nss_wifi_msg));
    auth_msg = &wifimsg->msg.auth_msg;
    auth_msg->peer_id = peer->peer_ids[0];
    auth_msg->auth_flag = auth_flag;

    msg_cb = (nss_wifi_msg_callback_t)osif_nss_ol_event_receive;
    ol_txrx_peer_unref_delete((ol_txrx_peer_handle)peer);

    /*
     * Send peer authorization message
     */
    nss_cmn_msg_init(&wifimsg->cm, ifnum, NSS_WIFI_PEER_AUTH_MSG,
            sizeof(struct nss_wifi_peer_auth_msg), msg_cb, NULL);

    status = nss_wifi_tx_msg(nss_wifiol_ctx, wifimsg);
    qdf_mem_free(wifimsg);
    if (status != NSS_TX_SUCCESS) {
        qdf_print("Peer authorize flag failed to send to NSS");
        return QDF_STATUS_E_FAILURE;
    }
...
static struct nss_wifi_radio_ops nss_wifi_funcs_radio = {
    ...
    osif_nss_ol_set_peer_isolation,
    osif_nss_ol_stats_clr,
#ifdef QCA_SUPPORT_WDS_EXTENDED
    osif_nss_wifiol_peer_set_4addr_cfg,
#endif /* QCA_SUPPORT_WDS_EXTENDED */
    osif_nss_wifiol_set_dbdc_no_backhaul_radio,
};
...
struct nss_vdev_ops nss_wifi_funcs_be = {
    ...
    NULL,
    osif_nss_ol_be_vdev_prepare_wifi_mac_db,
    osif_nss_ol_be_vdev_wifi_mac_db_pool_entries_send,
    osif_nss_ol_be_vdev_wifi_mac_db_is_ready,
    osif_nss_ol_be_vdev_wifi_mac_db_reset_state,
};
...
static struct nss_wifi_internal_ops nss_wifi_funcs_ol_be = {
    ...
    osif_nss_ol_be_check_valid_vdev_for_id,
    ...
    osif_nss_ol_be_get_peer_mac_by_id,
...
#ifdef QCA_SUPPORT_WDS_EXTENDED
    osif_nss_ol_ext_vdev_rx,
#endif /* QCA_SUPPORT_WDS_EXTENDED */
    osif_nss_ol_be_vdev_get_per_pkt_vdev_id_check,
};
#endif
...
struct nss_wifi_soc_ops nss_wifibe_soc_ops = {
    ...
    osif_nss_wifiol_peer_update_auth_flag,
};
