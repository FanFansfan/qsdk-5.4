...
int is_hostie(const uint8_t *ie, uint8_t ie_len)
{
    ...
    /* Do not add any open code other than IE checks in any case */
    switch(elem_id) {
	...
	case IEEE80211_ELEMID_EXTN:
	    /**
             * Skip HE IEs to remove duplication
             *  1. HE Capabilities
             *  2. HE Operations
             *  3. Spatial Reuse Pattern
             *  4. MU EDCA Params
             */
	    if (ie[2] == IEEE80211_ELEMID_EXT_HECAP ||
                    ie[2] == IEEE80211_ELEMID_EXT_HEOP ||
                    ie[2] == IEEE80211_ELEMID_EXT_SRP ||
                    ie[2] == IEEE80211_ELEMID_EXT_MUEDCA) {
                ret = 1;
                goto ret_exit;
            }
            break;
