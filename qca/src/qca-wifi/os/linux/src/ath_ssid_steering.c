...
unsigned int netlink_ath_ssid_event = NETLINK_ATH_SSID_EVENT;
module_param(netlink_ath_ssid_event, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
...
#endif
{
    ...
    while ((skb = skb_get(__skb)) !=NULL){
        ...
        while ((skb = skb_dequeue(&sk->sk_receive_queue)) != NULL) {
            ...
            qdf_nofl_info("recv skb from user space uid:%d pid:%d seq:%d,\n",uid,pid,seq);
            qdf_nofl_info("data is :%s\n",(char *)data);
...
qdf_export_symbol(ath_ssid_steering_netlink_send);
qdf_export_symbol(ath_ssid_steering_netlink_init);
qdf_export_symbol(ath_ssid_steering_netlink_delete);
#endif
