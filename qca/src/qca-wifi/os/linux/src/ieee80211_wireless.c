...
#define PLANNED_ESS_MASK     0x1
#define EDGE_ESS_MASK        0x2
#define BSS_RSSI_THRESH_MASK 0x3F
                            ...
                            struct ieee80211req_athdbg *req)
{
    ...
    wlan_enable_frame_injector(vap, req->data.param[0], req->data.param[1],
                        req->data.param[2], req->data.param[3], req->dstmac);
    return 0;
}

int
ieee80211dbg_addtpeie(struct net_device *dev,
                       struct ieee80211req_athdbg *req)
{
    osif_dev *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;

    wlan_add_tpe_ie(vap, req->data.tpe_conf.tpe_payload.tpe_info_cnt,
                    req->data.tpe_conf.tpe_payload.tpe_info_intrpt,
                    req->data.tpe_conf.tpe_payload.tpe_info_cat,
                    req->data.tpe_conf.tpe_val_len,
                    req->data.tpe_conf.tpe_payload.local_max_txpwr);
    return 0;
}

int
ieee80211dbg_deltpeie(struct net_device *dev,
                       struct ieee80211req_athdbg *req)
{
    osif_dev *osifp = ath_netdev_priv(dev);
    wlan_if_t vap = osifp->os_if;

    wlan_del_tpe_ie(vap, req->data.param[0], req->data.param[1]);
    return 0;
}
...
static const struct iw_priv_args ieee80211_priv_args[] = {
    ...
    */
    ...
    { IEEE80211_PARAM_IGMP_ME,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "igmpmcasten" },
    { IEEE80211_PARAM_IGMP_ME, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_igmpmcasten" },
   ...
   { IEEE80211_PARAM_GET_ACS,
    ...
    { IEEE80211_PARAM_RAWMODE_SIM_DEBUG_LEVEL,
  ...
  { IEEE80211_PARAM_RXINFO_PERPKT,
    ...
    { IEEE80211_PARAM_CLEAR_MIN_MAX_SNR,
    ...
    { IEEE80211_PARAM_DBEACON_SNR_THR,
...
#endif
    { IEEE80211_PARAM_WLAN_PRINT_RL,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0, "print_rl" },
    { IEEE80211_PARAM_WLAN_SCHED_TIMEOUT,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "sched_timeout" },
    ...
    { IEEE80211_PARAM_RAWSIM_DEBUG_NUM_ENCAP_FRAMES,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "rsim_en_frmcnt" },
    { IEEE80211_PARAM_RAWSIM_DEBUG_NUM_ENCAP_FRAMES,
      0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_rsim_en_frmcnt" },
    { IEEE80211_PARAM_RAWSIM_DEBUG_NUM_DECAP_FRAMES,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "rsim_de_frmcnt" },
    { IEEE80211_PARAM_RAWSIM_DEBUG_NUM_DECAP_FRAMES,
      0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_rsim_de_frmcnt" },
#endif /* ATH_PERF_PWR_OFFLOAD && QCA_SUPPORT_RAWMODE_PKT_SIMULATION */
    { IEEE80211_PARAM_MBSS_TXVDEV,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "mbss_tx_vdev" },
    { IEEE80211_PARAM_MBSS_TXVDEV,
    0,IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_mbss_tx_vdev" },
    { IEEE80211_PARAM_HLOS_TID_OVERRIDE,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "hlos_tidoverride" },
    { IEEE80211_PARAM_SM_GAP_PS_ENABLE,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "sm_gap_ps" },
    { IEEE80211_PARAM_SM_GAP_PS_ENABLE, 0,
    IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_sm_gap_ps" },
    { IEEE80211_PARAM_PEER_AUTHORIZE,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "peer_authorize" },
    { IEEE80211_PARAM_AP_CHAN_RPT_FILTER,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "rpt_filter" },
    { IEEE80211_PARAM_AP_CHAN_RPT_FILTER, 0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_rpt_filter" },
    { IEEE80211_PARAM_AP_MAX_AUTH_FAIL,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "max_auth_fail" },
    { IEEE80211_PARAM_AP_MAX_AUTH_FAIL, 0,
      IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "g_max_auth_fail" },
};
    ...
    struct iw_freq *freq, char *extra)
{
    ...
    */
    ...
    if (vap->iv_ic->ic_wideband_capable) {
        if (IS_UP(dev)) {
            if ((BIT(wlan_reg_freq_to_band(vap->iv_ic->ic_curchan->ic_freq)) |
                 BIT(wlan_reg_freq_to_band(ifreq))) ==
                                  (BIT(REG_BAND_5G) | BIT(REG_BAND_6G))) {
                qdf_err("No wideband channel change operation allowed");
                return -EINVAL;
            }
        }
    }
    ...
    struct iw_point *data, char *extra)
{
    ...
    * something; however, there are 128 different values.. */
    ...
    nchans = wlan_get_channel_list(ic, bandmask, chans, IEEE80211_CHAN_MAX);
    range->num_channels = nchans;
    range->num_frequency = 0;

    for (i = 0; i < nchans; i++)
    {
        qdf_freq_t c = chans[i];
        range->freq[range->num_frequency].i =
            wlan_reg_freq_to_chan(ic->ic_pdev_obj, c);
...
waplist_cb(void *arg, wlan_scan_entry_t se)
{
    ...
    /* Use the default NF value if nf_val is invalid */
    if (!nf_val)
        nf_val = IEEE80211_DEFAULT_NOISE_FLOOR;

    set_quality((void *)&req->qual[i], se_snr, nf_val);
...
#define MAX_IE_LENGTH 30 + 255 * 2 /* "xxx_ie=" + encoded IE */
    ...
    set_quality((void *)&iwe.u.qual, se_snr, nf_val);
    current_ev = iwe_stream_add_event(CURRENT_EV,
    ...
    void *w, char *extra)
{
    ...
    if ((retval = ieee80211_ucfg_set_phymode(vap, s, wri->length, true)) < 0) {
        return retval;
    }
    return 0;
}
    ...
    struct iw_request_info *info, void *w, char *extra)
{
    ...
    struct ieee80211req_channel_list *channel_list = NULL;
    ...
    channel_list = (struct ieee80211req_channel_list *)qdf_mem_malloc(
            sizeof(*channel_list));
    if (!channel_list) {
        qdf_err("NULL channel_list");
        qdf_mem_free(channel);
        return -ENOMEM;
    }

    wlan_get_chaninfo(vap, false, channel->ic_chans, channel_list->chans,
                      &channel_list->nchans);

    if (channel_list->nchans > nchans_max) {
        channel_list->nchans = nchans_max;
    }

    qdf_mem_copy(extra, channel_list, channel_list->nchans *
            sizeof(struct ieee80211_channel_info) + sizeof(__u32));
    ...
    void *w, char *extra)
{
    ...
    switch(u->data.flags)
    {
        ...
        case IEEE80211_PARAM_ADD_MAC_LIST_SEC:
            ...
            if (copy_from_user(macaddr, sa->sa_data, QDF_MAC_ADDR_SIZE)) {
                QDF_TRACE(QDF_MODULE_ID_ATF, QDF_TRACE_LEVEL_ERROR, "%s: copy from user error \n", __func__);
                return EFAULT;
            }
            rc = wlan_set_acl_add(vap, macaddr, IEEE80211_ACL_FLAG_ACL_LIST_2);
            if (rc)
                return EFAULT;
            break;
        ...
        case IEEE80211_PARAM_DEL_MAC_LIST_SEC:
            ...
            rc = wlan_set_acl_remove(vap, macaddr, IEEE80211_ACL_FLAG_ACL_LIST_2);
            if (rc)
                return EFAULT;
            break;
...
static void
wlan_debug_dump_nodes_tgt_iter_cb(void *arg, struct ieee80211_node *ni)
{
...
wlan_debug_dump_nodes_tgt(void)
{
    ...
    for_each_netdev(&init_net, dev) {
        ...
        /* node information */
        wlan_mlme_iterate_node_list(ic, wlan_debug_dump_nodes_tgt_iter_cb,
                                    NULL, (IEEE80211_NODE_ITER_F_ASSOC_STA |
...
#if QCA_SUPPORT_SON
    (iw_handler) son_ioctl_ald_getStatistics,   /* SIOCWFIRSTPRIV+25 */
#else
...
void wlan_get_co_located_stats(struct wlan_objmgr_psoc *psoc,
                               void *msg, uint8_t index)
{
    struct ol_ath_radiostats *scn_stats = (struct ol_ath_radiostats *)msg;
    ...
    for (id = 0; id < WLAN_UMAC_MAX_PDEVS; id++) {
        ...
        if (pdev) {
            ...
            if (ic && IEEE80211_IS_CHAN_6GHZ(ic->ic_curchan)) {
                scn_stats->created_vap = wlan_pdev_get_vdev_count(pdev);
                scn_stats->active_vap = ieee80211_get_num_ap_vaps_up(ic);
                scn_stats->rnr_count = ieee80211_get_rnr_count(ic);

                scn = OL_ATH_SOFTC_NET80211(ic);
                soc = scn->soc;
                scn_stats->soc_status_6ghz = soc->target_status;
            }
        }
    }
}
                              ...
                              struct ol_ath_radiostats *scn_stats_user)
{
    ...
    /* get 6ghz radio stats */
    wlan_objmgr_iterate_psoc_list(wlan_get_co_located_stats,
                                  scn_stats_user, WLAN_MLME_NB_ID);
    return 0;
}
...

{
     ...
     * cp_stats component
     */
    ni_stats_user->ns_rx_mgmt_rssi = peer_cp_stats.cs_rx_mgmt_snr;
...
#if WLAN_SUPPORT_TWT
    ni_stats_user->ns_twt_event_type = peer_cp_stats.cs_twt_event_type;
    ni_stats_user->ns_twt_flow_id = peer_cp_stats.cs_twt_flow_id;
    ni_stats_user->ns_twt_bcast = peer_cp_stats.cs_twt_bcast;
    ni_stats_user->ns_twt_trig = peer_cp_stats.cs_twt_trig;
    ni_stats_user->ns_twt_announ = peer_cp_stats.cs_twt_announ;
    ni_stats_user->ns_twt_dialog_id = peer_cp_stats.cs_twt_dialog_id;
    ni_stats_user->ns_twt_wake_dura_us = peer_cp_stats.cs_twt_wake_dura_us;
    ni_stats_user->ns_twt_wake_intvl_us = peer_cp_stats.cs_twt_wake_intvl_us;
    ni_stats_user->ns_twt_sp_offset_us = peer_cp_stats.cs_twt_sp_offset_us;
#endif
    return 0;
}
int wlan_update_vdev_cp_stats(struct ieee80211vap *vap,
                              ...
                              struct ieee80211_mac_stats *mcast_stats_usr)
{
    ...
    vap_stats_usr->fils_frames_sent = vdev_cp_stats.stats.cs_fils_frames_sent;
    vap_stats_usr->fils_frames_sent_fail =
                vdev_cp_stats.stats.cs_fils_frames_sent_fail;
    vap_stats_usr->tx_offload_prb_resp_succ_cnt =
                vdev_cp_stats.stats.cs_tx_offload_prb_resp_succ_cnt;
    vap_stats_usr->tx_offload_prb_resp_fail_cnt =
                vdev_cp_stats.stats.cs_tx_offload_prb_resp_fail_cnt;
    vap_stats_usr->tx_20TU_prb_resp = vap->iv_he_6g_bcast_prob_rsp;
    vap_stats_usr->tx_20TU_prb_interval = vap->iv_he_6g_bcast_prob_rsp_intval;
    ...
    vap_stats_usr->prob_req_drops = vdev_cp_stats.stats.cs_prob_req_drops;
    vap_stats_usr->oob_probe_req_count =
                            vdev_cp_stats.stats.cs_oob_probe_req_count;
                            ...
                            struct ieee80211_mac_stats *vap_multicast_stats)
{
    ...
    vap_stats->ims_tx_packets = vdev_stats->tx.comp_pkt.num;
    vap_stats->ims_tx_bytes = vdev_stats->tx.comp_pkt.bytes;
    return 0;
}
...
#if WLAN_SUPPORT_TWT
    ni_stats->ns_rx_data_twt  = peer_stats->rx.to_stack_twt.num;
#endif
...
#ifdef QCA_SUPPORT_CP_STATS
    ni_stats->ns_rx_decryptcrc += peer_cp_stats_rx_decryptcrc_get(peer);
#endif
...
#if WLAN_SUPPORT_TWT
    ni_stats->ns_tx_data_success_twt = peer_stats->tx.tx_success_twt.num;
#endif
    ...
    ni_stats->ns_tx_ucast = peer_stats->tx.ucast.num;
    {
        ...
        for (mode = 0; mode < DOT11_MAX; mode++) {
            for (mcs = 0; mcs < IEEE80211_MAX_MCS; mcs++) {
                ni_stats->mcs_tx[mode][mcs] =
                peer_stats->tx.pkt_type[mode].mcs_count[mcs];
                ni_stats->mcs_rx[mode][mcs] =
                peer_stats->rx.pkt_type[mode].mcs_count[mcs];
            }
        }
    }
    {
        ...
        for (ru_index = 0; ru_index < RU_INDEX_MAX; ru_index++)
            ni_stats->ru_tx[ru_index] =
            peer_stats->tx.ru_loc[ru_index].num_msdu;
    }
...
int ieee80211_config_oce_tx_power(wlan_if_t vap, char *extra)
{
    int *val = (int*)extra;
    qdf_info("txpwr cmd: %d ,txpwr en/dl: %d , txpwr default(zero)/user def(non zero): %d ", val[0], val[1], val[2]);

    vap->iv_tx_power_enable = val[1];
    vap->iv_tx_power_value = val[2];
    return 0;
}

int
ieee80211_config_oce_ipsubnet_id(wlan_if_t vap, char *params)
{
    uint32_t *config = (uint32_t *) params;
    uint64_t ip_subnet = (uint64_t) config[1] << 32 | config[2];

    vap->iv_ip_subnet_id = ip_subnet;
    qdf_info("ip_subnet_id configured: %llx", vap->iv_ip_subnet_id);
    return 0;
}

int
ieee80211_config_oce_ess_rpt_param(wlan_if_t vap, char *extra)
{

    uint8_t param[2];
    uint32_t *config = (uint32_t *) extra;


    param[0] = (uint8_t)(config[1] & 0x000000ff);
    param[1] = (uint8_t)(config[2] & 0x000000ff);

    if (param[0] & PLANNED_ESS_MASK) {
        vap->iv_planned_ess = 1;
    } else {
        vap->iv_planned_ess = 0;
        vap->iv_edge_ess = 0;
        vap->iv_bss_rssi_threshold = 0;
        return 0;
    }

    if (param[0] & EDGE_ESS_MASK)
        vap->iv_edge_ess = 1;
    else
        vap->iv_edge_ess = 0;

    vap->iv_bss_rssi_threshold = param[1] & BSS_RSSI_THRESH_MASK;

   qdf_info("planned ess: %d, edge of ess: %d, bss rssi threshold value: %d",
            vap->iv_planned_ess, vap->iv_edge_ess,
            vap->iv_bss_rssi_threshold);

    return 0;

}

int ieee80211_config_rsnx(wlan_if_t vap, char *extra)
{
    int *val = (int*)extra;
    vap->iv_rsnx_override = (uint8_t)(val[1] & 0x000000ff);
    qdf_info("RSNX IE override: 0x%x", vap->iv_rsnx_override);
                           ...
                           struct ol_ath_radiostats *scn_stats)
{
    ...
    if ((target_type == TARGET_TYPE_QCA8074) ||
        ...
        if (!cdp_host_get_pdev_stats(soc_txrx_handle, pdev_id, pdev_stats)) {
            ...
            scn_stats->rx_rssi_comb = pdev_stats->rx.snr;
...
static int
ieee80211dbg_sendmscsresp(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    wlan_if_t vap = NETDEV_TO_VAP(dev);
    struct ieee80211_mscs_resp *mscs_resp = &req->data.mscsresp;

    return wlan_send_mscs_resp(vap, req->dstmac, mscs_resp);
}
...
ieee80211acs_ioctl_setchanlist(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    for (i = chan_idx; chans[i].chan; i++) {
         freq = wlan_band_id_chan_to_freq_from_cur_chan_list(ic,
                                                             chans[i].chan,
                                                             chans[i].band);
         if (!freq) {
             qdf_alert("Invalid input chan: %d, band: %d\n", chans[i].chan, chans[i].band);
             return -EINVAL;
         }
...
ieee80211dbg_ioctl_get_last_beacon_rssi(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    ni = ieee80211_find_node(vap->iv_ic, info->macaddr, WLAN_MLME_SB_ID);
    if (ni == NULL) {
        qdf_info("ni not found with provided macaddr\n");
        return -EINVAL;
    }

    info->stats.ni_last_bcn_snr = ni->ni_last_bcn_snr;
...
ieee80211_ioctl_supported_freq_band(struct net_device *dev,struct ieee80211req_athdbg *req)
{
    ...
    for (i=0; i < nmodes; i++) {
        if (ieee80211_is_phymode_11g(modes[i]))
            support_2g = 1;
        if (ieee80211_is_phymode_11a(modes[i]))
            support_5g = 1;
    }
...
ieee80211tr69_ioctl_get11hsupported(struct net_device *dev, struct ieee80211req_athdbg *req)
{
	...
	for (i = 0; i < nmodes; i++) {
		if (ieee80211_is_phymode_11a(modes[i]) ||
		    ieee80211_is_phymode_11ac(modes[i]) ||
		    ieee80211_is_phymode_11na(modes[i])){
			*supported = 1;
			return;
		}else
			*supported = 0;
	...
	switch(cmdid){
		...
		case TR069_GET_RRM_UTIL:
			...
			if (chan) {
				...
				rrm_util->radar_detect = IEEE80211_IS_CHAN_RADAR(ic, chan);
			}
#ifdef QCA_SUPPORT_CP_STATS
			rrm_util->chann_util =
...
ieee80211dbg_ioctl_mu_scan(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    ...
    for (i = 0; i < sizeof(modes) / sizeof(modes[0]); i++) {
        ...
        channel = ieee80211_find_dot11_channel(ic, wlan_reg_legacy_chan_to_freq(ic->ic_pdev_obj, reqptr->mu_channel), 0,
        ...
        retv = wlan_set_channel(vap, wlan_reg_legacy_chan_to_freq(ic->ic_pdev_obj, reqptr->mu_channel), 0);
        if (!retv) {
            TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
...
ieee80211dbg_ioctl_atf_dump_nodestate(struct net_device *dev, struct ieee80211req_athdbg *req)
{
    int ret = -EINVAL;
#if QCA_AIRTIME_FAIRNESS
    atf_err("This command is deprecated\n");
#endif
    return ret;
}
                       ...
                       struct ieee80211req_athdbg *req)
{
    ...
    ni = ieee80211_find_node(ic, wh->i_addr2, WLAN_MLME_SB_ID);
    if (ni == NULL) {
...
ieee80211_ioctl_getstastats(struct net_device *dev, struct iwreq *iwr)
{
    struct ieee80211vap *vap = NETDEV_TO_VAP(dev);
    struct ieee80211_node *ni;
    struct ieee80211_nodestats *node_stats_user = NULL;
    u_int8_t macaddr[QDF_MAC_ADDR_SIZE];
    ...
    if (ni == NULL)
        return -EINVAL;     /* XXX */
    node_stats_user = (struct ieee80211_nodestats *)qdf_mem_malloc(sizeof(struct ieee80211_nodestats));
    qdf_mem_zero(node_stats_user, sizeof(struct ieee80211_nodestats));
    if (!node_stats_user)
        return -EINVAL;
    ...
    node_stats_user->ns_rssi = ni->ni_snr;
...
static int
ieee80211_ioctl_me_list(struct net_device *dev, struct iwreq *iwr)
{
    int ret = 0;
    wlan_if_t vap = NETDEV_TO_VAP(dev);
    struct ieee80211_wlanconfig *config;
        ...
        struct iwreq *iwr)
{
    ...
    wlan_get_chaninfo(vap, true, channel->ic_chans, channel_list->chans,
                      &channel_list->nchans);
    ...
    if(copy_to_user(buf, channel_list, channel_list->nchans *
            sizeof(struct ieee80211_channel_info) + sizeof(__u32))){
...
ieee80211_ioctl_config_generic(struct net_device *dev, struct iwreq *iwr)
{
    ...
    switch (cmdtype) {
        ...
        case IEEE80211_WLANCONFIG_ME_LIST_DUMP:
            return ieee80211_ioctl_me_list(dev, iwr);
#endif
#if UMAC_SUPPORT_WNM
                             ...
                             void *wri_pointer, unsigned int data_size)
{
    ...
    switch (req->cmd) {
        ...
        case IEEE80211_DBGREQ_FRAME_INJECTOR:
            ...
            break;
        case IEEE80211_DBGREQ_ADD_TPE:
            retv = ieee80211dbg_addtpeie(dev, req);
            break;
        case IEEE80211_DBGREQ_DEL_TPE:
            retv = ieee80211dbg_deltpeie(dev, req);
            break;
...
#endif
#if WLAN_SUPPORT_MSCS
        case IEEE80211_DBGREQ_SENDMSCSRESP:
            retv = ieee80211dbg_sendmscsresp(dev, req);
            break;
#endif
...
#define ATH_FIPS_TIMEOUT ((CONVERT_SEC_TO_SYSTEM_TIME(2)/100)) /* 20 ms */
    ...
    case IEEE80211_DBGREQ_MESH_SET_GET_CONFIG:
        retv = wlan_mesh_set_get_params(dev, req, wri_pointer);
        break;

    case IEEE80211_DBGREQ_PEER_LATENCY_PARAM_CONFIG:
        retv = wlan_mesh_set_peer_latency_param_config(vap, req);
        break;
