 ...
 */
int osif_nss_be_vdev_set_peer_nexthop(osif_dev *osif, uint8_t *addr, nss_if_num_t if_num)
{
...
bool osif_nss_ol_be_check_valid_vdev_for_id(struct cdp_soc_t *soc, uint8_t vdev_id)
{
    ...
    return ol_txrx_get_vdev_from_pdev(pdev, vdev_id)?true:false;
}
...
void
osif_nss_ol_be_rsim_rx_decap(os_if_t osif, qdf_nbuf_t *pdeliver_list_head, qdf_nbuf_t *pdeliver_list_tail, uint8_t *peer_mac)
{
#if QCA_SUPPORT_RAWMODE_PKT_SIMULATION
...
int32_t osif_nss_be_vap_updchdhdr(osif_dev *osifp)
{
    ...
    if (!NSS_IF_IS_TYPE_DYNAMIC(nss_ifnum)) {
        qdf_err(" vap transmit called on invalid interface %p", osifp);
        return -1;
    }
 ...
 */
bool osif_nss_ol_be_vdev_wifi_mac_db_is_ready(osif_dev *osdev)
{
	return true;
}
 ...
 */
bool osif_nss_ol_be_vdev_wifi_mac_db_reset_state(osif_dev *osdev)
{
	return true;
}
