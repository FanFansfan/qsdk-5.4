    ...
    IEEE80211_WLANCONFIG_CMDTYPE cmdtype, char *addr1, char *addr2)
{
    ...
    switch (cmdtype) {
        case IEEE80211_WLANCONFIG_HMWDS_ADD_ADDR:
            memcpy(config->data.hmwds.wds_ni_macaddr, temp2, QDF_MAC_ADDR_SIZE);
            config->data.hmwds.wds_macaddr_cnt = 1;
            memcpy(config->data.hmwds.wds_macaddr, temp1, QDF_MAC_ADDR_SIZE);
            break;
        ...
        case IEEE80211_WLANCONFIG_HMWDS_READ_ADDR:
            ...
            for (i = 0; i < config->data.hmwds.wds_macaddr_cnt; i++) {
                printf("\t%02x:%02x:%02x:%02x:%02x:%02x\n",
                        *(config->data.hmwds.wds_macaddr + i * QDF_MAC_ADDR_SIZE),
                        *(config->data.hmwds.wds_macaddr + i * QDF_MAC_ADDR_SIZE + 1),
                        *(config->data.hmwds.wds_macaddr + i * QDF_MAC_ADDR_SIZE + 2),
                        *(config->data.hmwds.wds_macaddr + i * QDF_MAC_ADDR_SIZE + 3),
                        *(config->data.hmwds.wds_macaddr + i * QDF_MAC_ADDR_SIZE + 4),
                        *(config->data.hmwds.wds_macaddr + i * QDF_MAC_ADDR_SIZE + 5));
            }
            break;
        case IEEE80211_WLANCONFIG_HMWDS_READ_TABLE:
            printf("WDS nodes: \n");
