...
int main(int argc, char *argv[])
{
    ...
    } else if (streq(cmd, "deny_list")) {
        handle_command = handle_command_deny_list;
#endif
    ...
    else if (streq(cmd, "peer_isolation")) {
        handle_command = handle_command_peer_isolation;
    }
#endif
    else if (streq(cmd,"rtt")) {
        handle_command = handle_command_rtt;
    }
#if WLAN_CFR_ENABLE
...
void usage(void)
{
    ...
    fprintf(stderr, "usage: wlanconfig athX deny_list add ip mask\n");
    fprintf(stderr, "usage: wlanconfig athX deny_list del ip mask\n");
    fprintf(stderr, "usage: wlanconfig athX deny_list dump\n");
#endif
#if UMAC_SUPPORT_WNM
    ...
    fprintf(stderr, "usage: wlanconfig athX showatfstats\n");
    fprintf(stderr, "usage: wlanconfig athX showatfacstats\n");
...
#endif
#if QCA_SUPPORT_PEER_ISOLATION
    fprintf(stderr, "usage: wlanconfig athX peer_isolation add <addr eg: xx:xx:xx:xx:xx:xx>\n");
    fprintf(stderr, "usage: wlanconfig athX peer_isolation del <addr eg: xx:xx:xx:xx:xx:xx>\n");
    fprintf(stderr, "usage: wlanconfig athX peer_isolation list\n");
    fprintf(stderr, "usage: wlanconfig athX peer_isolation flush\n");
#endif
    fprintf(stderr, "                  wlanmode [sta|adhoc|ap|monitor|wrap|p2pgo|p2pcli|p2pdev|specialvap|mesh|smart_monitor|lp_iot_mode]\n"
                    "                  [wlanaddr <mac_addr>] [mataddr <mac_addr>] [bssid|-bssid] [vapid <0-15>] [nosbeacon]\n");
#if WLAN_CFR_ENABLE
    ...
    fprintf(stderr, "wlanconfig athx cfr capture_count <16 bit count_value>\n");
    fprintf(stderr, "wlanconfig athx cfr capture_intervalmode_sel <value 0:duration mode 1:count mode>\n");
#endif
#endif
#ifdef WLAN_SUPPORT_RX_FLOW_TAG
