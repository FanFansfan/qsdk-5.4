...
static void usage(void)
{
    ...
    fprintf(stderr, "cmd: [sendmscsresp] \n");
    ...
    fprintf(stderr, "cmd: [ap_twt_btwt_invite_sta]\n");
    fprintf(stderr, "cmd: [ap_twt_btwt_remove_sta]\n");
#endif
#endif
    ...
    fprintf(stderr, "cmd: [add_tpe]\n");
    fprintf(stderr, "cmd: [peer_latency_param_config]\n");
    exit(-1);
}
...
{
    fprintf(stderr, "usage: wifitool athX offchan_rx_test [-band X] chan dwell_time bw_mode(optional) sec_chan(only for 80+80) \n");
    exit(-1);
}
...
{
    fprintf(stderr, "usage: wifitool athX frame_injector_en <frametype> <enable|disable> <inject_period> <dest_macaddr> <frame_duration>\n");
    exit(-1);
}

static void usage_add_tpe(void)
{
    fprintf(stderr, "usage: wifitool athX add_tpe <txpwr_cnt> <txpwr_intrpt> <txpwr_cat> <txpwr_val1>..... \n");
    exit(-1);
}

static void usage_del_tpe(void)
{
    fprintf(stderr, "usage: wifitool athX del_tpe <txpwr_intrpt> <txpwr_cat> \n");
    exit(-1);
}
...
{
    fprintf(stderr, "usage: wifitool athX sendbstmreq <mac_addr> <candidate_list> <disassoc_timer> <validity_itrv> "
            "[disassoc_imminent] [bss_term_inc] [bss_term_tsf] [bss_term_duration] [abridged] [opt_ie...] \n");
    exit(-1);
}
...
static void usage_sendmscsresp(void)
{
    fprintf(stderr, "usage: wifitool athX sendmscsresp <mac_addr> <status_code>\n");
    exit(-1);
}
...
static void usage_set_peer_latency_param_config(void)
{
   fprintf(stderr, "usage: wifitool athX set_set_peer_latency_param_config <peer_mac> <tid> <dl/ul> <service interval> <burst size> <add/sub>\n");
}
...
#define MAX_CHANNUM_PER_REGCLASS 59
    typedef enum {
        REGCLASS_1 = 1,
        REGCLASS_2 = 2,
        REGCLASS_3 = 3,
    ...
    while (token != NULL) {
        ...
        if (bcnrpt->regclass[0] < REGCLASS_131) {
            switch (channum) {
                case 1:
                case 2:
                case 3:
    ...
    for (regclassnum = REGCLASS_1; regclassnum < REGCLASS_MAX; regclassnum++) {
        if (regclass[regclassnum].numchans > 0) {
            ...
            if ((bcnrpt->regclass[0] == 81) || (bcnrpt->regclass[0] == 115) || (bcnrpt->regclass[0] == 131))
                bcnrpt->apchanrep[num_chanrep].regclass = bcnrpt->regclass[0];
            else
...
static int send_rpt(const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    int chan_rptmode = 0;
    bool is_multi_regclass = false;
    switch (req_cmd) {
        case IEEE80211_DBGREQ_SENDBCNRPT:
            cmd = IEEE80211_DBGREQ_SENDBCNRPT;
            arg_index = 3;
            if (streq(argv[arg_index + 1], "--num-opclass")) {
                is_multi_regclass = true;
                arg_num = 19;
            } else {
                arg_num = 14;
            }
            ...
            case IEEE80211_DBGREQ_SENDBCNRPT:
                {
                    ...
                    if (is_multi_regclass) {
                        arg_index++;
                        bcnrpt->num_regclass = 0;
                        for (i = 0; i < IEEE80211_NUM_REGCLASS; i++) {
                            if (atoi(argv[arg_index + i]) != 0) {
                                bcnrpt->regclass[i] = atoi(argv[arg_index + i]);
                                bcnrpt->num_regclass++;
                            } else {
                                break;
                            }
                        }
                        if (!bcnrpt->num_regclass) {
                            errx(1, "All operating classes are zero");
                            return -1;
                        }
                        arg_index += IEEE80211_NUM_REGCLASS;
                    } else {
                        bcnrpt->num_regclass = 1;
                        bcnrpt->regclass[0] = atoi(argv[arg_index++]);
                    }
                }
                bcnrpt->channum = atoi(argv[arg_index++]);
                bcnrpt->random_ivl = atoi(argv[arg_index++]);
                bcnrpt->duration = atoi(argv[arg_index++]);
                bcnrpt->mode = atoi(argv[arg_index++]);
                bcnrpt->req_ssid = atoi(argv[arg_index++]);
                bcnrpt->rep_cond = atoi(argv[arg_index++]);
                bcnrpt->rep_detail = atoi(argv[arg_index++]);
                bcnrpt->req_ie = atoi(argv[arg_index++]);
                chan_rptmode = atoi(argv[arg_index++]);
                if (argc < (arg_index + 1)) {
                    memset(bcnrpt->bssid,0xff,QDF_MAC_ADDR_SIZE);
                } else {
                    if (!wifitool_mac_aton(argv[arg_index++], bcnrpt->bssid, 6)) {
                        errx(1, "Invalid BSSID");
                        return -1;
                    }
                }
                if (argc < (arg_index + 1)) {
                    ...
                    bcnrpt->lastind = atoi(argv[arg_index++]);
                }
                if (!chan_rptmode) {
                    bcnrpt->num_chanrep = 0;
                    /*Skip 16th argument as chan_rptmode is set to 0*/
                    arg_index++;
                } else if (argc < (arg_index + 1)) {
                    /* If no channel is provided, use pre-canned channel values */
                    if (bcnrpt->regclass[0] == 81) {
                        /* Global op class 81 */
                        bcnrpt->num_chanrep = 1;
                        bcnrpt->apchanrep[0].regclass = bcnrpt->regclass[0];
                        bcnrpt->apchanrep[0].numchans = 2;
                        bcnrpt->apchanrep[0].channum[0] = 1;
                        bcnrpt->apchanrep[0].channum[1] = 6;
                    } else if (bcnrpt->regclass[0] == 115) {
                        /* Global op class 115 */
                        bcnrpt->num_chanrep = 1;
                        bcnrpt->apchanrep[0].regclass = bcnrpt->regclass[0];
                        bcnrpt->apchanrep[0].numchans = 2;
                ...
                } else if (bcnrpt_append_chan_report(bcnrpt, argc, argv, arg_index++) < 0) {
                ...
                if (argc < (arg_index + 1)) {
                    ...
                    bcnrpt->reqmode = atoi(argv[arg_index++]);
                }
...
static int set_bssidpref(const char *ifname,int argc, char *argv[])
{
 ...
 /*
  * send_mscs_resp: Sends MSCS Response to the STA supporting
  * MSCS Procedures.
  * @ifname: interface name
  * @argc: total number of command line arguments
  * @argv: values of command line arguments
...
static int send_mscsresp (const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req = {0};
    int arg_num = 5;
    int cmd = IEEE80211_DBGREQ_SENDMSCSRESP;
    usage_cmd = usage_sendmscsresp;

    if (argc < arg_num) {
        usage_sendmscsresp();
    } else {
        struct ieee80211_mscs_resp* mscs_info = &req.data.mscsresp;
        req.cmd = cmd;
        req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
        ...
        mscs_info->status_code = atoi(argv[4]);
        mscs_info->mscs_dialog_token = 0;
        send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
    }
    return 0;
}
...
static int send_req (const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    if (argc < arg_num) {
        ...
        if (req_cmd == IEEE80211_DBGREQ_SENDBSTMREQ) {
            ...
            if (argc <= 12 + ieee80211_bstm_req_max_optie) {
                i = 0;
                while (argc > (12 + i)) {
                    reqinfo->optie_buf[i] = atoi(argv[12 + i]);
                    if (++i >= ieee80211_bstm_req_max_optie) {
                        break;
                    }
                }
                reqinfo->optie_len = i;
            } else {
                errx(1, "Vendor IE too big, supported is %d bytes", ieee80211_bstm_req_max_optie);
            }
        }
        send_command(&sock_ctx, ifname, &req, sizeof(req), NULL, QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
    }
...
void print_channel_survey_stats (struct cfg80211_data *arg)
{
    ...
    if (!chan_stats) {
        fprintf(stderr, "Channel stats are not populated\n");
        return;
    }
    ...
    if (flags == CFG80211_GET_CHAN_SURVEY_HOME_CHANNEL_STATS) {
        if (num_chan_stats != 1) {
            fprintf(stderr, "Unexpected number of channel stats %u", num_chan_stats);
            return;
        }
        printf("Home channel survey stats\n");
        ...
        for (chan_stats_index = 0; chan_stats_index < num_chan_stats; chan_stats_index++) {
...
int add_tpe_ie(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req;
    uint8_t iter;
    ...
    if (argc < 7) {
        ...
        memset(&req, 0, sizeof(struct ieee80211req_athdbg));
        req.cmd = IEEE80211_DBGREQ_ADD_TPE;
        req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
        req.data.tpe_conf.tpe_payload.tpe_info_cnt = atoi(argv[3]);
        req.data.tpe_conf.tpe_payload.tpe_info_intrpt = atoi(argv[4]);
        req.data.tpe_conf.tpe_payload.tpe_info_cat = atoi(argv[5]);
        /* Calculate the number of Max Tx Pwr values input by user
         * Arguments argv[6] to argv[n] will be Tx Pwr values input by user
         *
         * Total Tx Pwr values input by user = (total args - 6)*/
        req.data.tpe_conf.tpe_val_len = (argc - 6);

        for(iter = 0; iter < req.data.tpe_conf.tpe_val_len; iter++) {
            req.data.tpe_conf.tpe_payload.local_max_txpwr[iter] =
                                                        atoi(argv[iter + 6]);
        }
...
int del_tpe_ie(const char *ifname, int argc, char *argv[])
{
    struct ieee80211req_athdbg req;
    ...
    if (argc < 5) {
        ...
        memset(&req, 0, sizeof(struct ieee80211req_athdbg));
        req.cmd = IEEE80211_DBGREQ_DEL_TPE;
        req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
        req.data.param[0] = atoi(argv[3]);
        req.data.param[1] = atoi(argv[4]);
...
int bsteer_set_cmd (const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    mesh_req->mesh_cmd = cmd;
    req.cmd = IEEE80211_DBGREQ_MESH_SET_GET_CONFIG;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;

    switch (req_cmd) {
        case MESH_BSTEERING_SET_PARAMS:
            mesh_req->mesh_data.bsteering_param.inactivity_timeout_overload = atoi(argv[4]);
            mesh_req->mesh_data.bsteering_param.utilization_sample_period = atoi(argv[5]);
            mesh_req->mesh_data.bsteering_param.utilization_average_num_samples = atoi(argv[6]);
            mesh_req->mesh_data.bsteering_param.low_rssi_crossing_threshold = atoi(argv[9]);
            mesh_req->mesh_data.bsteering_param.inactivity_check_period = atoi(argv[10]);
            mesh_req->mesh_data.bsteering_param.low_tx_rate_crossing_threshold = atoi(argv[11]);
            mesh_req->mesh_data.bsteering_param.low_rate_rssi_crossing_threshold = atoi(argv[13]);
            mesh_req->mesh_data.bsteering_param.interference_detection_enable = atoi(argv[16]);
            mesh_req->mesh_data.bsteering_param.client_classification_enable = atoi(argv[17]);
            for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
                mesh_req->mesh_data.bsteering_param.inactivity_timeout_normal[index] = atoi(argv[3]);
                mesh_req->mesh_data.bsteering_param.inactive_rssi_xing_high_threshold[index] = atoi(argv[7]);
                mesh_req->mesh_data.bsteering_param.inactive_rssi_xing_low_threshold[index] = atoi(argv[8]);
                mesh_req->mesh_data.bsteering_param.high_tx_rate_crossing_threshold[index] = atoi(argv[12]);
                mesh_req->mesh_data.bsteering_param.high_rate_rssi_crossing_threshold[index] = atoi(argv[14]);
                mesh_req->mesh_data.bsteering_param.ap_steer_rssi_xing_low_threshold[index] = atoi(argv[15]);
            }
            break;

        case MESH_BSTEERING_SET_DBG_PARAMS:
            mesh_req->mesh_data.bsteering_dbg_param.raw_chan_util_log_enable = atoi(argv[3]);
            mesh_req->mesh_data.bsteering_dbg_param.raw_rssi_log_enable = atoi(argv[4]);
            mesh_req->mesh_data.bsteering_dbg_param.raw_tx_rate_log_enable = atoi(argv[5]);
            break;
        ...
        case MESH_BSTEERING_SET_STEERING:
            ...
            mesh_req->mesh_data.value = atoi(argv[4]);
            break;

        case MESH_BSTEERING_SET_OVERLOAD:
            mesh_req->mesh_data.value = atoi(argv[3]);
            break;
...
void print_acl_params(ieee80211_acl_cli_param_t *cli)
{
    ...
    PRINT_STRUCTURE_VALUE(cli, inact_snr_xing);
    PRINT_STRUCTURE_VALUE(cli, low_snr_xing);
    PRINT_STRUCTURE_VALUE(cli, low_rate_snr_xing);
    PRINT_STRUCTURE_VALUE(cli, high_rate_snr_xing);
...
void get_acl_params(ieee80211_acl_cli_param_t *cli, char *argv[], int start_arg)
{
    ...
    GET_STRUCTURE_VALUE(cli, inact_snr_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, low_snr_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, low_rate_snr_xing, argv[start_arg++]);
    GET_STRUCTURE_VALUE(cli, high_rate_snr_xing, argv[start_arg++]);
...
int bsteer_get_cmd(const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    req.cmd = IEEE80211_DBGREQ_MESH_SET_GET_CONFIG;
    req.needs_reply = DBGREQ_REPLY_IS_REQUIRED;
    if (req_cmd == MESH_BSTEERING_GET_DATARATE_INFO) {
        if (!wifitool_mac_aton(argv[3], &req.dstmac[0], 6)) {
            errx(1, "Invalid mac address");
            return -1;
        }
    ...
    switch (req_cmd) {
        case MESH_BSTEERING_GET_PARAMS:
            ...
            printf("Overload inactivity timeout: %u s\n",
                    mesh_req->mesh_data.bsteering_param.inactivity_timeout_overload);
            printf("Utilization sampling period: %u s\n",
                    mesh_req->mesh_data.bsteering_param.utilization_sample_period);
            printf("Utilization num samples to average: %u\n",
                    mesh_req->mesh_data.bsteering_param.utilization_average_num_samples);
            printf("Low RSSI crossing threshold: %u dB\n",
                    mesh_req->mesh_data.bsteering_param.low_rssi_crossing_threshold);
            printf("Inactivity check interval: %u s\n",
                    mesh_req->mesh_data.bsteering_param.inactivity_check_period);
            printf("Active steering low threshold: Tx rate %u Kbps, RSSI %u dB\n",
                    mesh_req->mesh_data.bsteering_param.low_tx_rate_crossing_threshold,
                    mesh_req->mesh_data.bsteering_param.low_rate_rssi_crossing_threshold);
            printf("Interference detection enable: %u\n",
                    mesh_req->mesh_data.bsteering_param.interference_detection_enable);
            printf("Client classification enable: %u\n",
                    mesh_req->mesh_data.bsteering_param.client_classification_enable);
            for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
                printf("Normal inactivity timeout [Class %d]: %u s\n",
                        index, mesh_req->mesh_data.bsteering_param.inactivity_timeout_normal[index]);
                printf("Inactive RSSI crossing high threshold [Class %d]: %u dB\n",
                        index, mesh_req->mesh_data.bsteering_param.inactive_rssi_xing_high_threshold[index]);
                printf("Inactive RSSI crossing low threshold [Class %d]: %u dB\n",
                        index, mesh_req->mesh_data.bsteering_param.inactive_rssi_xing_low_threshold[index]);
                printf("Active steering high threshold [Class %d]: Tx rate %u Kbps, RSSI %u dB\n",
                        index, mesh_req->mesh_data.bsteering_param.high_tx_rate_crossing_threshold[index],
                        mesh_req->mesh_data.bsteering_param.high_rate_rssi_crossing_threshold[index]);
                printf("AP steering low RSSI threshold [Class %d]: %u dB\n",
                        index, mesh_req->mesh_data.bsteering_param.ap_steer_rssi_xing_low_threshold[index]);
            }
            break;
        ...
        case MESH_BSTEERING_GET_DBG_PARAMS:
            ...
            printf("Enable raw channel utilization logging: %s\n",
                    mesh_req->mesh_data.bsteering_dbg_param.raw_chan_util_log_enable ?
                    "yes" : "no");
            printf("Enable raw RSSI measurement logging: %s\n",
                    mesh_req->mesh_data.bsteering_dbg_param.raw_rssi_log_enable ?
                    "yes" : "no");
            break;
...
int bsteer_enable(const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    err = send_command(&sock_ctx, ifname, &req, sizeof(req), NULL,
                       QCA_NL80211_VENDOR_SUBCMD_DBGREQ, IEEE80211_IOCTL_DBGREQ);
    if (req_cmd == MESH_BSTEERING_ENABLE) {
        if (err < 0) {
            fprintf(stderr, "band steering enable failed\n");
        }
    }
    return 0;
}
...
void bsteer_getrssi(const char *ifname, int argc, char *argv[])
{
    ...
    mesh_req = &req.data.mesh_dbg_req;
    mesh_req->mesh_cmd = MESH_BSTEERING_GET_RSSI;
    req.cmd = IEEE80211_DBGREQ_MESH_SET_GET_CONFIG;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
...
int bsteer_setauthallow(const char *ifname, int argc, char *argv[])
{
    ...
    mesh_req = &req.data.mesh_dbg_req;
    mesh_req->mesh_cmd = MESH_BSTEERING_SET_AUTH_ALLOW;
    req.cmd = IEEE80211_DBGREQ_MESH_SET_GET_CONFIG;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
...
int bsteer_set_sta_stats_update_interval_da(const char *ifname, int argc, char *argv[])
{
    ...
    mesh_req = &req.data.mesh_dbg_req;
    mesh_req->mesh_cmd = MESH_BSTEERING_SET_DA_STAT_INTV;
    mesh_req->mesh_data.bsteering_sta_stats_update_interval_da = atoi(argv[3]);
    req.cmd = IEEE80211_DBGREQ_MESH_SET_GET_CONFIG;
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
...
#define FCS_LEN 4
                    ...
                    if (output_fips->data_len < sizeof(exp_op) && exp_op_len < sizeof(exp_op) &&
                        memcmp(ptr, exp_op, exp_op_len) == 0)
                        fips_fail = 0;
                    else
                        fips_fail = 1;
...
int offchan_rx_get_params(char *str, uint8_t *bw_mode, uint8_t *sec_chan_offset)
{
  ...
  *sec_chan_offset = OFFCHAN_RX_SCN;
  ...
  } else if (strcmp("80+80", str) == 0) {
     printf("\n80+80 bwmode is not supported ");
     return -1;
  } else {
...
static void set_peer_latency_param_config(const char *ifname, int argc, char *argv[]) {
    struct ieee80211req_athdbg req = {0};
    if ((argc < 9) || (argc > 9)) {
        usage_set_peer_latency_param_config();
        return;
    }
    memset(&req, 0, sizeof(struct ieee80211req_athdbg));
    req.cmd = IEEE80211_DBGREQ_PEER_LATENCY_PARAM_CONFIG;
    req.data.wlan_latency_info.tid = atoi(argv[4]);
    req.data.wlan_latency_info.dl_ul_enable = atoi(argv[5]);
    req.data.wlan_latency_info.service_interval = atoi(argv[6]);
    req.data.wlan_latency_info.burst_size = atoi(argv[7]);
    req.data.wlan_latency_info.burst_size_add_or_sub = atoi(argv[8]);
    if (!wifitool_mac_aton(argv[3],
                    &req.data.wlan_latency_info.peer_mac[0], 6)) {
        errx(1, "Invalid mac address");
        return;
    }
    req.needs_reply = DBGREQ_REPLY_IS_NOT_REQUIRED;
...
int ap_twt_dialog_req_cmd (const char *ifname, int argc, char *argv[], int req_cmd)
{
    ...
    switch (req_cmd) {
...
#ifdef WLAN_SUPPORT_BCAST_TWT
            arg_num = 6;
            strlcpy(usage_msg,"wifitool athX ap_twt_del_dialog <peer_mac> <dialog id> <b_twt_recommendation>\n",sizeof(usage_msg));
#else
            arg_num = 5;
        ...
        case IEEE80211_DBGREQ_TWT_BTWT_INVITE_STA:
            cmd = IEEE80211_DBGREQ_TWT_BTWT_INVITE_STA;
            arg_num = 5;
            strlcpy(usage_msg,"Usage:wifitool athX ap_twt_btwt_invite_sta <peer_mac> <dialog_id>\n",sizeof(usage_msg));
            break;

        case IEEE80211_DBGREQ_TWT_BTWT_REMOVE_STA:
            cmd = IEEE80211_DBGREQ_TWT_BTWT_REMOVE_STA;
            arg_num = 5;
            strlcpy(usage_msg,"wifitool athX ap_twt_btwt_remove_sta <peer_mac> <dialog_id>\n",sizeof(usage_msg));
            break;
#endif
        default:
            return -1;
    ...
    switch (req_cmd) {
...
#ifdef WLAN_SUPPORT_BCAST_TWT
            req.data.twt_del_pause.b_twt_persistence = atoi(argv[5]);
#endif
            break;
        ...
        case IEEE80211_DBGREQ_TWT_BTWT_INVITE_STA:
        case IEEE80211_DBGREQ_TWT_BTWT_REMOVE_STA:
            req.data.twt_btwt_sta_inv_remove.dialog_id = atoi(argv[4]);
            break;
#endif
...
int main(int argc, char *argv[])
{
    ...
    if (streq(cmd, "dp_trace")) {
    ...
    } else if (streq(cmd, "sendmscsresp")) {
        send_mscsresp(ifname, argc, argv);
    ...
    } else if (streq(cmd, "add_tpe")) {
        add_tpe_ie(ifname, argc, argv);
    } else if (streq(cmd, "del_tpe")) {
        del_tpe_ie(ifname, argc, argv);
        ...
        bsteer_get_cmd(ifname, argc, argv, MESH_BSTEERING_GET_PARAMS);
        ...
        bsteer_set_cmd(ifname, argc, argv, MESH_BSTEERING_SET_PARAMS);
    } else if (streq(cmd, "bsteer_getdbgparams")) {
        bsteer_get_cmd(ifname, argc, argv, MESH_BSTEERING_GET_DBG_PARAMS);
    } else if (streq(cmd, "bsteer_setdbgparams")) {
        bsteer_set_cmd(ifname, argc, argv, MESH_BSTEERING_SET_DBG_PARAMS);
    } else if (streq(cmd, "bsteer_enable")) {
        bsteer_enable(ifname, argc, argv, MESH_BSTEERING_ENABLE);
    } else if (streq(cmd, "bsteer_enable_events")) {
        bsteer_enable(ifname, argc, argv, MESH_BSTEERING_ENABLE_EVENTS);
    } else if (streq(cmd, "bsteer_setoverload")) {
        bsteer_set_cmd(ifname, argc, argv, MESH_BSTEERING_SET_OVERLOAD);
    } else if (streq(cmd, "bsteer_getoverload")) {
        bsteer_get_cmd(ifname, argc, argv, MESH_BSTEERING_GET_OVERLOAD);
    } else if (streq(cmd, "bsteer_getrssi")) {
        bsteer_getrssi(ifname, argc, argv);
    }  else if (streq(cmd, "bsteer_setproberespwh")) {
        bsteer_set_cmd(ifname, argc, argv, MESH_BSTEERING_SET_PROBE_RESP_WH);
    } else if (streq(cmd, "bsteer_getproberespwh")) {
        bsteer_get_cmd(ifname, argc, argv, MESH_BSTEERING_GET_PROBE_RESP_WH);
        ...
        bsteer_get_cmd(ifname, argc, argv, MESH_BSTEERING_GET_DATARATE_INFO);
        ...
        bsteer_set_cmd(ifname, argc, argv, MESH_BSTEERING_SET_STEERING);
...
#ifdef WLAN_SUPPORT_BCAST_TWT
    } else if (streq(cmd, "ap_twt_btwt_invite_sta")) {
        ap_twt_dialog_req_cmd(ifname, argc, argv, IEEE80211_DBGREQ_TWT_BTWT_INVITE_STA);
    } else if (streq(cmd, "ap_twt_btwt_remove_sta")) {
        ap_twt_dialog_req_cmd(ifname, argc, argv, IEEE80211_DBGREQ_TWT_BTWT_REMOVE_STA);
#endif
#endif
    ...
    } else if (streq(cmd, "peer_latency_param_config")) {
        set_peer_latency_param_config(ifname, argc, argv);
#if WLAN_SUPPORT_PRIMARY_ALLOWED_CHAN
