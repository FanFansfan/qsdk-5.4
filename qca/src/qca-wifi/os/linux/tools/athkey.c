	...
	*/
	...
	if (tb[NL80211_ATTR_KEY_CIPHER])
		memcpy(data, nla_data(tb[NL80211_ATTR_KEY_CIPHER]),
			min_int(nla_len(tb[NL80211_ATTR_KEY_CIPHER]), 1));
	data = data + 1;
 ...
 * return NL state.
 */
static int send_key_nlmsg(wifi_cfg80211_context *ctx, struct nl_msg *nlmsg,
		   ...
		   void *data)
{
	...
	if (valid_handler)
		nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, valid_handler, data);
				...
				void* req_key)
{
	...
	case NL80211_CMD_GET_KEY:
		...
		res = send_key_nlmsg(ctx, nlmsg, resp_handler, buffer);
		break;
	case NL80211_CMD_NEW_KEY:
		...
		res = send_key_nlmsg(ctx, nlmsg, NULL, buffer);
		break;
	default:
				...
				void *buf)
{
	...
	if (IS_CFG80211_ENABLED(athkey)) {
		...
		if (op == IEEE80211_IOCTL_GETKEY) {
			get_buf = &buffer.data;
			memcpy(&key->req_key.ik_type, get_buf, 1);
			get_buf = get_buf + 1;
			memcpy(&key->req_key.ik_keylen, get_buf, 1);
			get_buf = get_buf + 1;
			memcpy(&key->req_key.ik_keydata, get_buf,
			       (key->req_key.ik_keylen));
		}
	} else
#endif /* UMAC_SUPPORT_CFG80211 */
...
main(int argc, char *argv[])
{
	...
	case IEEE80211_IOCTL_DELKEY:
		...
		err = athkey_send_command(&athkey, ifname, op,
						(void *)athkey_key);
		goto exit;
	...
	case IEEE80211_IOCTL_GETKEY:
		...
		if (argv[1] == NULL)
			memset(getkey.ik_macaddr, 0xff, QDF_MAC_ADDR_SIZE);
		else
...
exit:
	...
	memset(&setkey, 0, sizeof(struct ieee80211_key));
	memset(&getkey, 0, sizeof(struct ieee80211req_key));
	return err;
}
