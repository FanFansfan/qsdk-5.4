...
static void print_usage() {
    ...
    fprintf(stdout, "1: exttool --chanswitch"
            ...
            " --force <force CSA even if target channel parameters are"
            " same as current channel parameters>"
            "--cfg80211\n");
    ...
    fprintf(stdout, "4: exttool --list_chan_info"
            " --interface wifiX\n");

    fprintf(stdout, "5: exttool --list_chan_state"
            " --interface wifiX\n");
...
static char *show_dfs_state(enum wlan_channel_dfs_state state)
{
    switch (state) {
        case WLAN_CH_DFS_S_CAC_REQ:
            return "DFS_CAC_REQUIRED";
        ...
        case WLAN_CH_DFS_S_PRECAC_STARTED:
            return "DFS_PRECAC_STARTED";
        case WLAN_CH_DFS_S_PRECAC_COMPLETED:
            return "DFS_PRECAC_COMPLETED";
        default:
            return "DFS_INVALID";
            break;
    }
...
static void convert_freq_to_index(uint16_t freq, int8_t *index)
{
#define FIRST_DFS_CHAN_NUM        (52)
#define CHAN_NUM_SPACING           (4)
#define WLAN_5_GHZ_BASE_FREQ    (5000)
#define WLAN_CHAN_SPACING_5MHZ     (5)
    uint16_t chan_num = (freq - WLAN_5_GHZ_BASE_FREQ) / WLAN_CHAN_SPACING_5MHZ;
    int8_t tmp_ind = (chan_num - FIRST_DFS_CHAN_NUM) / CHAN_NUM_SPACING;
    *index = ((tmp_ind >= 0) && (tmp_ind < NUM_DFS_CHANS)) ?
              tmp_ind : INVALID_INDEX;
}

static void show_chaninfo_full(struct ieee80211req_chaninfo_full *chans)
{
    int i;
    struct ieee80211_ath_channel *chan;
    int8_t arr_ind;

    for (i = 0; i < chans->req_chan_info.ic_nchans; i++) {
        chan = &chans->req_chan_info.ic_chans[i];
        printf("chan %d (%d)\n", chan->ic_ieee, chan->ic_freq);
        if (chan->ic_flagext & IEEE80211_CHAN_DFS) {
            convert_freq_to_index(chan->ic_freq, &arr_ind);
            if (arr_ind != INVALID_INDEX)
                printf("dfs state: %s\n", show_dfs_state(chans->dfs_chan_state_arr[arr_ind]));
        } else {
            printf("dfs state: NON_DFS\n");
        }
...
static void show_dfs_chan_state(struct ieee80211req_chaninfo_full *chans)
{
    int i;
    struct ieee80211_ath_channel *chan;
    int8_t arr_ind;
...
int main(int argc, char *argv[])
{
    ...
    static struct option exttool_long_options[] = {
        ...
        {"list_chan_state",no_argument,      NULL, 'y'},
        {"list_chan_info", no_argument,      NULL, 'Y'},
        {0,               0,                 0,     0},
    };
    ...
    while (TRUE)
    {
        option = getopt_long (argc, argv, "hi:csz:a:w:n:o:f:u:d:x:r:m:ge:pq:b:t:l:j:yY",
        ...
        switch (option) {
            ...
            case 'l': /* idle time  */
                ...
                break;
            case 'j':
                extended_cmd.ext_data.channel_switch_req.force = 1;
                break;
            case 'e': /* channel to scan  */
                if ((extended_cmd.ext_data.channel_scan_req.chanlist.n_chan +
                                                atoi(optarg)) >= CHAN_SCAN_MAX) {
                    fprintf(stderr, "Too many channels. Please keep channels under %u\n",
                            CHAN_SCAN_MAX);
                    return -1;
                }
    ...
    if (subioctl_cmd == EXTENDED_SUBIOCTL_CHANNEL_SWITCH) {
        ...
        /* The force CSA option does not work with repeater move option */
        extended_cmd.ext_data.channel_switch_req.force = 0;
    ...
    if (subioctl_cmd == EXTENDED_SUBIOCTL_GET_CHANINFO)
        list_chan_info ? show_chaninfo_full(&chans) : show_dfs_chan_state(&chans);
