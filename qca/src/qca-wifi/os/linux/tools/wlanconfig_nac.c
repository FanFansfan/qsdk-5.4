    ...
    IEEE80211_WLANCONFIG_CMDTYPE cmdtype, int argc, char *argv[])
{
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_ADDR_ADD ||
        ...
        for(i=0, j=5; i < max_addrlimit && j < argc ; i++, j++) {
            ...
            memcpy(config.data.nac.mac_list[i], temp, QDF_MAC_ADDR_SIZE);
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_ADDR_LIST) {
        ...
        for (i = 0; i < max_addrlimit; i++) {
            if (memcmp(config.data.nac.mac_list[i], nullmac, QDF_MAC_ADDR_SIZE) != 0) {
    ...
    IEEE80211_WLANCONFIG_CMDTYPE cmdtype, int argc, char *argv[])
{
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_ADD ||
        ...
        memcpy(config.data.nac_rssi.mac_bssid, temp, QDF_MAC_ADDR_SIZE);
        ...
        memcpy(config.data.nac_rssi.mac_client, temp, QDF_MAC_ADDR_SIZE);
    }
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_ADD) {
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_NAC_RSSI_ADDR_LIST) {
        ...
        if ((memcmp(config.data.nac_rssi.mac_bssid, nullmac, QDF_MAC_ADDR_SIZE) != 0)
                && (memcmp(config.data.nac_rssi.mac_client, nullmac, QDF_MAC_ADDR_SIZE) != 0)){
