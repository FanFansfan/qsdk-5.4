    ...
    char *in_cap_info, char *in_ftype_map)
{
    ...
    if (cmdtype == IEEE80211_WLANCONFIG_VENDOR_IE_ADD || cmdtype == IEEE80211_WLANCONFIG_VENDOR_IE_UPDATE) {
        ...
        if (strlen(in_cap_info) != ((len - VENDORIE_OUI_LEN) * 2)) {
            /*
             * strlen is treated as size_t. For 64-bit platforms,
             * size_t is identified as "long unsigned int".
             * For 32-bit, it is "unsigned int". As a result,
             * all printf-like API's will complain about the format-strings.
             * As it can be safely assumed that strlen or sizeof will never
             * actually return huge values, typecasting to "uint32_t" will
             * keep the compiler happy for both platfrms.
             */
            printf("Invalid len , capability len =%u not matching with total len=%d (bytes) , format: xxxxxxxx)\n", (uint32_t)strlen(in_cap_info),len);
            return -1;
        }
    ...
    printf("\n sizeof vendorie: %u \n", (uint32_t)sizeof(struct ieee80211_wlanconfig_vendorie));
