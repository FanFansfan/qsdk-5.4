...
#define SA_WORD_MASK   0xFFFF

/* Lower 16bit have 11 bit rate code higher 16bit have 3 bit Bandwidth code */
#define SA_GET_BW_FROM_RC_PARAM(x) ((x) & SA_MAX_BW)
...
struct sa_testmode_train_params {
    u_int16_t ratecode[SA_MAX_BW]; /* training rate 0-Legacy/20MHz, 1-40MHz, 2-80MHz */
    u_int32_t antenna; /* training antenna */
    u_int16_t numpkts; /* Number of packets required for training*/
};
