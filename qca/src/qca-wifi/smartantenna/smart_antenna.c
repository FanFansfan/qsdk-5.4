/*
* Copyright (c) 2013, 2019, 2021 Qualcomm Innovation Center, Inc.
* All Rights Reserved.
...
#define SA_PARAM_STR_LENGTH 40

/**
 * sa_num_module_param() - Update smart antenna module params
 *
 *
 * Read the file which has wifi module params, parse and update
 * smart antenna module params.
 *
 * Return: void
 */
void initialize_sa_module_param_from_ini(void);

enum sa_num_module_param {
    PARAM_SA_DEFAULT_ANTENNA,
    PARAM_SA_ENABLE_TEST_MODE,
    SA_PARAM_MAX,
};

static char sa_module_param[SA_PARAM_MAX][SA_PARAM_STR_LENGTH] = {
    "sa_default_antenna",
    "sa_enable_test_mode",
};
...
/**
 * sa_module_param_handler() - update smart antenna module param
 *
 * This function receives any one of smart antenna module param and its value as input
 *
 * Return : QDF_STATUS_SUCCESS on Success
 */
QDF_STATUS sa_module_param_handler(void *context, const char *str_param, const char *str_value)
{
    QDF_STATUS status;
    uint16_t param=0;

    while ( param < SA_PARAM_MAX ) {
        if (qdf_str_eq(sa_module_param[param], str_param)) {
            switch(param) {
                case PARAM_SA_DEFAULT_ANTENNA:
                    status = qdf_uint32_parse(str_value, &sa_default_antenna);
                    break;
                case PARAM_SA_ENABLE_TEST_MODE:
                    status = qdf_uint32_parse(str_value, &sa_enable_test_mode);
                    break;
                default:
                    break;
            }
...
void initialize_sa_module_param_from_ini()
{
    QDF_STATUS status;
    char *path = QDF_WIFI_MODULE_PARAMS_FILE;

    status = qdf_ini_parse(path, NULL, sa_module_param_handler, NULL);
...
int32_t sa_update_ratestats(struct sa_node_ccp *node, struct sa_tx_feedback *feedback)
{
    ...
    f_rate = feedback->ratemaxphy[0] & SA_BYTE_MASK;
    s_rate = feedback->ratemaxphy[1] & SA_BYTE_MASK;
    t_rate = feedback->ratemaxphy[2] & SA_BYTE_MASK;
    if (f_rate && s_rate && t_rate) {
        node->rate_stats.rate_maxphy = (uint32_t)(f_rate || (s_rate << 8) || (t_rate << 16));
    } else {
...
{
#ifdef QCA_WIFI_MODULE_PARAMS_FROM_INI
    initialize_sa_module_param_from_ini();
#endif
