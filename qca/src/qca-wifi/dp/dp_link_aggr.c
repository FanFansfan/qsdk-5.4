...
void dp_lag_soc_enable(struct wlan_objmgr_pdev *pdev, bool value)
{
    ...
    qdf_print("pdev(%p) Enabling DBDC Repeater %d  soc_lag %p", pdev_lag, value, soc_lag);
}
...
#else
                if (qdf_unlikely(dp_is_extap_enabled(pdev_lag->sta_vdev) || dp_wrap_vdev_is_psta(vap->vdev_obj)))
#endif
                    return;
#endif
...
bool dp_lag_rx_ap(struct dp_pdev_link_aggr *pdev_lag, qdf_nbuf_t nbuf)
{
...
#else
    if (dp_wrap_vdev_is_mpsta(vap->vdev_obj) || dp_is_extap_enabled(pdev_lag->sta_vdev)) {
#endif
        if (ast_entry_found) {
    ...
    if (!ast_entry_found) {
        ...
        ast_type = ast_entry_info.type;
        if ((pdev_id != pdev_lag->pdev_id) && pdev_lag->sta_vdev && (ast_type != CDP_TXRX_AST_TYPE_STATIC)) {
            enqueue_to_sta_vap = true;
        }
    }
