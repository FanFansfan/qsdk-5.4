...
print_target_node_bitmap(struct ieee80211_node *ni)
{
    ...
    qdf_nofl_info("%s : ", __FUNCTION__);
    for (i = 0; i < HTC_MAX_NODE_NUM; i++) {
        qdf_nofl_info("%d ", ni->ni_ic->target_node_bitmap[i].ni_valid);
    }
    qdf_nofl_info("\n");
}
...
{
    qdf_nofl_info("------------------------------------------------------------\n");
    qdf_nofl_info("ni_associd: 0x%x\n", nt->ni_associd);
    //qdf_nofl_info("ni_txpower: 0x%x\n", nt->ni_txpower);

    qdf_nofl_info("mac: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
        nt->ni_macaddr[0], nt->ni_macaddr[1], nt->ni_macaddr[2],
        nt->ni_macaddr[3], nt->ni_macaddr[4], nt->ni_macaddr[5]);
    qdf_nofl_info("bssid: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
        nt->ni_bssid[0], nt->ni_bssid[1], nt->ni_bssid[2],
        nt->ni_bssid[3], nt->ni_bssid[4], nt->ni_bssid[5]);     

    qdf_nofl_info("nodeindex: %d\n", nt->ni_nodeindex);
    qdf_nofl_info("vapindex: %d\n", nt->ni_vapindex);
    //qdf_nofl_info("ni_vapnode: 0x%x\n", nt->ni_vapnode);
    qdf_nofl_info("ni_flags: 0x%x\n", nt->ni_flags);
    qdf_nofl_info("ni_htcap: %d\n", nt->ni_htcap);
    //qdf_nofl_info("ni_valid: %d\n", nt->ni_valid);
    qdf_nofl_info("ni_capinfo: 0x%x\n", nt->ni_capinfo);
    //qdf_nofl_info("ni_ic: 0x%08x\n", nt->ni_ic);
    //qdf_nofl_info("ni_vap: 0x%08x\n", nt->ni_vap);
    //qdf_nofl_info("ni_txseqmgmt: %d\n", nt->ni_txseqmgmt);
    qdf_nofl_info("ni_is_vapnode: %d\n", nt->ni_is_vapnode);
    qdf_nofl_info("ni_maxampdu: %d\n", nt->ni_maxampdu);
    //qdf_nofl_info("ni_iv16: %d\n", nt->ni_iv16);
    //qdf_nofl_info("ni_iv32: %d\n", nt->ni_iv32);
#ifdef ENCAP_OFFLOAD
    //qdf_nofl_info("ni_ucast_keytsc: %d\n", nt->ni_ucast_keytsc);
    //qdf_nofl_info("ni_ucast_keytsc: %d\n", nt->ni_ucast_keytsc);
#endif
    qdf_nofl_info("------------------------------------------------------------\n");
}
...
ieee80211_chk_vap_target(struct ieee80211com *ic)
{
	...
	for (i = 0; i < HTC_MAX_VAP_NUM; i++) {
		...
		if (ic->target_vap_bitmap[i].vap_valid == 0) {
			qdf_nofl_info("%s: target_vap_bitmap(%d) is invalid\n", __FUNCTION__, i);
			break;
		}
	}
	...
	if (i == HTC_MAX_VAP_NUM) {
		qdf_nofl_info("%s: exceed maximum vap number\n", __FUNCTION__);
		return 1 ;
	}
...
ieee80211_add_vap_target(struct ieee80211vap *vap)
{
    ...
    for (i = 0; i < HTC_MAX_VAP_NUM; i++) {
        ...
        if (IEEE80211_ADDR_EQ(ic->target_vap_bitmap[i].vap_macaddr, vap->iv_myaddr)) {
            qdf_nofl_info("%s: add the same vap twice\n", __FUNCTION__);
            return;
        }
...
#if 0        	
            qdf_nofl_info("%s: target_vap_bitmap(%d) vap_valid = %d\n", __FUNCTION__, i,
            ic->target_vap_bitmap[i].vap_valid);
#endif            
        }
    }

    qdf_nofl_info("%s: vap index: %d\n", __FUNCTION__, vapindex);
    ...
    qdf_mem_copy(&vt.iv_myaddr, &(vap->iv_myaddr), QDF_MAC_ADDR_SIZE);
...
ieee80211_chk_node_target(struct ieee80211com *ic)
{
   int i;
	...
	if (i == HTC_MAX_NODE_NUM) {
		qdf_nofl_info("%s: exceed maximum node number\n", __FUNCTION__);
		return 1;
	}
	return 0;
    ...
    struct ieee80211vap *vap, u_int8_t is_reset_bss)
{
    ...
    //qdf_nofl_info("%s: vap: 0x%08x, ni: 0x%08x, is_reset_bss: %d\n", __FUNCTION__, vap, ni, is_reset_bss);
    ...
    for (i = 0; i < HTC_MAX_NODE_NUM; i++) {
        if (ni->ni_ic->target_node_bitmap[i].ni_valid == 1) {
            ...
            if (IEEE80211_ADDR_EQ(ic->target_node_bitmap[i].ni_macaddr, ni->ni_macaddr)) {
                ...
                OS_MEMZERO(ic->target_node_bitmap[i].ni_macaddr, QDF_MAC_ADDR_SIZE);
    ...
    if (i == HTC_MAX_NODE_NUM) {
        /*qdf_nofl_info("%s: Can't find desired node\n", __FUNCTION__);*/
        return;
    }

    /* qdf_nofl_info("%s: ni: 0x%08x, nodeindex: %d, is_reset_bss: %d\n", __FUNCTION__,
...
ieee80211_update_node_target(struct ieee80211_node *ni, struct ieee80211vap *vap)
{
    ...
    //qdf_nofl_info("%s: vap: 0x%08x, ni: 0x%08x\n", __FUNCTION__, vap, ni);
    //print_target_node_bitmap(ni);
    ...
    if ((i < HTC_MAX_NODE_NUM) && (ni->ni_ic->target_node_bitmap[i].ni_valid == 1)) {
        //qdf_nofl_info("%s : find node index (%d)\n", __FUNCTION__, i);
        nodeindex = i;            
    }
    else {
        //qdf_nofl_info("%s:  can't find the update node\n", __FUNCTION__);
        return;
    }
...
ieee80211_delete_vap_target(struct ieee80211vap *vap)
{
    ...
    for (i = 0; i < HTC_MAX_VAP_NUM; i++) {
        if (ic->target_vap_bitmap[i].vap_valid == 1) {
            ...
            if (IEEE80211_ADDR_EQ(ic->target_vap_bitmap[i].vap_macaddr, vap->iv_myaddr)) {
                ...
                OS_MEMZERO(ic->target_vap_bitmap[i].vap_macaddr, QDF_MAC_ADDR_SIZE);
                break;
            }
        }
    }
    ...
    if (i == HTC_MAX_VAP_NUM) {
        qdf_nofl_info("%s: Can't find desired vap\n", __FUNCTION__);
        return;
    }

    //qdf_nofl_info("%s: vapindex: %d\n", __FUNCTION__, vapindex);
