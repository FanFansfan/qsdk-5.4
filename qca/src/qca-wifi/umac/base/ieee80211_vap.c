...
#define IEEE80211_C_OPMODE                                              \
...
#else
    struct ieee80211vap *mpsta_vap = wlan_get_vap(dp_wrap_get_mpsta_vdev(ic->ic_pdev_obj));
#endif
#endif
    ...
    if (opmode == IEEE80211_M_HOSTAP) {
        ...
        if (vap->iv_create_flags & IEEE80211_LP_IOT_VAP) {
            if (!is_ema_ap_enabled || is_tx_vap) {
    ...
    if(wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj, WLAN_PDEV_F_MBSS_IE_ENABLE))
        vap->iv_max_aid = ic->ic_num_clients + (1 << ic->ic_mbss.max_bssid) + 1;
    else
        /* default max aid(should be a config param ?) */
        vap->iv_max_aid = ic->ic_num_clients + 1;

    /*
     * iv_mbss_max_aid should follow iv_max_aid in default cases. It will change
     * to a value less than iv_max_aid only if explicitly done so using
     * a set_param command.
     */
    ...
    vap->iv_smps_snrthresh   = ic->ic_reg_parm.smpsSnrThreshold;
...
#else
        if ((vap->iv_ic->ic_wrap_vap_sgi_cap) && !(dp_wrap_vdev_is_mpsta(vap->vdev_obj)) && (dp_wrap_vdev_is_psta(vap->vdev_obj)))
#endif
     ...
     */
     if (ic->ic_he_target) {
        ...
        vap->iv_he_multi_tid_aggr =
                HECAP_MAC_MTIDRXSUP_GET_FROM_IC(ic_hecap_mac_low);
        vap->iv_he_multi_tid_aggr_tx =
                HECAP_MAC_MTIDTXSUP_GET_FROM_IC(ic_hecap_mac_high);
        vap->iv_he_max_ampdu_len_exp  =
                HECAP_MAC_MAXAMPDULEN_EXPEXT_GET_FROM_IC(ic_hecap_mac_low);
#endif
        vap->iv_he_min_frag_size  =
                HECAP_MAC_MINFRAGSZ_GET_FROM_IC(ic_hecap_mac_low);
        ...
        vap->iv_he_omi            =
                HECAP_MAC_OMI_GET_FROM_IC(ic_hecap_mac_low);
        ...
        if(vap->iv_opmode == IEEE80211_M_HOSTAP) {
            ...
            if((ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCA8074V2) ||
               ...
               (ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCA5018) ||
               (ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCN6122) ||
        ...
        if(ic->ic_get_tgt_type(ic) == TARGET_TYPE_QCA5018) {
            if(vap->iv_he_mu_bfer) {
               /* Disable HE-MU-BFER */
               vap->iv_he_mu_bfer = !ENABLE_HE_MU_BFER;
            }
            if(opmode == IEEE80211_M_STA) {
               /* Enable HE-MU-BFEE for STA vap */
               vap->iv_he_mu_bfee = ENABLE_HE_MU_BFEE;
            }
        }

        vap->iv_twt_rsp           = HECAP_MAC_TWTRSP_GET_FROM_IC(ic_hecap_mac_low);
#if !SUPPORT_11AX_D3
        ...
        vap->iv_he_amsdu_in_ampdu_suprt = HECAP_MAC_AMSDUINAMPDU_GET_FROM_IC
                                                            (ic_hecap_mac_high);
        vap->iv_he_ulmu_data_disable_rx =
                        HECAP_MAC_OMCTRLULMU_DISRX_GET_FROM_IC(ic_hecap_mac_high);
        vap->iv_he_1024qam_lt242ru_rx =
                                HECAP_PHY_TX1024QAMLT242TONERU_GET_FROM_IC
                                        ((&(ic->ic_he.hecap_phyinfo
                                            [IC_HECAP_PHYDWORD_IDX0])));
#endif
        ...
        vap->iv_he_max_nc         = HECAP_PHY_MAX_NC_GET_FROM_IC
                                            ((&(ic->ic_he.hecap_phyinfo
                                                [IC_HECAP_PHYDWORD_IDX0])));
        vap->iv_he_full_bw_ulmumimo =  HECAP_PHY_UL_MU_MIMO_GET_FROM_IC
                                            ((&(ic->ic_he.hecap_phyinfo
                                                [IC_HECAP_PHYDWORD_IDX0])));
        vap->iv_he_dcm_max_cons_rx  =  HECAP_PHY_DCMRX_GET_FROM_IC
                                            ((&(ic->ic_he.hecap_phyinfo
                                                [IC_HECAP_PHYDWORD_IDX0])));
    ...
    vap->max_cont_auth_fail = MLME_MAX_AUTH_FAIL_CNT;
    ...
    if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && ic->ic_curchan) {
        vdev_mlme->mgmt.rate_info.bcn_tx_rate =
                wlan_vdev_get_bcn_tx_rate(vdev_mlme, ic->ic_curchan);
    }
    ...
    vap->nbr_scan_period = NBR_SCAN_PERIOD;
        ...
        vap->iv_dbeacon_snr_thr = DEFAULT_DBEACON_SNR_THRESHOLD;
    ...
    qdf_spinlock_create(&vap->peer_isolation_list.lock);
#endif

    if (is_ema_ap_enabled && soc->ema_ap_support_wps_6ghz) {
        /* In EMA mode if user chooses to enalbe WPS by
         * using INI ema_ap_support_wps_6ghz then force
         * 6g-compiance indicator variable to false
         */
        vap->iv_6g_comp = false;
    } else {
        /* This will be changed to true in near future -
         * this comment needs to be removed at that point
         */
        vap->iv_6g_comp = false;
    }
    vap->iv_keymgmt_6g_mask = 0xFFFFFFF;
...
ieee80211_vap_detach(struct ieee80211vap *vap)
{
    ...
    if ((apvaps_cnt == 0) && (vap->iv_opmode == IEEE80211_M_HOSTAP)) {
        /* If it is a last APVAP, stop the Green AP feature */
...
#if QCA_SUPPORT_PEER_ISOLATION
    wlan_peer_isolation_flush(vap);
    qdf_spinlock_destroy(&vap->peer_isolation_list.lock);
#endif

    if (IS_MBSSID_EMA_EXT_ENABLED(ic) &&
            IEEE80211_VAP_IS_MBSS_NON_TRANSMIT_ENABLED(vap)) {
        if (vap->iv_mbss.non_tx_pfl_ie_pool != NULL) {
...
void wlan_vap_up_check_beacon_interval(struct ieee80211vap *vap, enum ieee80211_opmode opmode)
{
    ...
    ol_ath_soc_softc_t *soc = scn->soc;
    uint8_t pdev_idx = wlan_objmgr_pdev_get_pdev_id(ic->ic_pdev_obj);
    ...
    if (opmode == IEEE80211_M_HOSTAP) {
        ...
        if (vap->iv_create_flags & IEEE80211_LP_IOT_VAP)  {
            if (!ic->ic_num_lp_iot_vaps) {
                ...
                soc->soc_lp_iot_vaps_mask |= (uint8_t)(1 << pdev_idx);
...
{
    bool is_vap_up = false;

    if (vap)
        is_vap_up =
            (wlan_vdev_is_up(vap->vdev_obj) ==
             QDF_STATUS_SUCCESS) ? true : false;
...
#endif
#if QCA_SUPPORT_RAWMODE_PKT_SIMULATION
    wlan_create_attach_vdev_rawsim_ctxt(vap);
#endif /* QCA_SUPPORT_RAWMODE_PKT_SIMULATION */
    return vap;
}
...
void ic_reset_params(struct ieee80211com *ic)
{
    ...
    /* Update current channel for DFS */
    psoc = wlan_pdev_get_psoc(pdev);
    if (!psoc) {
        qdf_nofl_err("\npsoc is NULL;trying to update dfs curchan");
        return;
    }
    ...
    if (dfs_rx_ops && dfs_rx_ops->dfs_set_current_channel_for_freq) {
        if (wlan_objmgr_pdev_try_get_ref(pdev, WLAN_DFS_ID) !=
            QDF_STATUS_SUCCESS) {
            qdf_nofl_err("Unable to get pdev's ref to update dfs curchan");
            return;
        }
    ...
    ic->ic_rtt_req_id = 0;
    ic->ic_ftmrr_meas_token = 0;
    ic->ic_ftmrr_dialogtoken = 0;
    ...
    /* Re-enable bsscolor in wifi down path if it
     * was disabled previously. According to current
     * design the current bsscolor is retained across
     * wifi down/up and it will be re-usable only if
     * we re-enable the color
     */
#if SUPPORT_11AX_D3
    ...
    if (ieee80211_vap_deleted_is_clear(vap)) /* if not deleted then it is on the list */
        {
...
#if ATH_SUPPORT_WRAP
#if !WLAN_QWRAP_LEGACY
                dp_wrap_pdev_set_isolation(ic->ic_pdev_obj, WRAP_ISOLATION_DEFVAL);
#endif
#endif
            }
            ...
            if (stop) {
                ...
                if (ieee80211_ucfg_bringdown_txvap(vap)) {
                   /* If ucfg tx vap bringdown fails, invoke bringdown sequence
                      for this vap alone
                    */
                    if (vap->iv_opmode == IEEE80211_M_STA)
                        wlan_mlme_cm_stop(vap->vdev_obj, CM_OSIF_DISCONNECT,
...
void wlan_stop(struct ieee80211com *ic)
{
    ...
    if (ic->ic_is_target_lithium(scn->soc->psoc_obj)) {
        ieee80211_user_rnr_free(ic);
    }

}
qdf_export_symbol(wlan_stop);
                           ...
                           u_int8_t *bssid, struct ieee80211_clone_params *cp)
{
    ...
    /* No need to clear slot id for MBSS enabled case
     * and wideband enabled case as it is not used/assigned
     * during VAP create
     */
...
wlan_phy_stats(wlan_dev_t devhandle, enum ieee80211_phymode mode)
{
    ...
    KASSERT(ieee80211_is_phymode_valid(mode),
             ("Invalid PHY mode\n"));
...
static void ieee80211_get_beaconing_vap_iter_func(void *arg, wlan_if_t vap)
{
    u_int16_t *beaconing_vaps = (u_int16_t *) arg;

    if (vap->iv_opmode != IEEE80211_M_HOSTAP)
        return;

    if (wlan_is_non_beaconing_ap_vap(vap))
        return;
...
{
    u_int16_t num_beaconing_vaps = 0;

    wlan_iterate_vap_list(comhandle, ieee80211_get_beaconing_vap_iter_func,
                          (void *)&num_beaconing_vaps);
    return num_beaconing_vaps;
}
...
bool wlan_is_non_beaconing_ap_vap(wlan_if_t vap)
{
    ...
    if ((vap->iv_opmode == IEEE80211_M_HOSTAP) &&
        ...
        && !WMI_HOST_VDEV_IS_BEACON_SUPPORTED(avn->vdev_param_capabilities)) ||
...
int wlan_getsnr(wlan_if_t vaphandle, wlan_snr_info *snr_info, u_int32_t flags)
{
    struct ieee80211vap   *vap = vaphandle;
    ...
    if (bss_node) {
        err =  wlan_node_getsnr(bss_node,snr_info,flags);
...
{
    uint32_t bcn_rate = 0;
    uint32_t mgmt_rate = 0;
    uint32_t rtscts_rate = 0;
    wlan_chan_t chan;
    ...
    if (IEEE80211_M_HOSTAP == vap->iv_opmode) {
        ...
        wlan_util_vdev_mlme_get_param(vap->vdev_mlme,
                WLAN_MLME_CFG_TX_RTSCTS_RATE, &rtscts_rate);
        ...
        if (rtscts_rate) {
            wlan_set_param(vap, IEEE80211_NON_BASIC_RTSCTS_RATE, rtscts_rate);
        }
    }
}
...
ieee80211_vap_is_connecting(wlan_if_t vap)
{
    ...
    return wlan_cm_is_vdev_connecting(vap->vdev_obj);
}

bool
ieee80211_vap_is_roaming(wlan_if_t vap)
{
    if (!vap) {
        qdf_err("Error vap: 0x%pK  ", vap);
        return false;
    }
    ...
    return wlan_cm_is_vdev_roaming(vap->vdev_obj);
}
...
ieee80211_vap_is_connected(wlan_if_t vap)
{
    ...
    return wlan_cm_is_vdev_connected(vap->vdev_obj);
}
