                         ...
                         int flags)
{
    ...
    /* Update the NSS capability */
    ni->ni_maxrxstreams = ni->ni_rxstreams;
    ni->ni_maxtxstreams = ni->ni_txstreams;
                         ...
                         int flags)
{
    ...
    /* Update the NSS capability */
    if (rx_rrs.num_streams > ni->ni_maxrxstreams) {
        ni->ni_maxrxstreams = rx_rrs.num_streams;
    }
    if (tx_rrs.num_streams > ni->ni_maxtxstreams) {
        ni->ni_maxtxstreams = tx_rrs.num_streams;
    }
...
ieee80211_get_nss_from_hecap_mcsnssset_6g(struct ieee80211_node *ni)
{
    ...
    tx_map = INTERSECT_11AX_MCSNSS_MAP(temp_self_rxmap, temp_peer_txmap);
    ni->ni_txstreams = get_nss_frm_mcsnssmap(tx_map);

    rx_map = INTERSECT_11AX_MCSNSS_MAP(temp_self_txmap, temp_peer_rxmap);
    ni->ni_rxstreams = get_nss_frm_mcsnssmap(rx_map);
    ni->ni_streams = QDF_MAX(ni->ni_streams, ni->ni_rxstreams);

    /* Update the NSS capability */
    if (get_nss_frm_mcsnssmap(temp_peer_rxmap) > ni->ni_maxrxstreams) {
        ni->ni_maxrxstreams = get_nss_frm_mcsnssmap(temp_peer_rxmap);
    }
    if (get_nss_frm_mcsnssmap(temp_peer_txmap) > ni->ni_maxtxstreams) {
        ni->ni_maxtxstreams = get_nss_frm_mcsnssmap(temp_peer_txmap);
    }
}
...
#define	RV(v)	((v) & IEEE80211_RATE_VAL)
    ...
    for (i = 0; i < rrs->rs_nrates; i++) {
        ...
        if (rrs->rs_rates[i] & IEEE80211_RATE_BASIC) {
            ...
            if (j == srs->rs_nrates)
            {
                if(rrs->rs_rates[i] == 0xfb || (rrs->rs_rates[i] == 0xff && is_ht))
                {
                    continue;
                }
                else {
                         ...
                         u_int32_t *nrates)
{
    ...
    KASSERT(!ieee80211_is_phymode_auto(mode), ("Unsupported PHY mode\n"));
