...
void atf_open(struct wlan_objmgr_psoc *psoc)
{
    ...
    if (!tx_ops) {
        atf_err("tx_ops is NULL \n");
        return;
    }

    tx_ops->atf_tx_ops.atf_open(psoc);
}
...
void atf_enable(struct wlan_objmgr_psoc *psoc)
{
    ...
    tx_ops->atf_tx_ops.atf_register_event_handler(psoc);
}
...
void atf_disable(struct wlan_objmgr_psoc *psoc)
{
    ...
    tx_ops->atf_tx_ops.atf_unregister_event_handler(psoc);
}
...
static void atf_reset_vdev(struct wlan_objmgr_pdev *pdev)
{
    if (NULL == pdev) {
				   ...
				   uint8_t *gindex)
{
	...
	for (i = 0; i < cfg->grp_num_cfg; i++) {
		...
		if ((qdf_str_len(group->group_name) == qdf_str_len(gname)) &&
		    (!qdf_str_cmp((char *)(group->group_name),
				  (uint8_t *)(gname)))) {
			qdf_str_lcopy(cfg->atfgroup[i].grpname,
				      group->group_name,
...
QDF_STATUS atf_config_add_pdev_group(struct pdev_atf *pa, uint8_t *gname)
{
	...
	if (groupindex >= 0 && groupindex < ATF_ACTIVED_MAX_ATFGROUPS) {
		qdf_mem_zero(cfg->atfgroup[groupindex].grpname,
			     WLAN_SSID_MAX_LEN + 1);
...
atf_set_ssid_sched_policy(struct wlan_objmgr_vdev *vdev, uint16_t value)
{
	...
	struct wlan_lmac_if_atf_tx_ops *atf_tx_ops = NULL;
	struct wlan_lmac_if_tx_ops *tx_ops;
	...
	if (atf_group_vap_sched(vdev, value) == QDF_STATUS_SUCCESS) {
		...
		atf_tx_ops = &tx_ops->atf_tx_ops;
		if (atf_tx_ops && atf_tx_ops->atf_ssid_sched_policy) {
			if (atf_tx_ops->atf_ssid_sched_policy(vdev, value) ==
							QDF_STATUS_SUCCESS) {
				retval = QDF_STATUS_SUCCESS;
			}
		} else {
...
int32_t atf_update_tbl_to_fm(struct wlan_objmgr_pdev *pdev)
{
    ...
    if (retv == 0) {
        ...
        tx_ops->atf_tx_ops.atf_set_grouping(pdev, &pa->atf_group_req,
                            pa->atf_tput_based);
        tx_ops->atf_tx_ops.atf_set_group_ac(pdev, &pa->atf_group_ac_req,
                            pa->atf_tput_based);
        tx_ops->atf_tx_ops.atf_set(pdev, &pa->atf_req, pa->atf_tput_based);
        tx_ops->atf_tx_ops.atf_send_peer_request(pdev, &pa->atf_peer_req,
                            pa->atf_tput_based);
    }
...
void atf_cfg_timer_handler(void *arg)
{
	...
	retv = atf_update_tbl_to_fm(pdev);
	if (retv != 0)
...
int32_t atf_build_bwf_for_fm(struct wlan_objmgr_pdev *pdev)
{
    ...
    tx_ops->atf_tx_ops.atf_set_bwf(pdev, bwf);
...
int32_t atf_set(struct wlan_objmgr_vdev *vdev)
{
    ...
    if (ac->atf_fmcap && ac->atf_mode) {
        ...
        tx_ops->atf_tx_ops.atf_enable_disable(vdev, 1);
    } else {
...
int32_t atf_clear(struct wlan_objmgr_vdev *vdev)
{
    ...
    if (pa->atf_stats_enable) {
        atf_set_ppdu_stats(pdev, 0);
        atf_stats_stop_timer(pdev);
    }
    pa->atf_stats_enable = false;

    tx_ops->atf_tx_ops.atf_enable_disable(vdev, 0);

    return retv;
}

int32_t atf_set_maxclient(struct wlan_objmgr_pdev *pdev, uint8_t enable)
{
...
static void atf_stats_print(struct wlan_objmgr_pdev *pdev)
{
    struct pdev_atf *patf = NULL;
    struct atf_config *cfg = NULL;
    struct atf_actual_airtime *act_airtime = NULL;
    uint8_t actual = 0;
    ...
    cfg = &patf->atfcfg_set;
    if (!patf->actual_total_airtime)
        return;
    if (patf->atf_ssidgroup && cfg->grp_num_cfg) {
        qdf_info("****************** ATF STATS For SSID Groups *************************");
        qdf_info("GroupID  GroupName  Configured  Actual  Borrowed  Unused  Duration(us)");
        for (i = 0; (i < cfg->grp_num_cfg) &&
             (i < ATF_ACTIVED_MAX_ATFGROUPS); i++) {
            borrowed = unused = 0;
            ac_cfg = &cfg->atfgroup[i].atf_cfg_ac;
            act_airtime = &cfg->atfgroup[i].act_airtime;
            configured = cfg->atfgroup[i].grp_cfg_value /
                    ATF_AIRTIME_CONVERSION_FACTOR;
            actual = (act_airtime->actual_airtime * 100) /
                  patf->actual_total_airtime;
            if (actual > configured) {
                borrowed = actual - configured;
            } else {
                unused = configured - actual;
            }
            if (i) {
                qdf_info("  %d     %s\t\t%d%%  \t  %d%%  \t  %d%%  \t  %d%%  \t  %u",
                     i, cfg->atfgroup[i].grpname, configured, actual, borrowed,
                     unused, act_airtime->actual_airtime);
            } else {
                qdf_info("  %d     %s   \t%d%%  \t  %d%%  \t  %d%%  \t  %d%%  \t  %u",
                     i, "Default", configured, actual, borrowed,
                     unused, act_airtime->actual_airtime);
            }
            group = cfg->atfgroup[i].grplist_entry;
            if (group && ac_cfg->ac_cfg_flag) {
                for (j = 0; j < WME_NUM_AC; j++) {
                    borrowed = 0;
                    unused = 0;
                    if (!atf_config_is_subgroup_exist(cfg,
                        group, &j, &subgroup,
                        ATF_SUBGROUP_TYPE_AC))
                        continue;
                    if (!subgroup)
                        continue;
                    configured = subgroup->sg_atf_units /
                        ATF_AIRTIME_CONVERSION_FACTOR;
                    subgroup = NULL;
                    actual =
                    (act_airtime->actual_ac_airtime[j] * 100) /
                    patf->actual_total_airtime;
                    if (actual > configured) {
                        borrowed = actual - configured;
                    } else {
                    ...
                    qdf_info("          AC(%s)     \t%d%%  \t  %d%%  \t  %d%%  \t  %d%% \t  %u",
                         ac[j], configured, actual, borrowed, unused,
                         act_airtime->actual_ac_airtime[j]);
                }
            }
        }
    } else if (cfg->vdev_num_cfg) {
        qdf_info("****************** ATF STATS For SSID ***************************");
        qdf_info("VdevID  ESSID  Configured  Actual  Borrowed  Unused  Duration(us)");
        for (i = 0; (i < cfg->vdev_num_cfg) &&
                 (i < ATF_CFG_NUM_VDEV); i++) {
            borrowed = 0;
            unused = 0;
            index = 0;
            group = NULL;
            ...
            ac_cfg = &cfg->vdev[i].atf_cfg_ac;
            configured = cfg->vdev[i].vdev_cfg_value /
                    ATF_AIRTIME_CONVERSION_FACTOR;
            actual = (cfg->vdev[i].actual_airtime * 100) /
                  patf->actual_total_airtime;
            if (actual > configured) {
                borrowed = actual - configured;
            } else {
            ...
            ssid = cfg->vdev[i].essid;
            qdf_info("  %d     %s \t  %d%%  \t  %d%%  \t  %d%%  \t  %d%% \t  %u",
                 i, ssid, configured, actual, borrowed, unused,
                 cfg->vdev[i].actual_airtime);

            if (atf_config_is_group_exist(cfg, ssid, &index)) {
                if (index < ATF_ACTIVED_MAX_ATFGROUPS) {
                    group =
                    cfg->atfgroup[index].grplist_entry;
                    act_airtime =
                    &cfg->atfgroup[index].act_airtime;
                }
            }
            if (group && act_airtime && ac_cfg->ac_cfg_flag) {
                for (j = 0; j < WME_NUM_AC; j++) {
                    borrowed = 0;
                    unused = 0;
                    if (!atf_config_is_subgroup_exist(cfg,
    ...
    if (!cfg->peer_num_cal) {
        qdf_info("Pdev Total Airtime(us)\t %u", patf->actual_total_airtime);
        qdf_info("****************************************************************\n");
        return;
    }
    qdf_info("****************** ATF STATS For PEERs ***************************");
    if (patf->atf_ssidgroup) {
        qdf_info("      PeerMAC      GroupName  Configured  Actual  Borrowed  Unused  Duration(us)");
    } else {
        qdf_info("      PeerMAC      SSIDName  Configured  Actual  Borrowed  Unused  Duration(us)");
    }
    for (i = 0; (i < cfg->peer_num_cal) &&
         (i < ATF_ACTIVED_MAX_CLIENTS); i++) {
        borrowed = unused = 0;
        configured = cfg->peer_id[i].sta_cal_value /
                ATF_AIRTIME_CONVERSION_FACTOR;
        actual = (cfg->peer_id[i].actual_airtime * 100) /
             patf->actual_total_airtime;
        if (actual > configured) {
            borrowed = actual - configured;
        } else {
        ...
        if (patf->atf_ssidgroup) {
            if (cfg->peer_id[i].index_group &&
                ...
                index = cfg->peer_id[i].index_group - 1;
                ssid = cfg->atfgroup[index].grpname;
            } else {
                ssid = "Default";
            }
            qdf_info("%s  %s      %d%%  \t  %d%% \t  %d%%  \t  %d%%  \t  %u",
                 ether_sprintf(cfg->peer_id[i].sta_mac), ssid,
                 configured, actual, borrowed, unused,
                 cfg->peer_id[i].actual_airtime);
        } else if ((cfg->peer_id[i].index_vdev - 1) <
               ATF_CFG_NUM_VDEV) {
            index = cfg->peer_id[i].index_vdev - 1;
            ssid = cfg->vdev[index].essid;
            qdf_info("%s  %s      %d%%  \t  %d%% \t  %d%%  \t  %d%%  \t  %u",
                 ether_sprintf(cfg->peer_id[i].sta_mac), ssid,
                 configured, actual, borrowed, unused,
                 cfg->peer_id[i].actual_airtime);
...
static void atf_stats_timer_handler(void *arg)
{
    struct wlan_objmgr_pdev *pdev = (struct wlan_objmgr_pdev *)arg;
    struct pdev_atf *pa;
    ...
    qdf_atomic_set(&pa->atf_stats_pause, 1);
    atf_stats_print(pdev);
    atf_stats_reset(pdev);
    qdf_atomic_set(&pa->atf_stats_pause, 0);
    if (pa->atf_stats_enable)
        atf_stats_start_timer(pdev);
}

void atf_pdev_atf_init(struct wlan_objmgr_pdev *pdev, struct pdev_atf *pa)
{
    struct atf_group_list *group = NULL;
    struct atf_subgroup_list *subgroup = NULL;
    uint8_t groupindex = 0;
    ...
    pa->atf_ssidgroup = 0;
    /* reset */
    qdf_mem_zero(&pa->atfcfg_set, sizeof(struct atf_config));
...
void atf_pdev_atf_deinit(struct wlan_objmgr_pdev *pdev, struct pdev_atf *pa)
{
    struct atf_group_list *group = NULL;
    struct atf_subgroup_list *subgroup = NULL;
    ...
    pa->atf_stats_enable = false;
    pa->atf_stats_timeout = 0;
    qdf_timer_free(&pa->atf_stats_timer);
...
void atf_stats_start_timer(struct wlan_objmgr_pdev *pdev)
{
	struct pdev_atf *pa;
	...
	if ((pa->atf_stats_timeout < ATF_STATS_MIN_TIMEOUT) ||
	    (pa->atf_stats_timeout > ATF_STATS_MAX_TIMEOUT)) {
		pa->atf_stats_timeout = ATF_STATS_DEFAULT_TIMEOUT;
		atf_info("Start ATF stats timer with %d secs timeout period",
			 pa->atf_stats_timeout);
	}

	qdf_timer_mod(&pa->atf_stats_timer, pa->atf_stats_timeout * 1000);
}

void atf_stats_stop_timer(struct wlan_objmgr_pdev *pdev)
{
	struct pdev_atf *pa;
	...
	qdf_timer_stop(&pa->atf_stats_timer);
}

QDF_STATUS atf_set_ppdu_stats(struct wlan_objmgr_pdev *pdev, uint8_t value)
{
	...
	atf_tx_ops = &tx_ops->atf_tx_ops;
	if (atf_tx_ops->atf_set_ppdu_stats)
		atf_tx_ops->atf_set_ppdu_stats(pdev, value);

	return QDF_STATUS_SUCCESS;
}

void atf_stats_reset(struct wlan_objmgr_pdev *pdev)
{
	struct pdev_atf *patf = NULL;
	struct atf_config *cfg = NULL;
	uint8_t i;
	...
	patf->actual_total_airtime = 0;
	cfg = &patf->atfcfg_set;
	for (i = 0; i < ATF_ACTIVED_MAX_CLIENTS; i++) {
		cfg->peer_id[i].actual_airtime = 0;
	}
	for (i = 0; i < ATF_CFG_NUM_VDEV; i++) {
		cfg->vdev[i].actual_airtime = 0;
	}
	for (i = 0; i < ATF_ACTIVED_MAX_ATFGROUPS; i++) {
		qdf_mem_zero(&(cfg->atfgroup[i].act_airtime),
			     sizeof(struct atf_actual_airtime));
	}
}
