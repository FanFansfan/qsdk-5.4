...
int32_t ucfg_atf_set_maxclient(struct wlan_objmgr_pdev *pdev, uint8_t enable)
{
	...
	retval = atf_set_maxclient(pdev, enable);
...
int32_t ucfg_atf_enable_stats(struct wlan_objmgr_pdev *pdev, uint32_t value)
{
	struct pdev_atf *pa;
	struct atf_context *ac;

	if (!pdev) {
		atf_err("Invalid Pdev!");
		return -1;
	}
	...
	pa = wlan_objmgr_pdev_get_comp_private_obj(pdev, WLAN_UMAC_COMP_ATF);
	if (!pa) {
		atf_err("Invalid pdev atf object");
		return -1;
	}

	if (!pa->atf_commit || !ac->atf_fmcap || !ac->atf_mode) {
		atf_err("ATF stats cannot be enabled without enabling ATF!\n");
		return -1;
	}

	if (value) {
		pa->atf_stats_enable = true;
...
int32_t ucfg_atf_is_stats_enabled(struct wlan_objmgr_pdev *pdev)
{
	struct pdev_atf *pa;
	...
	return pa->atf_stats_enable;
}

int32_t ucfg_atf_set_stats_timeout(struct wlan_objmgr_pdev *pdev, uint32_t value)
{
	struct pdev_atf *pa;
	struct atf_context *ac;
	...
	if (!pa->atf_commit || !ac->atf_fmcap || !ac->atf_mode) {
		atf_err("ATF stats timeout cannot be configured without enabling ATF!\n");
		return -1;
	}

	if ((value < ATF_STATS_MIN_TIMEOUT) || (value > ATF_STATS_MAX_TIMEOUT)) {
		atf_info("ATF stats timout value %d is invalid; Must be within %d - %d!",
			value, ATF_STATS_MIN_TIMEOUT, ATF_STATS_MAX_TIMEOUT);
		value = ATF_STATS_DEFAULT_TIMEOUT;
		atf_info("Proceed with default value %d\n", value);
	}

	pa->atf_stats_timeout = value;

	return 0;
}

int32_t ucfg_atf_get_stats_timeout(struct wlan_objmgr_pdev *pdev)
{
	struct pdev_atf *pa;
	...
	return pa->atf_stats_timeout;
}

static int32_t atf_stats_group_table(struct wlan_objmgr_vdev *vdev,
				     struct atftable *buf)
{
	struct pdev_atf *pa = NULL;
	struct wlan_objmgr_peer *peer = NULL;
	...
	uint32_t leftperuint = 0;
	uint8_t grp_index[ATF_CFG_NUM_VDEV] = {0};
	uint8_t i = 0, j = 0, k =0, l = 0, group_mark = 0, len = 0;
	uint8_t *sta_mac = NULL;
	uint8_t ssid[WLAN_SSID_MAX_LEN+1] = {0};
	uint8_t pdev_id;
	...
	psoc = wlan_vdev_get_psoc(vdev);
	pdev_id = wlan_objmgr_pdev_get_pdev_id(wlan_vdev_get_pdev(vdev));
	buf->pdev_stats_airtime = pa->actual_total_airtime;
	if (cfg->percentage_unit == 0)
		leftperuint = 1000;
	else
		leftperuint = cfg->percentage_unit;
	...
	for (i = 0; i < cfg->grp_num_cfg; i++) {
		...
		for (j = 0; j < ATF_ACTIVED_MAX_CLIENTS; j++) {
			...
			if (!group_mark) {
				l = buf->info_cnt;
				act_grp = &cfg->atfgroup[i].act_airtime;
				len = qdf_str_len(cfg->atfgroup[i].grpname);
				qdf_mem_copy(buf->atf_info[l].grpname,
					     cfg->atfgroup[i].grpname, len);
				buf->atf_info[l].value =
						act_grp->actual_airtime;
				buf->atf_info[l].cfg_value =
					cfg->atfgroup[i].grp_cfg_value;
				buf->atf_info[l].info_mark = 0;
				buf->atf_info[l].atf_ac_cfg =
			...
			wlan_vdev_mlme_get_ssid(vdev_temp, ssid, &len);
			qdf_mem_copy(buf->atf_info[l].ssid, ssid, len);
			buf->atf_info[l].value = cfg->peer_id[j].actual_airtime;
			/**
			* Assigning peer percentage from Global value, though we
			* don't use this value if grouping is enabled in ATF.
			**/
	...
	for (j = 0; j < ATF_ACTIVED_MAX_CLIENTS; j++) {
		...
		if (cfg->peer_id[j].index_group == 0xFF) {
			sta_mac = &cfg->peer_id[j].sta_mac[0];
			l = buf->info_cnt;
			buf->atf_info[l].value =
					cfg->peer_id[j].actual_airtime;
			buf->atf_info[l].cfg_value =
					cfg->peer_id[j].sta_cal_value;
			buf->atf_info[l].assoc_status =
					cfg->peer_id[j].sta_assoc_status;
...
static int32_t atf_stats_vdev_table(struct wlan_objmgr_vdev *vdev,
				    struct atftable *tbl)
{
	struct pdev_atf *pa = NULL;
	uint8_t *sta_mac = NULL;
	...
	tbl->info_cnt = 0;
	tbl->pdev_stats_airtime = pa->actual_total_airtime;
		...
		for (j = 0; j < ATF_ACTIVED_MAX_CLIENTS; j++) {
			...
			if (!init_flag) {
				...
				tbl->atf_info[l].cfg_value =
						cfg->vdev[i].vdev_cfg_value;
				tbl->atf_info[l].info_mark = 0;
				tbl->atf_info[l].atf_ac_cfg = atfac->ac_bitmap;
				tbl->info_cnt++;
				init_flag++;
				vdev_index[i] = i + 1;
				k += 1;
			...
			len = qdf_str_len(cfg->vdev[i].essid);
			l = tbl->info_cnt;
			qdf_mem_copy(tbl->atf_info[l].ssid, cfg->vdev[i].essid,
				     len);
			tbl->atf_info[l].cfg_value =
						cfg->peer_id[j].sta_cal_value;
			tbl->atf_info[l].assoc_status =
	...
	qdf_mem_copy(tbl->atf_info[tbl->info_cnt].ssid, OTHER_SSID,
		     qdf_str_len(OTHER_SSID));
	tbl->atf_info[tbl->info_cnt].value = leftperuint;
	tbl->atf_info[tbl->info_cnt].cfg_value = 0;
	tbl->atf_info[tbl->info_cnt].info_mark = 0;
	...
	for (j = 0; j < ATF_ACTIVED_MAX_CLIENTS; j++) {
		...
		if (cfg->peer_id[j].index_vdev == 0xff) {
			...
			qdf_mem_copy(tbl->atf_info[l].sta_mac, sta_mac,
				     QDF_MAC_ADDR_SIZE);
			qdf_mem_copy(tbl->atf_info[l].ssid, OTHER_SSID,
				     strlen(OTHER_SSID));
			tbl->atf_info[l].info_mark = 1;
			tbl->info_cnt++;
		}
...
int32_t ucfg_atf_get_stats(struct wlan_objmgr_vdev *vdev, struct atftable *tbl)
{
	struct pdev_atf *pa = NULL;
	struct atf_config *cfg = NULL;
	int32_t retv = 0;
	...
	if (pa->atf_ssidgroup && cfg->grp_num_cfg) {
		retv = atf_stats_group_table(vdev, tbl);
	} else {
		retv = atf_stats_vdev_table(vdev, tbl);
	}

	return 0;
}

int32_t ucfg_atf_get_ac_stats(struct wlan_objmgr_vdev *vdev,
			      struct atfgrouplist_table *buf)
{
	uint32_t i = 0, k = 0;
	uint8_t j = 0, total_count = 0;
	uint8_t index = 0;
	struct pdev_atf *pa = NULL;
	struct atf_config *cfg = NULL;
	struct atf_group_list *group = NULL;
	...
	buf->pdev_stats_airtime = pa->actual_total_airtime;
	total_count = (cfg->grp_num_cfg < ATF_ACTIVED_MAX_ATFGROUPS) ?
				cfg->grp_num_cfg : ATF_ACTIVED_MAX_ATFGROUPS;

	for (index = 0; index < total_count; index++) {
		group = cfg->atfgroup[index].grplist_entry;
		act_grp = &cfg->atfgroup[index].act_airtime;
		if (!group) {
			continue;
		}
		i = buf->info_cnt;
		qdf_mem_copy(buf->atf_list[i].grpname,
			     group->group_name, WLAN_SSID_MAX_LEN);
		buf->atf_list[i].grp_value = group->group_airtime;
		for (j = 0; j < WME_NUM_AC; j++) {
			if (!atf_config_is_subgroup_exist(cfg, group, &j,
							  &subgroup,
							  ATF_SUBGROUP_TYPE_AC))
				continue;
			if (!subgroup)
				continue;
			k = buf->atf_list[i].num_subgroup;
			buf->atf_list[i].sg_table[k].subgrp_type =
							subgroup->sg_type;
			buf->atf_list[i].sg_table[k].subgrp_ac_id =
							subgroup->ac_id;
			actual_airtime = act_grp->actual_ac_airtime[
							subgroup->ac_id];
			buf->atf_list[i].sg_table[k].subgrp_value =
							actual_airtime;
			buf->atf_list[i].sg_table[k].subgrp_cfg_value =
							subgroup->sg_atf_units;
			buf->atf_list[i].num_subgroup++;
		}
		buf->info_cnt++;
	}
