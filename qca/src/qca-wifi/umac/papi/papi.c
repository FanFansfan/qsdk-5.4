                                     ...
                                     u_int8_t event)
{
    ...
    OS_MEMCPY(assoc.mac, ni->ni_macaddr, QDF_MAC_ADDR_SIZE);
                                         ...
                                         struct ieee80211_rx_status *rs)
{
    ...
    if (ni == vap->iv_bss) {
        ...
        sender_ni = ieee80211_find_rxnode(ic, (struct ieee80211_frame_min *)
                                          wbuf_header(wbuf), WLAN_MISC_ID);
        if (sender_ni) {
            /* probe from associated sta ignore */
            ieee80211_free_node(sender_ni, WLAN_MISC_ID);
            return;
        }
        ...
        probe.band = IEEE80211_IS_CHAN_2GHZ(rs->rs_full_chan) ? PAPI_BAND_24_GHZ :
                     (IEEE80211_IS_CHAN_5GHZ(rs->rs_full_chan) ? PAPI_BAND_5_GHZ : PAPI_BAND_6_GHZ);
