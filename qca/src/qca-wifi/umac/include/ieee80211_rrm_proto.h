...
typedef struct ieee80211_rrm
{
    ...
    u_int8_t                            send_radiom_req, send_radiom_resp, recv_radiom_req, recv_radiom_resp;
    u_int8_t                            send_neigh_resp, recv_neigh_req;
}*ieee80211_rrm_t;
...
/**
 * Check node with given mac address
 * is rrm capable or not.
 * param vap:pointer to vap structure 
...
#else
static inline int ieee80211_set_rrmstats_misc(wlan_if_t vap, int val)
{
    return EINVAL;
}
static inline int ieee80211_get_rrmstats_misc(wlan_if_t vap)
{
    return EINVAL;
}
