...
#define IEEE80211_DELIVER_EVENT_MLME_UNPROTECTED_DEAUTH_INDICATION( _vap,_macaddr,_associd, _reason, _wbuf)  do {    \
        int i;                                                                                                \
        for(i=0;i<IEEE80211_MAX_VAP_MLME_EVENT_HANDLERS; ++i) {                                               \
            if ((_vap)->iv_mlme_evtable[i] &&                                                                 \
            (_vap)->iv_mlme_evtable[i]->mlme_unprotected_deauth_indication) {                                 \
                (* (_vap)->iv_mlme_evtable[i]->mlme_unprotected_deauth_indication)                            \
                                           ((_vap)->iv_mlme_arg[i], _macaddr, _associd, _reason, _wbuf);      \
             }                                                                                                \
        }                                                                                                     \
    } while(0)
...
#define IEEE80211_DELIVER_EVENT_NODE_SMPS_UPDATE( _vap,_val,_len)  do {                               \
    int i;                                                                                   \
    for(i=0;i<IEEE80211_MAX_MISC_EVENT_HANDLERS; ++i) {                                      \
        if ((_vap)->iv_misc_evtable[i] &&                                                    \
                        (_vap)->iv_misc_evtable[i]->wlan_recv_smpsupdate) {                        \
            (* (_vap)->iv_misc_evtable[i]->wlan_recv_smpsupdate)                                   \
             ((_vap)->iv_misc_arg[i],_val,_len);                                             \
        }                                                                                    \
    }                                                                                        \
...
#define IEEE80211_DELIVER_EVENT_OPMODE_UPDATE( _vap,_val,_len)  do {                               \
    int i;                                                                                   \
    for(i=0;i<IEEE80211_MAX_MISC_EVENT_HANDLERS; ++i) {                                      \
        if ((_vap)->iv_misc_evtable[i] &&                                                    \
                        (_vap)->iv_misc_evtable[i]->wlan_recv_opmodeupdate) {                        \
            (* (_vap)->iv_misc_evtable[i]->wlan_recv_opmodeupdate)                                   \
             ((_vap)->iv_misc_arg[i],_val,_len);                                             \
        }                                                                                    \
    }                                                                                        \
...
bool ieee80211_is_basic_txrx_mcsnss_requirement_met(
                    struct ieee80211_node *ni, uint8_t mcsnss_idx);
...
/* wlan_tmpl_update_6ghz_frm: Function to update the FILS/20TU frm
 * of 6ghz band to indicate if 6Ghz only AP or not in order to
 * add RNR IE of Non Tx APs
 */
