				  ...
				  struct scan_event *event)
{
	...
	switch (event->type) {
		...
		case SCAN_EVENT_TYPE_PREEMPTED:
		case SCAN_EVENT_TYPE_START_FAILED:
			offchan_err("Scan failed, type %d\n",event->type);
			qdf_mem_zero(evt_tstamp, sizeof(*evt_tstamp));
			break;
		default:
			break;
	}
						...
						struct offchan_tx_req *req)
{
	...
	if (req->high_priority)
		scan_params->scan_req.scan_priority = SCAN_PRIORITY_VERY_HIGH;
	else
	...
	if(req->wide_scan.bw_mode) {
		...
		status = ucfg_scan_init_chanlist_params(scan_params, 1,
							&req->chan, NULL);
		if (QDF_IS_STATUS_ERROR(status)) {
			status = QDF_STATUS_E_INVAL;
			goto free_scan_params;
