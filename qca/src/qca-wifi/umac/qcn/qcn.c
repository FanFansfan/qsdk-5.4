                                                ...
                                                u_int8_t *frm)
{
    ...
    *frm++ = (ic->ic_he.hecap_info_internal &
    ...
    qdf_mem_copy(&ic_hecap_mac_high, &ic->ic_he.hecap_macinfo[HECAP_MACBYTE_IDX4], sizeof(ic_hecap_mac_high));

    *frm++ = vap->iv_he_mcs12_13_supp ? ((ic_hecap_mac_high >> IEEE80211_HE_MCS_12_13_SUPP_L80) & IEEE80211_HE_MCS_12_13_SUPP_MASK) : 0;
    *frm++ = vap->iv_he_mcs12_13_supp ? ((ic_hecap_mac_high >> IEEE80211_HE_MCS_12_13_SUPP_G80) & IEEE80211_HE_MCS_12_13_SUPP_MASK) : 0;

    if (vap->iv_bsscolor_rptr_info) {
        /* Add Repeater info attribute */
        frm = ieee80211_add_qcn_attr_header_info(frm, QCN_ATTRIB_REPEATER_INFO,
                REPEATER_INFO_ATTRIB_LEN, &len);

        *frm++ = vap->iv_bsscolor_rptr_info;
    }
    return len;
}
...
{
        uint16_t self_higher_mcs_supp;
        uint32_t ic_hecap_mac_high;

        qdf_mem_copy(&ic_hecap_mac_high, &ic->ic_he.hecap_macinfo[HECAP_MACBYTE_IDX4], sizeof(ic_hecap_mac_high));
        self_higher_mcs_supp = ic_hecap_mac_high >> IEEE80211_HE_MCS_12_13_SUPP_L80;
        ...
        struct ieee80211_node *ni,u_int8_t * data)
{
    ...
    if(data) {
            ...
            switch(attribute_id){
                ...
                case QCN_ATTRIB_HE_DL_MUMIMO_SUPP:
                    ...
                    updated_len += (attribute_len +
                                sizeof(struct ieee80211_qcn_header));
                    break;
                case QCN_ATTRIB_REPEATER_INFO:
                    attribute_len = *frm++;
                    ni->ni_bsscolor_rept_info = *frm++;
                    if ((ni->ni_bsscolor_rept_info != QCN_REPT_INVALID_TYPE) &&
                            (ni->ni_bsscolor_rept_info != QCN_REPT_ONLY_STAVAP)) {
                        ic->ic_rept_clients = 1;
                    }
                    updated_len += (attribute_len +
                                sizeof(struct ieee80211_qcn_header));
                    break;
        ...
        IEEE80211_DPRINTF(ni->ni_vap, IEEE80211_MSG_MLME,
                ...
                "HE MCS12/13 supp: 0x%x BSSCOLOR REPEATER INFO: 0x%x\n",
