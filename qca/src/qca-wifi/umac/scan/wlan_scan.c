...
wlan_find_full_channel(struct ieee80211com *ic, uint32_t freq)
{
    ...
    c = NULL;
    if (IEEE80211_IS_FLAG_2GHZ(band_flag)) {
    ...
    } else if (IEEE80211_IS_FLAG_5GHZ(band_flag)) {
    ...
    } else if (IEEE80211_IS_FLAG_6GHZ(band_flag)) {
        ...
        struct scan_cache_entry *scan_entry)
{
    ...
    if (ap_vhtop) {
        if (htinfo) {
            if ((ieee80211_is_phymode_8080(ap_phymode)) &&
        ...
        if((!chan) && ic->ic_emiwar_80p80 && \
                (ieee80211_is_phymode_11ac_vht80_80(ap_phymode))) {
            /*
...
static const u_int16_t default_scan_order[] = {
    ...
    /* 6G channels */
    5935, 5955, 5975, 5995, 6015, 6035, 6055, 6075, 6095, 6115, 6135, 6155,
    6175, 6195, 6215, 6235, 6255, 6275, 6295, 6315, 6335, 6355, 6375, 6395,
    6415, 6435, 6455, 6475, 6495, 6515, 6535, 6555, 6575, 6595, 6615, 6635,
    6655, 6675, 6695, 6715, 6735, 6755, 6775, 6795, 6815, 6835, 6855, 6875,
...
#define IEEE80211_2GHZ_FREQUENCY_THRESHOLD    3000  /* in kHz */
    ...
    for (i = 0; i < scan_order_size; ++i) {
        ...
        if (c && !IEEE80211_IS_CHAN_RADAR(ic, c))
#else
            if (c)
#endif
    ...
    if (wlan_wide_band_scan_enabled(ic)) {
        ...
        for (i = 0; i < scan_order_size; ++i) {
            ...
            /* AX HE 20MHz 6G channels are added already as part of full channel list*/
            if (IEEE80211_SUPPORT_PHY_MODE(ic, IEEE80211_MODE_11AXA_HE20) &&
                (!(IEEE80211_IS_FLAG_6GHZ(band_flag)))) {
