/*
 * Copyright (c) 2013,2017,2021 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
                                        ...
                                        ieee80211_rrm_nrreq_info_t* nrreq_info)
{
    ...
    /* If this AP supports FTM responder mode & LCI/LCR capable & Neighbor request has LCI/LCR set, add itself in Nbr element */
    if((vap->rtt_enable & RTT_RESPONDER_MODE) &&
       (vap->lcr_enable || vap->lci_enable) &&
...
#if QCA_SUPPORT_SON
    ieee80211_rrm_t rrm = vap->rrm;
    struct ev_rrm_report_data rrm_bcnrpt_event_data = {0};
#endif
    ...
    switch(rsp->rsptype)
    {
        ...
        case IEEE80211_MEASRSP_BEACON_REPORT:
             ...
             if (rsp->rspmode & (IEEE80211_RRM_MEASRPT_MODE_BIT_INCAPABLE |
                                    ...
                                    IEEE80211_RRM_MEASRPT_MODE_BIT_REFUSED)) {
                 if (rrm->rrm_app_filter) {
                     rrm_bcnrpt_event_data.dialog_token = dialog_token;
                     rrm_bcnrpt_event_data.rrm_type = RRM_TYPE_BCNRPT;
                     qdf_mem_copy(rrm_bcnrpt_event_data.macaddr, vap->rrm->rrm_macaddr, QDF_MAC_ADDR_SIZE);
                     rrm_bcnrpt_event_data.mode = rsp->rspmode;
                     IEEE80211_DELIVER_EVENT_BCNRPT_AP(vap, &rrm_bcnrpt_event_data, sizeof(struct ev_rrm_report_data));
                     break;
                 }
            }
        ...
        u_int8_t *sfrm, u_int32_t frm_len, u_int8_t dialog_token)
{
    ...
    u_int32_t rrm_bcnrpt_ev_data_len = 0;
    struct ev_rrm_report_data *rrm_bcnrpt_event_data = NULL;
    ieee80211_rrm_t rrm = vap->rrm;
    struct son_act_frm_info rrm_info;
...
#if QCA_SUPPORT_SON
    mie = (struct ieee80211_measrsp_ie *)frame;
    reports = (ieee80211_bcnrpt_t *)qdf_mem_malloc(IEEE80211_RRM_NUM_BCNRPT_MAX * sizeof(ieee80211_bcnrpt_t));
    if (reports) {
        qdf_mem_zero(reports, IEEE80211_RRM_NUM_BCNRPT_MAX * sizeof(ieee80211_bcnrpt_t));
        num_bcnrpt_requested = IEEE80211_RRM_NUM_BCNRPT_MAX;
    } else {
        num_bcnrpt_requested = 0;
    }
#endif
            ...
            case IEEE80211_ELEMID_MEASREP:
                {
                    ...
                    if (num_bcnrpt_requested) {
                        ...
                        if (bcnrpt_count == num_bcnrpt_requested) {
                            if (rrm->rrm_app_filter) {
                                rrm_bcnrpt_ev_data_len = ((bcnrpt_count <= IEEE80211_RRM_NUM_BCNRPT_MAX ?
                                                           bcnrpt_count : IEEE80211_RRM_NUM_BCNRPT_MAX)
                                                           * sizeof(ieee80211_bcnrpt_t)) + sizeof(struct ev_rrm_report_data);
                                rrm_bcnrpt_event_data = (struct ev_rrm_report_data *)qdf_mem_malloc(rrm_bcnrpt_ev_data_len);
                                if (!rrm_bcnrpt_event_data) {
                                    qdf_mem_free(reports);
                                    ieee80211_free_node(ni, WLAN_RRM_ID);
                                    RRM_FUNCTION_EXIT;
                                    return -ENOMEM;
                                }
                                qdf_mem_zero(rrm_bcnrpt_event_data, rrm_bcnrpt_ev_data_len);
                                rrm_bcnrpt_event_data->dialog_token = dialog_token;
                                rrm_bcnrpt_event_data->bcnrpt_count = bcnrpt_count;
                                rrm_bcnrpt_event_data->rrm_type = RRM_TYPE_BCNRPT;
                                qdf_mem_copy(rrm_bcnrpt_event_data->macaddr, vap->rrm->rrm_macaddr, QDF_MAC_ADDR_SIZE);
                                rrm_bcnrpt_event_data->mode = IEEE80211_RRM_MEASRPT_MODE_SUCCESS;
                                qdf_mem_copy(&rrm_bcnrpt_event_data->bcnrpt, reports, (bcnrpt_count <= IEEE80211_RRM_NUM_BCNRPT_MAX ?
                                             bcnrpt_count : IEEE80211_RRM_NUM_BCNRPT_MAX) * sizeof(ieee80211_bcnrpt_t));
                                IEEE80211_DELIVER_EVENT_BCNRPT_AP(vap, rrm_bcnrpt_event_data, rrm_bcnrpt_ev_data_len);
                                qdf_mem_free(rrm_bcnrpt_event_data);
                            }
                            bcnrpt_count = 0;
                        }
                    }
    ...
    if (total_bcnrpt_count) {
        ...
        frame -= 3; /* To retrieve category, action, dialog_token */
        ia = (struct ieee80211_action *) frame; /* cat and action */
        rrm_info.ia = ia;
        rrm_info.ald_info = 0;
        frame += 3;
        rrm_info.data.rrm_data.dialog_token = IEEE80211_ACTION_RM_TOKEN;
        qdf_mem_copy(rrm_info.data.rrm_data.macaddr, vap->rrm->rrm_macaddr, QDF_MAC_ADDR_SIZE);
        rrm_info.data.rrm_data.num_meas_rpts = total_bcnrpt_count;
        son_update_mgmt_frame(vap->vdev_obj, ni->peer_obj, IEEE80211_FC0_SUBTYPE_ACTION,
                              frame, frm_len, &rrm_info);
    }
#endif
...
#endif
    /* Set is_ftm flag if AP supports RTT responder mode */
    ninfo.is_ftm = (vap->rtt_enable & RTT_RESPONDER_MODE) ? 1 : 0;
