...
QDF_STATUS ieee80211_fill_nrinfo(void *arg, wlan_scan_entry_t se)
{
    ...
    if(ninfo.is_he) {
        ...
        if(heop) {
            ...
            ninfo.is_he_er_su = !HEOP_PARAM_ER_SU_DISABLE_GET(heop_param);  /* If AP is HE ER SU capable */
        }
    }
#endif
...
u_int8_t ieee80211_rrm_get_chwidth_for_wbc(u_int64_t ic_flags)
{
    ...
    if (IEEE80211_IS_FLAG_80_80(ic_flags)) {
        chwidth = IEEE80211_CWM_WIDTH80_80;
    }
    else if (IEEE80211_IS_FLAG_160(ic_flags)) {
        chwidth = IEEE80211_CWM_WIDTH160;
    }
    else if (IEEE80211_IS_FLAG_80(ic_flags)) {
        chwidth = IEEE80211_CWM_WIDTH80;
    }
    else if (IEEE80211_IS_FLAG_40(ic_flags)) {
        chwidth = IEEE80211_CWM_WIDTH40;
    }
    else if (IEEE80211_IS_FLAG_TURBO(ic_flags) ||
             IEEE80211_IS_FLAG_CCK(ic_flags) ||
             IEEE80211_IS_FLAG_OFDM(ic_flags) ||
             IEEE80211_IS_FLAG_DYN(ic_flags) ||
             IEEE80211_IS_FLAG_GFSK(ic_flags) ||
             IEEE80211_IS_FLAG_STURBO(ic_flags) ||
             IEEE80211_IS_FLAG_HALF(ic_flags) ||
             IEEE80211_IS_FLAG_QUARTER(ic_flags) ||
             IEEE80211_IS_FLAG_20(ic_flags)) {
                             ...
                             ieee80211_rrm_beaconreq_info_t *binfo)
{
    ...
    if(IEEE80211_ADDR_EQ(vap->iv_myaddr,macaddr)) {
        ...
        for (i = 0; i < binfo->num_regclass; i++) {
             frm = ieee80211_add_measreq_beacon_ie((u_int8_t *)(frm), ni, binfo, wbuf_header(wbuf), i);
        }
...
int wlan_send_stastats_req(wlan_if_t vap, u_int8_t *macaddr, ieee80211_rrm_stastats_info_t *stastats)
{
    ...
    ni = ieee80211_find_node(ic, macaddr, WLAN_RRM_ID);
...
static
void wlan_get_rrm_sta_count_iter_cb(void *arg, struct ieee80211_node *ni)
{
    int *count = (int *)arg;
...
int wlan_get_rrm_sta_count(wlan_if_t vap)
{
    ...
    wlan_mlme_iterate_node_list(vap->iv_ic, wlan_get_rrm_sta_count_iter_cb,
                                (void *)&count,
...
int ieee80211_rrm_scan_start(wlan_if_t vap, bool active_scan)
{
    ...
    if ((opmode == IEEE80211_M_STA) || (opmode == IEEE80211_M_IBSS)) {
        ...
        if(rrm_req_params->chnum) {
            if (!rrm_req_params->regclass) {
                IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM,
                        "%s: Invalid regclass passed for rrm request: %u. Using ic_curchan instead", rrm_req_params->regclass);
                channel_freq = wlan_reg_chan_band_to_freq(ic->ic_pdev_obj,
                                                rrm_req_params->chnum,
                                                BIT(wlan_reg_freq_to_band(ic->ic_curchan->ic_freq)));
            } else {
                channel_freq = wlan_reg_chan_opclass_to_freq_auto(rrm_req_params->chnum,
                                                                  rrm_req_params->regclass,
                                                                  false);
            }

            if (!channel_freq) {
                scan_err("Invalid channel passed for rrm request: %u\n", rrm_req_params->chnum);
                ieee80211_rrm_free_scan_resource(rrm);
                qdf_mem_free(scan_params);
                wlan_objmgr_vdev_release_ref(vdev, WLAN_OSIF_SCAN_ID);
                return -EINVAL;
            ...
            chan = ieee80211_find_dot11_channel(ic, channel_freq, 0, IEEE80211_MODE_AUTO);
            ...
            status = ucfg_scan_init_chanlist_params(scan_params, 1, &channel_freq, NULL);
            if (status) {
...
bool ieee80211_node_is_rrm(wlan_if_t vap,char *mac)
{
    struct ieee80211_node *ni  = NULL;
    bool result = false;

    ni = ieee80211_find_node(vap->iv_ic, mac, WLAN_RRM_ID);
...
int ieee80211_set_rrmstats(wlan_if_t vap, int val)
{
    ...
    if (rrm) {
        rrm->send_radiom_req = rrm->send_radiom_resp =
             rrm->recv_radiom_req = rrm->recv_radiom_resp =
             rrm->send_neigh_resp = rrm->recv_neigh_req = 0;
    }

    return ieee80211_set_rrmstats_misc(vap, val);
}
...
int ieee80211_get_rrmstats(wlan_if_t vap)
{
    ...
    if (rrm) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "sent %d radio measurement request\n", rrm->send_radiom_req);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "received %d radio measurement request\n", rrm->recv_radiom_req);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "sent %d radio measurement response\n", rrm->send_radiom_resp);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "received %d radio measurement response\n", rrm->recv_radiom_resp);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "sent %d neighbor response\n", rrm->send_neigh_resp);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_RRM, "received %d neighbor request\n", rrm->recv_neigh_req);
    }

    return ieee80211_get_rrmstats_misc(vap);
}
