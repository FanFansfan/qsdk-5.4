 ...
 * @return 
 */
int ieee80211_set_rrmstats_misc(wlan_if_t vap, int val)
{
    ieee80211_rrm_t rrm = vap->rrm;
    if(val)
    {
 ...
 * @return 
 */
inline int ieee80211_get_rrmstats_misc(wlan_if_t vap)
{
    ieee80211_rrm_t rrm = vap->rrm;
    return rrm->rrmstats;
...
struct ieee80211_rrm_misc_stats
{
    ieee80211_rrm_t rrm_p;
    ieee80211_rrm_chloadreq_info_t *chinfo_p;
    ieee80211_rrm_nhist_info_t *nhist_p;
    ieee80211_rrm_cca_info_t *cca_p;
    ieee80211_rrm_rpihist_info_t *rpihist_p;
    ieee80211_rrm_stastats_info_t *stastats_p;
};

static void rrm_event_iter_cb(void *arg, struct ieee80211_node *ni)
{
    struct ieee80211_rrm_misc_stats *rrm_stats = (struct ieee80211_rrm_misc_stats *)arg;
    if (ni->ni_flags & IEEE80211_NODE_RRM) {
        switch(rrm_pre_to_rrm_type(ni->ni_rrmreq_type))
        {
            case IEEE80211_MEASREQ_CHANNEL_LOAD_REQ:
                wlan_send_chload_req(rrm_stats->rrm_p->rrm_vap,ni->ni_macaddr,rrm_stats->chinfo_p);
            break;
            case IEEE80211_MEASREQ_NOISE_HISTOGRAM_REQ:
                rrm_stats->nhist_p->regclass = DEFAULT_CLASS;
                rrm_stats->nhist_p->chnum = rrm_stats->rrm_p->rrm_vap->iv_bsschan->ic_freq;
                rrm_stats->nhist_p->m_dur = DEFAULT_DURATION;
                wlan_send_nhist_req(rrm_stats->rrm_p->rrm_vap, ni->ni_macaddr, rrm_stats->nbisht_p);
            break;
            ...
            case IEEE80211_MEASREQ_STA_STATS_REQ:
                rrm_stats->stastats_p->gid = rrm_stats->rrm_p->pre_gid ;
                if (rrm_stats->rrm_p->pre_gid == IEEE80211_STASTATS_GID10)
                    rrm_stats->rrm_p->pre_gid = IEEE80211_STASTATS_GID0;
                else if (rrm_stats->rrm_p->pre_gid == IEEE80211_STASTATS_GID1)
                    rrm_stats->rrm_p->pre_gid = IEEE80211_STASTATS_GID10;
                else if(rrm_stats->rrm_p->pre_gid == IEEE80211_STASTATS_GID0)
                    rrm_stats->rrm_p->pre_gid = IEEE80211_STASTATS_GID1;
                rrm_stats->stastats->m_dur = DEFAULT_DURATION; /*default value */
                wlan_send_stastats_req(rrm_stats->rrm_p->rrm_vap, ni->ni_macaddr,rrm_stats->stastats_p);
            break;
            default:
            break;
        }
...
#define DEFUALT_CHLOAD_TRAP_VAL 60    
    ...
    && rrm->slwindow)
    {
        struct ieee80211_rrm_misc_stats rrm_stats = {rrm, &chinfo, &nhist, &cca,
                                                     &rpihist, &stastats};
        ...
        wlan_mlme_iterate_node_list(ic, rrm_event_iter_cb, (void *)&rrm_stats,
