                ...
                void *htrates, u_int16_t vhtrates, u_int16_t herates)
{
    ...
    /* If add client already called before,
     * and Qos bit changed this time, return err.
     */
    if (wlan_peer_is_extflag_set(peer, IEEE80211_NODE_ADD_CLIENT_DONE)) {
        is_qos_set = wlan_peer_is_flag_set(peer, WLAN_PEER_F_QOS) ? 1 : 0;
        if (is_qos_set != qos) {
            splitmac_err("%s: peer %s Qos bit changed, return err!\n",__func__,
    ...
    if (status == 0) {
           wlan_set_peer_extflag(peer, IEEE80211_NODE_ADD_CLIENT_DONE);
