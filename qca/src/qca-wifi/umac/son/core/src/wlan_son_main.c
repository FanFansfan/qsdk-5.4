					       ...
					       void *arg)
{
	...
	psoc_priv->psoc = psoc;
	qdf_mem_zero(&psoc_priv->buff_thresh, sizeof(struct ald_record));
					       ...
					       void *arg)
{
	...
	if (son_ald_vattach(vdev, vdev_priv) != QDF_STATUS_SUCCESS) {
		SON_LOGF("SON ald attach failed");
		status = QDF_STATUS_E_FAILURE;
		goto attach_failure;
	}
					       ...
					       void *arg)
{
	...
	if (son_ald_vdetach(vdev)) {
		SON_LOGF("SON ald detach failed");
		status = QDF_STATUS_E_FAILURE;
	}
						...
						void *arg)
{
	...
	if (pd_priv) {
		SON_LOCK(&pd_priv->son_lock);
		if (wlan_peer_mlme_flag_get(peer, WLAN_PEER_F_EXT_STATS))
			pd_priv->peer_ext_stats_count--;
		SON_UNLOCK(&pd_priv->son_lock);
	}
...
PUBLIC QDF_STATUS wlan_son_psoc_close(struct wlan_objmgr_psoc *psoc)
{
	...
	if ( EOK != son_ald_destroy_netlink()) {
		return QDF_STATUS_E_FAILURE;
	}
...
int8_t son_core_get_wifi6_sta_stats(struct wlan_objmgr_vdev *vdev,
				    map_wifi6_stastats_t *wifi6_stats) {
	int retv = -EINVAL;
	struct wlan_objmgr_peer *peer = NULL;
	struct wlan_objmgr_pdev *pdev = NULL;
	struct wlan_objmgr_psoc *psoc = NULL;
	struct son_peer_priv *pe_priv = NULL;
	size_t n = 1, i = 0; // need to revisit

	pdev = wlan_vdev_get_pdev(vdev);
	psoc = wlan_pdev_get_psoc(pdev);
	peer = wlan_objmgr_get_peer(psoc, wlan_objmgr_pdev_get_pdev_id(pdev),
				    wifi6_stats->macaddr, WLAN_SON_ID);
	if (!peer) {
		return retv;
	}
	...
	for (i = 0 ; i < n ; i ++){
		wifi6_stats->tidinfo[i].tid = 1;
		wifi6_stats->tidinfo[i].queueSize = 0xff;
	}
			   ...
			   u_int8_t rssi_num_samples)
{
	...
	do{
		...
		tx_ops->son_tx_ops.son_send_null(pdev, dstmac, vdev);
...
{
	struct ieee80211_uplinkinfo rootinfo = {0};
...
int son_core_get_map_apcap(struct wlan_objmgr_vdev *vdev, mapapcap_t *apcap)
{
	...
	opclass = wlan_pdev_get_multi_ap_opclass(pdev, apcap, NULL /* map_op_chan */,
						 NULL /* opclass info */, 0 /* dfs not required */);
	if (opclass) {
		apcap->hwcap.num_supported_op_classes = opclass;
...
int son_core_enable_disable_peer_ext_stats(struct wlan_objmgr_peer *peer,
					   uint32_t enable)
{
	struct wlan_objmgr_vdev *vdev = NULL;
	struct wlan_objmgr_pdev *pdev = NULL;
	...
	if (pd_priv) {
		...
		if (tx_ops->son_tx_ops.peer_ext_stats_enable) {
			status = tx_ops->son_tx_ops.peer_ext_stats_enable(pdev, peer->macaddr, vdev,
									  pd_priv->peer_ext_stats_count, enable);
 ...
 *
 */
void son_core_ald_record_set_free_descs(struct wlan_objmgr_psoc *psoc,
					u_int32_t descs)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	ps_priv->buff_thresh.free_descs = descs;
}
 ...
 *
 */
u_int32_t son_core_ald_record_get_free_descs(struct wlan_objmgr_psoc *psoc)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	return ps_priv->buff_thresh.free_descs;
}
 ...
 *
 */
void son_core_ald_record_set_buff_full_warn(struct wlan_objmgr_psoc *psoc,
					    u_int8_t enable)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	ps_priv->buff_thresh.ald_buffull_wrn = enable;
}
 ...
 *
 */
u_int8_t son_core_ald_record_get_buff_full_warn(struct wlan_objmgr_psoc *psoc)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	return ps_priv->buff_thresh.ald_buffull_wrn;
}
 ...
 *
 */
void son_core_ald_record_set_buff_lvl(struct wlan_objmgr_psoc *psoc,
				      int thres)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	if ((thres >= MIN_BUFF_LEVEL_IN_PERCENT) && (thres <= 100)) {
		ps_priv->buff_thresh.ald_free_buf_lvl = ps_priv->buff_thresh.pool_size -
							((ps_priv->buff_thresh.pool_size * thres) / 100);
		SON_LOGI("Buff Warning Level=%d\n",
			 (ps_priv->buff_thresh.pool_size - ps_priv->buff_thresh.ald_free_buf_lvl));
	} else {
		SON_LOGI("ERR: Buff Thresh(in %%) should be >=%d and <=100\n",
 ...
 *
 */
u_int16_t son_core_ald_record_get_buff_lvl(struct wlan_objmgr_psoc *psoc)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	return ps_priv->buff_thresh.ald_free_buf_lvl;
}
 ...
 *
 */
void son_core_ald_record_set_pool_size(struct wlan_objmgr_psoc *psoc,
				       u_int32_t sz)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	ps_priv->buff_thresh.pool_size = sz;
}
 ...
 *
 */
u_int32_t son_core_ald_record_get_pool_size(struct wlan_objmgr_psoc *psoc)
{
	struct son_psoc_priv *ps_priv = NULL;

	ps_priv = wlan_son_get_psoc_priv(psoc);
	return ps_priv->buff_thresh.pool_size;
}
