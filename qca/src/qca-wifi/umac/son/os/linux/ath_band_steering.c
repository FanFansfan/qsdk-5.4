...
unsigned int netlink_band_steering_event = NETLINK_BAND_STEERING_EVENT;
module_param(netlink_band_steering_event, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
...
{
    if (!band_steering_nl) {
        qdf_nofl_info("%s band_steering_nl is NULL\n", __func__);
        return -ENODEV;
    }
...
qdf_export_symbol(ath_band_steering_netlink_send);
qdf_export_symbol(ath_band_steering_netlink_init);
qdf_export_symbol(ath_band_steering_netlink_delete);
#endif
