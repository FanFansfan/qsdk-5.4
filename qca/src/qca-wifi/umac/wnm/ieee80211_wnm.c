/*
 *
 * Copyright (c) 2014-2019,2021 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
                              ...
                              u_int8_t *frm, u_int8_t frm_len)
{
    ...
    case IEEE80211_ACTION_BSTM_QUERY:
        if (vap->iv_fwd_action_frames_to_app)
            return -EINVAL;
...
int wlan_send_mscs_resp(wlan_if_t vaphandle, u_int8_t *macaddr,
                        struct ieee80211_mscs_resp *mscs_resp)
{
    struct ieee80211vap      *vap = vaphandle;
    struct ieee80211_node    *ni;
    int error = 0;
    u_int16_t status = mscs_resp->status_code;

    ni = ieee80211_vap_find_node(vap, macaddr, WLAN_WNM_ID);
    if (ni == NULL || ni->ni_mscs == NULL) {
        error = -EINVAL;
        if (ni != NULL)
            ieee80211_free_node(ni, WLAN_WNM_ID);
        goto exit;
    ...
    if (status == IEEE80211_MSCS_TCLAS_PROCESSING_TERMINATED ||
        status == IEEE80211_MSCS_INSUFFICIENT_TCLAS_PROCESSING_RESOURCES) {
        /* MSCS Session for that STA has to be cleared */
        if (ni->ni_mscs)
            OS_MEMSET(ni->ni_mscs, 0 , sizeof(struct ieee80211_mscs_data));
    }

    error = ieee80211_send_mscs_resp(ni, mscs_resp->status_code,
                                    mscs_resp->mscs_dialog_token);
    ieee80211_free_node(ni, WLAN_WNM_ID);
exit:
    return error;
}
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
        if ((bssmax->idleperiod < 1) || (bssmax->idleperiod > 65534)) {
            ...
            if(vap->iv_ic->ic_get_tgt_type(vap->iv_ic) >= TARGET_TYPE_QCA8074) {
                /* Max BSS Idle period is advertised in units of 1000TUs.
                 * Convert the user input value to seconds.
                 */
                bssmax_period = ((vap->wnm->wnm_bss_max_idle_period *
                             TIME_UNIT_TO_MICROSEC) / 1000);
                /* Add a buffer to the configured Max BSS Idle period for FW
                 * tracking. This buffer will account for the Keep-alive frames
                 * sent by the peer just before the Max BSS Idle period expiry
                 * giving AP time to process it.
                 */
                bssmax_period += MAX_BSS_IDLE_PERIOD_BUFFER_SECS;
                /* Enable FW tracking of Max BSS Idle Timeout for lithium targets */
                status = wlan_set_param(vap, IEEE80211_RUN_INACT_TIMEOUT,
                                        bssmax_period);
            } else {
                status = 0;
            }
...
int wlan_wnm_set_tfs(wlan_if_t vaphandle, void *data)
{
    ...
    ni = ieee80211_find_node(vap->iv_ic, vap->iv_bss->ni_bssid, WLAN_WNM_ID);
    if (ni == NULL) {
        wlan_wnm_free_tfs(&tfs_req_head);
        return -EINVAL;
    }
...
static
void ieee80211_wnm_timbcast_cansend_iter_cb(void *arg,
                                            struct ieee80211_node *ni)
{
    uint8_t *match = (uint8_t *)arg;
    uint8_t sleep;

    /**
     * If ni with timbcast_status = IEEE80211_WNM_TIMREQUEST_ACCEPT has been
     * found, then no need to check for the remaining nodes
     */
    if (*match == 1)
        return;

    /* Check timbcast_status for ni */
...
int ieee80211_wnm_timbcast_cansend(wlan_if_t vap)
{
    ...
    wlan_iterate_all_sta_list(vap, ieee80211_wnm_timbcast_cansend_iter_cb,
                              (void *)&match);
...
int ieee80211_set_wnmstats(wlan_if_t vap, int val)
{
    ieee80211_wnm_t wnm = vap->wnm;

    if (wnm) {
        wnm->recv_bstm_query = wnm->recv_bstm_resp =
                               wnm->send_bstm_req = val = 0;
    }

    return 0;
}

int ieee80211_get_wnmstats(wlan_if_t vap)
{
    ieee80211_wnm_t wnm = vap->wnm;

    if (wnm) {
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_WNM, "Received %d BTM query\n", wnm->recv_bstm_query);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_WNM, "Received %d BTM response\n", wnm->recv_bstm_resp);
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_WNM, "Sent %d BTM request\n", wnm->send_bstm_req);
    }
                                     ...
                                     struct ieee80211_action *ia)
{
    ...
    info.ia = ia;
    info.ald_info = 1;
    qdf_mem_copy(info.data.macaddr, ni->ni_macaddr, QDF_MAC_ADDR_SIZE);
...
#define MACSTR "%02x:%02x:%02x:%02x:%02x:%02x"
    ...
    p2p_event.u.rx_frame.frame_snr = rs->rs_snr;
...
static
void wlan_wnm_set_bssterm_iter_cb(void *arg, struct ieee80211_node *ni)
{
    struct ieee80211_bstm_reqinfo *bstmreq = (struct ieee80211_bstm_reqinfo *)
                                              arg;
    struct ieee80211vap *vap = ni->ni_vap;

    IEEE80211_DPRINTF(vap, IEEE80211_MSG_WNM, "Send BSTM Request to "
                      "%s. BSS will terminate at %llu \n",
                      ether_sprintf(ni->ni_macaddr),
                      vap->wnm->wnm_bssterm_tsf);
    ieee80211_send_bstm_req(ni->ni_vap, ni, bstmreq);
}
...
int wlan_wnm_set_bssterm(wlan_if_t vaphandle, void *data)
{
    ...
    wlan_iterate_station_list(vap, wlan_wnm_set_bssterm_iter_cb,
                              (void *)&bstmreq);
...
static void wlan_get_btm_sta_count_iter_cb(void *arg, struct ieee80211_node *ni)
{
    int *count = (int *)arg;
...
int wlan_get_btm_sta_count(wlan_if_t vap)
{
    ...
    wlan_mlme_iterate_node_list(vap->iv_ic, wlan_get_btm_sta_count_iter_cb,
                                (void *)&count,
