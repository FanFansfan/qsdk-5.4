...
/* Hawkeye RSSI Chains */
#define MAX_SA_RSSI_CHAINS 8
#define MAX_SA_BW 7

#define MAX_EVM_SIZE_LEGACY 16 /* Max 16 pilot EVMs for 11 ac */
#define MAX_EVM_SIZE 32 /* Max 32 pilot EVMs */
...
/* Maximum MCS rates supported (Number of stream * Number of MCS code) = 8*12;
 * 11-bit ratecode packed in 16 bits */
#define MAX_MCS_RATES 96
#define MAX_VALID_RATES (MAX_MCS_RATES + MAX_CCK_OFDM_RATES) /* 96 MCS rates + 12 OFDM and CCK rates */
#define MAX_RATE_COUNTERS 4

#define SMART_ANT_BSS_MODE_AP         0x00
#define SMART_ANT_BSS_MODE_STA        0x01

#define SA_MASK_LOWER_NIBBLE          0x0f

#define SA_NIBBLE_BITS 4
...
enum sa_band_id {
	SA_BAND_UNSPECIFIED = 0, /* band not specified. Should be treated as 2G or 5G */
	SA_BAND_2GHZ = 1, /* 2G band */
	SA_BAND_5GHZ = 2, /* 5G band */
	SA_BAND_6GHZ = 3, /* 6G band */
	SA_BAND_MAX,
};
...
struct sa_config {
	...
	uint8_t channel_num;
	enum sa_band_id band; /* band corresponding to channel number */
...
struct sa_rate_cap {
	uint16_t ratecode_legacy[MAX_CCK_OFDM_RATES]; /* Rate code array for CCK OFDM */
	uint16_t ratecode_20[MAX_MCS_RATES];          /* Rate code array for 20MHz BW */
	uint16_t ratecode_40[MAX_MCS_RATES];          /* Rate code array for 40MHz BW */
	uint16_t ratecode_80[MAX_MCS_RATES];          /* Rate code array for 80MHz BW */
...
struct sa_tx_feedback {
	...
	uint32_t  rssi[MAX_SA_RSSI_CHAINS];  /* rssi of ACK/ Block ACK */
	...
	uint16_t ratemaxphy[MAX_RATE_COUNTERS]; /* max phy rate for all bandwidth */
...
struct  sa_rx_feedback {
	uint8_t  rx_rate_index;  /* received rate index */
	...
	int8_t   rx_rssi[MAX_SA_RSSI_CHAINS][MAX_SA_BW]; /* rssi of received packet */
