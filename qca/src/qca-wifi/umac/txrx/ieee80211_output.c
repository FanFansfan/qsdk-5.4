...
void ieee80211_kick_node(struct ieee80211_node *ni)
{
   ...
   if (ni->ni_associd != 0) {
#if QCA_SUPPORT_SON
       if (!son_is_steer_in_prog(ni->peer_obj)) {
...
ieee80211_complete_wbuf(wbuf_t wbuf, struct ieee80211_tx_status *ts )
{
    ...
    if (ic && (type == IEEE80211_FC0_TYPE_DATA) && (tidno == OFFCHAN_EXT_TID_NONPAUSE)) {
        uint32_t delayed_cleanup = 0;
        delayed_cleanup = (ts->ts_flags & IEEE80211_TX_ERROR_NO_SKB_FREE);
	 ...
	 */
        ...
        if ((ia->ia_category == IEEE80211_ACTION_CAT_HT) &&
            (ia->ia_action == IEEE80211_ACTION_HT_SMPOWERSAVE) &&
            ...
            ieee80211_pwrsave_smps_txrx_event_handler(ni->ni_vap->vdev_obj, &evt,
                                                      ni->ni_vap->iv_pwrsave_smps);
        }
#endif
