...
/*
 * In case of MBSSIE, one response needs to be sent on receiving a broadcsat
 * probe request.
 */
static void ieee80211_input_handle_mbss_probereq(struct ieee80211com *ic,
                                                 uint8_t *addr2)
{
    if (wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj,
                                   WLAN_PDEV_F_MBSS_IE_ENABLE)) {

        struct ieee80211vap *transmit_vap,*tmpvap;
        ...
        if (!transmit_vap) {
            mbss_debug("Tx vap is NULL!");
            return;
        }

        if (transmit_vap->iv_mbss.mbssid_send_bcast_probe_resp) {

            /* if SSID of all VAPs is hidden, return without sending a response
             */
            if (IEEE80211_VAP_IS_HIDESSID_ENABLED(transmit_vap) &&
                !ic->ic_mbss.prb_req_ssid_match_vap) {
                TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
                    if ((tmpvap != transmit_vap) &&
                        !IEEE80211_VAP_IS_HIDESSID_ENABLED(tmpvap)) {
                        flag = 1;
                        break;
                    }
                }
            ...
            if (!ni) {
                mbss_debug("Tx vap ni is NULL!");
                return;
            }

            OS_MEMZERO(&extractx, sizeof(extractx));
            extractx.is_broadcast_req = true;
            ieee80211_send_proberesp(ni, addr2, NULL, 0, &extractx);
            ic->ic_mbss.resp_sent = 1;

skip:
            /* clear the error flag */
            TAILQ_FOREACH(tmpvap, &ic->ic_vaps, iv_next) {
                tmpvap->iv_mbss.mbssid_send_bcast_probe_resp = 0;
            }
        }
    } /* WLAN_PDEV_F_MBSS_IE_ENABLE */
...
#define HAS_SEQ(type, subtype)   (((type & 0x4) == 0) && ((type | subtype) != QOS_NULL))
    ...
    if ((wlan_vdev_chan_config_valid(vap->vdev_obj) != QDF_STATUS_SUCCESS) ||
          ...
          ((wlan_vdev_chan_config_valid(vap->vdev_obj) == QDF_STATUS_SUCCESS) &&
           (!ieee80211_mbssid_txvap_is_active(vap)))) {
...
int ieee80211_input_iwspy_update_snr(struct ieee80211com *ic, u_int8_t *address, u_int8_t snr)
{
	struct ieee80211vap *vap = TAILQ_FIRST(&ic->ic_vaps);

	if (vap)
	{
		vap->iv_evtable->wlan_iwspy_update(vap->iv_ifp, address, snr);
	}
                     ...
                     enum mgmt_frame_type txrx_type)
{
    ...
    if(!WLAN_REG_IS_49GHZ_FREQ(ic->ic_curchan->ic_freq)) {
        if (rx_status->rs_snr < ic->mgmt_rx_snr) {
            if(wbuf_next(wbuf) != NULL) {
                wbuf_t wbx = wbuf;
                while (wbx) {
                    wbuf = wbuf_next(wbx);
