...
#define CW_20_40_SWITCH_DELAY 1000
...
ieee80211_wme_updateparams_locked(struct ieee80211vap *vap)
{
    ...
    if (vap->iv_opmode == IEEE80211_M_HOSTAP) {   /* XXX ibss? */
        ...
        vap->iv_muedcastate.muedca_param_update_count = temp_info_count;
    }
...
ieee80211_change_cw(struct ieee80211com *ic)
{
    ...
    wlan_mlme_iterate_node_list(ic, get_20MHz_only, &num_20MHz_only,
                                IEEE80211_NODE_ITER_F_ASSOC_STA);

    if (cw_width == IEEE80211_CWM_WIDTH40) {
        if (num_20MHz_only) {
            qdf_timer_sync_cancel(&ic->ic_cw_timer);
            ic->ic_bss_to20(ic);
        }
    } else if (cw_width == IEEE80211_CWM_WIDTH20) {
        if (num_20MHz_only == 0) {
            OS_SET_TIMER(&ic->ic_cw_timer, CW_20_40_SWITCH_DELAY);
        }
    }
}
