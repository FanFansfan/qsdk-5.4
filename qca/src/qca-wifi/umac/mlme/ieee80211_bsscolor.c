...
static void ieee80211_bsscolor_post_color_selection(struct ieee80211com *ic) {
    ...
    bool is_bcca_ongoing_for_any_vap = false;
        ...
        TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
            if (ieee80211_mbss_is_beaconing_ap(vap)) {
                ...
                if ((wlan_vdev_is_up(vap->vdev_obj) == QDF_STATUS_SUCCESS)) {
                    is_bcca_ongoing_for_any_vap = true;
                    vap->iv_he_bsscolor_change_ongoing = true;
                }
            }
        }

        if(is_bcca_ongoing_for_any_vap) {
                                           ...
                                           wlan_scan_entry_t scan_entry)
{
    ...
    if ((HEOP_BSSCOLORINFO_BSS_COLOR_GET(heop_bsscolor_info) < IEEE80211_HE_BSS_COLOR_MIN) ||
        (HEOP_BSSCOLORINFO_BSS_COLOR_GET(heop_bsscolor_info) > IEEE80211_HE_BSS_COLOR_MAX))  {
        /* please do not remove following log.
         * intended to be enabled in debug build
         * only
         */
...
#if SUPPORT_11AX_D3
        node->color     = HEOP_BSSCOLORINFO_BSS_COLOR_GET(heop_bsscolor_info);
#else
...
ieee80211_bsscolor_init(struct ieee80211com *ic)
{
    ...
    ini_def = cfg_get(scn->soc->psoc_obj, CFG_OL_BSS_COLOR_DEFAULT_VAL);

    if (ini_def <= IEEE80211_HE_BSS_COLOR_MAX) {
        if(!ini_def) {
            ic->ic_he_bsscolor = IEEE80211_BSS_COLOR_DEFAULT;
            bsscolor_hdl->state =
                        IEEE80211_BSS_COLOR_CTX_STATE_COLOR_USER_DISABLED;
        } else {
            /* Set BSS Color to ini defined defualt if valid and exists */
            ic->ic_he_bsscolor = ini_def;
        }
        ic->ic_bsscolor_hdl.selected_bsscolor = ic->ic_he_bsscolor;
        ic->ic_bsscolor_hdl.prev_bsscolor = ic->ic_he_bsscolor;
        ieee80211_set_ic_heop_bsscolor_disabled_bit(ic, !ini_def);
    } else {
    ...
    if (sel_color != IEEE80211_BSS_COLOR_INVALID) {
        ...
        if (prev_state ==
            ...
            if(!IS_RUNNING(dev) || ieee80211_get_num_vaps_up(ic) == 0 ||
                    ic->ic_mesh_mode || is_mbssid_enabled) {
                bsscolor_hdl->bcca_not_reqd = true;
            } else {
...
int ieee80211_is_bcca_ongoing_for_any_vap(struct ieee80211com *ic) {
    ...
    TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
        if (ieee80211_mbss_is_beaconing_ap(vap) &&
           ...
           return (wlan_vdev_get_id(vap->vdev_obj) + 1);
        }
    }
                         ...
                         struct ieee80211_ath_channel *chan)
{
    ...
    if (!IEEE80211_IS_CHAN_11AX(chan)) {
        QDF_TRACE(QDF_MODULE_ID_BSSCOLOR, QDF_TRACE_LEVEL_INFO,
            "%s  Avoiding BSS Color setup for non-11ax mode",__func__);
        QDF_TRACE(QDF_MODULE_ID_BSSCOLOR, QDF_TRACE_LEVEL_DEBUG,
                                                "%s<<", __func__);
        return;
    }
