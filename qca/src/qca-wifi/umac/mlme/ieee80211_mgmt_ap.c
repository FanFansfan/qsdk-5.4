...
ieee80211_update_ht_vht_he_flags(struct ieee80211_node *ni)
{
    ...
    switch(phymode) {
	    ...
	    /* ht , vht , & he are not applicable */
            ...
            ieee80211node_clear_flag(ni, IEEE80211_NODE_HT | IEEE80211_NODE_VHT);
            ieee80211node_clear_extflag(ni, IEEE80211_NODE_HE);
            break;
...
ieee80211_recv_asreq(struct ieee80211_node *ni, wbuf_t wbuf, int subtype)
{
    ...
    struct bs_assoc_reject_ind assoc_reject_event_data = {0};
    uint8_t reason;
    int reassoc, resp;
    uint8_t *athextcap, *opmode, *xcaps, *bwnss_oui, *supp_op_cl = NULL;
    uint8_t *ftcap = NULL;
    uint8_t *rsnxe = NULL;
    uint8_t *mscs_ie = NULL;
...
#define ADD_NI_RATES_NUM 8
    ...
    if (qdf_atomic_read(&(ni->ni_auth_tx_completion_pending))) {
        IEEE80211_NOTE(vap, IEEE80211_MSG_AUTH, ni,
                       "%s:Recv assoc req when AUTH tx completion is pending",
                       __func__);
    }
    ...
    while (((frm + 1) < efrm) && (frm + frm[1] + 1) < efrm) {
        ...
        case IEEE80211_ELEMID_BSSMAX_IDLE_PERIOD:
            ni->ni_ext_flags |= IEEE80211_NODE_MAX_BSS_IDLE_EN;
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_INACT,
                              "%s: Max BSS Idle enabled client",
                              __func__);
            break;
    ...
    son_update_mgmt_frame(vap->vdev_obj, ni->peer_obj, subtype, wbuf_header(wbuf), wbuf_get_pktlen(wbuf), NULL);
    ...
    if (vap->iv_mscs && mscs_ie) {
        ni->ni_mscs->mscs_assoc.retval = ieee80211_parse_mscs_ie(ni,
                  (struct ieee80211_mscs_descriptor *)mscs_ie);
        ni->ni_mscs->mscs_assoc.is_present = true;
    }
    ...
    if (ni == vap->iv_bss) {
        ...
        logically_deleted_ni =
            _ieee80211_find_logically_deleted_node(vap->iv_ic,
    ...
    qdf_mem_copy(assoc_reject_event_data.client_addr, wh->i_addr2, QDF_MAC_ADDR_SIZE);

    if (ic->ic_flags & IEEE80211_F_CHANSWITCH) {
#ifdef DROP_ASSOC_DURING_CSA
        qdf_info("Assoc request dropped as Channel switch is in progress");
#else
        qdf_info("Assoc request rejected as Channel switch is in progress");
#if WLAN_SUPPORT_SPLITMAC
        /*
...
#endif
            ieee80211_send_assocresp(ni, reassoc, IEEE80211_STATUS_REJECT_TEMP, NULL);
#if UMAC_SUPPORT_ACFG
            acfg_assoc_failure_indication_event(vap, (wh->i_addr2),
                                           IEEE80211_STATUS_REJECT_TEMP);
#endif
            assoc_reject_event_data.reason = IEEE80211_STATUS_REJECT_TEMP;
            son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
#if WLAN_SUPPORT_SPLITMAC
        if (!splitmac_is_enabled(vdev))
#endif
            IEEE80211_NODE_LEAVE(ni);
...
#endif
            assoc_reject_event_data.reason = IEEE80211_STATUS_POOR_CHAN_CONDITION;
            son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
            return -EINVAL;
        } else
#endif
...
#endif
                assoc_reject_event_data.reason = IEEE80211_STATUS_INVALID_RSNE_CAP;
                son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
#ifdef QCA_SUPPORT_CP_STATS
...
#endif
                assoc_reject_event_data.reason = status;
                son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
                IEEE80211_NODE_LEAVE(ni);
#ifdef QCA_SUPPORT_CP_STATS
                vdev_cp_stats_rx_assoc_wpaie_err_inc(vap->vdev_obj, 1);
...
#endif
            assoc_reject_event_data.reason = IEEE80211_STATUS_CAPINFO;
            son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
#ifdef QCA_SUPPORT_CP_STATS
...
#endif
            assoc_reject_event_data.reason = IEEE80211_STATUS_BASIC_RATE;
            son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
#ifdef QCA_SUPPORT_CP_STATS
    ...
    if (wpa != NULL) {
        ...
        if (!(ieee80211_is_pmf_enabled(vap, ni) && ieee80211_node_is_authorized(ni)))
    ...
    if (wme != NULL) {
         ...
         */
        if ((ni->ni_associd != 0) && (!ni->ni_wme_ie || !ieee80211node_has_flag(ni, IEEE80211_NODE_QOS))) {
#if WLAN_SUPPORT_SPLITMAC
            /*
             ...
             */
             if (splitmac_is_enabled(vdev)) {
                ...
                acfg_assoc_failure_indication_event(vap, (wh->i_addr2),
                                                ACFG_REASON_ASSOC_CAP_MISMATCH);
#endif
                assoc_reject_event_data.reason = IEEE80211_STATUS_INVALID_ELEMENT;
                son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
                return -EINVAL;
            } else
#endif
...
#endif
            assoc_reject_event_data.reason = IEEE80211_STATUS_NO_HT;
            son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
            return -EINVAL;
        } else
#endif
...
#endif
                assoc_reject_event_data.reason = IEEE80211_STATUS_UNSPECIFIED;
                son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
                return -EINVAL;
            } else
#endif
    ...
    if (ieee80211_is_phymode_11a(ni->ni_phymode)) {
        ...
        if (crypto_rx_ops && WLAN_CRYPTO_RX_OPS_ENCAP(crypto_rx_ops)) {
            ...
            if (QDF_STATUS_SUCCESS != ret) {
                ...
                if (!reason) {
                    ...
                    qdf_info("Still send Assoc Resp with Status Code %d", reason);
                }
            }
        }
    }
                          ...
                          struct ieee80211_app_ie_t* optie)
{
    ...
    *(u_int16_t *)frm = htole16(reason);
    ...
    if (reassoc) {
        ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
                IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_RIC_DATA, -1, &frm,
                TYPE_ALL_BUF, optie, true);
    }
...
#define MAX_BSS_IDLE_PERIOD_BUFFER_SECS 2
        /*
        ...
        /* Include Max BSS Idle Time IE in assoc response with default
         * Max BSS Idle period of 300(default for FW inactivity timer),
         * if the Max BSS Idle feature is disabled on the AP, but the STA
         * sends the Max BSS Idle IE in the association request frame.
         */
        if ((ni->ni_ext_flags & IEEE80211_NODE_MAX_BSS_IDLE_EN) &&
                                !ieee80211_wnm_bss_is_set(vap->wnm)) {
            /* Account for a buffer of 2secs to the default Max BSS Idle
             * period for FW tracking. This buffer will account for the
             * Keep-alive frames sent by the peer just before the Max BSS Idle
             * period expiry giving AP time to process it.
             */
            maxbssperiod_tus =
            (DEFAULT_WLAN_VDEV_AP_KEEPALIVE_MAX_UNRESPONSIVE_TIME_SECS -
             MAX_BSS_IDLE_PERIOD_BUFFER_SECS);

            /* Max BSS Idle Period value in the IE is in uints of 1000TUs.
             * Convert default FW inactivity timer value of 300secs in units
             * of 1000TUs before adding to IE.
             */
            maxbssperiod_tus = ((maxbssperiod_tus * 1000) /
                                        TIME_UNIT_TO_MICROSEC);
            vap->wnm->wnm_bss_max_idle_period = maxbssperiod_tus;

            /* Update FW with the Max BSS Idle period in units of secs.
             */
            wlan_set_param(vap, IEEE80211_RUN_INACT_TIMEOUT,
                    (DEFAULT_WLAN_VDEV_AP_KEEPALIVE_MAX_UNRESPONSIVE_TIME_SECS));
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_INACT,
                              "%s: Setting BSS Idle Time to %d for Max BSS Idle client",
                              __func__, vap->wnm->wnm_bss_max_idle_period);
        }
...
#if WLAN_SUPPORT_MSCS
    if (vap->iv_mscs) {
        if (ni->ni_mscs->mscs_assoc.is_present) {
            frm = ieee80211_add_mscs_ie(ni->ni_mscs, frm);
            ieee80211_send_mscs_info(ni, ni->ni_mscs->mscs_assoc.retval);
        }
    }
     ...
     */
    if ((ni->omit_rsnxe == false) && (vap->iv_sae_pwe != SAE_PWE_LOOP)) {
    ...
    if (IEEE80211_VAP_IS_WDS_ENABLED(vap) &&
        ...
        ieee80211_add_or_retrieve_ie_from_app_opt_ies(vap,
                    IEEE80211_FRAME_TYPE_ASSOCRESP, IEEE80211_ELEMID_VENDOR, IEEE80211_ELEMID_VENDOR_SON_AP,
...
#endif
#if QCA_SUPPORT_SON
    struct ieee80211_smps_update_data smps_update_event_data = {0};
#endif
...
#if QCA_SUPPORT_SON
            qdf_mem_copy(smps_update_event_data.macaddr, ni->peer_obj->macaddr, QDF_MAC_ADDR_SIZE);
            smps_update_event_data.is_static = (!smen && (ni->ni_updaterates & IEEE80211_NODE_SM_PWRSAV_STAT)) ? 1 : 0;
            IEEE80211_DELIVER_EVENT_NODE_SMPS_UPDATE(ni->ni_vap, &smps_update_event_data, sizeof(struct ieee80211_smps_update_data));
#endif
        ni->ni_updaterates = 0;
    }
}
        ...
        ieee80211_xmit_status *ts)
{
    ...
    if (ni &&(vap->iv_opmode == IEEE80211_M_HOSTAP) && (subtype == IEEE80211_FC0_SUBTYPE_AUTH)) {
        IEEE80211_NOTE(vap, IEEE80211_MSG_AUTH, ni,
                       "%s: AUTH frame completion handler for ni=0x%pK",
                       __func__, ni);
...
#define ADD_NI_RATES_NUM 8
    ...
    if ((ieee80211vap_vhtallowed(vap)) &&
        ...
        acfg_assoc_failure_indication_event(vap, (wh->i_addr2),
                                           ACFG_REASON_ASSOC_PUREAC_NOVHT);
#endif
        assoc_reject_event_data.reason = IEEE80211_STATUS_NO_VHT;
        son_update_mlme_event(vap->vdev_obj, ni->peer_obj, SON_EVENT_BSTEERING_TX_ASSOC_FAIL, &assoc_reject_event_data);
        return -EINVAL;
    }
