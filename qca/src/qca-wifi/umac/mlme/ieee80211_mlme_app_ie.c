    ...
    )
{
    ...
    STAILQ_FOREACH(ie_entry, &vap->iv_app_ie_list[ftype], link_entry) {
        ...
        if (IEEE80211_VAP_IS_HIDESSID_ENABLED(vap) && (ftype == IEEE80211_FRAME_TYPE_BEACON) && iswpsoui(ie_entry->app_ie.ie) ) {
            continue;
        } else if ((isqca_son_rept_oui(ie_entry->app_ie.ie, QCA_OUI_WHC_REPT_INFO_SUBTYPE)) || (is_qca_son_oui(ie_entry->app_ie.ie, QCA_OUI_WHC_AP_INFO_SUBTYPE))) {
            continue; //since SON IEs are added separately in frame, to avoid duplication
        } else {
...
int wlan_mlme_app_ie_delete(wlan_mlme_app_ie_t app_ie_handle, ieee80211_frame_type ftype, u_int8_t *app_ie)
{
    ...
    if (ftype == IEEE80211_FRAME_TYPE_BEACON) {
       ...
       vap->iv_app_ie_list[IEEE80211_FRAME_TYPE_BEACON].changed = true;
    ...
    u_int8_t                identifier)
{
    ...
    /* don't allow WPS for a MBSS VAP */
    if (wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj,
        ...
        if (!(is_ema_ap_enabled && soc->ema_ap_support_wps_6ghz)) {
            qdf_err("WPS is not allowed for MBSS VAP (id %d)!",
                     vap->iv_unit);
            error = -ENOTSUPP;
            goto exit;
        }
