...
#define BSS_RSSI_THRESHOLD_MASK 0x3F
#define SET_MIN_TXPOWER 1
#define SET_MAX_TXPOWER 2
...
#if QCA_SUPPORT_SON
    struct son_ald_assoc_allow_event_info info;
#endif
...
#define MBO_MAX_ASSOC_DISALLOW 0x06
            if (!ieee80211_vap_mbo_is_set(vap)) {
                ...
                qdf_mem_copy(info.bssid, vap->iv_myaddr, QDF_MAC_ADDR_SIZE);
                info.status = !(mbo->usr_assoc_disallow);
                son_update_mlme_event(vap->vdev_obj, NULL,
                                      SON_EVENT_ALD_ASSOC_ALLOWANCE_STATUS, &info);
#endif
            } else {
                               ...
                               u_int8_t *macaddr)
{
     ...
     */
     ni = ieee80211_find_node(vap->iv_ic, macaddr, WLAN_MBO_ID);
...
static void ieee80211_clear_opclass_vars(struct ieee80211_node *ni)
{
    ...
    ni->ni_supp_op_cl.bands_supported = 0;
}
...
{
    u_int8_t i, opclass_num = 0, temp_class = 0;
    char country_iso[4];
    ...
    for (i = 0; i < opclass_num; i++) {
         ...
         temp_class = ni->ni_supp_op_cl.supp_class[i];
         ieee80211_getCurrentCountryISO(ni->ni_ic, country_iso);

         /* Check for band supported only if it is not present already */
         if (!(ni->ni_supp_op_cl.bands_supported & BIT(REG_BAND_2G))
                 && wlan_reg_is_2ghz_op_class(country_iso, temp_class)) {
             ni->ni_supp_op_cl.bands_supported |= BIT(REG_BAND_2G);
         } else if (!(ni->ni_supp_op_cl.bands_supported & BIT(REG_BAND_5G))
                 && wlan_reg_is_5ghz_op_class(country_iso, temp_class)) {
             ni->ni_supp_op_cl.bands_supported |= BIT(REG_BAND_5G);
         } else if (!(ni->ni_supp_op_cl.bands_supported & BIT(REG_BAND_6G))
                 && wlan_reg_is_6ghz_op_class(NULL, temp_class)) {
             ni->ni_supp_op_cl.bands_supported |= BIT(REG_BAND_6G);
         }
    }
...
int ieee80211_oce_vattach (struct ieee80211vap *vap)
{
	...
	oce->oce_version_override = OCE_CAP_REL_DEFAULT;
	vap->oce = oce;
...
uint8_t* ieee80211_add_ess_rpt_ie(uint8_t *ie, struct ieee80211vap *vap)
{
    uint8_t len = 0, *start = NULL;
    struct ieee80211_ess_rpt_ie *ess_rpt_ie = (struct ieee80211_ess_rpt_ie *)ie;
    struct ieee80211_ess_rpt_field *ess_rpt_field;

    if (!ess_rpt_ie)
        return ie;

    ess_rpt_ie->element_id = IEEE80211_ELEMID_EXTN;
    ess_rpt_ie->element_id_extension = IEEE80211_ELEMID_EXT_ESS_REPORT;
    start = (uint8_t *)&ess_rpt_ie->ess_rpt_list[0];
    ess_rpt_field = (struct ieee80211_ess_rpt_field*)(start);

    if (vap->iv_planned_ess) {
        ess_rpt_field->planned_ess = 1;
    }
    if (vap->iv_edge_ess) {
        ess_rpt_field->edge_ess = 1;
    }
    if (vap->iv_bss_rssi_threshold) {
        ess_rpt_field->bss_rssi_threshold =
                                 vap->iv_bss_rssi_threshold & BSS_RSSI_THRESHOLD_MASK;
    }

    len = sizeof(struct ieee80211_ess_rpt_field);
    ess_rpt_ie->len = len + sizeof(struct ieee80211_ess_rpt_ie) - 2;
...
u_int8_t ieee80211_setup_oce_attrs (int f_type, struct ieee80211vap *vap, u_int8_t *frm, struct ieee80211_node *ni)
{
	...
	if ((f_type == IEEE80211_FC0_SUBTYPE_ASSOC_RESP) ||
		...
		if (vap->iv_tx_power_enable == 1) {
			tx_power = (struct ieee80211_oce_transmit_power *) frm;

			tx_power->header.ie = OCE_ATTRIB_TRANSMIT_POWER;
			tx_power->header.len = sizeof (struct ieee80211_oce_transmit_power) - 2;
			tx_power->tx_pow_value =  (int8_t)(vap->iv_bss->ni_txpower/2);  /* Current TX power of the chan */
			if (!vap->iv_tx_power_value) {
				tx_power->tx_pow_value =  (int8_t)(vap->iv_bss->ni_txpower/2);  /* Current TX power of the chan */
			}  else if (vap->iv_tx_power_value == SET_MIN_TXPOWER) {
				tx_power->tx_pow_value =  min_tx_power;  /* Minimum supported tx power */
			} else if (vap->iv_tx_power_value == SET_MAX_TXPOWER) {
				tx_power->tx_pow_value =  max_tx_power;  /* Maximum supported tx power */
			}
			frm += sizeof (struct ieee80211_oce_transmit_power);
		} else if (vap->iv_tx_power_enable == 2) {
			tx_power = (struct ieee80211_oce_transmit_power *) frm;

			tx_power->header.ie = OCE_ATTRIB_TRANSMIT_POWER;
			tx_power->header.len = sizeof (struct ieee80211_oce_transmit_power) - 2;
			tx_power->tx_pow_value =  vap->iv_tx_power_value;  /* User configured TX power for vap */
			frm += sizeof (struct ieee80211_oce_transmit_power);
		}
		if (vap->iv_ip_subnet_id) {
			ip_subnet_id = (struct ieee80211_oce_ip_subnet_id *) frm;

			ip_subnet_id->header.ie = OCE_ATTRIB_IP_SUBNET_IND;
			ip_subnet_id->header.len = sizeof (struct ieee80211_oce_ip_subnet_id) - 4;
			ip_subnet_id->ip_subnet_id_value = vap->iv_ip_subnet_id;  // user defined value IP Subnet value
			frm += sizeof (struct ieee80211_oce_ip_subnet_id) - 2;
		}
	}
...
ieee80211_update_11b_ap_presence (struct ieee80211vap *vap)
{
    ...
    TAILQ_FOREACH_SAFE(nbr_vap, &ic->ic_vaps, iv_next, tvap) {
        ...
        if (ieee80211_is_phymode_11b(nbr_vap->iv_des_mode)) {
...
ieee80211_fill_ap_chan_rpt_entry(void *iarg, wlan_scan_entry_t se)
{
    ...
    char country_iso[4];
    struct ieee80211_ath_channel *chan;
    u_int8_t op_class, vap_op_class;
    struct wlan_ssid *se_ssid = NULL;
    u_int8_t filter_ssid[WLAN_SSID_MAX_LEN + 1] = {0};
    u_int8_t ssid_len = 0;
    bool ssid_matched = FALSE;
    bool opclass_matched = FALSE;

    if (ap_chan_info_cnt >= ap_chan_info_cnt_max)
        return -ENOMEM;

    if (vap->ap_chan_rpt_ssid_filter) {
        wlan_vdev_mlme_get_ssid(vap->vdev_obj, filter_ssid, &ssid_len);
        se_ssid = util_scan_entry_ssid(se);
        if (se_ssid && se_ssid->ssid && (se_ssid->length == ssid_len) &&
            !qdf_mem_cmp(se_ssid->ssid, filter_ssid, ssid_len)) {
            ssid_matched = TRUE;
        }
    } else {
            ssid_matched = TRUE;
    ...
    if (vap->ap_chan_rpt_opclass_filter) {
        ieee80211_getCurrentCountryISO(vap->iv_ic, country_iso);
        vap_op_class = regdmn_get_opclass(country_iso, vap->iv_ic->ic_curchan);
    }

    if (ssid_matched == TRUE) {
        country_ie = (struct ieee80211_country_ie *) util_scan_entry_country(se);
        chan = wlan_util_scan_entry_channel(se);
        if (country_ie) {
        ...
        if (vap->ap_chan_rpt_opclass_filter) {
            if (vap_op_class == op_class)
                opclass_matched = TRUE;
        } else {
            opclass_matched = TRUE;
        }

        if (opclass_matched == TRUE) {
