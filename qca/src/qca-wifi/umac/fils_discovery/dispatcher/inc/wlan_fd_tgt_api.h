...
/**
 * tgt_fd_offload() - API to offload FILS Discovery frame
 * @vdev: Pointer to VDEV object
 * @vdev_id: VDEV ID
 *
 * This API is invoked from ol_if layer to offload the FD frame
 * by invoking FD Rx ops in UMAC layer during VAP up.
 *
 * Return: QDF_STATUS_SUCCES is success or QDF_STATUS_E_INVAL for error
 */
QDF_STATUS tgt_fd_offload(struct wlan_objmgr_vdev *vdev, uint32_t vdev_id);

/**
 * tgt_fd_tmpl_update() - API to update the FD template
 * @vdev: Pointer to VDEV object
 *
 * This API is invoked from the ol_if layer to  update hte FD template
 * by invoking FD Rx ops in UMAC layer during any FD parameter update.
 *
 * Return: QDF_STATUS_SUCCES is success or QDF_STATUS_E_INVAL for error
 */
