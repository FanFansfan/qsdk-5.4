...
{
    if (ni->ni_inact_snr_xing) {
        *inact_rssi_low_threshold  = ni->ni_inact_snr_xing;
        *inact_rssi_high_threshold = ni->ni_inact_snr_xing;
    }

    if (ni->ni_low_snr_xing)
        *low_rssi_threshold = ni->ni_low_snr_xing;

    if (ni->ni_low_rate_snr_xing)
        *low_rate_rssi_threshold = ni->ni_low_rate_snr_xing;

    if (ni->ni_high_rate_snr_xing)
        *high_rate_rssi_threshold = ni->ni_high_rate_snr_xing;
}
                                    ...
                                    const u_int8_t *mac_addr)
{
    ...
    {
        u_int8_t params[] = {IEEE80211_ACL_PARAM_INACT_SNR_XING,
                             IEEE80211_ACL_PARAM_LOW_SNR_XING,
                             IEEE80211_ACL_PARAM_LOW_RATE_SNR_XING,
                             IEEE80211_ACL_PARAM_HIGH_RATE_SNR_XING};

        u_int8_t *values[] = {&ni->ni_inact_snr_xing,
                              &ni->ni_low_snr_xing,
                              &ni->ni_low_rate_snr_xing,
                              &ni->ni_high_rate_snr_xing};
...
static int
get_cli_params_snr_xing_values(struct ieee80211vap *vap,
                                struct ieee80211req_athdbg *req)
{
    ...
    u_int8_t *values[] = {&req->data.acl_cli_param.inact_snr_xing,
                          &req->data.acl_cli_param.low_snr_xing,
                          &req->data.acl_cli_param.low_rate_snr_xing,
                          &req->data.acl_cli_param.high_rate_snr_xing};
                              ...
                              struct ieee80211req_athdbg *req)
{
    ...
    ret = get_cli_params_snr_xing_values(vap, req);
    if (ret < 0)
        return ret;
...
static int
set_cli_params_snr_xing_values(struct ieee80211vap *vap,
                              ...
                              const struct ieee80211req_athdbg *req)
{
    ...
    ret = set_cli_params_snr_xing_values(vap, req);
    if (ret < 0)
        return ret;
     ...
     */
    wlan_acl_apply_node_snr_thresholds(vap, req->dstmac);
    return EOK;
}
