...
void ieee80211_cbs_post_event (ieee80211_cbs_t cbs, enum ieee80211_cbs_event event)
{
    ...
    status = wlan_objmgr_vdev_try_get_ref(cbs->cbs_vap->vdev_obj, WLAN_OSIF_SCAN_ID);

    if (QDF_IS_STATUS_ERROR(status)) {
        qdf_info("unable to get reference");
        return;
    }
    cbs->cbs_event = event;
...
void cbs_work(void *arg)
{
    ...
    /* Release the vdev ref which was taken in ieee80211_cbs_post_event() */
    wlan_objmgr_vdev_release_ref(cbs->cbs_vap->vdev_obj, WLAN_OSIF_SCAN_ID);
}
...
static void ieee80211_cbs_scan_evhandler(struct wlan_objmgr_vdev *vdev, struct scan_event *event, void *arg)
{
    ...
    if (originator == NULL) {
        qdf_info("Vdev pointer is invalid");
        return;
    }
...
int ieee80211_cbs_event(ieee80211_cbs_t cbs, enum ieee80211_cbs_event event)
{
    ...
    case IEEE80211_CBS_SCAN_START:
        ...
        case IEEE80211_CBS_WAIT: /* notice fallthrough */
            ...
            /* channel to scan */
            for (; cbs->chan_list_idx < cbs->nchans; cbs->chan_list_idx++) {
                chan = ieee80211_find_dot11_channel(ic, cbs->chan_list[cbs->chan_list_idx], 0, IEEE80211_MODE_AUTO);
                if (chan && !IEEE80211_IS_CHAN_RADAR(ic, chan))
                    break;
            }
	    ...
	    if ((status = cbs_scan_start(vap, cbs) != EOK)) {
                type = ALD_CBS_CANCELLED;
                son_update_mlme_event(vap->vdev_obj, NULL, SON_EVENT_ALD_CBS, &type);
                ieee80211_cbs_cancel(cbs);
                rc = -EINVAL;
                goto release_ref;
            }
...
{
    QDF_STATUS status = qdf_cancel_work(&(*cbs)->cbs_work);
    /* Check if an event was in progress during cancelling work, release the vdev ref if true. */
    if (status && (*cbs)->cbs_event != 0) {
        wlan_objmgr_vdev_release_ref((*cbs)->cbs_vap->vdev_obj, WLAN_OSIF_SCAN_ID);
        (*cbs)->cbs_event = 0;
    }
...
int ieee80211_cbs_detach(ieee80211_cbs_t *cbs)
{
    ...
    status = qdf_cancel_work(&(*cbs)->cbs_work);
    /* Check if an event was in progress during cancelling work, release the vdev ref if true. */
    if (status && (*cbs)->cbs_event != 0) {
        wlan_objmgr_vdev_release_ref((*cbs)->cbs_vap->vdev_obj, WLAN_OSIF_SCAN_ID);
        (*cbs)->cbs_event = 0;
...
int ieee80211_cbs_set_param(ieee80211_cbs_t cbs, int param , int val)
{
    ...
    switch (param) {
        case IEEE80211_CBS_ENABLE:
            if (val < 0 || val > 1) {
                qdf_print("Enable value should be either 0 or 1");
                ret = -EINVAL;
                break;
            }
...
int ieee80211_cbs_api_change_home_channel(ieee80211_cbs_t cbs,
                                          int dcs_wideband_policy)
{
    struct ieee80211com *ic = cbs->cbs_ic;
    struct ieee80211vap *vap;
    int chan_freq, first_valid_chan_freq = 0, found = 0, i = 1;
    ...
    if (ic->cw_inter_found &&
        (check_inter_band_switch_compatibility(ic) == QDF_STATUS_SUCCESS)) {
    ...
    do{
        ...
        channel = ieee80211_find_dot11_channel(ic, chan_freq, 0, vap->iv_des_mode);

        /* If DCS is enabled, ensure that channel is selected as per the
         * wideband policy */
        if (channel && ic->cw_inter_found &&
            IEEE80211_ARE_CHANS_INTERWIDEBAND(vap->iv_ic->ic_curchan, channel) &&
            (!is_wideband_csa_allowed ||
             (dcs_wideband_policy != DCS_WIDEBAND_POLICY_INTERBAND))) {
            channel = NULL;
            chan_freq = 0;
            continue;
        }

        if (channel && !first_valid_chan_freq) {
            first_valid_chan_freq = chan_freq;
        } else if (!chan_freq || !channel) {
            break;
        }
    ...
    if (!chan_freq || !channel) {
        if (first_valid_chan_freq) {
            chan_freq = first_valid_chan_freq;
        } else {
            /* If no valid channel, return */
            cbs_trace("No channel found");
            return -EINVAL;
        }
    }
