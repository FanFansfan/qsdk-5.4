...
MODULE_DESCRIPTION("Qualcomm Tehnologies packet-logging infrastructure Module");
...
#define VM_FAULT_MINOR  0 /* this macro was removed in 5.4 kernel, so added here*/
#endif
/*
...
#	define set_ctl_name(a, b)	pl_info_lnx->sysctls[a].ctl_name = b
    ...
    set_ctl_name(16, CTL_AUTO);
    pl_info_lnx->sysctls[16].procname = "ssr_hit";
    pl_info_lnx->sysctls[16].mode = PKTLOG_PROCSYS_PERM;
    pl_info_lnx->sysctls[16].proc_handler = proc_dointvec;
    pl_info_lnx->sysctls[16].data = &pl_info_lnx->info.ssr_hit;
    pl_info_lnx->sysctls[16].maxlen = sizeof(pl_info_lnx->info.ssr_hit);
...
pktlogmod_init(void)
{
    ...
    for (i = 0; i < MAX_WLANDEV; i++) {
        ...
        if ((dev = (struct net_device *)
                   qdf_net_if_get_dev_by_name(dev_name)) != NULL) {
#else
...
pktlog_attach(ath_generic_softc_handle scn)
{
    ...
    if (pktlog_sysctl_register(scn)) {
        qdf_err(PKTLOG_TAG "[%s]: sysctl register failed \n", proc_name);
        goto attach_fail2;
    }
    return 0;
...
pktlog_detach(ath_generic_softc_handle scn)
{
    ...
    if (pl_info) {
        if (pl_dev) {
            pl_info_lnx = PL_INFO_LNX(pl_dev->pl_info);
            qdf_info(PKTLOG_TAG "[ %s ] remove entry %pK root %pK",
                     pl_dev->name, pl_info_lnx->proc_entry, g_pktlog_pde);
            remove_proc_entry(pl_dev->name, g_pktlog_pde);
        } else {
            remove_proc_entry(PKTLOG_PROC_SYSTEM, g_pktlog_pde);
        }

        /* The unregister API is ok to accept NULL handle */
...
int
pktlog_tcpip(struct ath_softc *sc, wbuf_t wbuf, struct llc *llc, u_int32_t *proto_log, int *proto_len, bool *isSack)
{
    struct iphdr *ip;
    struct tcphdr *tcp;
    u_int32_t *proto_hdr;
