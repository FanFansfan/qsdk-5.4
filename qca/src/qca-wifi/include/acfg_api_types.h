/*
 * Copyright (c) 2019,2021 Qualcomm Innovation Center, Inc.
 * All Rights Reserved.
...
enum {
    ...
    ACFG_MAX_DPP_CONNECTOR_LEN = 1024,
};
...
enum acfg_param_vap {
    ...
    ACFG_PARAM_6G_SECURITY_COMP     = IEEE80211_PARAM_6G_SECURITY_COMP,
    ACFG_PARAM_6G_KEYMGMT_MASK      = IEEE80211_PARAM_6G_KEYMGMT_MASK,
}; /* Use IEEE80211_PARAM_* directly in acfg application instead of defining new params */
...
enum acfg_param_radio {
     ...
     * */
    ACFG_PARAM_RADIO_ACS_SNRVAR = 120 | ATH_PARAM_SHIFT,
    /*
...
}; /* Use OL_ATH_PARAM_* directly in acfg application instead of defining new params */
...
enum acfg_req_cmd {
    ...
    ACFG_REQ_ADD_GENERIC_BUF,   /* Send general buffer to driver */
...
enum acfg_ev_id {
    ...
    ACFG_EV_HW_MODE_CHANGE_STATUS           = 131,  /** Dynamic HW mode change
                                                        status */
    ACFG_EV_DPP_EVENT_CONF_RECEIVED         = 132,  /* DPP Conf Recvd */
    ACFG_EV_DPP_EVENT_CONFOBJ_AKM           = 133,  /* DPP Conf Recvd AKM */
    ACFG_EV_DPP_EVENT_CONFOBJ_SSID          = 134,  /* DPP Conf Recvd SSID */
    ACFG_EV_DPP_EVENT_CONFOBJ_PASS          = 135,  /* DPP Conf Recvd Pass */
    ACFG_EV_DPP_EVENT_CONFOBJ_CONNECTOR     = 136,  /* DPP Conf Recvd Connector */
    ACFG_EV_DPP_EVENT_CONFOBJ_CSIGN         = 137,  /* DPP Conf Recvd Csign */
    ACFG_EV_DPP_EVENT_CONFOBJ_NETACCESSKEY  = 138,  /* DPP Conf Recvd Netaccesskey */
...
enum {
    ACFG_SET_HE_BSSCOLOR     = 1,
    ACFG_ENABLE_MESH_MODE    = 2,
};
...
enum acfg_buf_type {
    ACFG_USER_RNR_ENTRY_ADD = 1,
    ACFG_USER_RNR_ENTRY_DEL = 2,
    ACFG_USER_RNR_ENTRY_DUMP = 3,
};

#define GENERIC_BUF_MAX_SIZE  (1024*2)
typedef struct acfg_generic_buf {
    enum acfg_buf_type buf_type;
    uint8_t        buf[GENERIC_BUF_MAX_SIZE];
    uint32_t       buf_len;
} acfg_generic_buf_t;

#define ACFG_USER_RNR_MAX_NUM_TBTT_INFO (16)
#define ACFG_USER_RNR_MAX_UID (16)
/*
 * 510 - 212(taken by 16 co-located 6Ghz APs - 13 bytes for each vap)
 * - 4 (for header if buffer gets divided over 2 IEs) = 294
 */
#define ACFG_USER_RNR_DATA_MAX_SIZE (294)


typedef struct acfg_rnr_bss_params {
    uint8_t oct_recommended:1;
    uint8_t same_ssid:1;
    uint8_t mbssid_set:1;
    uint8_t tx_bssid:1;
    uint8_t colocated_lower_band_ess:1;
    uint8_t probe_resp_20tu_active:1;
    uint8_t co_located_ap:1;
} __packed acfg_rnr_bss_params_t;

typedef struct acfg_rnr_tbtt_info_set {
    uint8_t  tbtt_offset;
    uint8_t  bssid[IEEE80211_ADDR_LEN];
    uint32_t short_ssid;
    struct  acfg_rnr_bss_params bss_params;
    uint8_t  psd_20mhz;
} __packed acfg_rnr_tbtt_info_set_t;

typedef struct acfg_rnr_nbr_ap_info {
    uint8_t hdr_info_cnt;
    uint8_t hdr_info_len;
    uint8_t op_class;
    uint8_t channel;
    struct acfg_rnr_tbtt_info_set tbtt_info[ACFG_USER_RNR_MAX_NUM_TBTT_INFO];
} __packed acfg_rnr_nbr_ap_info_t;

typedef struct acfg_user_rnr_data {
    uint8_t uid;
    uint8_t num_ap;
    uint8_t user_buf_len;
    uint8_t user_buf[ACFG_USER_RNR_DATA_MAX_SIZE];
} acfg_user_rnr_data_t;
