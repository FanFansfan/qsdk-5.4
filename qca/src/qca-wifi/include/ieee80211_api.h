...
* @param cp            : cloneparams of the VAP
* @returns EOK if successful .
...
*/
int wlan_vap_free_mac_addr(wlan_dev_t devhandle,
                           u_int8_t *bssid, struct ieee80211_clone_params *cp);
...
bool ic_is_sta_vap(struct ieee80211com *ic);
 ...
 */
int wlan_get_channel_list(wlan_dev_t devhandle, uint32_t band_bitmap,
                          qdf_freq_t chanlist[], u_int32_t nchan);
 ...
 * @param  devhandle    : handle to the radio.
 * @param  regdmn       : Regdomain code: 2.4_5Ghz pair (LSB 16 bits) +
 * 6Ghz super domain (MSB 16 bits).
 */
int wlan_set_regdomain(wlan_dev_t devhandle, uint32_t regdmn);
 ...
 * get snr info
 * @param      vaphandle   : vap handle
 * @param      snr_info    : handle for the return value .
 * @param      snrtype     : type of snr.
 *
 * @return : 0  on success and -ve on failure.
 *           only valid for STA vaps;
 *
 */
int wlan_getsnr(wlan_if_t vaphandle, wlan_snr_info *snr_info, wlan_snr_type snrtype);
 ...
 *
 */
int wlan_node_getsnr(wlan_node_t node, wlan_snr_info *snr_info, wlan_snr_type snrtype);
...
struct ieee80211_node_iter_arg {
    uint32_t count;
    wlan_dev_t ic;
    uint32_t flag;
    struct ieee80211_node *nodes[WLAN_UMAC_PSOC_MAX_PEERS];
};

/**
 * Function to iterate over nodes present at the ic level
 * @param ic            : pointer to the radio device
 * @param iter_func     : function to be called for each node
 * @param arg           : arg passed back to the iteration function
 * @param flag          : flag to specify the type of nodes for AP VAP
 *                        (Associated / Unassociated / Both)
 * @return              : none
 */
...
/**
 * get tx and rx streams capability of a node
 * @param node           : node handle.
 * @return tx and rx NSS.
 */
 ...
 * @param duration          : Duration field of frame
...
/**
* Validate the user input for Max Tx Power
...
/**
 * Add TPE IE for local config
 * @param vaphandle         : handle to a vap
 * @param txpwr_cnt         : Tx Pwr Count - TPE info
 * @param txpwr_intrpt      : Tx Pwr Interpretation - TPE info
 * @param txpwr_cat         : Tx Pwr Category - TPE info
 * @param count             : Number of Tx Pwr values
 * @param txpwr_val         : Pointer to user input Tx Pwr values
 * @return 0 on success and -ve on failure.
 */
...
/**
 * Delete TPE IE for local config
 * @param vaphandle         : handle to a vap
 * @param txpwr_intrpt      : Tx Pwr Interpretation - TPE info
 * @param txpwr_cat         : Tx Pwr Category - TPE info
 * @return 0 on success and -ve on failure.
 ...
 *  @return 0  on success and -ve on failure.
 */
int ieee80211_del_key(wlan_if_t vaphandle,int32_t authmode, u_int16_t keyix, u_int8_t *macaddr);
 ...
 * @is_160chan_info     : if true, 160 channel info will be obtained;
 ...
 * @nchan : number of channels
 */
int wlan_radio_get_chan_info(struct ieee80211com *ic, bool is_160chan_info,
 ...
 * @param maximum_age          : maximum age a scan entry may have to be included in the list.
 * @param bssid_hint           : Bssid hint mac address
 *
 * Return                      : candidate list with filtering, scoring and sorting done.
 */
qdf_list_t *wlan_candidate_list_build(wlan_if_t vaphandle,
                                      bool strict_filtering,
...
int wlan_send_bstmreq_target(wlan_if_t vap, u_int8_t *macaddr,struct ieee80211_bstm_reqinfo_target *bstmreq);
 ...
 /**
  * send MSCS Response to the STA
  * supporting MSCS procedures with the corresponding status code
  * @param vap       :  handle to the vap
  * @param macaddr   : macaddress of the station
  * @param mscsresp  : pointer to mscs response
  *                    struct
  */
int wlan_send_mscs_resp(wlan_if_t vap, u_int8_t *macaddr,struct ieee80211_mscs_resp *mscsresp);
...
/**
 * get number of beaconing vaps for which iv_active is set
 * @param comhandle    : handle to the ic.
 * @return  number of beaconing vaps.
 */
u_int32_t  ieee80211_get_num_beaconing_vaps(wlan_dev_t comhandle);

/**
 * Check non-beaconing AP vap or not
 * @param vaphandle    : handle to the vap.
 * @return  true if non beaconing vap.
 */
bool wlan_is_non_beaconing_ap_vap(wlan_if_t vap);
...
int wlan_peer_isolation_add_mac(wlan_if_t vaphandle, struct ieee80211_wlanconfig_isolation *peer);

int wlan_peer_isolation_del_mac(wlan_if_t vaphandle, struct ieee80211_wlanconfig_isolation *peer);

int wlan_peer_isolation_list(wlan_if_t vaphandle, struct ieee80211_wlanconfig_isolation_list *out);

int wlan_peer_isolation_flush(wlan_if_t vaphandle);
#endif
 ...
 *
 * @param  ic           : Handle to the radio of ieee80211com type.
 * @param  scan_entry   : Scan entry structure
 * @return              : Return true if Radar found in the current channel.
 */
bool wlan_mlme_is_chan_radar(struct ieee80211com *ic, struct scan_cache_entry *scan_entry);
...
/*
 * set rnr ap info bss param sub field
 * @ic: Ieee80211 commom object for pdev
 * @type: rnr ap info bss param sub field
 * @value: value to be set
 */
int ieee80211_set_rnr_bss_param(struct ieee80211com *ic,
        enum ieee80211_rnr_bss_param type, int value);

/* ieee80211_user_rnr_frm_update: Allow user to select
 * frame that advertizes user rnr data
 * @ic: ieee80211 commom object for pdev
 * @value: Frame selection value
 * @is_buf_update: Flag to indicate user buffer update
 */
void ieee80211_user_rnr_frm_update(struct ieee80211com *ic,
                                  int value, bool is_buf_update);

/*
 * wlan_send_vdev_tsf_tstamp_action_wmi_cmd() - Send TSF Action Command
 * @param ic: ieee80211 common object
 * @param vap_id: ID of VDEV for which to get TSF
 *
 * Return: QDF_STATUS_SUCCESS if event sent via WMI layer
 */
QDF_STATUS wlan_send_vdev_tsf_tstamp_action_wmi_cmd(struct ieee80211com *ic,
                                                    uint32_t vap_id);

/*
 * ieee80211_is_phymode_##field() - Checks for the specific phymode
 * @param phymode: phymode to be checked with ##field
 *
 * Return: true if phymode matches else false.
 */
/* ieee80211_is_phymode_##mode(uint32_t phymode) - function declaration */
#define IEEE80211_IS_PHYMODE_PROTO(mode) bool ieee80211_is_phymode_##mode(uint32_t phymode);
/* ieee80211_is_phymode_##mode(uint32_t phymode) - function definition */
#define IEEE80211_IS_PHYMODE_DEF(mode, field) bool ieee80211_is_phymode_##mode(uint32_t phymode) \
{ \
    if (phymode == IEEE80211_MODE_##field) \
        return true; \
    return false; \
} \
qdf_export_symbol(ieee80211_is_phymode_##mode);

IEEE80211_IS_PHYMODE_PROTO(auto);
IEEE80211_IS_PHYMODE_PROTO(11a);
IEEE80211_IS_PHYMODE_PROTO(turbo_a);
IEEE80211_IS_PHYMODE_PROTO(11b);
IEEE80211_IS_PHYMODE_PROTO(11g);
IEEE80211_IS_PHYMODE_PROTO(11na_ht20);
IEEE80211_IS_PHYMODE_PROTO(11na_ht40);
IEEE80211_IS_PHYMODE_PROTO(11ng_ht20);
IEEE80211_IS_PHYMODE_PROTO(11ng_ht40);
IEEE80211_IS_PHYMODE_PROTO(11axa_he40);
IEEE80211_IS_PHYMODE_PROTO(11ac_vht40);
IEEE80211_IS_PHYMODE_PROTO(11axg_he40);
IEEE80211_IS_PHYMODE_PROTO(11ac_vht40minus);
IEEE80211_IS_PHYMODE_PROTO(11ac_vht40plus);
IEEE80211_IS_PHYMODE_PROTO(11axa_he40plus);
IEEE80211_IS_PHYMODE_PROTO(11axa_he40minus);
IEEE80211_IS_PHYMODE_PROTO(11ac_vht80);
IEEE80211_IS_PHYMODE_PROTO(11ac_vht160);
IEEE80211_IS_PHYMODE_PROTO(11ac_vht80_80);
IEEE80211_IS_PHYMODE_PROTO(11axa_he80);
IEEE80211_IS_PHYMODE_PROTO(11axa_he160);
IEEE80211_IS_PHYMODE_PROTO(11axa_he80_80);

/**
 * ieee80211_is_phymode_g40() - Checks for the 2g specific 40Mhz phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_40() - Checks for the specific 40Mhz phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_40plus() - Checks for the specific 40Mhz with
 * SCN above phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
...
/**
 * ieee80211_is_phymode_40minus() - Checks for the specific 40Mhz with
 * SCN below phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
...
/**
 * ieee80211_is_phymode_not_basic() - Checks for the all phymodes
 * except basic modes like turbo,a,b,g,n,turbo_a,turbo_g and auto as well
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_11ac_160or8080() - Checks for the 11ac specific
 * 160MHz or 80+80MHz phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_11axa_160or8080() - Checks for the 11axa specific
 * 160MHz or 80+80MHz phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
...
/**
 * ieee80211_is_phymode_160() - Checks for the 160MHz specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_80() - Checks for the 80MHz specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_8080() - Checks for the 80+80MHz specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_160_or_8080() - Checks for the specific
 * 160MHz or 80+80MHz phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
...
/**
 * ieee80211_is_phymode_11ax() - Checks for the 11axa specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_11ac() - Checks for the 11ac specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_11na() - Checks for the 11na specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_11ng() - Checks for the 11ng specific modes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_valid() - Checks for the mode greater
 * than IEEE80211_MODE_AUTO and less than IEEE80211_MODE_MAX phymode
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_allowed() - Checks for the mode is
 * IEEE80211_MODE_AUTO or is valid phymode.
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_equal_or_above_11axa_he40plus() - Checks for the mode equal or greater
 * than IEEE80211_MODE_11AXA_HE40PLUS phymode
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/**
 * ieee80211_is_phymode_2g() - Checks for the 2g specific phymodes
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
 */
...
/*
 * ieee80211_is_phymode_5g_or_6g() - Checks for the phymodes which are applicable
 * to both 5g and 6g
 * @mode: phymode to be checked
 *
 * Return: true if phymode matches else false.
...
/**
 * @brief: Gets number of bits set in chainmask
 * @mask:  Chainmask value
 *
...
/*
 * mlme_get_active_req_type: Api to return active mlme req
 * @vap: vap obect
 *
 * Return: Active mlme req
 */
enum mlme_req mlme_get_active_req_type(struct ieee80211vap *vap);

#if WLAN_SUPPORT_GREEN_AP
/*
 * ieee80211_get_apvaps_count: Api to return APVAPs count
 * @ic: radio obect
 *
 * Return: APVAPs count
 */
int ieee80211_get_apvaps_count(struct ieee80211com *ic);
#endif

/*
 * wlan_mesh_set_peer_latency_param_config() - Set peer wmi latency config param
 * @param vaphandle: Vap interface handle
 * @param req: pointer to the structure ieee80211req_athdbg
 *
 * Return: QDF_STATUS_SUCCESS if event sent via WMI layer
 */
QDF_STATUS wlan_mesh_set_peer_latency_param_config(wlan_if_t vaphandle, struct ieee80211req_athdbg *req);

/*
 * wlan_cm_update_advance_filter() - Custom scan filter options
 * @vdev: Vdev object
 * @filter: Scan filter to be updated
 *
 * Return: void
 */
...
/*
 * wlan_cm_candidate_list_custom_sort() - Custom sort API
 * @vdev: vdev object
 * @candidate_list: Scored candidate list from connection manager
 *
 * Return: void
 */
