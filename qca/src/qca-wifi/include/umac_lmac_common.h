 ...
 */
#define MBSSID_IS_TX_VAP_SET(ic) ((!ieee80211_mbssid_get_num_vaps_in_mbss_cache((ic))) ? true : false)
#define MBSSID_POOL_SIZE(ic) (1 << (ic)->ic_mbss.max_bssid)
#define MBSSID_GROUP_SIZE MBSSID_POOL_SIZE
#define MBSSID_GET_BSSIDX(ic) \
    (qdf_ffz((ic)->ic_mbss.bssid_index_bmap[IEEE80211_DEFAULT_MBSS_SET_IDX]) + 1)

#define ATH_BSSID_INDEX 5

/* Assign mac to ap vap in WB radio */
#define MBSSID_ATH_SET_VAP_BSSID_WB(ic, bssid, hwbssid)                        \
    do {                                                                       \
        if (ieee80211_get_num_beacon_ap_vaps(ic)) {                            \
            uint8_t temp =  bssid[ATH_BSSID_INDEX] &                           \
                                (MBSSID_POOL_SIZE(ic) - 1);                    \
            bssid[ATH_BSSID_INDEX] &= ~(MBSSID_POOL_SIZE(ic) - 1);             \
            bssid[ATH_BSSID_INDEX] |=                                          \
            (temp + MBSSID_GET_BSSIDX(ic)) % MBSSID_POOL_SIZE(ic);             \
            /* Mark the bssid_idx as ocupied in the bssid_idx pool */          \
            current_bssid_ix = MBSSID_GET_BSSIDX(ic);                          \
            qdf_set_bit(MBSSID_GET_BSSIDX(ic) - 1,                             \
               (unsigned long *)                                               \
               &ic->ic_mbss.bssid_index_bmap[IEEE80211_DEFAULT_MBSS_SET_IDX]); \
        } else {                                                               \
            /* ref_bssid is bssid of tx-vap */                                 \
        }                                                                      \
    } while(0)
...
#define ATH_SET_VAP_BSSID(bssid, hwbssid, id)                                  \
    do {                                                                       \
       if (!is_mbssid_enabled && ic->ic_wideband_capable &&                    \
           (cp->icp_opmode == QDF_SAP_MODE)) {                                 \
           MBSSID_ATH_SET_VAP_BSSID_WB(ic, bssid, hwbssid);                    \
       } else {                                                                \
           if (!is_mbssid_enabled && ic->ic_wideband_capable &&                \
               (cp->icp_opmode != QDF_SAP_MODE)) {                             \
               bssid[ATH_BSSID_INDEX] = hwbssid[ATH_BSSID_INDEX];              \
