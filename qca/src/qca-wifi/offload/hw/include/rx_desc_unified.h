/*
 * Copyright (c) 2015,2017,2020-2021 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved.
...
#define FN_RX_DESC_MPDU_WLAN_PREAMBLE(target) \
static u_int32_t _##target##_rx_desc_ppdu_wlan_preamble(ar_handle_t arh, \
        void *rx_desc) \
{ \
    struct rx_desc_base *desc = \
        (struct rx_desc_base *) rx_desc; \
    int preamble =\
            ((*(((u_int32_t *) &desc->ppdu_start) + 5)) &\
...
{ \
    struct rx_desc_base *cbf_rx_desc; \
        cbf_rx_desc = ar_rx_desc(head_msdu); \
...
#define RX_DEFAULT_NOISEFLOOR (-96)
        ...
        struct ieee80211_rx_status *rs)
{
    ...
    /* SNR */
    rs->rs_snr = ((*((u_int32_t *)ppdu_start+4) &
                  RX_PPDU_START_4_RSSI_COMB_MASK) >> RX_PPDU_START_4_RSSI_COMB_LSB);

    /* Calculate RSSI in dBm by adding default noisefloor */
    rs->rs_rssi = rs->rs_snr + RX_DEFAULT_NOISEFLOOR;
                                                  ...
                                                  uint32_t permsdu_flag)
{
    ...
    if (!permsdu_flag || (is_head)) {
        ...
        if (first_mpdu) {
            ...
            rx_status->rs_snr = rx_info.rs_snr;
            ...
            if(rx_status->rs_snr > 127){
                rx_status->rs_snr = 127;
            }
        }
