 ...
 */
#define HTT_RX_IND_MIN_LEN 4 * ( 2 + HTT_RX_PPDU_DESC_SIZE32 )
...
/**
 * @brief Indicate PPDU current preamble value
 * @details
 *  Indicate preamble from the PPDU that the specified MSDU belonged to.
 *
 * @param pdev - the HTT instance the rx data was received on
 * @param msdu_desc - the abstract descriptor for the MSDU in question
 * @return
 *      return preamble value from ppdu_desc
 */
static inline int htt_rx_ppdu_wlan_preamble(
        htt_pdev_handle pdev, void *msdu_desc)
{
    return pdev->htt_rx_ppdu_wlan_preamble(pdev->arh,
            msdu_desc);
}
...
/**
 * @brief Clear discard flag so that the MSDU is pushed to OS layer.
 * @details
 ...
 */
void
htt_rx_msdu_discard_clear(htt_pdev_handle pdev, void *msdu_desc);
