...
#ifdef DEBUG_HOST_RC
    qdf_nofl_info("%s : peer_nss: %d ni_streams: %d\n", __func__, peer_nss,
...
ratectrl_update_rate_sched(struct rate_node *an, RC_TX_RATE_SCHEDULE *rate_sched, void *pkt_info_rcf)
{
    ...
    if(wmi_node_update_rate_sched(((struct ol_ath_softc_net80211 *)pdev->scnctx)->sc_pdev,
            peer->mac_addr.raw, vdev->vdev_id, rate_sched)) {
        qdf_nofl_info("%s: Unable to send the updated rate-schedule peer\n", __func__);
    }
#endif
