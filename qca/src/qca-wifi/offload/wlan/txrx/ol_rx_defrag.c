    ...
    int seq_num)
{
    ...
    int seq;
    struct ol_rx_reorder_t *rx_tid = &peer->tids_rx_reorder[tid];
    ...
    unsigned tid)
{
    ...
    qdf_spin_lock_bh(&pdev->rx.defrag.defrag_lock);
    TAILQ_INSERT_TAIL(&pdev->rx.defrag.waitlist, rx_reorder,
            defrag_waitlist_elem);
    qdf_spin_unlock_bh(&pdev->rx.defrag.defrag_lock);
}
    ...
    struct ol_txrx_pdev_t *pdev)
{
    ...
    TAILQ_FOREACH_SAFE(rx_reorder, &pdev->rx.defrag.waitlist,
        ...
        if(tid >= OL_TXRX_NUM_EXT_TIDS) {
            TXRX_PRINT(TXRX_PRINT_LEVEL_ERR, "%s: TID out of bounds: %d",
                       __func__, tid);
            qdf_assert(0);
            continue;
        }
    ...
    qdf_nbuf_t frag_list)
{
    ...
    while (cur) {
        ...
        if (!ol_rx_pn_check_base(vdev, peer, tid, cur, 1)) {
    ...
    if (index != txrx_sec_ucast) {
       ol_rx_frames_free(htt_pdev, frag_list);
       TXRX_PRINT(TXRX_PRINT_LEVEL_ERR, "ol_rx_defrag: MCAST/BCAST frags to be dropped\n");
       return;
    }
