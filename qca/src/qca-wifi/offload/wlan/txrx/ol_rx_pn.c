    ...
    bool is_frag)
{
    ...
    while (mpdu) {
        ...
        mac_hdr = (struct ieee80211_frame *) qdf_nbuf_data(mpdu);
        fragno = qdf_le16_to_cpu(*(u_int16_t *) mac_hdr->i_seq) & IEEE80211_SEQ_FRAG_MASK;

        sec_type = peer->security[index].sec_type;
        if (is_frag && fragno) {
            if (!(sec_type == htt_sec_type_none || sec_type == htt_sec_type_wep128
                 || sec_type == htt_sec_type_wep104 || sec_type == htt_sec_type_wep40)) {
                 if (new_pn.pn128[1] == last_pn->pn128[1]) {
                        frag_out_of_order = (new_pn.pn128[0] - last_pn->pn128[0] != 1);
                 }
                 else {
                        frag_out_of_order = (new_pn.pn128[1] - last_pn->pn128[1] != 1);
                 }
            }
            if (!frag_out_of_order) {
                update_last_pn = 1;
            }
            goto pn_check_process_done;
        }
...
pn_check_process_done:
        if (pn_is_replay || frag_out_of_order) {
            qdf_nbuf_t msdu;
            /*
...
ol_rx_pn_check(
    ...
    qdf_nbuf_t msdu_list)
{
    msdu_list = ol_rx_pn_check_base(vdev, peer, tid, msdu_list, 0);
