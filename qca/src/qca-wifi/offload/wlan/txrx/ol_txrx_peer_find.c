...
ol_txrx_peer_find_hash_attach(struct ol_txrx_pdev_t *pdev)
{
    ...
    hash_elems = ol_cfg_max_peer_id((ol_soc_handle)dp_soc->psoc_obj) + 1;
...
ol_txrx_peer_config_max_buf(struct ol_txrx_pdev_t *pdev)
{
    ...
    if (!pdev->scnctx || !(obj_pdev = ((struct ol_ath_softc_net80211 *)pdev->scnctx)->sc_pdev))
        return;

    lmac_get_pdev_target_type(obj_pdev, &tgt_type);
