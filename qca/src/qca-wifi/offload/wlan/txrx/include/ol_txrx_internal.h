/*
 * Copyright (c) 2011, 2015, 2020-2021 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved
...
{ \
    ol_vap_tx_lock(_vdev->osif_vdev); \
    OL_TX_LL_WRAPPER(_vdev, _msdu, _oshandle, HTT_INVALID_TID); \
    ol_vap_tx_unlock(_vdev->osif_vdev); \
}
#else
#define OL_VDEV_TX(_vdev, _msdu, _oshandle) \
...
extern int dp_igmp_me_mcast_convert(struct cdp_soc_t *soc,
                                    uint8_t vdev_id,
                                    uint8_t pdev_id,
                                    wbuf_t wbuf);
...
{ \
        ...
        if ((dp_me_mcast_convert(_soc, _vdev->vdev_id, _pdev_id, _msdu) > 0) || \
            (dp_igmp_me_mcast_convert(_soc, _vdev->vdev_id, _pdev_id, _msdu) > 0)) { \
            ol_vap_tx_unlock(_vdev->osif_vdev); \
        ...
        struct ol_txrx_peer_t *_srcobj)
{
		...
		OL_TXRX_STATS_AGGR(_tgtobj, _srcobj, tx.dropped.age_out);
								\
		OL_TXRX_STATS_UPD_STRUCT(_tgtobj, _srcobj, rx.snr);
       ...
       struct ol_txrx_peer_t *_srcobj)
{
		...
		OL_TXRX_VDEV_STATS_UPD_STRUCT(_tgtobj, _srcobj, rx.snr);
       ...
       struct cdp_vdev_stats * _srcobj)
{
		...
		OL_TXRX_PDEV_STATS_UPD_STRUCT(_tgtobj, _srcobj, rx.snr);
		...
		_srcobj->rx.unicast.num =
			_srcobj->rx.to_stack.num -
			_srcobj->rx.multicast.num;
		_srcobj->rx.unicast.bytes =
			_srcobj->rx.to_stack.bytes -
			_srcobj->rx.multicast.bytes;
