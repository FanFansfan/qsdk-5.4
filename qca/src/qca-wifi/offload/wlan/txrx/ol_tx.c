...
ol_tx_rawsim_getastentry(struct cdp_soc_t *soc, uint8_t vdev_id, qdf_nbuf_t *pnbuf, struct cdp_raw_ast *raw_ast_handle)
{
    ...
    struct rawsim_ast_entry *raw_ast = (struct rawsim_ast_entry *)raw_ast_handle;
...
#undef TID_MASK
        ...
        else if(qdf_unlikely(ip->protocol == IP_PROTOCOL_IGMP)) {
            nbuf_class->is_igmp = 1;
            if (pdev->igmpmld_override) {
                nbuf_class->tid = pdev->igmpmld_tid;
                return;
            }

            if (vdev->igmp_mcast_enhanc_en) {
        ...
        tos_dscp = pri;
#endif
        if (((pdev->igmpmld_override) || (vdev->igmp_mcast_enhanc_en)) &&
...
ol_tx_ll(struct cdp_soc_t *soc, uint8_t vdev_id, qdf_nbuf_t msdu_list)
{
    ...
    if (OL_CFG_RAW_TX_LIKELINESS(vdev->tx_encap_type == htt_pkt_type_raw)) {
        OL_TX_LL_WRAPPER((ol_txrx_vdev_handle)vdev, msdu, vap->iv_ic->ic_qdf_dev, HTT_INVALID_TID);
        goto out;
    }
    ...
    if((IEEE80211_IS_IPV4_MULTICAST((eh)->ether_dhost) ||
       ...
       (!IEEE80211_IS_BROADCAST((eh)->ether_dhost))) {
        ...
        if (vdev->igmp_mcast_enhanc_en &&
            dp_igmp_me_mcast_convert((struct cdp_soc_t *)(vdev->pdev->soc),
                                vdev->vdev_id,
                                vdev->pdev->pdev_id,
                                msdu) > 0 ) {
                goto out;
...
#if (HOST_SW_TSO_SG_ENABLE || HOST_SW_SG_ENABLE)
...
#endif	/* HOST_SW_TSO_SG_ENABLE || HOST_SW_SG_ENABLE */
    {
        ...
        ret = OL_TX_LL_WRAPPER((ol_txrx_vdev_handle)vdev, msdu, vap->iv_ic->ic_qdf_dev, HTT_INVALID_TID);
    }
                ...
                struct cdp_tx_exception_metadata *tx_exc)
{
    ...
    ret = OL_TX_LL_WRAPPER((ol_txrx_vdev_handle)vdev, msdu, vap->iv_ic->ic_qdf_dev, tx_exc->tid);
...
int
ol_tx_ll_pflow_ctrl(ol_txrx_vdev_handle _vdev, qdf_nbuf_t netbuf, uint8_t override_tid)
{
    struct ol_txrx_vdev_t *vdev = (struct ol_txrx_vdev_t *)_vdev;
    struct ol_txrx_pdev_t *pdev = ((struct ol_txrx_vdev_t *)vdev)->pdev;
    uint32_t status;
    ...
    if((pdev->pflow_ctrl_mode == HTT_TX_MODE_PUSH_NO_CLASSIFY) ||
...
#else
        {
            ...
            if (qdf_unlikely(ftype == CB_FTYPE_MCAST2UCAST)
                ...
                if ((ftype == CB_FTYPE_MCAST2UCAST) && (override_tid != HTT_INVALID_TID))
                    tid = override_tid;
                ...
                if (pdev->carrier_vow_config && status == OL_TXRX_SUCCESS) {
                    ...
                    if (override_tid != HTT_INVALID_TID)
                        nbuf_class.pkt_tid = override_tid;
        ...
        uint32_t num_msdus)
{
    ...
    for (i = 0; i < num_msdus; i++) {
        nbuf = nbuf_arr[i];
        ol_tx_ll_pflow_ctrl(vdev, nbuf, HTT_INVALID_TID);
    }
...
ol_tx_delay_stats_update(struct ol_txrx_pdev_t *pdev, qdf_nbuf_t netbuf)
{
    ...
    delta_msecs  = qdf_ktime_to_ms(qdf_nbuf_net_timedelta(netbuf->tstamp));
    ...
    while (tx_pkts <= max_pkts) {
        ...
        if (qdf_likely(netbuf)) {
            ...
            if (qdf_likely(tx_desc)) {
                ...
                if (qdf_unlikely(pdev->delay_counters_enabled && netbuf->tstamp))
#else
                ...
                if (pdev->prof_trigger) {
                    delta_msecs  = qdf_ktime_to_ms(
                                        qdf_nbuf_net_timedelta(netbuf->tstamp));
...
ol_txrx_per_peer_stats(struct cdp_soc_t *soc, char *addr)
{
    ...
    ni = ieee80211_find_node(ic, addr, WLAN_MLME_SB_ID);
    if (ni == NULL) {
