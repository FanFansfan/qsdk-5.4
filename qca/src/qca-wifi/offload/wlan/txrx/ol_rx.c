/*
 * Copyright (c) 2011-2015,2017-2021 Qualcomm Innovation Center, Inc.
 * All Rights Reserved
...
extern void ol_ath_mgmt_handler(struct wlan_objmgr_pdev *pdev, struct ol_ath_softc_net80211 *scn, wbuf_t wbuf, struct ieee80211_frame * wh, struct mgmt_rx_event_params rx_event, bool null_data_handler);
                                ...
                                void *rx_mpdu_desc )
{
    ...
    ni = ieee80211_find_node(ic, peer->mac_addr.raw, WLAN_MLME_SB_ID);
                            ...
                            struct cdp_rx_indication_ppdu *cdp_rx_ppdu)
{
    ...
    snr = (ppdu_start[co] & cm) >> cs;
    cdp_rx_ppdu->rssi = snr;

    for (chain = 0; chain < WLANSTATS_MAX_CHAIN_LEGACY; chain++) {
         for (ht = 0; ht < WLANSTATS_MAX_BW_LEGACY; ht++) {
              snr = ppdu_start[chain];
              snr >>= ht * WLANSTATS_SNR_OFFSET;
              snr &= WLANSTATS_SNR_MASK;

              if (snr & WLANSTATS_SNR_MAX)
                  continue;
              cdp_rx_ppdu->rssi_chain[chain][ht] = snr;
         }
    }
}
                          ...
                          qdf_nbuf_t msdu)
{
    ...
    if (peer->stats.rx.avg_snr == CDP_INVALID_SNR)
        peer->stats.rx.avg_snr = CDP_SNR_IN(snr);
    else
        CDP_SNR_UPDATE_AVG(peer->stats.rx.avg_snr, snr);
                    ...
                    void *rx_desc)
{
    ...
    if (qdf_unlikely(soc->rdkstats_enabled
#if UNIFIED_SMARTANTENNA
		    ...
		    if (htt_rx_msdu_first_msdu_flag(htt_pdev, rx_mpdu_desc) && i == 0) {
                ...
                if (qdf_unlikely(pdev->ap_stats_tx_cal_enable))
                    ol_rx_update_peer_stats(pdev, head_msdu, peer_id, status);
                     ...
                     /* If enable_ol_stats is disabled, update to_stack stats
                      * for ifconfig
                      */
                     temp_msdu = head_msdu;
                     while (temp_msdu) {
                        peer->stats.rx.to_stack.num += 1;
                        peer->stats.rx.to_stack.bytes += qdf_nbuf_len(temp_msdu);
                        temp_msdu = qdf_nbuf_next(temp_msdu);
                     }
                }
                    ...
                    if (msdu->len >= sizeof(struct ieee80211_frame)) {
                        if ((type == IEEE80211_FC0_TYPE_DATA) && (subtype == IEEE80211_FC0_SUBTYPE_NODATA)) {
                            ...
                            if (!(htt_pdev->ar_rx_ops->is_fcs_err(rx_desc))) {
                                ...
                                msdu->next = NULL;
                                ol_ath_mgmt_handler(spdev, scn, msdu, wh, rx_event, true);
    ...
    void *rx_desc)
{
    ...
    if (vdev->filters_num > 0) {
        ...
        if (ether_type == ETHERTYPE_VLAN) {
            ether_type = (wh[offset + sizeof(struct vlan_hdr)] << 8) |
                                      wh[offset + sizeof(struct vlan_hdr) + 1];
        }
    ...
    qdf_nbuf_t msdu_list)
{
	...
	if (pdev->host_80211_enable) {
         ...
         */
        if (!peer->authorize && !(qdf_nbuf_is_ipv4_eapol_pkt(msdu) || qdf_nbuf_is_ipv4_wapi_pkt(msdu))) {
             qdf_nbuf_free(msdu);
             pdev->pdev_stats.peer_unauth_rx_pkt_drop++;
             msdu = next;
...
ol_rx_peer_init(struct ol_txrx_pdev_t *pdev, struct ol_txrx_peer_t *peer)
{
	...
	qdf_spinlock_create(&peer->tids_rx_reorder[tid].tid_lock);
    }
    /*
