...
/* Definitions for values of command flag in WMI_CHAN_INFO_EVENT */
...
#define OL_ATH_RATE_OUT(x)  (((x) != DUMMY_MARKER) ? \
			     (OL_ATH_EP_RND((x), OL_ATH_RATE_EP_MULTIPLIER)) : \
			     DUMMY_MARKER)
#define OL_ATH_EP_RND(x, mul)  ((((x)%(mul)) >= ((mul)/2)) ? ((x) + \
				((mul) - 1)) / (mul) : (x)/(mul))

#define RSSI_INV     0x80 /*invalid RSSI */

#define RSSI_DROP_INV(_curr_val, _new_val) (_new_val == RSSI_INV ? \
					    _curr_val: _new_val)

#define RSSI_CHAIN_STATS(ppdu, rx_chain)     do { \
	(rx_chain).rx_rssi_pri20 = \
		RSSI_DROP_INV((rx_chain).rx_rssi_pri20, \
			      ((ppdu) & RX_RSSI_CHAIN_PRI20_MASK)); \
	(rx_chain).rx_rssi_sec20 = \
		RSSI_DROP_INV((rx_chain).rx_rssi_sec20, \
			      ((ppdu) & RX_RSSI_CHAIN_SEC20_MASK) >> \
			      RX_RSSI_CHAIN_SEC20_SHIFT) ; \
	(rx_chain).rx_rssi_sec40 = \
		RSSI_DROP_INV((rx_chain).rx_rssi_sec40, \
			      ((ppdu) & RX_RSSI_CHAIN_SEC40_MASK) >> \
			      RX_RSSI_CHAIN_SEC40_SHIFT); \
	(rx_chain).rx_rssi_sec80 = \
		RSSI_DROP_INV((rx_chain).rx_rssi_sec80, \
			      ((ppdu) & RX_RSSI_CHAIN_SEC80_MASK) >> \
			      RX_RSSI_CHAIN_SEC80_SHIFT); \
} while ( 0 )


#define RSSI_CHAIN_PRI20(ppdu, rx_chain)       do { \
	rx_chain = RSSI_DROP_INV((rx_chain), ((ppdu) & \
				 HTT_T2H_EN_STATS_RSSI_PRI_20_M) >> \
...
struct ath_pktlog_stats_txctl {
	...
	struct {
		...
		uint32_t txdesc_ctl[PKTLOG_STATS_MAX_TXCTL_WORDS];
	} priv;
}__attribute__ ((packed));

/**
 * ol_ath_wlan_profile_data_event_handler() - Handler for wmi profile data event
 * @sc: pointer to soft context
 * @data: pointer to profile data event buffer
 * @datalen: profile event data len
 *
...
/**
 * ol_ath_chan_info_attach() - Registers WMI event handler for
 * channel info event
 * @ic: pointer to ic
 *
 * Return: none
 */
void ol_ath_chan_info_attach(struct ieee80211com *ic);

/**
 * ol_ath_stats_soc_attach() - Registers target stats event handlers
 * @soc: pointer to ath soft context
 *
 * Return: none
 */
...
/**
 * ol_ath_stats_soc_detach() - Unregisters target stats event handlers
 * @soc: pointer to ath soft context
 *
 * Return: none
 */
...
/**
 * ol_ath_stats_attach() - Enables pdev,vdev,peer, periodic chan stats event
 * Creates work queue for tx ppdu stats and rx ppdu stats
 * @ic: ic pointer
 *
 * Return: none
 */
void ol_ath_stats_attach(struct ieee80211com *ic);

/**
 * ol_ath_stats_detach() - Disables pdev,vdev,peer, periodic chan stats event
 * Destorys work queue of tx ppdu stats and rx ppdu stats
 * @ic: ic pointer
 *
 * Return: none
 */
void ol_ath_stats_detach(struct ieee80211com *ic);

/**
 * ol_ath_soc_chan_info_attach - Registers WMI event handler for chan info event
 * @soc: pointer to ath soft context
 *
 * Return: none
 */
...
void ol_ath_soc_chan_info_attach(ol_ath_soc_softc_t *soc);

/**
 * ol_ath_soc_chan_info_detach - Unregister WMI event handler of chan info event
 * @soc: pointer to ath soft context
 *
 * Return: none
 */
...
/**
 * acfg_chan_stats_event() - Sends a netlink broadcast event containing
 * self bss utilization, obss utilization, noise floor and current
 * channel centre frequency
 * @ic: pointer to ieee80211com
 * @self_util: self bss channel utilization
 * @obss_util: other bss channel utilization
 *
 * Return: None
 */
void acfg_chan_stats_event(struct ieee80211com *ic, uint8_t self_util,
			   uint8_t obss_util);

/**
 * ol_find_chan_stats_delta() - Updates channel periodic stats delta based on
 * new and past periodic channel stats
 * @pstats: past periodic channel stats
 * @nstats: new periodic channel stats
 * @delta: to hold delta of new and past periodic channel stats
 *
