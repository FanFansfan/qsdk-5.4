                ...
                ath_pktlog_hdr_t *pl_hdr)
{
    ...
    if(plarg.flags & PHFLAGS_INTERRUPT_CONTEXT) {
        /*
         * We are already in interrupt context, no need to make it intsafe
...
A_STATUS
process_pktlog_frag_mgmt_pkt(void *context, void *log_data, uint16_t log_type)
{
    struct ol_pktlog_dev_t *pl_dev = (struct ol_pktlog_dev_t *)context;
    struct ath_pktlog_info *pl_info;
    ath_pktlog_hdr_t pl_hdr;
    struct ath_pktlog_rx_info rxstat_log;
    size_t log_size;
    qdf_frag_t frag_addr;
    uint32_t frag_size, num_nr_frags, idx;
    void *pos;
    int nbuf_data_len;
     ...
     */
    log_size = ((qdf_nbuf_len(nbuf) + 3) >> 2) << 2;
    ...
    pos = rxstat_log.rx_desc;

    nbuf_data_len = qdf_nbuf_headlen(nbuf);
    qdf_mem_copy(pos, qdf_nbuf_data(log_data), nbuf_data_len);
    pos += nbuf_data_len;

    idx = 0;
    while(idx < num_nr_frags) {
        frag_addr =
            qdf_nbuf_get_frag_addr(nbuf, idx);
        frag_size = qdf_nbuf_get_frag_size_by_idx(nbuf, idx);
        qdf_mem_copy(pos, frag_addr, frag_size);
	pos += frag_size;
	idx++;
    }
...
A_STATUS
process_pktlog_skb_mgmt_pkt(void *context, void *log_data, uint16_t log_type)
{
    struct ol_pktlog_dev_t *pl_dev = (struct ol_pktlog_dev_t *)context;
    struct ath_pktlog_info *pl_info;
    ath_pktlog_hdr_t pl_hdr;
    ...
    pos = rxstat_log.rx_desc;
    while(nbuf) {
	nbuf_data_len = qdf_nbuf_get_data_len(nbuf);
        qdf_mem_copy(pos, qdf_nbuf_data(nbuf), nbuf_data_len);
	pos += nbuf_data_len;
...
A_STATUS
process_pktlog_mgmt_pkt(void *context, void *log_data, uint16_t log_type)
{
    if (qdf_nbuf_get_nr_frags(log_data))
        return process_pktlog_frag_mgmt_pkt(context, log_data, log_type);
    else
        return process_pktlog_skb_mgmt_pkt(context, log_data, log_type);
}
#else
A_STATUS
process_pktlog_mgmt_pkt(void *context, void *log_data, uint16_t log_type)
