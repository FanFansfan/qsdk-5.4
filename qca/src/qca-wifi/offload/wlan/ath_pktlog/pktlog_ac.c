...
int pktlog_set_pktlog_ssr_dump(struct ol_ath_softc_net80211 * scn);
...
struct ol_pl_arch_dep_funcs ol_pl_funcs = {
    ...
    .pktlog_set_pktlog_ssr_dump = pktlog_set_pktlog_ssr_dump,
};
        ...
        uint32_t log_state)
{
    ...
    if (log_state & ATH_PKTLOG_ACTION_NOACK) {
        types |= WMI_HOST_PKTLOG_EVENT_CBF;
    }
    ...
    u_int16_t peer_id, uint32_t status)
{
	...
	case WDI_EVENT_RX_CBF:
        {
            /*
             * process mgmt pkt for monitor mode
             */
            if (process_pktlog_mgmt_pkt(context, log_data, PKTLOG_TYPE_LITE_T2H)) {
                AR_DEBUG_PRINTF(ATH_DEBUG_TRC, (" Unable to process mgmt pkt\n"));
                return;
            }
            break;
        }
...
wdi_pktlog_unsubscribe(struct ol_pktlog_dev_t *pl_dev, uint32_t log_state)
{
    ...
    } else if (log_state & ATH_PKTLOG_CBF) {
        if (cdp_wdi_event_unsub(soc_txrx_handle,
                    pdev_id,
                    &(pl_dev->PKTLOG_RX_CBF_SUBSCRIBER),
                    WDI_EVENT_RX_CBF)) {
...
pktlog_init_3_0(struct ol_ath_softc_net80211 *scn)
{
    ...
    pl_dev->PKTLOG_RX_CBF_SUBSCRIBER.callback = pktlog_callback_3_0;
...
pktlog_enable(struct ol_ath_softc_net80211 *scn, int32_t log_state)
{
    ...
    if(log_state != 0) {
        ...
        status = hif_config_ce_pktlog(hif_hdl);
    }
...
int pktlog_set_pktlog_ssr_dump(struct ol_ath_softc_net80211 * scn)
{
    ...
    /* this setting will reflect in sysctl "ssr_hit" i.e.
     * /proc/sys/ath_pktlog/wifiX/ssr_hit. In hotplug/acfg dump collection
     * script, this sysctl would be checked and pktlog dump
     * file woule be TFTPd from /proc/ath_pktlog/wifiX
     */
    pl_info->ssr_hit = 1;
    return 0;
}
