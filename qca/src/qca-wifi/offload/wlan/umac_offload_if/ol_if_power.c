                             ...
                             uint32_t param, uint32_t value)
{
    ...
    ps_param.vdev_id = wlan_vdev_get_id(vap->vdev_obj);
    ps_param.param = param;
    ps_param.value = value;
    status = wmi_unified_ap_ps_cmd_send(pdev_wmi_handle,
                                        anode->an_node.ni_macaddr,
                                        &ps_param);
    return qdf_status_to_os_return(status);
}

int ol_power_set_sta_ps_param(struct ieee80211vap *vap, uint32_t param,
                              uint32_t value)
{
    struct wlan_objmgr_pdev *pdev = NULL;
    struct wmi_unified *pdev_wmi_handle = NULL;
    ...
    status = wmi_unified_sta_ps_cmd_send(pdev_wmi_handle, &ps_param);
    return qdf_status_to_os_return(status);
}

/**
 * ol_power_set_psmode() - set power save mode
 * @vap: pointer to vap
 * @psmode: station power save mode
 *
 * Return: 0 on success, other value on failure
 */
static int ol_power_set_psmode(struct ieee80211vap *vap,
                               ieee80211_pwrsave_mode psmode)
{
    ...
    if (ol_target_lithium(psoc)) {
        ...
        status = wmi_unified_set_sta_ps_mode(pdev_wmi_handle,
        ...
        param.psmode = psmode;
        status = wmi_unified_set_psmode_cmd_send(pdev_wmi_handle, &param);
    }
    return qdf_status_to_os_return(status);
}

int ol_power_sta_set_pspoll(struct ieee80211vap *vap, uint32_t pspoll)
{
    ...
    if (!sta_handle->force_sleep) {
        ...
        err = ol_power_set_sta_ps_param(vap,
                                        WMI_HOST_STA_PS_PARAM_RX_WAKE_POLICY,
                                        value);
    }
    return err;
}
        ...
        ieee80211_pspoll_moredata_handling mode)
{
    ...
    if (!sta_handle->force_sleep) {
        ...
        err = ol_power_set_sta_ps_param(vap, WMI_HOST_STA_PS_PARAM_PSPOLL_COUNT,
                                        value);
    }
    return err;
}
...
int ol_power_sta_set_mode(struct ieee80211vap *vap,
                          ieee80211_pwrsave_mode mode)
{
    ...
    if (!sta_handle->force_sleep) {
        ...
        err = ol_power_set_sta_ps_param(vap,
                            ...
                            sta_handle->inactivity_time);
        if (err)
            goto error;

        err = ol_power_set_psmode(vap, sta_handle->psmode);
        if (err)
            goto error;
...
int ol_power_sta_set_inactive_time(struct ieee80211vap *vap,
                                   ieee80211_pwrsave_mode mode,
        ...
        bool enable)
{
    ...
    err = ol_power_set_sta_ps_param(vap, WMI_HOST_STA_PS_PARAM_RX_WAKE_POLICY,
                                    rx_wake_policy);
    if (err)
        goto error;
    ...
    err = ol_power_set_sta_ps_param(vap, WMI_HOST_STA_PS_PARAM_INACTIVITY_TIME,
                                    inactivity_time);
    if (err)
        goto error;

    err = ol_power_set_psmode(vap, psmode);
    if (err)
        goto error;
