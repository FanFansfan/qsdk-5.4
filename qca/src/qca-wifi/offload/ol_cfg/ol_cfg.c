...
int ol_cfg_max_peer_id(ol_soc_handle psoc)
{
    ...
    tgt_psoc_info = wlan_psoc_get_tgt_if_handle(psoc_obj);
    if (!tgt_psoc_info) {
        qdf_err("null tgt_psoc_info");
        return 0;
    }
    ...
    if (soc->soc_attached) {
        num_vdev = wlan_psoc_get_max_vdev_count(psoc_obj);
        if (ol_target_lithium(psoc_obj))
            def_tgt_max_peer = wlan_psoc_get_max_peer_count(psoc_obj);
    } else {
        num_vdev = init_deinit_get_total_vdevs_for_pdev_id(psoc_obj, 0);
        if (ol_target_lithium(psoc_obj))
            def_tgt_max_peer = init_deinit_get_total_peers_for_pdev_id(psoc_obj, 0);
    }
...
int ol_cfg_max_vdevs(ol_pdev_handle pdev)
{
    ...
    return wlan_pdev_get_max_vdev_count(scn->sc_ic.ic_pdev_obj);
}
...
ol_cfg_target_tx_credit(ol_soc_handle ol_soc)
{
    ...
    tgt_psoc_info = wlan_psoc_get_tgt_if_handle(
            psoc_obj);
