...
{
    u_int8_t *fw_desc = (u_int8_t *)(((u_int8_t *) msdu_desc)
                        + pdev->rx_fw_desc_offset);
    *fw_desc &= ~FW_RX_DESC_DISCARD_M;
}
qdf_export_symbol(htt_rx_msdu_discard_clear);
...
htt_rx_attach(struct htt_pdev_t *pdev)
{
	...
	pdev->htt_rx_mpdu_desc_seq_num = (htt_rx_mpdu_desc_seq_num_cb)pdev->ar_rx_ops->mpdu_desc_seq_num;
        ...
        pdev->htt_rx_ppdu_wlan_preamble = (htt_rx_ppdu_wlan_preamble_cb)pdev->ar_rx_ops->ppdu_wlan_preamble;
