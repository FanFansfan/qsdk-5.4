/*
 * Copyright (c) 2013,2017-2018,2021 Qualcomm Innovation Center, Inc.
 *
 * All Rights Reserved.
...
ath_sysfs_diag_init(struct ol_ath_soc_softc *soc)
{
    ...
    ret = qdf_vfs_create_binfile(
                            ...
                            (struct qdf_vf_bin_attr *)diag_fsattr);
    if (ret) {
...
ath_sysfs_CE_init(struct ol_ath_softc *sc)
{
    ...
    ret = qdf_vfs_create_binfile((struct qdf_dev_obj *)&sc->dev->kobj,
                                 (struct qdf_vf_bin_attr *)&CE_fsattr);
    BUG_ON(ret);
...
{
    qdf_vfs_delete_binfile((struct qdf_dev_obj *)&sc->dev->kobj,
                           (struct qdf_vf_bin_attr *)&CE_fsattr);
