 ...
 *                2462,5180 ..
 */
uint32_t ol_scan_freqlist[IEEE80211_CUSTOM_SCAN_ORDER_MAXSIZE];
int ol_scan_chanlist_size = 0;
module_param_array(ol_scan_freqlist,
        uint, &ol_scan_chanlist_size, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
...
#define QCA_OL_PARAM_STR_LENGTH 40

enum qca_ol_num_module_param {
    PARAM_AR900B_20_TARG_CLK,
    PARAM_OL_DUAL_BAND_2G_RADIO,
    PARAM_TESTMODE,
    PARAM_DFS_DISABLE,
    PARAM_OL_DUAL_BAND_5G_RADIOS,
#if defined(HIF_PCI)
    PARAM_WAR1,
#endif
    PARAM_FRAC,
    PARAM_OL_SCAN_FREQLIST,
#if defined(HIF_PCI)
    PARAM_WAR1_ALLOW_SLEEP,
#endif
    PARAM_INTVAL,
    PARAM_POLLEDMODE,
    PARAM_IPQ4019_20_TARG_CLK,
    PARAM_QCA9888_20_TARG_CLK,
    QCA_OL_PARAM_MAX,
};

static char qca_ol_module_param[QCA_OL_PARAM_MAX][QCA_OL_PARAM_STR_LENGTH] = {
    "ar900b_20_targ_clk",
    "ol_dual_band_2g_radios",
    "testmode",
    "dfs_disable",
    "ol_dual_band_5g_radios",
#if defined(HIF_PCI)
    "war1",
#endif
    "frac",
...
void config_param_scan_freqlist(char *str_value)
{
    char *freq;
    int32_t i = 0;

    /* Param ol_scan_freqlist is an array of int, i.e
     * param=5180,5200,5220,5240
     * So, using ',' as delimter in strsep()
     */
    while ((freq = strsep(&str_value, ",")) &&
           (i < IEEE80211_CUSTOM_SCAN_ORDER_MAXSIZE)) {

        if (kstrtouint(freq, 10, &ol_scan_freqlist[i]) < 0) {
            qdf_err("ERROR!!Invalid scan freqency param: %s", freq);
            return;
        }
       i++;
    }
...
/**
 * qca_ol_module_param_handler() - update qca_ol module param
 *
 * This function receives any one of qca_ol module param and its value as input
 *
 * Return : QDF_STATUS_SUCCESS on Success
 */
QDF_STATUS qca_ol_module_param_handler(void *context, const char *str_param,
                                       const char *str_value)
{
    QDF_STATUS status = QDF_STATUS_E_FAILURE;
    uint16_t param=0;

    while ( param < QCA_OL_PARAM_MAX ) {
        if (qdf_str_eq(qca_ol_module_param[param], str_param)) {
            switch(param) {
                case PARAM_AR900B_20_TARG_CLK:
                    status = qdf_int32_parse(str_value, &ar900b_20_targ_clk);
                    break;
                case PARAM_OL_DUAL_BAND_2G_RADIO:
                    status = qdf_uint32_parse(str_value,
                                              &ol_dual_band_2g_radios);
                    break;
                case PARAM_TESTMODE:
                    status = qdf_int32_parse(str_value, &testmode);
                    break;
                case PARAM_DFS_DISABLE:
                    status = qdf_uint32_parse(str_value, &dfs_disable);
                    break;
                case PARAM_OL_DUAL_BAND_5G_RADIOS:
                    status = qdf_uint32_parse(str_value,
                                              &ol_dual_band_5g_radios);
                    break;
#if defined(HIF_PCI)
                case PARAM_WAR1:
                    status = qdf_int32_parse(str_value, &war1);
                    break;
#endif
                case PARAM_FRAC:
                    status = qdf_int32_parse(str_value, &frac);
                    break;
                case PARAM_OL_SCAN_FREQLIST:
                    /* Param is an array of int, i.e  param=5180,5200,5220,5240*/
                    config_param_scan_freqlist((char *)str_value);
                    break;
#if defined(HIF_PCI)
                case PARAM_WAR1_ALLOW_SLEEP:
                    status = qdf_int32_parse(str_value, &war1_allow_sleep);
                    break;
#endif
                case PARAM_INTVAL:
                    status = qdf_int32_parse(str_value, &intval);
                    break;
                case PARAM_POLLEDMODE:
                    status = qdf_uint32_parse(str_value, &polledmode);
                    break;
                case PARAM_IPQ4019_20_TARG_CLK:
                    status = qdf_int32_parse(str_value, &ipq4019_20_targ_clk);
                    break;
                case PARAM_QCA9888_20_TARG_CLK:
                    status = qdf_int32_parse(str_value, &qca9888_20_targ_clk);
                    break;
                default:
                    break;
            }
 ...
 * Return: void
 */
void initialize_qca_ol_module_param_from_ini(void)
{
    QDF_STATUS status;
    char *path = QDF_WIFI_MODULE_PARAMS_FILE;

    status = qdf_ini_parse(path, NULL, qca_ol_module_param_handler, NULL);
...
qdf_export_symbol(initialize_qca_ol_module_param_from_ini);
#endif
...
#define IC_UINT32_FIELD(x) seq_printf(m, #x ": %lu\n", ic->x)
#define IC_UINT64_FIELD(x) seq_printf(m, #x ": %llu\n", ic->x)
    ...
    IC_UINT32_FIELD(ic_lock_flags);
    IC_UINT32_FIELD(ic_main_sta_lock_flags);
    IC_UINT32_FIELD(ic_chan_change_lock_flags);
    IC_UINT_FIELD(ic_chanchange_serialization_flags);
    IC_UINT32_FIELD(ic_beacon_alloc_lock_flags);
    ...
    IC_UINT_FIELD(ic_rpt_max_phy);
    IC_UINT_FIELD(ic_rpt_ap_needs_dfs);
    IC_UINT_FIELD(ic_mbss_automode);
    IC_UINT_FIELD(ic_strict_pscan_enable);
    IC_UINT_FIELD(ic_min_snr_enable);
    ...
    IC_UINT_FIELD(obss_snr_threshold);
    IC_UINT_FIELD(obss_rx_snr_threshold);
    ...
    if (wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj,
        ...
        IC_UINT_FIELD(ic_mbss.rot_factor);
        IC_UINT_FIELD(ic_mbss.pp_change);
        IC_UINT_FIELD(ic_mbss.current_pp);
        IC_UINT_FIELD(ic_mbss.known_bssid_map);
        IC_UINT_FIELD(ic_mbss.non_inherit_enable);
        IC_UINT_FIELD(ic_mbss.lsb_of_last_user_bssid);
    }
#if ATH_SUPPORT_DFS
    IC_UINT_FIELD(ic_scan_over_cac);
#endif
    IC_UINT_FIELD(ic_externalacs_enable);

    return 0;
}

static int
proc_dump_mbss_cache_show(struct seq_file *m, void *v)
{
    struct ieee80211com *ic = (struct ieee80211com *) m->private;
    struct ieee80211_mbss_ie_cache_node *node;
    struct ieee80211_mbss_non_tx_profile_sub_ie *profile;
    struct wlan_objmgr_vdev *vdev;
    struct wlan_objmgr_psoc *psoc;
    uint8_t ie_header_len = sizeof(struct ieee80211_ie_header);
    struct ol_ath_softc_net80211 *scn = OL_ATH_SOFTC_NET80211(ic);
    int idx;

    psoc = wlan_pdev_get_psoc(ic->ic_pdev_obj);
    if (!psoc) {
        seq_printf(m, "psoc is NULL!\n");
        return 0;
    }
    ...
    if (ieee80211_mbssid_get_num_vaps_in_mbss_cache(ic) <= 1) {
    ...
    if (!node) {
        seq_printf(m, "MBSS cache node is NULL!\n");
        return 0;
    }
    ...
    for (idx = 0; idx < scn->soc->ema_ap_num_max_vaps; idx++) {
        ...
        if (node && node->used) {
            ...
            profile = &node->non_tx_profile.ntx_pf;
            ...
            if (vap == ic->ic_mbss.transmit_vap) {
                node_idx = scn->soc->ema_ap_num_max_vaps -
            ...
            if (wlan_vdev_is_up(vdev) == QDF_STATUS_SUCCESS) {
                ...
                struct ieee80211_mbss_ie_bssid_index *bssidx;

                if (vap == ic->ic_mbss.transmit_vap)
                    seq_printf(m,"\nTransmitting VAP profile (Node index %d):\n",
                               node_idx);
                else
                    seq_printf(m,"\nNon-transmitting VAP profile (Node index %d):\n",
                               node_idx);

                seq_printf(m, "Sub Element: ID:%d, Length:%d\n",
                               profile->sub_elem.element_id,profile->sub_elem.length);
                seq_puts(m, "Capability element:\n");
                seq_printf(m, "\tID:%d, Length:%d Capability:0x%x 0x%x\n",
                                profile->cap_elem.hdr.element_id,
                                profile->cap_elem.hdr.length,
                                profile->cap_elem.non_tx_cap[0],
                                profile->cap_elem.non_tx_cap[1]);

                if (profile->ssid_elem.hdr.length &&
                    (profile->ssid_elem.hdr.length <= WLAN_SSID_MAX_LEN)) {

                        uint8_t target_ssid[WLAN_SSID_MAX_LEN + 1] = {0};

                        qdf_mem_copy(target_ssid,
                                     profile->ssid_elem.ssid,
                                     profile->ssid_elem.hdr.length);

                        seq_puts(m, "SSID element:\n");
                        seq_printf(m, "\tID:%d Length:%d SSID: %s\n",
                                   profile->ssid_elem.hdr.element_id,
                                   profile->ssid_elem.hdr.length,
                                   target_ssid);

                        bssidx = (struct  ieee80211_mbss_ie_bssid_index *)
                            ((uint8_t *) profile +
                             (3 * ie_header_len) +
                             profile->cap_elem.hdr.length +
                             profile->ssid_elem.hdr.length);
                    ...
                    bssidx = &profile->idx_elem;
                }

                if (vap != ic->ic_mbss.transmit_vap) {
                    seq_puts(m, "BSSID Index element:\n");
                    seq_printf(m,"\tID:%d Length:%d BSSID Index:%d DTIM period:%d\n",
                               bssidx->hdr.element_id,
                               bssidx->hdr.length,
                               bssidx->bssid_idx,
                               bssidx->dtim_period);
                }
                seq_printf(m, "Beacon position:%d\n", node->pos);
                seq_printf(m, "Vdev ID:%d\n", node->vdev_id);
            }
...
static int
proc_dump_mbss_bssid_idx_pool_show(struct seq_file *m, void *v)
{
    struct ieee80211com *ic = (struct ieee80211com *) m->private;
    bool is_mbssid_enabled  = wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj,
                                                WLAN_PDEV_F_MBSS_IE_ENABLE);
    unsigned long *bssid_index_bmap;
    uint8_t bssid_idx;
    ...
    bssid_index_bmap = (unsigned long *)
                &ic->ic_mbss.bssid_index_bmap[IEEE80211_DEFAULT_MBSS_SET_IDX];

    for (bssid_idx = 1; bssid_idx < MBSSID_GROUP_SIZE(ic); bssid_idx++) {
        if (qdf_test_bit((bssid_idx - 1), bssid_index_bmap)) {
            seq_printf(m, "bssid_idx: %d is occupied\n", bssid_idx);
        } else {
            seq_printf(m, "bssid_idx: %d is free\n", bssid_idx);
        }
    } /* end for */
...
static int
proc_dump_mbss_cache_attributes_show(struct seq_file *m, void *v)
{
    struct ieee80211com *ic = (struct ieee80211com *) m->private;
    bool is_mbssid_enabled  = wlan_pdev_nif_feat_cap_get(ic->ic_pdev_obj,
                                                WLAN_PDEV_F_MBSS_IE_ENABLE);
    ...
    struct ieee80211vap *vap;
    uint8_t *vap_bssid;
    uint8_t node_idx, bssidx, idx;

    if (is_mbssid_enabled) {
        seq_printf(m, "node_idx       bssid            bssidx   beacon_pos\n");
        seq_printf(m, "---------------------------------------------------\n");
        ...
        for (idx = 0; idx < scn->soc->ema_ap_num_max_vaps; idx++) {
            ...
            if (node->used) {
                ...
                vap_bssid = wlan_vdev_mlme_get_macaddr(vdev);
                ...
                /* Derive bssidx */
                bssidx = vap->vdev_mlme->mgmt.mbss_11ax.profile_idx;

                seq_printf(m, "  %2d", node_idx);
                seq_printf(m, "     %s", ether_sprintf(vap_bssid));
                ...
                /* Dump bssidx */
                if (vap != ic->ic_mbss.transmit_vap) {
                    seq_printf(m, "        %2d", bssidx);
                } else {
                    seq_printf(m, "       %2s", "-");
                }

                /* Dump beacon-position */
                if (vap != ic->ic_mbss.transmit_vap) {
                    seq_printf(m, "        %2d\n", node->pos);
                } else {
                    seq_printf(m, "       %3s\n", "all");
...
GENERATE_PROC_STRUCTS(dump_mbss_cache);
GENERATE_PROC_STRUCTS(dump_mbss_bssid_idx_pool);
GENERATE_PROC_STRUCTS(dump_mbss_cache_attributes);

struct pdev_proc_info pdev_proc_infos[] = {
  PROC_FOO(ic_config,0),
  PROC_FOO(dump_mbss_cache, 0),
  PROC_FOO(dump_mbss_bssid_idx_pool, 0),
  PROC_FOO(dump_mbss_cache_attributes, 0),
};
...
static ssize_t wifi_soc_target_type_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    struct net_device *net = to_net_dev(dev);
    ol_ath_soc_softc_t *soc = ath_netdev_priv(net);
    ...
    switch(target_type) {
        case TARGET_TYPE_QCN9000:
            qdf_str_lcopy(buf, "qcn9000", qdf_str_len("qcn9000") + 1);
            return qdf_str_len(buf);
        default:
            return 0;
    }
...
static DEVICE_ATTR(target_type, S_IRUGO, wifi_soc_target_type_show, NULL);
...
static void ath_vdev_netdev_save_state(struct wlan_objmgr_pdev *pdev, void *obj, void *args)
{
    osif_dev  *osifp;
	...
	if(flags == VDEV_DOWN_STATE) {
	   /* save state of the vdev */
           if (IS_UP(netdev)) {
...
ath_netdev_stop(struct net_device *dev)
{
    ...
    /* This iterates through all VDEVs, save VDEV state, before invoking VDEV down,
       This is needed mainly for EMA AP mode, since Tx VDEV can bring down all VDEVs
     */
    wlan_objmgr_pdev_iterate_obj_list(scn->sc_pdev, WLAN_VDEV_OP,
                                     ath_vdev_netdev_save_state, &flags, 1,
                                     WLAN_MLME_NB_ID);
    wlan_objmgr_pdev_iterate_obj_list(scn->sc_pdev, WLAN_VDEV_OP,
                                     ath_vdev_netdev_state_change, &flags, 1,
                                     WLAN_MLME_NB_ID);
...
/**
 * ieee80211_get_req_chan_info() - Get the channel information from the driver
 * and copy it to user.
 * @dfs: Pointer to ieee80211com structure.
 * @extended_cmd: Command structure pointer from user space to store output data.
 * @error: Failure reason used as return value in case of failure.
 * @reply: Used as return value incase of positive response.
 *
 * Return: void.
 */
#if defined(WLAN_DISP_CHAN_INFO)
static
void ieee80211_get_req_chan_info(struct ieee80211com *ic,
                                 ...
                                 int *reply)
{
    ...
    if (extended_cmd->data_len != sizeof(struct ieee80211req_chaninfo_full)) {
        *error = -EFAULT;
        return;
    }
    ...
    req_chan =
        ...
        qdf_mem_malloc(sizeof(struct ieee80211req_chaninfo_full));
    ...
    ol_ioctl_get_chan_info(ic, req_chan);

    ret = _copy_to_user(extended_cmd->data,
            req_chan,
            sizeof(struct ieee80211req_chaninfo_full));
        ...
        void *vextended_cmd)
{
    ...
    switch (extended_cmd->cmd) {
        ...
        case EXTENDED_SUBIOCTL_GET_CHANINFO:
            ieee80211_get_req_chan_info(ic, extended_cmd, &error, &reply);
            break;
#endif /* WLAN_DISP_CHAN_INFO */
        default:
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 36)
    return stats;
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 24)
    qdf_mem_copy(stats64, stats, sizeof(struct rtnl_link_stats64));
    return;
#else
    qdf_mem_copy(stats64, stats, sizeof(struct rtnl_link_stats64));
    return stats64;
...
ol_swap_seg_alloc (ol_ath_soc_softc_t *soc, struct swap_seg_info **ret_seg_info, u_int64_t **scn_cpuaddr, const char* filename, int type)
{
    ...
    qdf_dev_alloc_mem((struct qdf_dev *)soc->sc_osdev->device,
                      (void *)&seg_info,
                      sizeof(*seg_info), GFP_KERNEL);
    if (!seg_info) {
...
static void ol_ath_release_vap_ifce(void *arg)
{
    ...
    struct ieee80211vap *vap;
    int32_t pdev_idx = 0xffffffff;

    if (ic->ic_pdev_obj)
        pdev_idx = (int32_t)wlan_objmgr_pdev_get_pdev_id(ic->ic_pdev_obj);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 24)
        dev_change_flags(pdev_netdev, pdev_netdev->flags & ~IFF_UP, NULL);
#else
...
#ifdef WLAN_IOT_SIM_SUPPORT
void ol_ath_iot_sim_register_cb(struct wlan_objmgr_psoc *psoc,
                                struct wlan_objmgr_pdev *pdev)
{
    struct wlan_lmac_if_iot_sim_rx_ops *iot_sim_ops = NULL;
    struct wlan_lmac_if_rx_ops *rx_ops = NULL;
    struct iot_sim_cbacks iot_sim_cb;
    QDF_STATUS ret;

    rx_ops = wlan_psoc_get_lmac_if_rxops(psoc);
    if (rx_ops) {
        if (rx_ops->iot_sim_rx_ops.iot_sim_register_cb) {
            iot_sim_ops = &rx_ops->iot_sim_rx_ops;
            iot_sim_cb.update_beacon_trigger = (void (*)(mlme_pdev_ext_t *))
                                               wlan_pdev_beacon_update;
            ret = iot_sim_ops->iot_sim_register_cb(pdev, &iot_sim_cb);
            if (QDF_IS_STATUS_ERROR(ret))
                qdf_err("bcn update handler registration fail");
        }
    }
}
#else
...
__ol_ath_attach(void *hif_hdl, struct ol_attach_t *ol_cfg, osdev_t osdev, qdf_device_t qdf_dev)
{
    ...
    log_mask = cfg_get(psoc, CFG_OL_CONSOLE_LOG_MASK);
    wlan_set_console_log_levels(log_mask);
    ...
    if (cfg_get(psoc, CFG_OL_STRICT_CHANNEL_MODE)) {
        wlan_psoc_nif_feat_cap_set(psoc, WLAN_SOC_F_STRICT_CHANNEL);
    }
    ...
    if (cfg_get(psoc, CFG_OL_B_TWT_ENABLE))
        wlan_psoc_nif_feat_cap_set(psoc, WLAN_SOC_F_BCAST_TWT);
...
#define LTEU_SUPPORT2    0x4
    ...
    /* Initialize the atomic variables used for global vap count */
    wlan_rnr_init_cnt();
	...
	cdp_pdev_post_attach(soc_txrx_handle, pdev_id);
	...
	ic->ic_cfg80211_config = cfg_get(psoc, CFG_OL_CFG80211_CONFIG);
        ...
        ic->ic_enable_additional_triplets = 0;
        ic->ic_mcst_of_rootap = 0;
        ic->ic_has_rootap_done_cac = false;
        ic->ic_opclass_tbl_idx = OPCLS_TAB_IDX_NONE;

        /* Copy ini setting to ic as ini and cfg80211tool
         * cmd map to ic flag */
        if(cfg_get(psoc, CFG_OL_6GHZ_SELECTIVE_NONTX_ADD))
            ic->ic_flags_ext2 |= IEEE80211_FEXT2_RNR_SELECTIVE_ADD;
        else
            ic->ic_flags_ext2 &= ~IEEE80211_FEXT2_RNR_SELECTIVE_ADD;
        ic->ic_user_rnr_frm_ctrl = IEEE80211_BCN_PROBERSP_RNR_EN; /* Set 2 bits for beacon and probe resp */
     ...
     */
        ...
        pdev_netdev->max_mtu = ATH_MAX_MTU;
#endif
...
// Enable ethtool support
...
#endif
        if (!pdev_netdev->dev_addr) {
            qdf_err("pdev_netdev->dev_addr is NULL\n");
            error = -ENOMEM;
            goto bad8;
        }
...
#if DBDC_REPEATER_SUPPORT
        /*
         * The addition of radio is done here as wiphy structure
         * is allocated inside ic_cfg80211_radio_attach function.
         */
        if (ic->ic_wiphy) {
            qca_multi_link_add_radio(ic->ic_wiphy);
        }
#endif
	...
	os_if_spectral_netlink_init(scn->sc_pdev);
        ...
        ol_ath_iot_sim_register_cb(psoc, pdev);
        /*
    ...
    pld_get_ramdump_device_name(osdev->device,
                                (char *)soc->fw_dump_file_name,
                                sizeof(soc->fw_dump_file_name));
...
void ath_vdev_recover_delete(struct wlan_objmgr_psoc *psoc, void *obj, void *args)
{
    ...
    if (vdev) {
         mlme_ext_vap_recover_init(vdev);
...
__ol_ath_detach(struct net_device *dev)
{
...
#if defined(OL_ATH_SMART_LOGGING) && !defined(REMOVE_PKT_LOG)
    {
        ...
        wlan_minidump_remove((void *)scn, sizeof(*scn), psoc,
                             WLAN_MD_CP_EXT_PDEV, "ol_ath_softc_net80211");
        qdf_mem_free(scn->sc_osdev);
        free_netdev(pdev_netdev);
    }
    ...
    wlan_minidump_remove((void *)soc, sizeof(*soc), psoc,
                          WLAN_MD_CP_EXT_PSOC, "ol_ath_soc_softc");
    target_if_free_psoc_tgt_info(psoc);
    wlan_objmgr_psoc_obj_delete(psoc);
    free_netdev(dev);
