                                  ...
                                  uint8_t *buffer, int32_t Length)
{
    ...
    /* Pass negated value init_enable to match with cfg80211tool implementation */
    qdf_nofl_info("%s[%d] gpio_num %d input %d pull_type %d, intr_mode %d mux_config %d drive %d, init_enable %d\n", __func__,__LINE__, ptr->gpio_num, ptr->input, ptr->pull_type, ptr->intr_mode, ptr->mux_config_val, ptr->drive, !(ptr->init_enable));
    return tgt_gpio_config(psoc, ptr->gpio_num, ptr->input, ptr->pull_type, ptr->intr_mode, ptr->mux_config_val, ptr->drive, !(ptr->init_enable));
}
                                  ...
                                  uint8_t *buffer, int32_t Length)
{
    ...
    if(tgt_gpio_output(psoc, ptr->gpio_num, ptr->set)) {
             ...
             uint8_t *buffer, int32_t Length)
{
    ...
    return ol_ath_ucfg_set_he_mesh_config(scn, bss_color_t);
}
             ...
             uint8_t *buffer, int32_t Length)
{
    ...
    return ol_ath_ucfg_get_he_mesh_config(scn, (int *)&(bss_color_t->bsscolor),
                                                                SET_HE_BSSCOLOR);
}
...
int
ol_acfg_add_generic_buf(void *ctx, uint16_t cmdid,
                   uint8_t *buffer, int32_t Length)
{
    struct net_device *dev = (struct net_device *) ctx;
    struct ol_ath_softc_net80211 *scn = ath_netdev_priv(dev);
    acfg_os_req_t     *req = NULL;
    acfg_generic_buf_t *ptr = NULL;
    acfg_user_rnr_data_t *rnr_buf = NULL;
    int ret = 0;
    ...
    switch (ptr->buf_type) {
    case ACFG_USER_RNR_ENTRY_ADD:
        rnr_buf = (acfg_user_rnr_data_t *)ptr->buf;
        ret = ieee80211_add_user_rnr_entry(&scn->sc_ic,
                                           rnr_buf->uid,
                                           rnr_buf->user_buf,
                                           rnr_buf->user_buf_len);
        break;
    case ACFG_USER_RNR_ENTRY_DEL:
        rnr_buf = (acfg_user_rnr_data_t *)ptr->buf;
        ret = ieee80211_del_user_rnr_entry(&scn->sc_ic,
                                           rnr_buf->uid);
        break;
    case ACFG_USER_RNR_ENTRY_DUMP:
        ieee80211_dump_user_rnr_entries(&scn->sc_ic);
        break;
    default:
        break;
    }
...
acfg_dispatch_entry_t ol_acfgdispatchentry[] =
{
    ...
    { ol_acfg_add_generic_buf,    ACFG_REQ_ADD_GENERIC_BUF, 0 },
