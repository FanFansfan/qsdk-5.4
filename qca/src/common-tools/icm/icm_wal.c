...
int icm_wal_get_channel_vendorsurvey_info(ICM_INFO_T* picm, ICM_BAND_T band)
{
    ...
    if (pconf->walflag == ICM_WAL_IOCTL) {
#ifdef WLAN_SPECTRAL_ENABLE
        return icm_wal_ioctl_get_channel_vendorsurvey_info(picm, band);
#endif /* WLAN_SPECTRAL_ENABLE */
    } else if (pconf->walflag == ICM_WAL_CFG) {
...
#ifdef ICM_RTR_DRIVER
/*
 * Function     : icm_wal_get_channel_status
...
int icm_wal_get_channel_status(ICM_INFO_T * picm)
{
    ...
    if (pconf->walflag == ICM_WAL_IOCTL) {
        ...
        return icm_wal_mblcfg_get_channel_status(picm);
    } else {
        return -1;
    }
    return 0;
...
/*
 * Function     : icm_wal_clear_radio_ext_acs_request_in_progress
 * Description  : Clear the indication of external ACS being in progress,
 *                on a given radio.
 * Input params : pointer to icm info structure
 * Return       : SUCCESS/FAILURE
 */
...
#ifdef ICM_RTR_DRIVER
        return icm_wal_rtrcfg_clear_radio_ext_acs_request_in_progress(picm);
#else
        /* This is not currently required for MCL config */
        return SUCCESS
#endif /* ICM_RTR_DRIVER */
    } else {
        return FAILURE;
