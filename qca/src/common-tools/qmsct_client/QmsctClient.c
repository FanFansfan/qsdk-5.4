...
int main(int narg, char *arg[])
{
    ...
    for(iarg=1; iarg<narg; iarg++)
    {
        if ( strncmp(arg[iarg], "-ip", sizeof( "-ip" ) ) == 0)
        {
            if((iarg+1) < narg)
            {
                if (strlcpy(pClient->response.ip, arg[iarg+1], IP_SIZE) >= IP_SIZE)
                {
        ...
        if (gethostname (hostName, sizeof(hostName)) == 0)
        {
           if ((hostInfo = gethostbyname(hostName)) != NULL)
           {
             ...
             if (ipStr)
             {
                ...
                if (strlcpy(pClient->response.ip, ipStr, IP_SIZE) >= IP_SIZE)
                {
                    printf("strlcpy error !\n");
                    exit(1);
                }
    ...
    if (strlcpy(pClient->broadcast.ip, pClient->response.ip, IP_SIZE) >= IP_SIZE)
    {
        printf("strlcpy error !\n");
        exit(1);
    }
    ...
    if (strlcpy(pClient->broadcast.buf, pClient->response.ip, BUF_SIZE) >= BUF_SIZE)
    {
        printf("strlcpy error !\n");
        exit(1);
    }
    ...
    while(++i)  // UDP wait response timeout is 1 sec in every loop.
    {
       ...
       if (recv(pClient->response.fd, pClient->response.buf, BUF_SIZE, 0) > 0)
       {
         ...
         if (strcmp(pClient->response.buf, pClient->broadcast.buf) != 0)
         {
             ...
             DbgPrintf("Response Msg: %s\n", pClient->response.buf);
             if (strlcpy(pClient->keepalive.ip, pClient->response.buf, IP_SIZE) >= IP_SIZE)
             {
                 printf("strlcpy error !\n");
                 exit(1);
             }
             ...
             *pStr = '\0';
             ...
             if (strlcpy(pClient->keepalive.buf, "Alive", BUF_SIZE) >= BUF_SIZE)
             {
                 printf("strlcpy error !\n");
                 exit(1);
             }
