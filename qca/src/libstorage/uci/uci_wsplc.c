...
struct ws_translate_option{
    ...
    int (*optparser)(struct uci_context *ctx, struct wsplc_s *ws, char *name, char *value);
};
...
static int opt_runmode(char *optval, char *ucival)
{
    ...
    if (isdigit(*optval))
    {
        ...
        snprintf(ucival, UCI_MAX_STR_LEN,"%s", optval);

    return 0;
}

static int opt_parse_responsercvd(struct uci_context *ctx, struct wsplc_s *ws, char *name, char *value)
{
    ...
    ret = uciGet(ctx, "wsplcd", option, ucival);

    if (ret || strcmp(value, ucival))
    {
        ret = uciSet(ctx, "wsplcd", option, value);
        ws->changed = 0;
    }
    return ret;
}
...
{
    { "RunMode",        opt_runmode,       NULL},
    { "ConfigSta",      opt_configsta,     NULL},
    { "ResponseRcvd",   NULL,              opt_parse_responsercvd},
    { NULL }
};
...
static int wsplc_set(void *cookie, struct package_s *pkg, char *name, char *value)
{
    ...
    if (iopt && iopt->optparser &&
            iopt->optparser(ctx, ws, name, value) != 0)
        return -1;
