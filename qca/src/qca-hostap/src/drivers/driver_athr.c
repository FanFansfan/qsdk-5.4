...
int
driver_atheros_set_key(void *priv, struct wpa_driver_set_key_params *params)
{
	struct driver_atheros_data *drv = priv;
	...
	int key_idx = params->key_idx, set_tx = params->set_tx;
	const u8 *seq = params->seq, *key = params->key;
	size_t seq_len = params->seq_len, key_len = params->key_len;
