...
static struct ieee1905State_t
{
    ...
    ieee1905SecurityCheckCallback_f ieee1905SecurityCheckCallback;
    ieee1905MultiApVersion_e mapVersionEnabled;
...
static u_int8_t ieee1905XmitBufLarge[ IEEE1905_LARGE_BUFFER_SIZE ];
...
void ieee1905DispatchMessage(u_int8_t *frame, u_int32_t len, u_int32_t type) {
    switch (type) {
        ...
        case IEEE1905_MSG_TYPE_AP_CAP_REPORT:
            ...
            break;
        case IEEE1905_MSG_TYPE_Q_CAP_QUERY:
        case IEEE1905_MSG_TYPE_Q_CAP_REPORT:
            ieee1905S.ieee1905ReceiveCallback( IEEE1905_EVENT_Q_CAP, frame, len );
            break;
        ...
        case IEEE1905_MSG_TYPE_COMBINED_INFRASTRUCTURE_METRICS:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_COMBINED_INFRASTRUCTURE_METRICS, frame,
                                              len);
            break;
        ...
        case IEEE1905_MSG_TYPE_ASSOC_STATUS_NOTIFICATION:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_ASSOC_STATUS_NOTIFICATION_MESSAGE,
                                              frame, len);
            break;
        case IEEE1905_MSG_TYPE_BACKHAUL_STA_CAP_QUERY_MESSAGE:
        case IEEE1905_MSG_TYPE_BACKHAUL_STA_CAP_REPORT_MESSAGE:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_BACKHAUL_STA_CAPABILITY, frame, len);
            break;
        case IEEE1905_MSG_TYPE_DPP_CCE_INDICATION:
        case IEEE1905_MSG_TYPE_DPP_PROXIED_ENCAP:
        case IEEE1905_MSG_TYPE_DPP_DIRECT_ENCAP:
        case IEEE1905_MSG_TYPE_DPP_RECONFIG_TRIGGER:
        case IEEE1905_MSG_TYPE_DPP_CHIRP:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_DPP_ONBOARDING, frame, len);
            break;
        case IEEE1905_MSG_TYPE_DPP_BSS_CONFIG_REQUEST:
        case IEEE1905_MSG_TYPE_DPP_BSS_CONFIG_RESPONSE:
        case IEEE1905_MSG_TYPE_DPP_BSS_CONFIG_RESULT:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_DPP_BSS_CONFIG, frame, len);
            break;
        case IEEE1905_MSG_TYPE_ENCAP_EAPOL:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_EAPOL, frame, len);
            break;
        case IEEE1905_MSG_TYPE_DPP_BS_URI_NOTIFICATION:
        case IEEE1905_MSG_TYPE_DPP_BS_URI_QUERY:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_DPP_BOOTSTRAPPING, frame, len);
            break;
        case IEEE1905_MSG_TYPE_FAILED_CONNECTION_MESSAGE:
            ...
            break;
        case IEEE1905_MSG_TYPE_SP:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_SERVICE_PRIORTIZATION, frame, len);
            break;
        case IEEE1905_MSG_TYPE_AGENT_LIST:
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_AGENTS_LIST, frame, len);
            break;
        default:
            break;
    }
...
static void ieee1905EventDispatch( u_int8_t *frame, u_int32_t len, u_int32_t type )
{
    ...
    if (TLV) {
        ...
        if (ieee1905AreEqualMACAddrs(
                alId->ether_addr_octet,
                ieee1905S.Interface[HYBRID_BRIDGE_INDEX].addr.ether_addr_octet)) {
    ...
    if (ieee1905S.mapVersionEnabled >= ieee1905MultiApVersion_3) {
        TLV = (ieee1905TLV_t *)message->content;
        if (TLV && TLV->type == IEEE1905_TLV_TYPE_ENCRYPTED) {
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_ENCRYPTED_MESSAGE, frame, len);
            return;
        }

        TLV = ieee1905TLVTypeFind((ieee1905TLV_t *)message->content, IEEE1905_TLV_TYPE_MIC);
        if (TLV) {
            ieee1905S.ieee1905ReceiveCallback(IEEE1905_EVENT_INTEGRITY_MESSAGE, frame, len);
            return;
        }
    }

    ieee1905DispatchMessage(frame, len, type);
}
...
{
    ieee1905Message_t *message = (ieee1905Message_t *)frame;
    u_int32_t contentLenOnWire =
        len - sizeof(struct ether_header) - sizeof(struct ieee1905Header_t);
    int32_t contentLen;
    int32_t index = -1;
    ...
    /* For mapR3 tlvlength can be greater than ETH_FRAME_LEN . So take content received on wire */
    if (ieee1905S.mapVersionEnabled >= ieee1905MultiApVersion_3) {
        contentLen = contentLenOnWire;
    }
...
/*--- ieee1905InitStage2 -- init encryption.
 */
void ieee1905InitStage2(ieee1905MultiApVersion_e mapVersionEnabled,
                        ieee1905SecurityCheckCallback_f ieee1905SecurityCheckCallback) {
    ieee1905Debug(DBGINFO, "Enter %s", __func__);

    if (!ieee1905S.IsInit) {
        ieee1905Debug(DBGERR, "ieee1905 not enabled %s", __func__);
        return;
    }

    ieee1905S.mapVersionEnabled = mapVersionEnabled;
    if (mapVersionEnabled >= ieee1905MultiApVersion_3) {
        if (ieee1905SecurityCheckCallback) {
            ieee1905S.ieee1905SecurityCheckCallback = ieee1905SecurityCheckCallback;
        }
    }
}
 ...
 */
void ieee1905Init(const char *bridgeName, ieee1905ReceiveCallback_f ieee1905ReceiveCallback,
                  ieee1905RelayCallback_f ieee1905RelayCallback) {
    u_int32_t i;
    ...
    if( bridgeName )
    {
        ...
        if (interfaceGetMACAddressForInterface(
                bridgeName,
                ieee1905S.Interface[HYBRID_BRIDGE_INDEX].addr.ether_addr_octet)) /* non-zero for
                                                                                    error */
        {
...
/*
 * ieee1905MessageBufferGetLarge - Get a pointer to the message buffer
 * with larger memory for r3 and above frames
 */
u_int8_t *ieee1905MessageBufferGetLarge( void )
{
    return ieee1905XmitBufLarge;
}
...
IEEE1905_STATUS ieee1905SecurityCheck(ieee1905Message_t *message, u_int8_t *frame, u_int32_t *len,
                                      const struct ether_addr *dest) {
    /* Fragmented messages will be encrypted in application */
    if (!ieee1905IsMessageFragmented(message->ieee1905Header.flags) &&
        message->ieee1905Header.fid == 0) {
        if (dest) {
            ieee1905TLV_t *TLV = (ieee1905TLV_t *)ieee1905MessageContentGet(frame);
            ieee1905TlvType_e tlvType = ieee1905TLVTypeGet(TLV);
            /* Should not get Encrypted TLV here */
            if (tlvType == IEEE1905_TLV_TYPE_ENCRYPTED) {
                return IEEE1905_NOK;
            }

            ieee1905S.ieee1905SecurityCheckCallback(IEEE1905_EVENT_ENCRYPTION_CHECK, frame, len,
                                                    dest);
        } else {
            ieee1905S.ieee1905SecurityCheckCallback(IEEE1905_EVENT_INTEGRITY_CHECK, frame, len,
                                                    dest);
        }
    }
                                     ...
                                     const struct ether_addr *src, const struct ether_addr *dest )
{
    ...
    /* Check if encryption needs to be performed */
    if (ieee1905S.mapVersionEnabled >= ieee1905MultiApVersion_3) {
        if (ieee1905SecurityCheck(message, frame, &len, dest) == IEEE1905_NOK) {
            return IEEE1905_NOK;
        }

        /* Drop R3 messages if encryption key not found */
        if (len == 0) {
            return IEEE1905_NOK;
        }
    }
    ...
    else
    {
        ...
        ieee1905CopyMACAddr(ieee1905S.Interface[HYBRID_BRIDGE_INDEX].addr.ether_addr_octet,
                            message->etherHeader.ether_shost);
    }
    ...
    if (!(message->ieee1905Header.flags & IEEE1905_HEADER_FLAG_LAST_FRAGMENT)) {
        ieee1905Debug(DBGERR, "%s: Sending IEEE1905.1 packet %d, src: " ieee1905MACAddFmt(
                                  ":") ", dst: " ieee1905MACAddFmt(":") ", frame length: %d",
                      __func__, htons(message->ieee1905Header.type),
                      ieee1905MACAddData(message->etherHeader.ether_shost),
                      ieee1905MACAddData(message->etherHeader.ether_dhost), len);
    }
